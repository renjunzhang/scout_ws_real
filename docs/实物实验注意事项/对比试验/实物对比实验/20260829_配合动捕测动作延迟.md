...
创建日期：2026-08-29
最后更新：2026-09-07（第 3 节使用新地图；第 5 节记录 C03 首次低速试跑及现有验收结果）

2026-08-30 延迟实验批次状态：`B0_DELAY_A_B_COMPLETE / FIXED_ROBOT_ONLY_SELECTED / FORMAL_R01_PENDING`

2026-09-07 状态：`C03_OFFLINE_VALIDATION_PASS / C03_PATH_SELECTION_A01_POSTFLIGHT_FAIL`。第 3、5 节分别使用 20260907 新地图和 C03 新路径；C02 路径、第 6～7 节延迟实验流程及已有结果仍属于 20260829 地图批次。C03/a01 已录制，现有自动报告未通过，不能继续标作“待执行”。同日晚另行完成的四组显式执行器消融 smoke 全部通过，使用不同入口和冻结参数，详见 [C03 四组实物分析](../实物对比试验分析/20260907_C03新地图四组消融内部液面高度分析.md)，不能用它们覆盖本节 a01 的失败记录。
...
### 20260907 动捕场地地图（第 3 节定位使用）

```text
/home/geist/scout_maps/real/20260907_mocap_exec/map_carto_20260907_mocap_exec_v1.pbstream
SHA-256: 86085992407b90571454d1f3c2a1379f602f0259df9535c60a6119470ccbcda6
```

地图于 `2026-09-07T20:40:14+08:00` 冻结，分辨率为 `0.02 m`；同目录的 `map_carto_20260907_mocap_exec_v1_map_validation.json` 为 `pass=true`，pbstream、YAML、PGM 的 SHA 清单已核对通过。新地图已生成下面的 C03 候选；地图资产与路径离线校验不能代替静止定位验收和实车试跑。

### 路径 C03（绑定 20260907 地图，第 5 节使用）

```text
/home/geist/fixed_paths/real/20260907_spmpc_mocap_execution_chain/candidates/mocap_compact_s_C03.json
SHA-256: bda11f74194bb65d7df3b4ba5f1f5c3cc756dafdfd7bc06e753a288e30f7e006
```

起点为采集时的 `map (0.021378, 0.004904)`，起始 yaw 为 `-0.010653 rad`；终点为点击的 `map (4.959137, 0.023974)`，终点 yaw 为 `0.014630 rad`。C03 共 171 点、长 `5.0808 m`、间距 `0.03 m`；为使曲率约束不超过 C02_v2，S 曲线幅值由 `0.30 m` 调整为 `0.25 m`。

几何、曲率与新地图净空报告均通过：最大曲率 `0.8491 1/m`，前 `0.5 m` 最大曲率 `0.4025 1/m`，最大曲率变化率 `1.4771 1/m²`；未知区按障碍处理，最小中心线净空 `0.8122 m`，扣除 `0.426 m` 车体/容器半径后余量 `0.3862 m`。考虑栅格和采样误差后的保守余量下界为 `0.3529 m`。上述是候选生成时的离线结论；后续 C03/a01 的实车报告见第 5 节，离线通过不等于实车通过。

### 20260829 动捕场地地图（历史延迟实验使用）
...
### 3. 使用 20260907 动捕小场地新地图启动传感器与定位

启动脚本按 `freeze.env` 指定的地图加载，不会自动选择日期最新的文件。旧命令 source 的 `20260829_mocap_exec` 仍指向旧地图，必须换成下面的 `20260907_mocap_exec`。

先停车并退出运动控制程序，再在原传感器栈终端按 `Ctrl+C`，等待其子进程退出；若建图仍在运行，也先退出建图。然后在新终端执行：
...
该入口启动底盘、雷达、Cartographer 定位、IMU 和 RealSense，保持终端运行。仅在另一终端重新 source 环境文件不会切换已经运行的定位节点。

启动完成后，在另一个已 source ROS 环境的终端只读核对：
...
应分别输出上面的 `20260907` pbstream 绝对路径和 `86085992407b90571454d1f3c2a1379f602f0259df9535c60a6119470ccbcda6`。启动日志目录中的 `localization_map_identity.env` 还会记录实际文件 SHA。

新地图的 `map` 坐标系不能假定与旧地图重合。C03 的新地图试跑入口见第 5 节；第 6～7 节仍是旧地图批次流程，其中第 6 节 runner 强制校验 C02 和 20260829 地图 SHA，不能直接用于 C03。开始运动前完成[静止定位验收](20260828_动捕场地建图定位与双场地切换SOP.md)，并核对现场路径净空。
...
### 5. 候选路径 C03 低速安全试跑（a01 已录制，自动验收未通过）

C03 是新地图下的新候选，首次试跑使用 `C03/a01`，属于 `PATH_SELECTION`，不占用正式 R01。使用新地图和 C03 各自的冻结文件，不再 source 旧的 `/tmp/mocap_execution_chain.env`，也不沿用 C02 的 `ATTEMPT=02`、重试标记或旧版归档理由。下面保留 a01 的启动命令用于追溯；该编号已有输出，不能原样重跑或覆盖。

此入口默认 variant 是 `B_slosh_matched5`，因此下面显式设置 `VARIANT=B0 W_SLOSH=0`。脚本内部固定 `DELAY_PHASE_MODE=shadow`；在命令前设置 `fixed_robot_only` 会被内部覆盖。本节保留第 5 节路径探索的 `shadow` 模式，不能将本次数据标作第 6 节的 `fixed_robot_only` 延迟诊断。`SPEED_SAFETY_ENABLE=true` 等参数会继续传入底层固定路径 runner，启用 `0.15 m/s` 硬限速。

原 a01 的运行前提为第 3、4 节的传感器、定位和 NOKOV 已运行，机器人位于 C03 起点附近，其他 planner/teleop 已退出。以下是当次命令；无运动参数校验使用 `VALIDATE_ONLY=true ARM_MOTION=NO`，原 a01 输出保留：
...
source devel/setup.bash && \
source /home/geist/scout_maps/real/20260907_mocap_exec/map_carto_20260907_mocap_exec_v1_freeze.env && \
source /home/geist/fixed_paths/real/20260907_spmpc_mocap_execution_chain/candidates/mocap_compact_s_C03_freeze.env && \
DATE=20260907 \
PATH_SELECTION_OUT_DIR=/home/geist/slosh_bags/real/20260907_spmpc_mocap_execution_chain/path_selection \
SELECTION_ID=C03 ATTEMPT=01 ACQUISITION_RETRY=false RETRY_REASON_FILE= \
VARIANT=B0 W_SLOSH=0 V_REF=0.10 \
SPEED_SAFETY_ENABLE=true V_SAFE_MAX=0.15 SPEED_SAFETY_TOLERANCE=0.0001 \
RECORD_SEC=90 START_POS_TOL=0.08 START_YAW_TOL=0.15 \
VALIDATE_ONLY=false ARM_MOTION=YES \
...
```text
/home/geist/slosh_bags/real/20260907_spmpc_mocap_execution_chain/path_selection/DEV_MOCAP_PATH_SELECTION_C03_B0_a01.bag
```

配套输出为同 stem 的 `_protocol.env` 和 `_mocap_path_selection_postflight.json`。无运动校验只核对参数、路径/地图资产及 SHA，不验证在线话题、定位质量、底层 launch 参数或实车跟踪表现，也不会占用 a01。正式执行会核对运行中的 Cartographer 地图身份，并拒绝已有 `/cmd_vel` publisher 和已有输出。

2026-09-07 已运行上述命令的 `VALIDATE_ONLY=true ARM_MOTION=NO` 版本，结果为 `validate-only PASS`，确认 `C03/a01`、B0、两项冻结 SHA 和新输出目录匹配，未启动运动。

随后于 `2026-09-07T21:18:19+08:00` 创建 a01 采集记录，现有 `_mocap_path_selection_postflight.json` 为 `pass=false`；报告列出 15 个不同的 `ACADOS_SOLVE_FAILED_4` 周期（682、684、690、691、716、724、725、726、1080、1105、1106、1107、1108、1114、1115），每个周期另有安全门拒绝/干预记录，不能将两条报错重复计为两次求解失败。路径匹配检查通过，`formal_trial_consumed=false`；本次仅核对已有报告，没有重放该 bag 或给失败机制定因。

a01 元数据为 `variant=B0`、`v_ref=0.10`、`execution_model_mode=explicit_actuator`、`delay_phase_mode=shadow`。同日晚四组消融使用另一协议、`v_ref=0.20`、legacy delay off 及共同软平滑配置；两个批次的 B0 名称不能代替实际配置核对。四组高度比较仍只使用分析文档列出的四包，不混入本节 a01。

只有低速试跑的 postflight 与跟踪检查均通过，才可将该入口状态更新为通过。如因采集中止需要重试，保留 a01 输出，使用下一 `ATTEMPT`，并按脚本要求提供新的非空重试理由文件；当前求解失败不能在未处理原因时直接改写成采集重试通过。

历史记录：旧地图上的 C02_v2/a02 已完成，`formal_trial_consumed=false`。其 bag 和自动 postflight 的 SHA-256 分别为 `ee08cdc2423bcfba0ee0654b4cd98e9e0baa3e2081fe159f5a94e7e549a4c293`、`21b04588321d0b7e363bdb2f2ea3c87ca5478aa4643b745904d502336788038f`；这些不是 C03 的试跑结果。
...
```text
src/scout_apps/control/spmpc_local_planner/scripts/mocap/run_spmpc_mocap_b0_delay_mode_trial.sh
```
