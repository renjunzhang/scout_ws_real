...
状态：`SHADOW_IMPLEMENTED_READY_FOR_B0_DATA_COLLECTION`。独立液体短时预测、四路诊断、同 bag 分析、隔离 replay、B0 专用 runner 和 postflight 已实现；但尚未采集三条 held-out 实物 bag，也没有允许 I1 进入含 slosh 求解器。因此当前只允许 B0 实物激励采集和 shadow 分析，不允许据此启动新的 slosh 实车闭环，也不占用正式 `R01～R05`。
...
| 功能 | 文件/输出 | 当前安全边界 |
|---|---|---|
| 液体短时传播 | `LiquidStateNowcaster` | 只按真实时间戳传播；默认最大 `50 ms`，激励年龄最大 `60 ms` |
| 四路在线诊断 | `/spmpc/debug/slosh_estimator_comparison` | 同周期发布 `O0/I0/I1/L22`；I1 在本版本强制 `applied_to_solver=false` |
| 同 bag 离线比较 | `analyze_slosh_nowcast_same_bag.py` | 固定 lag=0、scale=1、RGB 五点居中中位数；development 数据不能冒充 held-out |
| 无实车运动 replay | `run_spmpc_slosh_nowcast_replay.sh` | 私有 ROS master；命令只发往 `/spmpc_nowcast_replay/cmd_vel`，输出 bag 禁止出现真实 `/cmd_vel` |
| B0 实车采集 | `run_spmpc_mocap_slosh_nowcast_shadow_trial.sh` | `B0 + fixed_robot_only + v_ref=0.10 + v_safe_max=0.15`；四路全部 shadow |
| 实车自动检查 | `validate_slosh_nowcast_shadow_bag.py` | 检查覆盖率、时间戳、传播边界、来源/回退、RGB 和速度合同 |

已经用 D02 实物 bag 完成一次 25 秒隔离 replay smoke：共 `340` 个求解比较周期，I1 有效 `339` 个，覆盖率 `99.71%`；四路进入求解器次数为 `0`，processed-IMU 来源/回退错误为 `0`，隔离命令超速为 `0`，输出 bag 不含真实 `/cmd_vel`。该结果只说明影子证据链能工作，不说明 I1 已经优于 I0，也不说明 slosh 闭环可以放行。
...
VALIDATE_ONLY=true \
bash src/scout_apps/control/spmpc_local_planner/scripts/mocap/run_spmpc_mocap_slosh_nowcast_shadow_trial.sh
```

确认路径、急停、RGB 几何和起点后，采集一条新的 B0 shadow bag：
...
离线 replay 使用独立 master，默认完整、1 倍速回放：

```bash
cd /home/geist/scout_ws && \
SOURCE_BAG=/绝对路径/输入.bag \
bash src/scout_apps/control/spmpc_local_planner/scripts/protocols/run_spmpc_slosh_nowcast_replay.sh
```

收齐至少三条独立 bag 后再做同口径汇总；每条 bag 重复一个 `--bag`：

```bash
python3 src/scout_apps/control/spmpc_local_planner/scripts/analysis/analyze_slosh_nowcast_same_bag.py \
  --bag /绝对路径/N01.bag \
  --bag /绝对路径/N02.bag \
  --bag /绝对路径/N03.bag \
  --out-dir /绝对路径/nowcast_same_bag_analysis \
  --evidence-class held_out
```
