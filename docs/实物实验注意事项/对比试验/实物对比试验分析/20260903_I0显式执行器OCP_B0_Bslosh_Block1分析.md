...
进一步用现有 bag 的 NOKOV 独立运动参考（`/vrpn_client_node/Tracker0/pose`，100 Hz，标记点刚性连接车体）做三方对比，并结合操作者给出的容器尺寸（高 `53 mm`、直径 `37.5 mm`）做物理核算，把根因从"激励口径/液体耦合"聚焦为"车体 ~5 Hz 结构共振"：5 Hz 是真实车体振荡（NOKOV 与 IMU 同相，平移振幅 `0.18–0.21 mm`），但已发布命令的 5 Hz 仅 `1.5 mm/s`、经执行器只驱动 `0.0126 m/s²`，与模型预测 `0.0153 m/s²` 吻合（模型对"执行器响应命令"是对的）；实测车体 5 Hz 却是 IMU `0.082`、NOKOV `0.165 m/s²`。而容器实际内径 `34.5 mm`（外径 37.5 mm）→ slosh 固有频率 `5.15 Hz`；模型原 `4.973 Hz` 是容器半径误标（用了 18.5 mm 而非内半径 17.25 mm），已随 `d0c98a0` 修正并重生成 solver、重跑测试通过。液体约 `50 g`、对底盘零反作用（反作用力只够 `0.0002–0.0005 m/s²`，比实测额外加速度小 `200–400` 倍）——液体太轻摇不动车体。因此 5 Hz 车体振荡是结构/执行器层面的共振（因），液体晃动是被共振激励的果；模型真正缺的是"车体 ~5 Hz 结构共振"，不是 slosh 耦合。详见 12.8 节。
...
4. 容器 slosh 频率被误标、液体太轻。容器实际内径 `34.5 mm`（外径 37.5 mm、内半径 17.25 mm）→ slosh 固有频率 `5.15 Hz`；模型原 `omega_n=4.973 Hz` 是容器半径误用 18.5 mm 所致，已随 `d0c98a0` 修正并重生成 solver、重跑测试通过。液体约 `50 g`、对底盘零反作用（反作用力（`eta=0.55 mm`）≈ `11 mN`，只能给 23–45 kg 车体 `0.0002–0.0005 m/s²`，比实测额外 `0.07–0.15 m/s²` 小 `200–400` 倍）。

**最可能机制（当前证据支持，尚未唯一定位）：**

5 Hz 车体振荡是**车体/执行器层面的 ~5 Hz 结构共振（因）**，被转向/驱动激励；容器内液体固有 `5.15 Hz` 恰与之接近，被共振激励而大幅晃动（`eta_x≈0.55 mm`、`eta_y≈0.26–0.33 mm`）是**果**。模型的一阶执行器模型太理想，挡掉了 5 Hz 峰值，故预测车体加速度偏光滑（`0.015`）、连带 slosh 也偏小。**模型缺的是"车体 ~5 Hz 结构共振"，不是 slosh 耦合**（"液体反作用驱动车体"被容器尺寸排除；此前反事实中"μ≈13%–28% 合理"的估计因未掌握容器尺寸而不成立，实际 `μ=m_n/M_body≈0.0005`）。

**尚未证明：**

- 该 ~5 Hz 共振是"车体-悬架结构"还是"驱动/舵机"，尚不能分开；当前"车体 yaw > 轮子 yaw"指向结构，但可能被轮速 odom 低通滤波污染（需确认 `/odom` twist 是否平滑）。
- 液体对底盘零反作用（操作者确认，且质量占比仅约 `0.0005`）——该 5 Hz 共振与液体无关，slosh 只是被动指示；模型单向耦合正确，无需补液体反作用项。
