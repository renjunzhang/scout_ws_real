...
当前终点链已经不是“进入目标容差区后外层直接发 `cmd_vel=0`”的旧实现。  
当前更准确的口径是：

- 远场：tracking MPC
- 近终点可能进入外层 `terminal recovery`
- `goal_stop_pending_` 下：把 `v_des_cmd` 压到 `0`，由 MPC 继续做最后一段减速与纠偏

因此这件事和 IMU 融入仍然是两条线。  
不要把 near-goal 模式切换、terminal recovery 或末端收敛表现误判成 IMU 问题。
