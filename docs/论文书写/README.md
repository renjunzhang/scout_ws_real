...
- [20260916 双层防晃 MPCC 方法与试验章节书写思路](../../docs_for_offlineslosh/验证与论文/20260916_双层防晃MPCC方法与试验章节书写思路.md)：当前两层定位、方法六节、B0 主比较、外部防晃方法、匹配消融及最终 RGB 评价；状态为写作/试验建议，数值协议和结果尚未冻结。
- [spmpc_paper_core](草稿/spmpc_paper_core/README.md) 仍为旧的单层论文核心，后续按上述映射重构；旧五条件矩阵和试验计数不能直接作为当前两层任务的协议。
