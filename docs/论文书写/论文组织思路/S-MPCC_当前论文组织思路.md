...
> 本文档以 `docs/论文书写/草稿/spmpc_paper_core` 的当前定义为准，用于指导完整论文后续写作。旧文档中出现的 `B_ours`、四格因子实验、Reference Governor 核心消融、modal hard cap 和外部 planner 排名，不再属于当前论文主线。
...
7. 液体相位规划实验和实时性统计证明该方法确实是状态相关的在线规划，而不是离线生成固定速度曲线。
...
4. **实验：** 三种匹配内部方法、等完成时间对照、跨容器迁移、相位相关规划和实时性。
...
2. **Container-parameterized prediction and online propagation：** 给出圆柱容器几何到模态参数的明确映射，并基于 odometry 激励传播当前液体状态。
3. **Matched experimental evidence：** 使用三种匹配内部方法、等时间对照、跨容器重参数化和液体相位规划实验区分液体机制、普通平滑和减速效应。

第三项是实验设计贡献或证据结构，不应包装成新的控制理论贡献。
...
- warm start 和失败回退；
- 只执行第一步优化控制；
- OCP 内部约束与最终 executed-command limits 的区别。
...
- Fig. 1：完整闭环架构；
- 主要公式：projection、contour/lag、robot dynamics、liquid parameters、liquid dynamics、state propagation、\(H_{\mathrm{modal}}\)、OCP、stage/slosh cost、bounds 和 first action。
...
本章按四个研究问题组织，而不是分别按 RGB、轨迹、速度、模型和求解器拆章。每个 RQ 内同时使用物理液面、运动、激励、任务表现和计算指标回答一个明确问题。
...
- **RQ4 — State-dependent online planning：** 液体模态状态和相位是否改变未来规划，并且求解是否满足实时期限？
...
这里必须说明 \(H_{\mathrm{modal}}\) 和 \(H_{\mathrm{vis}}\) 是并行的模型与实验响应，不应画成前者导致后者的因果关系。
...
| --- | ---: | ---: | ---: | --- |
| Baseline MPCC | Off | Off | Nominal | 基础 MPCC 对照 |
| Smooth-only MPCC | Off | Off | Enhanced | 通用增强平滑对照 |
| S-MPCC | On | On | Nominal | 本文方法 |
...
不存在 `B_ours`。已有 “S-MPCC + enhanced smoothing” 数据只能作为补充开发结果，不能进入核心主表。
...
可选参数失配组：在 \(C_2\) 上使用 \(C_1\) 参数运行 S-MPCC，增加 8 次，只作为 Supporting Analysis。

RQ4 四相位规划和 RQ3 参数切换属于计算实验，不增加实物次数。

若实验资源不足，可按证据包缩减，但必须同步缩小论文主张：

| 证据包 | 正式实物组成（\(n=8\)） | 次数 | 可以保留的结论 |
| --- | --- | ---: | --- |
| 最小机制包 | \(C_1\) 高风险三方法 + 等时间两方法 | 40 | RQ1 高风险、RQ2、RQ4、实时性 |
| 双路径包 | 最小机制包 + \(C_1\) 低风险三方法 | 64 | 增加两条路径的一致性 |
...
- OCP：第一控制量和预测 \(v,\omega,v_s,H_{\mathrm{modal}}\)；
- 执行：部署层之后的最终命令；
- 激励：\(\dot v\) 和 \(v\omega\)；
...
必须区分预测控制与真实执行。运动和激励指标使用 odometry 与 executed command；预测量只用于机制解释。
...
主要结果固定为 10%–90% 路径进度窗口内的 trial-level RGB p95：
...
### 7.8 4.7 RQ4: State-Dependent Planning and Real-Time Feasibility

#### 4.7.1 Liquid-phase-dependent replanning

固定相同机器人状态和路径，使用四个等能量不同相位的液体初始状态：
...
其中 \(A=0.5\eta_{\mathrm{ref}}\) 是预先冻结的机制研究参数，不应描述成当前代码自动事实。
...
如果相同机器人状态和路径在不同液体相位下产生不同未来计划，就直接支撑“液体动态记忆参与在线决策”。
...
### 7.9 4.8 Supporting Model Consistency and Sensitivity

这不是第五个 RQ，也不能替代 RGB 主结果。本节只做支持性诊断：

- \(H_{\mathrm{modal}}\) 与 \(H_{\mathrm{vis}}\) 的代表性对齐时序；
- vision-normalized signed bias；
- absolute disagreement；
- \(\omega_1,\zeta,c_h\)、初始状态和执行延迟敏感性；
- 可选 \(C_2\) 使用 \(C_1\) 参数的 mismatch 实验。

详细敏感性曲线和数值表放补充材料。正文只保留能够帮助解释模型适用范围的简洁结果。
...
9. 可选 container mismatch 分析；
10. 正文放不下的低风险过程曲线和机制诊断。
...
| 核心判断 | 主要出现位置 | 必要证据 |
| --- | --- | --- |
| 液体具有动态记忆，普通平滑不能显式表示相位 | Introduction、Related Work、RQ4 | 文献定位与四相位规划结果 |
| S-MPCC 把液体状态放入在线 MPCC | Method | 增广状态、传播方程、slosh cost、first action |
...
| 模型可以按容器几何迁移 | RQ3 | \(C_1/C_2\) super-block 与参数切换规划 |
| 方法是真正的状态相关在线规划 | RQ4 | 不同液体相位的不同预测计划与 first action |
...
2. **冻结实验协议：** 三种方法、权重、\(\zeta\)、执行限制、RGB 处理、主要估计量、窗口和失败规则。
3. **完成实验准备：** 冻结两条路径、\(C_2\)、相机标定、随机区组表和正式软件提交。
4. **采集正式数据：** 优先完成最小机制包；资源允许时按完整 88 次方案一次性执行。
5. **填写 RQ1–RQ4：** 先生成原始配对点、过程曲线、效应量和区间，再写结果解释。
6. **反向修改 Introduction 与 Related Work：** 让贡献和主张与实际证据完全一致。
7. **最后写 Abstract 与 Conclusion：** 只使用正式结果中的最关键数字。
8. **同步完整论文：** `spmpc_paper_core` 确定后，再将方法和实验同步到 `spmpc_paper`。
...
- [ ] 10%–90% primary window、2 s rest criterion 和 5 s post-arrival window 已冻结；
- [ ] Smooth-match 只用独立 pilot 的完成时间调节；
- [ ] \(C_2\) 和统一 \(\lambda_H\) 已确定；
- [ ] OCP limits 与 executed-command limits 已分别记录；
...
> S-MPCC 的价值不在于单纯让机器人运动更平滑，也不在于提出新的高保真液体模型。它把一个可实时传播的低阶液体动态状态放入移动底盘的 path-progress MPCC 决策层，使当前液体位移、速度和相位能够改变下一段在线运动计划。论文通过匹配内部对照、等完成时间比较、跨容器重参数化、相位相关规划和实时求解统计，逐层验证这一有限但明确的主张。
