...
Therefore, ordinary local planners are natural same-layer baselines for testing whether online planning without explicit liquid-state propagation is already sufficient for open-liquid transport.
...
Ordinary local planners are therefore useful same-layer comparisons, but they do not directly represent liquid dynamic memory.
...
In contrast, \SPMPC{} inserts a low-order slosh state directly into an online \MPCC{} local-planning problem for a standard wheeled mobile base and resolves a short-horizon optimization problem at every control cycle.
...
\SPMPC{} defines this problem layer by selecting path progress, base control, and predicted liquid response in the same receding-horizon optimization problem.
...
Rather than claiming the first slosh-aware \MPC{} method, the paper integrates low-order slosh modal states, path progress, base-control constraints, control smoothness, and predicted liquid response into one receding-horizon local-planning problem.
The Slosh-risk Reference Governor is likewise confined to this local-planning interface: it scales the speed reference before the \MPCC{} solve and is not a post-\MPCC{} command override, a control-barrier-function safety layer, an RGB-based liquid-surface feedback controller, or a separate OCP with different state dimension.
