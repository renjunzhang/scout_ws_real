...
ControlCycleAuditDebug makeAuditFixture() {
    ...
    audit.timing.expected_publish_stamp_ns = 9500000010;
    audit.timing.publish_deadline_stamp_ns = 9600000010;
    audit.timing.command_publish_stamp_ns = 10000000011;
    audit.timing.estimated_dc_sec = 0.012;
    audit.timing.actual_dc_sec = 0.015;
    audit.timing.dc_error_sec = 0.003;
    ...
    audit.timing.publish_epoch_estimate_valid = true;
    audit.timing.publish_latency_observation_valid = true;
    audit.timing.expected_publish_deadline_missed = false;
    audit.timing.publish_deadline_missed = true;
    audit.timing.state_alignment_status = "INTERPOLATED";
    audit.timing.publish_timing_status = "PUBLISH_DEADLINE_MISSED";
...
TEST(TelemetryEncoderTest, MatchesFrozenControlCycleAuditWireImage) {
    ...
    EXPECT_EQ(4u, msg.schema_version);
    ...
    EXPECT_EQ(9500000010u, msg.expected_publish_stamp.toNSec());
    EXPECT_EQ(9600000010u, msg.publish_deadline_stamp.toNSec());
    EXPECT_DOUBLE_EQ(0.012, msg.estimated_dc_sec);
    EXPECT_DOUBLE_EQ(0.015, msg.actual_dc_sec);
    EXPECT_DOUBLE_EQ(0.003, msg.dc_error_sec);
    EXPECT_TRUE(msg.publish_epoch_estimate_valid);
    EXPECT_TRUE(msg.publish_latency_observation_valid);
    EXPECT_FALSE(msg.expected_publish_deadline_missed);
    EXPECT_TRUE(msg.publish_deadline_missed);
    EXPECT_EQ("PUBLISH_DEADLINE_MISSED", msg.publish_timing_status);
