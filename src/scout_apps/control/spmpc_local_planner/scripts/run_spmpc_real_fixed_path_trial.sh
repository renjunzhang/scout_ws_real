...
case "${SKIP_START_WAIT}" in
  true|false) ;;
  *) fail "SKIP_START_WAIT must be true or false, got '${SKIP_START_WAIT}'" ;;
esac
...
EXECUTION_MODEL_MODE="${EXECUTION_MODEL_MODE:-explicit_actuator}"
EXECUTION_MODEL_LINEAR_DELAY_SEC="${EXECUTION_MODEL_LINEAR_DELAY_SEC:-0.1666666665}"
EXECUTION_MODEL_ANGULAR_DELAY_SEC="${EXECUTION_MODEL_ANGULAR_DELAY_SEC:-0.3333333330}"
EXECUTION_MODEL_LINEAR_TAU_SEC="${EXECUTION_MODEL_LINEAR_TAU_SEC:-0.112}"
EXECUTION_MODEL_ANGULAR_TAU_SEC="${EXECUTION_MODEL_ANGULAR_TAU_SEC:-0.119}"
EXECUTION_MODEL_LINEAR_GAIN="${EXECUTION_MODEL_LINEAR_GAIN:-1.018}"
EXECUTION_MODEL_ANGULAR_GAIN="${EXECUTION_MODEL_ANGULAR_GAIN:-1.096}"
EXECUTION_MODEL_CMD_TIMEOUT_SEC="${EXECUTION_MODEL_CMD_TIMEOUT_SEC:-0.5}"
EXECUTION_MODEL_MAX_PREFIX_PREDICTION_SEC="${EXECUTION_MODEL_MAX_PREFIX_PREDICTION_SEC:-0.20}"
EXECUTION_MODEL_MAX_INTEGRATION_STEP_SEC="${EXECUTION_MODEL_MAX_INTEGRATION_STEP_SEC:-0.01}"
EXECUTION_MODEL_REQUIRE_COMPLETE_HISTORY="${EXECUTION_MODEL_REQUIRE_COMPLETE_HISTORY:-true}"
V_REF="${V_REF:-0.20}"
W_SLOSH="${W_SLOSH:--1.0}"
W_ACCEL="${W_ACCEL:--1.0}"
W_V="${W_V:--1.0}"
W_CONTOUR="${W_CONTOUR:--1.0}"
W_LAG="${W_LAG:--1.0}"
...
LIQUID_NOWCAST_ENABLE="${LIQUID_NOWCAST_ENABLE:-false}"
LIQUID_NOWCAST_PUBLISH_COMPARISON="${LIQUID_NOWCAST_PUBLISH_COMPARISON:-false}"
LIQUID_NOWCAST_MAX_PREDICTION_SEC="${LIQUID_NOWCAST_MAX_PREDICTION_SEC:-0.050}"
LIQUID_NOWCAST_MAX_EXCITATION_AGE_SEC="${LIQUID_NOWCAST_MAX_EXCITATION_AGE_SEC:-0.060}"
LIQUID_NOWCAST_MAX_FUTURE_SKEW_SEC="${LIQUID_NOWCAST_MAX_FUTURE_SKEW_SEC:-0.005}"
LIQUID_NOWCAST_MAX_STATE_EXCITATION_SKEW_SEC="${LIQUID_NOWCAST_MAX_STATE_EXCITATION_SKEW_SEC:-0.001}"
LIQUID_NOWCAST_MAX_INTEGRATION_STEP_SEC="${LIQUID_NOWCAST_MAX_INTEGRATION_STEP_SEC:-0.020}"
...
if truthy "${LIQUID_NOWCAST_ENABLE}" || truthy "${LIQUID_NOWCAST_PUBLISH_COMPARISON}"; then
  # I1 is fed by the accepted processed-IMU pipeline even when O0 remains the
  # nominal solver source.  Reuse the existing stationary READY safety gate.
  IMU_SHADOW_ENABLE=true
fi
...
if truthy "${LIQUID_NOWCAST_ENABLE}"; then
  LIQUID_NOWCAST_ENABLE=true
else
  LIQUID_NOWCAST_ENABLE=false
fi
if truthy "${LIQUID_NOWCAST_PUBLISH_COMPARISON}"; then
  LIQUID_NOWCAST_PUBLISH_COMPARISON=true
else
  LIQUID_NOWCAST_PUBLISH_COMPARISON=false
fi
...
case "${EXECUTION_MODEL_MODE}" in
  explicit_actuator|legacy_instantaneous) ;;
  *) fail "EXECUTION_MODEL_MODE must be explicit_actuator|legacy_instantaneous, got '${EXECUTION_MODEL_MODE}'" ;;
esac
if [[ "${EXECUTION_MODEL_MODE}" == "explicit_actuator" && \
      ( "${DELAY_PHASE_MODE}" == "fixed_closed_loop" || "${DELAY_PHASE_MODE}" == "fixed_robot_only" ) ]]; then
  fail "explicit_actuator cannot be combined with legacy closed-loop delay rollout"
fi
if truthy "${EXECUTION_MODEL_REQUIRE_COMPLETE_HISTORY}"; then
  EXECUTION_MODEL_REQUIRE_COMPLETE_HISTORY=true
else
  EXECUTION_MODEL_REQUIRE_COMPLETE_HISTORY=false
fi
for kv in \
  "GOAL_X=${GOAL_X}" \
  "GOAL_Y=${GOAL_Y}" \
  ...
  "EXECUTION_MODEL_LINEAR_DELAY_SEC=${EXECUTION_MODEL_LINEAR_DELAY_SEC}" \
  "EXECUTION_MODEL_ANGULAR_DELAY_SEC=${EXECUTION_MODEL_ANGULAR_DELAY_SEC}" \
  "EXECUTION_MODEL_LINEAR_TAU_SEC=${EXECUTION_MODEL_LINEAR_TAU_SEC}" \
  "EXECUTION_MODEL_ANGULAR_TAU_SEC=${EXECUTION_MODEL_ANGULAR_TAU_SEC}" \
  "EXECUTION_MODEL_LINEAR_GAIN=${EXECUTION_MODEL_LINEAR_GAIN}" \
  "EXECUTION_MODEL_ANGULAR_GAIN=${EXECUTION_MODEL_ANGULAR_GAIN}" \
  "EXECUTION_MODEL_CMD_TIMEOUT_SEC=${EXECUTION_MODEL_CMD_TIMEOUT_SEC}" \
  "EXECUTION_MODEL_MAX_PREFIX_PREDICTION_SEC=${EXECUTION_MODEL_MAX_PREFIX_PREDICTION_SEC}" \
  "EXECUTION_MODEL_MAX_INTEGRATION_STEP_SEC=${EXECUTION_MODEL_MAX_INTEGRATION_STEP_SEC}" \
  "ALPHA_MAX=${ALPHA_MAX}" \
...
awk -v value="${LIQUID_NOWCAST_MAX_PREDICTION_SEC}" 'BEGIN {exit !(value >= 0.0)}' || \
  fail "LIQUID_NOWCAST_MAX_PREDICTION_SEC must be >= 0"
awk -v value="${LIQUID_NOWCAST_MAX_EXCITATION_AGE_SEC}" 'BEGIN {exit !(value >= 0.0)}' || \
  fail "LIQUID_NOWCAST_MAX_EXCITATION_AGE_SEC must be >= 0"
awk -v value="${LIQUID_NOWCAST_MAX_FUTURE_SKEW_SEC}" 'BEGIN {exit !(value >= 0.0)}' || \
  fail "LIQUID_NOWCAST_MAX_FUTURE_SKEW_SEC must be >= 0"
awk -v value="${LIQUID_NOWCAST_MAX_STATE_EXCITATION_SKEW_SEC}" 'BEGIN {exit !(value >= 0.0)}' || \
  fail "LIQUID_NOWCAST_MAX_STATE_EXCITATION_SKEW_SEC must be >= 0"
awk -v value="${LIQUID_NOWCAST_MAX_INTEGRATION_STEP_SEC}" 'BEGIN {exit !(value > 0.0)}' || \
  fail "LIQUID_NOWCAST_MAX_INTEGRATION_STEP_SEC must be > 0"
if [[ "${LIQUID_NOWCAST_PUBLISH_COMPARISON}" == "true" && "${LIQUID_NOWCAST_ENABLE}" != "true" ]]; then
  fail "LIQUID_NOWCAST_PUBLISH_COMPARISON=true requires LIQUID_NOWCAST_ENABLE=true"
fi
...
planner_cmd=(
  ...
  "slosh_constraint_enable:=${SLOSH_CONSTRAINT_ENABLE:-inherit}"
  ...
  "execution_model_max_prefix_prediction_sec:=${EXECUTION_MODEL_MAX_PREFIX_PREDICTION_SEC}"
  "execution_model_max_integration_step_sec:=${EXECUTION_MODEL_MAX_INTEGRATION_STEP_SEC}"
  "execution_model_require_complete_history:=${EXECUTION_MODEL_REQUIRE_COMPLETE_HISTORY}"
...
run_meta="${RUN_OUT_DIR}/${NAME}_one_click_meta.env"
{
  ...
  echo "execution_model_mode=${EXECUTION_MODEL_MODE}"
  echo "execution_model_linear_delay_sec=${EXECUTION_MODEL_LINEAR_DELAY_SEC}"
  echo "execution_model_angular_delay_sec=${EXECUTION_MODEL_ANGULAR_DELAY_SEC}"
  echo "execution_model_linear_tau_sec=${EXECUTION_MODEL_LINEAR_TAU_SEC}"
  echo "execution_model_angular_tau_sec=${EXECUTION_MODEL_ANGULAR_TAU_SEC}"
  echo "execution_model_linear_gain=${EXECUTION_MODEL_LINEAR_GAIN}"
  echo "execution_model_angular_gain=${EXECUTION_MODEL_ANGULAR_GAIN}"
  echo "execution_model_cmd_timeout_sec=${EXECUTION_MODEL_CMD_TIMEOUT_SEC}"
  echo "execution_model_max_prefix_prediction_sec=${EXECUTION_MODEL_MAX_PREFIX_PREDICTION_SEC}"
  echo "execution_model_max_integration_step_sec=${EXECUTION_MODEL_MAX_INTEGRATION_STEP_SEC}"
  echo "execution_model_require_complete_history=${EXECUTION_MODEL_REQUIRE_COMPLETE_HISTORY}"
  echo "v_ref=${V_REF}"
  ...
  echo "slosh_constraint_enable=${SLOSH_CONSTRAINT_ENABLE:-inherit}"
...
echo "  liquid shadow = nowcast ${LIQUID_NOWCAST_ENABLE}, O0/I0/I1/L22 ${LIQUID_NOWCAST_PUBLISH_COMPARISON}"
if truthy "${IMU_SHADOW_ENABLE}"; then
