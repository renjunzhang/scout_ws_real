...
from spmpc_acados_constraints import set_constraints  # noqa: E402
...
def default_parameter_values(cfg, with_slosh):
...
def build_check(cfg, model_key):
    ...
    sym = MODELS[model_key]["export"]()
    stage = stage_cost_expr(sym, cfg)
    terminal = terminal_cost_expr(sym, cfg)
    p_default = default_parameter_values(cfg, MODELS[model_key]["with_slosh"])
...
def generate(cfg, output_root, model_key):
    ...
    with_slosh = MODELS[model_key]["with_slosh"]
    sym = MODELS[model_key]["export"]()
    cfg["nx"] = sym["nx"]
    ...
    ocp.parameter_values = default_parameter_values(cfg, with_slosh)
