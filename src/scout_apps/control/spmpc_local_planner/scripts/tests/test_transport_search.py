...
from trajectory.search_transport_duration import run_search
...
def test_failed_constrained_baseline_preserves_attempt_and_never_selects_old_plan(tmp_path):
    ...
    out = tmp_path/'search'
    with patch('trajectory.search_transport_duration.run_process', side_effect=failed):
        result = run_search(task_path, baseline, out, [4., 3.5], 1., 5., Path(sys.executable))
...
def test_interrupt_keeps_a_complete_attempt_list_without_executable_output(tmp_path):
    task, baseline = inputs(tmp_path)
    out = tmp_path/'search'
    with patch('trajectory.search_transport_duration.run_process', side_effect=KeyboardInterrupt):
