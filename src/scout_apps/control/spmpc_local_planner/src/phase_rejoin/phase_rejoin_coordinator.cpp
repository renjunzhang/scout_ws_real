...
bool PhaseRejoinCoordinator::validateRuntimeContract(
    ...
    if (!configured_) {
        ...
        error = "ARTIFACT_UNAVAILABLE";
               ...
               std::abs(dt - artifact_.metadata().dt) >
                   params_.artifact_dt_tolerance_sec) {
        error = "DT_MISMATCH";
    } else if (!std::isfinite(path_length) || path_length <= 0.0 ||
               std::abs(path_length - artifact_.metadata().path_length) >
        ...
        if (frame_id.empty() || frame_id != artifact_.metadata().frame_id) {
            error = "FRAME_ID_MISMATCH";
        } else if (!params_.required_frame_id.empty() &&
                   (frame_id != params_.required_frame_id ||
...
PhaseRejoinPreparation PhaseRejoinCoordinator::prepare(
    ...
    bool solver_origin_at_execution_front) const {
    ...
    preparation.nominal_cmd_v = current->u_pub_v;
    preparation.nominal_cmd_omega = current->u_pub_omega;
    preparation.recovery_cmd_v = current->kappa_v;
    preparation.recovery_cmd_omega = current->kappa_omega;
...
PhaseRejoinDecision PhaseRejoinCoordinator::decide(
    ...
    if (solver_success && decision.terminal_gate_accepted) {
        decision.residual_v = clamp(
            solver_output.cmd_v - preparation.nominal_cmd_v,
            -params_.max_residual_v, params_.max_residual_v);
        decision.residual_omega = clamp(
            solver_output.cmd_omega - preparation.nominal_cmd_omega,
            -params_.max_residual_omega, params_.max_residual_omega);
        decision.output_cmd_v = preparation.nominal_cmd_v +
            decision.residual_v;
        decision.output_cmd_omega = preparation.nominal_cmd_omega +
            decision.residual_omega;
        decision.command_intervened =
            std::abs(decision.output_cmd_v - solver_output.cmd_v) > 1e-9 ||
            std::abs(decision.output_cmd_omega - solver_output.cmd_omega) > 1e-9;
