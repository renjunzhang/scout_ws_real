...
2026-09-18 本机接续：原 `planned_slosh` Full 已在开发级 Gazebo 多次到点。首弯停滞主要由低速曲率变化代价过强引起，已将该权重从 0.01 降至 0.0001；当前默认固定为 N40 / 30 Hz（1.33 s），并消除重复尾段参考查询和 TF 查询竞态。ROS1 重编译及 19/19 真实 acados 原生测试通过。完整结果、负例与边界见[Full 定位修复](../../../../docs/实物实验注意事项/对比试验/仿真对比试验分析/02_问题定位与修复/20260918_Full首弯停滞与预算停车定位修复.md)。仍是 `formal=false`，未完成实物验收或证明防晃收益。下文 9 月 16 日的 ROS2 主机记录保留历史语境。
