...
2026-09-17 本机接续：ROS Noetic 整节点编译、21 个 ROS 测试程序（236 个用例）、19 个真实 acados 核心测试和 44 项 Python 检查通过。当前默认在线配置的 Gazebo B0/Full 启动仍被预测一致性与求解预算拦截；完整负结果、移植修复和复现入口见[ROS1 与 Gazebo 验证](../../../../docs/实物实验注意事项/对比试验/仿真对比试验分析/20260917_当前主线ROS1与Gazebo验证.md)。下文 9 月 16 日的 ROS2 主机记录保留历史语境。
