...
主比较是 `raw_mpcc` 与 `planned_slosh`：未经新增路径处理的普通 MPCC 对比完整方法。四组共同使用同一空间区域、运动限制、目标容差、期限和评价窗口。`planned_mpcc` 与 `planned_slosh` 隔离的是**下层液体项**；上层液体贡献需另生成液体目标关闭的计划，不能由这四组直接认定。
