...
[再次复盘](../../../../docs/实物实验注意事项/后续改进/20260916_当前主线复盘与剩余缺口.md)已合并两窗口证据：除停车速度、普通参考端点和回放配置外，还需处理发布时效/改写、定位与液体状态恢复、普通求解失败停车接管，以及启用液面硬上限时的周期内峰值漏检。CAN反馈新鲜度需核对部署驱动。这些尚未修复，前五项回归通过不代表整条运行链已验收。
