...
共同时间戳之外，ROS先按实测运动核对定位创新量，再允许投影更新进度。突跳观测不作为后续锚点；短暂跳变回到原坐标后可恢复，持续重定位需明确重置任务/重启。阈值不依赖贴线误差，不限制正常切角。液体观察器将状态时间增量与积分时长绑定；断流、epoch变化或传播失败后需要可信初始化，滤波READY不能自动解除。实物恢复沿用静置后重启流程，不在运动中自动把液体清零。
...
planned阶段几何使用归一化三次拟合，每阶段速度/进度速度参考保存四个progress knot。raw/geometry的cruise拟合也使用局部归一化坐标，结果转换为既有绝对progress系数供OCP消费；末端和过短窗口使用真实非零路径段，切向与曲率由参考模块统一处理。contour/lag、速度/进度速度、控制平滑、曲率和曲率变化项共同构成弱引导目标。低速曲率使用
...
区域或完整停车启用时，`TaskStopManager` 共享检查当前停车尾段和执行候选首命令后的停车尾段，覆盖真实FIFO、jerk制动、实际运动及quiet后的执行器残余位移。尾段固定当前cell，因而可能保守拒绝本可跨cell完成的制动。候选命令会失去停车余量时，停车模块接管并持续至任务重置；到达状态仍逐拍重查，位姿漂移后撤销成功也不会自动开动。`STOP_FIFO_REGION_VIOLATION` 表示模型中不可改变的FIFO前缀节点已越界，`STOP_REGION_UNSAFE` 只表示本制动策略未通过保守核验。它们都不构成模型误差、求解后延迟或故障发布路径下的实物安全保证。

停车模块复用共同的 `actual_v_min/v_max/omega_max`，核验初态、FIFO、每条制动命令及传播子步中的实际运动；不可改变的FIFO造成越界也会拒绝。无液体的B0停车只使用运动和区域条件。

停车尾段汇总已有RK4各子步完成状态的液面峰值，由同一液面policy决定是否核验cap/freeboard；液体软代价本身不启用硬上限。这仍是离散模型检查，OCP完整预测时域的硬约束仍按节点，不能宣称连续时间保证。普通数值求解失败或计算预算耗尽时，只有本周期状态和已核验尾段有效才进入持续平滑停车；传感/历史/配置错误保持故障分支，过期停车结果也不能发布。

在线RTI以配置次数为上限，按steady clock剩余预算决定继续；单次调用不可抢占，发布前再核验整周期耗时及solver输入epoch年龄。trajectory四组关闭重复shared limiter，并开启改写即拒绝；无输出回放也执行同一命令检查，仅不发送/记录新命令。日志中的rti_iterations记录实际次数，0表示本拍未执行RTI，具体原因看status。
...
作为 modal cost、可选 hard cap 和预测诊断量。准静态转弯抛物面修正只在明确的 governor/可视化口径中使用，不能与 modal 高度混称真实液面。本轮主评价为solver预测实际运动对应的高度、以及IMU实测运动驱动同一模型重建的高度；后者不是直接实测液面。RGB不作为本轮前置条件。B0的原始solver高度仍是缺少液体状态的零占位，须经旁路 `rotating_liquid_replay.py` 使用可信且同epoch的液体初态重建；该评价不反馈B0控制。
