...
共同时间戳尚不保证状态可信：定位前跳可能抬高不可回退的进度；IMU缺口重置液体状态后会自动重新接纳，odom长缺口还可漏掉传播区间。这些恢复合同尚未闭合，见[合并审查](../../../../docs/实物实验注意事项/后续改进/20260916_当前主线复盘与剩余缺口.md)。
...
当前OCP及尾段液面上限主要核验控制网格节点；四子步RK4的中间峰值未纳入该上限检查。普通solver失败仍由ROS直接归零，即使已有停车尾段valid也没有接管；求解后发布还缺有效期门，默认shared limiter可能改写命令。预测检查通过不能代表最终发布动作及连续区间满足全部合同，修复依赖见上述复盘第4～5节。
