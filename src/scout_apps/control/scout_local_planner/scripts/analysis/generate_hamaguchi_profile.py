#!/usr/bin/env python3
"""Compatibility wrapper for the Hamaguchi profile baseline generator.
...
TARGET_REL = Path("scout_profile_baselines/scripts/generate_hamaguchi_profile.py")
...
def main():
    for script in _candidate_scripts():
        if script.is_file():
            ...
            return
    raise RuntimeError(
        "Cannot find scout_profile_baselines/scripts/generate_hamaguchi_profile.py. "
        "Build/source the scout_profile_baselines package and use "
        "`rosrun scout_profile_baselines generate_hamaguchi_profile.py` for the new entrypoint."
    )
