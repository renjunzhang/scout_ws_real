...
class SelectionResult:
    ...
    # fb / takeover 语义码
    fb: int = -1       # -1 未 active; 0 takeover 成功; 1 only original safe; 2 几何可行 slosh 全失败; 3 无可用几何
    takeover: int = 0   # 1 if OSCRS actually changed the published path vs geometry selector
...
class AntiSloshParams:
    ...
    oscrs_score_w_energy_rms: float = 0.3
    oscrs_score_w_eta_dot_rms: float = 0.3
    oscrs_score_w_terminal_E: float = 0.2
