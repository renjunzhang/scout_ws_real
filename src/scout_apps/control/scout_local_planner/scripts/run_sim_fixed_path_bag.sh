...
case "${CONDITION}" in
    ...
    *)
        ...
        echo "Use NOM, PROFILE_ENERGY_GEO, PROFILE_REF_V2, FAS_Q5, FAS_Q5_DOT, FAS_Q10, FAS_Q5_TERM, PROP_Q5, ISR, CUSTOM, RAW_TUNED, GEOREF_TUNED, GEOREF_SLOSH_SCORE, GEOREF_SLOSH_SCORE_TUNED, or GEOREF_ORIGINAL." >&2
        exit 2
        ;;
esac
...
publish_template_goal() {
    rostopic pub -1 "${TEMPLATE_GOAL_TOPIC}" geometry_msgs/PoseStamped "
header:
  frame_id: '${TEMPLATE_GOAL_FRAME}'
pose:
  position:
    x: ${TEMPLATE_GOAL_X}
    y: ${TEMPLATE_GOAL_Y}
