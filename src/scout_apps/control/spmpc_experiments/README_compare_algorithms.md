...
> 当前采取低风险组织方式：Hamaguchi/Lim 已从老控制器目录拆到独立 `scout_profile_baselines` 包；旧 `rosrun scout_local_planner generate_*.py` 入口仅作为兼容 wrapper 保留。LT-DWA 作为 source-only vendored code 放在 `third_party/LT_DWA`，不进入 catkin 主构建。
...
spmpc_experiments/
  ...
  launch/sim/                SPMPC / TEB / DWA / mpc_local_planner sim launch wrappers
  scripts/                   suites / gates / evidence conversion / metrics extraction

scout_profile_baselines/
  scripts/                   Hamaguchi / Lim offline profile generators and helpers
...
third_party/LT_DWA/
                              upstream LT-DWA source-only vendor, adapter pending
```
...
| profile baseline | Hamaguchi-style | `config/profile_baselines/hamaguchi_profile.yaml` | `scout_profile_baselines/scripts/generate_hamaguchi_profile.py` | `scripts/run_fixed_path_profile_baseline_suite.sh` + common tracker | `profile_baseline_metrics_aggregate.csv`、combined comparison | supplementary profile baseline | generator/tracker 不消费 monitor；monitor 只做评价 |
| profile baseline | Lim-style | `config/profile_baselines/lim_profile.yaml` | `scout_profile_baselines/scripts/generate_lim_style_profile.py` | `scripts/run_fixed_path_profile_baseline_suite.sh` + common tracker | `profile_baseline_metrics_aggregate.csv`、combined comparison | supplementary profile baseline | generator/tracker 不消费 monitor；monitor 只做评价 |
| advanced candidate | LT-DWA | `config/benchmark/capability_matrix.yaml` | `third_party/LT_DWA/` source-only vendor；adapter pending | `scripts/bench_check_advanced_baseline_readiness.py` | readiness report | candidate；未通过前不进表 | 不适用 |
...
### TEB/DWA/mpc_local_planner fixed-path
...
`mpc_local_planner` 需要显式 opt-in；未确认依赖与 readiness 前不要放入主表。
...
`scout_local_planner/scripts/analysis/path_profile_utils.py` 继续保留给 TOPPRA/Ruckig/Biagiotti 旧 profile 工具使用；profile-baseline 正式 schema source 以 `scout_profile_baselines/scripts/path_profile_utils.py` 为准。

LT-DWA 当前是 source-only vendor：

```text
third_party/LT_DWA/
```

不要把该目录 symlink 到 `src/`，因为上游包含 `local_planner`、`navigation` 等与本 workspace 冲突的 catkin package 名称。进入可运行对比前需要另做 adapter 和 smoke gate。
