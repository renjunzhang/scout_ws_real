#!/usr/bin/env python3
"""Root lifecycle supervisor and host frame auditor for U3 C1 GenCase v5.
...
WORKSPACE_SUPERVISOR_PATH = WORKSPACE_PACKAGE_ROOT / "scripts/r8_liquid_target_u3_gencase_supervisor_v5.py"
...
EXPORT_FD_ENV = "R8_LIQUID_U3_GENCASE_V5_EXPORT_FD"
EXPORT_SHA256_ENV = "R8_LIQUID_U3_GENCASE_V5_EXPORT_SHA256"
ADMISSION_TOKEN = f"{CASE_ID}:single-gencase-attempt"
SNAPSHOT_ROOT = Path(f"/run/r8-liquid-{CASE_ID}.snapshot")

POLICY_NAME = "liquid_zrj_msi_u2404_u3_gencase_execution_policy_v5.json"
SCHEMA_NAME = "target_host_u3_gencase_execution_policy_v5.json"
PROFILE_NAME = "r8-liquid-u3-gencase-v5.profile"
GATE_NAME = "r8_liquid_target_u3_gencase_gate_v5.py"
HELPER_NAME = "r8_liquid_u3_gencase_bootstrap_helper_v5.py"
SUPERVISOR_NAME = "r8_liquid_target_u3_gencase_supervisor_v5.py"
...
OUTPUT_MAGIC = b"R8C1GENCASEV5\0\0\0"
...
os.write(1,(json.dumps({{"status":"ROOT_GENCASE_V5_SNAPSHOT_CREATED_NOT_EXECUTED","files":manifest}},sort_keys=True,separators=(",",":"))+"\\n").encode("ascii"))
'''
...
g={{"__name__":"__main__"}}
exec(compile(p,"<r8-liquid-gencase-v5-root-snapshot-bootstrap>","exec",dont_inherit=True,optimize=2),g,g)
'''
...
def verify_policy_static() -> dict[str, Any]:
    ...
    if policy.get("status") != "ADMITTED_SINGLE_GENCASE_RUNTIME_V5_AFTER_FROZEN_V11_ZERO_DENIAL_PROBE":
        raise SupervisorError("v5 one-shot admission status differs")
    if policy.get("allowed_gate_commands") != ["self-check", "internal-run"]:
        raise SupervisorError("v5 gate command surface differs")
    ...
    if (probe.get("required_receipt_sha256") != frozen_receipts["execution"][1]
        ...
        raise SupervisorError("v5 policy does not pin the frozen v11 PASS lifecycle")
    predecessor = policy.get("rejected_predecessor", {})
    v4_paths = {
        "execution": (
            Path("/home/zrj/scout_liquid_lab/audits/u3_c1_gencase_v4_20260808T065518Z.execution.json"),
            "356996f3d8c69839be1ba88f818268071f2c6705791b382f550c8261091ba584",
        ),
        "lifecycle": (
            Path("/home/zrj/scout_liquid_lab/audits/u3_c1_gencase_v4_20260808T065518Z.lifecycle_incomplete.json"),
            "5bf3eb525913cf3cfa9fb393e8e0cecc1f99fe60602279f1907320aaa35adb44",
        ),
    }
    v4_documents: dict[str, Any] = {}
    for name, (path, digest) in v4_paths.items():
        raw = read_regular_bytes(path, limit=256 * 1024)
        if sha256_bytes(raw) != digest:
            raise SupervisorError(f"frozen v4 {name} receipt digest differs")
        v4_documents[name] = json.loads(raw.decode("utf-8"))
    v4_execution = v4_documents["execution"]
    v4_lifecycle = v4_documents["lifecycle"]
    v4_audit = v4_execution.get("audit", {})
    denial = v4_audit.get("unexpected_denials", [])
    denial_fields = (
        {field["key"]: field["value"] for field in denial[0].get("fields", [])}
        if len(denial) == 1 else {}
    )
    gate = v4_execution.get("gate", {})
    if (predecessor.get("identity") != "u3_c1_gencase_v4_20260808T065518Z"
            or predecessor.get("execution_receipt_sha256") != v4_paths["execution"][1]
            or predecessor.get("lifecycle_receipt_sha256") != v4_paths["lifecycle"][1]
            or v4_execution.get("status") != "ATTEMPT_ABORTED_NO_RETRY_SAME_IDENTITY"
            or v4_execution.get("primary_error") != "SupervisorError: AppArmor journal window is not a closed zero-denial success window"
            or gate.get("returncode") != 2
            or gate.get("stdout_size_bytes") != 0
            or "transient output set differs: ['C1_static.bi4', 'C1_static.out', 'C1_static.xml']" not in gate.get("stderr_utf8_prefix", "")
            or v4_audit.get("matching_total") != 1
            or v4_audit.get("unexpected_total") != 1
            or len(denial) != 1
            or {key: denial_fields.get(key) for key in ("operation", "name", "comm", "requested_mask", "denied_mask")} != {
                "operation": "open", "name": "/proc/stat", "comm": "GenCase_linux64",
                "requested_mask": "r", "denied_mask": "r",
            }
            or v4_lifecycle.get("status") != "LIFECYCLE_OR_EXECUTION_INCOMPLETE_NO_RETRY_SAME_IDENTITY"
            or v4_lifecycle.get("cleanup_errors") != []
            or v4_lifecycle.get("cleanup_observation", {}).get("post_unload_stable_zero_scans") != [[], [], []]
            or any(v4_lifecycle.get("profiles_after_observation", {}).get("kernel_exact_counts", {}).values())
            or v4_lifecycle.get("sysctls_after_observation") != policy.get("profile_lifecycle", {}).get("expected_sysctls")):
        raise SupervisorError("frozen v4 fail-closed provenance differs")
    ...
    return {
        ...
        "frozen_v4_failure": {name: {"path": str(path), "sha256": digest} for name, (path, digest) in v4_paths.items()},
...
def parse_success_frame(raw: bytes) -> dict[str, Any]:
    ...
    if metadata["document_type"] != "SMPCC_R8_LIQUID_U3_C1_GENCASE_GUEST_FRAME_V5" or metadata["status"] != "GUEST_FRAME_COMPLETE_PENDING_HOST_REAUDIT_AND_O_EXCL_EXPORT":
...
def load_lifecycle_harness() -> Any:
    ...
    spec = importlib.util.spec_from_file_location("_r8_gencase_v5_lifecycle_harness", HARNESS_PATH)
...
def run_once(*, policy_sha256: str, admission_token: str) -> dict[str, Any]:
    ...
    with harness.TerminationGuard() as termination_guard:
        ...
        if policy.get("profile_lifecycle", {}).get("expected_sysctls") != sysctls_before:
            raise SupervisorError("current sysctls differ from the frozen v5 baseline")
        ...
        start_document = {
            "document_type": "SMPCC_R8_LIQUID_U3_C1_GENCASE_V5_START_RECEIPT",
        ...
        try:
            ...
            execution_status = "PASS_U3_C1_GENCASE_V5_FRAME_VALID_CLEANUP_PENDING"
        ...
        if parsed is not None and primary_error is None and not cleanup_errors:
            try:
                ...
                if (not isinstance(export_message, dict) or export_message.get("status") != "PASS_UID1000_O_EXCL_EXPORT_V5"
                ...
                execution_status = "PASS_U3_C1_GENCASE_V5_CASE_EXPORTED_CLEANUP_PENDING"
            except BaseException as exc:
                primary_error = f"{type(exc).__name__}: {exc}"[:4096]
        ...
        execution_document = {
            "document_type": "SMPCC_R8_LIQUID_U3_C1_GENCASE_V5_EXECUTION_RECEIPT",
        ...
        success = (
            execution_document["status"] == "PASS_U3_C1_GENCASE_V5_CASE_EXPORTED_CLEANUP_PENDING"
            and not cleanup_errors and primary_error is None and parsed is not None
            and export_verification is not None
        )
        if success:
            lifecycle_document = {
                "document_type": "SMPCC_R8_LIQUID_U3_C1_GENCASE_V5_LIFECYCLE_RECEIPT",
                "case_id": CASE_ID, "status": "PASS_U3_C1_GENCASE_V5_LIFECYCLE_CLEANUP_AND_CASE_EXPORT",
        ...
        failure_document = {
            "document_type": "SMPCC_R8_LIQUID_U3_C1_GENCASE_V5_LIFECYCLE_INCOMPLETE_RECEIPT",
        ...
        raise SupervisorError(f"v5 attempt failed closed; preserved {failure_record['path']}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if arguments.command == "self-check":
            ...
            print(json.dumps({
                "status": "PASS_V5_SUPERVISOR_STATIC_ADMITTED_EXECUTION_NOT_PERFORMED",
                "review": review,
                "snapshot_bootstrap": {
        ...
        if arguments.command == "export-frame":
            ...
            print(json.dumps({"status": "PASS_UID1000_O_EXCL_EXPORT_V5", "export": result}, ensure_ascii=False, sort_keys=True))
            return 0
