...
GATE_NAME = "r8_liquid_target_u3_proc_stat_boundary_probe_gate_v5.py"
...
ROOT="/run"
NAME="r8-liquid-u3_proc_stat_apparmor_boundary_probe_v5_20260809T042751Z.snapshot"
SOURCES=(
 ("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/scripts/r8_liquid_target_u3_proc_stat_boundary_probe_gate_v5.py","r8_liquid_target_u3_proc_stat_boundary_probe_gate_v5.py"),
 ("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/scripts/r8_liquid_target_u3_proc_stat_boundary_probe_supervisor_v5.py","r8_liquid_target_u3_proc_stat_boundary_probe_supervisor_v5.py"),
 ("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/config/apparmor_drafts/r8-liquid-u3-proc-stat-boundary-probe-v5.profile","r8-liquid-u3-proc-stat-boundary-probe-v5.profile"),
 ("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/schema/target_host_u3_proc_stat_boundary_probe_policy_v5.json","target_host_u3_proc_stat_boundary_probe_policy_v5.json"),
 ("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/config/target_hosts/liquid_zrj_msi_u2404_u3_proc_stat_boundary_probe_policy_v5.json","liquid_zrj_msi_u2404_u3_proc_stat_boundary_probe_policy_v5.json"),
)
def open_abs(path):
...
os.write(1,(json.dumps({"status":"ROOT_PROC_STAT_PROBE_V5_SNAPSHOT_CREATED_NOT_EXECUTED","files":manifest},sort_keys=True,separators=(",",":"))+"\n").encode("ascii"))
'''
...
g={{"__name__":"__main__"}}
exec(compile(p,"<r8-liquid-root-proc-stat-probe-bootstrap-v5>","exec",dont_inherit=True,optimize=2),g,g)
'''
...
def verify_v4_runtime_no_go_provenance() -> dict[str, Any]:
    """Independently verify v4's consumed NO-GO evidence and closed journal."""
    ...
    if (
        ...
        raise SupervisorError("frozen v4 runtime NO-GO snapshot root metadata differs")
    expected_names = {record["name"] for record in snapshot["files"].values()}
    if set(os.listdir(root)) != expected_names:
        raise SupervisorError("frozen v4 runtime NO-GO snapshot file set differs")

    def read_frozen(path: Path, record: Mapping[str, Any], context: str) -> bytes:
        try:
            descriptor = os.open(path, os.O_RDONLY | os.O_NOFOLLOW | os.O_CLOEXEC)
        except OSError as exc:
            raise SupervisorError(f"frozen v4 {context} is absent or unsafe") from exc
        try:
            ...
            if (
                ...
                raise SupervisorError(f"frozen v4 {context} metadata differs")
            raw = bytearray()
            ...
            if len(value) != record["size_bytes"] or sha256_bytes(value) != record["sha256"]:
                raise SupervisorError(f"frozen v4 {context} bytes differ")
            return value
        finally:
            os.close(descriptor)
    ...
    try:
        ...
        raise SupervisorError("frozen v4 runtime NO-GO snapshot policy JSON differs") from exc
    profiles = frozen["profiles"]
    ...
    if (
        ...
        raise SupervisorError("frozen v4 runtime NO-GO snapshot policy identity differs")
    ...
    for name, record in frozen["receipts"].items():
        ...
        try:
            ...
            raise SupervisorError(
                f"frozen v4 runtime NO-GO receipt JSON differs: {name}"
            ) from exc
        if (
            value.get("probe_id") != frozen["probe_id"]
            or value.get("document_type") != record["document_type"]
            ...
            raise SupervisorError(
                f"frozen v4 runtime NO-GO receipt semantics differ: {name}"
            )
        receipts[name] = value
        observed_receipts[name] = {
            "path": str(path), "sha256": record["sha256"],
    ...
    for path_value in frozen["other_receipt_paths"].values():
        try:
            ...
            continue
        raise SupervisorError(
            f"frozen v4 non-authoritative receipt must remain absent: {path_value}"
        )
    ...
    if (
        ...
        raise SupervisorError("frozen v4 runtime NO-GO receipt chain or evidence differs")
    return {
        "decision": frozen["decision"],
        "snapshot": {
            "root": str(root), "owner": list(snapshot["owner"]),
...
def verify_full_authority_delta(candidate_lines: tuple[str, ...]) -> dict[str, Any]:
    ...
    if normalized_digest != EXPECTED_NORMALIZED_PROFILE_LINES_SHA256:
        raise SupervisorError("normalized v5 full-profile digest differs")
...
def verify_repository_path_contract(source_path: Path | None = None) -> dict[str, Any]:
    ...
    if (
        ...
        raise SupervisorError("supervisor name must be a fresh v5 literal")
...
def repository_static_review() -> dict[str, Any]:
    ...
    if policy.get("provenance") != expected_provenance:
        raise SupervisorError(
            "frozen v6 and proc-stat v1-v4 NO-GO provenance policy differs"
        )
    ...
    if (
            payload.get("frame_magic_hex") != "523850524f435354415450524f42453500"
...
def verify_snapshot_producer_identity() -> dict[str, Any]:
    ...
    if SCRIPT_PATH != WORKSPACE_SUPERVISOR_PATH:
        raise SupervisorError("snapshot producer must be the exact workspace proc-stat v5 supervisor")
...
def verify_snapshot(policy_sha256: str) -> tuple[dict[str, Any], dict[str, dict[str, Any]]]:
    ...
    if (
        policy.get("status") != "STATIC_READY_V5_PENDING_INDEPENDENT_REVIEW_AND_EXPLICIT_SINGLE_ATTEMPT"
    ...
    if policy.get("provenance") != {
        ...
        raise SupervisorError("snapshot v6 and proc-stat v1-v4 NO-GO provenance differs")
    ...
    verify_v4_runtime_no_go_provenance()
...
def labeled_processes(proc_root: Path = Path("/proc")) -> list[dict[str, Any]]:
    """Authoritatively enumerate every task/thread carrying a fresh v5 label."""
...
def classify_execution(result: Mapping[str, Any], audit: Mapping[str, Any]) -> tuple[str, dict[str, Any]]:
    ...
    if (result.get("failure") is None and type(returncode) is int and returncode == 0
        ...
        return "PASS_U3_PROC_STAT_APPARMOR_BOUNDARY_PROBE_V5_CLEANUP_PENDING", evidence
...
def execution_receipt_document(
    *,
    ...
    return {
        "document_type": "SMPCC_R8_LIQUID_U3_PROC_STAT_APPARMOR_BOUNDARY_PROBE_V5_EXECUTION_RECEIPT",
        "probe_id": PROBE_ID,
        "status": status,
...
def lifecycle_document(
    *,
    ...
    return {
        "document_type": "SMPCC_R8_LIQUID_U3_PROC_STAT_APPARMOR_BOUNDARY_PROBE_V5_LIFECYCLE_RECEIPT",
        "probe_id": PROBE_ID,
...
def run_once(*, policy_sha256: str, admission_token: str) -> dict[str, Any]:
    with TerminationGuard() as termination_guard:
        try:
            ...
            document = {
                "document_type": "SMPCC_R8_LIQUID_U3_PROC_STAT_APPARMOR_BOUNDARY_PROBE_V5_PREFLIGHT_FAILURE_RECEIPT",
                "probe_id": PROBE_ID,
                "status": (
...
def _run_once_guarded(
    *,
    ...
    start_document = {
        "document_type": "SMPCC_R8_LIQUID_U3_PROC_STAT_APPARMOR_BOUNDARY_PROBE_V5_START_RECEIPT",
        "probe_id": PROBE_ID,
        "status": "ONE_SHOT_STARTED_NO_RETRY_CLEANUP_REQUIRED",
        "policy_sha256": policy_sha256,
        "snapshot": snapshot,
    ...
    failure = {
        "document_type": "SMPCC_R8_LIQUID_U3_PROC_STAT_APPARMOR_BOUNDARY_PROBE_V5_LIFECYCLE_INCOMPLETE_RECEIPT",
        "probe_id": PROBE_ID,
...
def recover_only(*, policy_sha256: str, recovery_token: str) -> dict[str, Any]:
    ...
    with TerminationGuard() as termination_guard:
        ...
        document = {
            "document_type": "SMPCC_R8_LIQUID_U3_PROC_STAT_APPARMOR_BOUNDARY_PROBE_V5_RECOVERY_RECEIPT",
            "probe_id": PROBE_ID,
            "status": "PASS_U3_PROC_STAT_APPARMOR_BOUNDARY_PROBE_V5_CLEANUP_ONLY_NO_PROBE_EXECUTED",
            "policy_sha256": policy_sha256,
            "snapshot": snapshot,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if arguments.command == "recover-only":
            ...
            print(json.dumps({"status": "PROC_STAT_BOUNDARY_PROBE_V5_CLEANUP_ONLY_RECORDED", "result": result}, ensure_ascii=False, sort_keys=True))
            return 0
        result = run_once(policy_sha256=arguments.policy_sha256, admission_token=arguments.admission_token)
        print(json.dumps({"status": "PROC_STAT_BOUNDARY_PROBE_V5_ONE_SHOT_LIFECYCLE_RECORDED", "result": result}, ensure_ascii=False, sort_keys=True))
        return 0 if result["execution_status"] == "PASS_U3_PROC_STAT_APPARMOR_BOUNDARY_PROBE_V5_CLEANUP_PENDING" else 2
