#!/usr/bin/env python3
"""Root lifecycle supervisor for one fresh U3 C1M cold-A settle v6.
...
WORKSPACE_SUPERVISOR_PATH = WORKSPACE_PACKAGE_ROOT / "scripts/r8_liquid_target_u3_solver_cpu_settle_cold_a_supervisor_v6.py"
...
EXPORT_FD_ENV = "R8_LIQUID_U3_SOLVER_CPU_SETTLE_COLD_A_V6_EXPORT_FD"
EXPORT_SHA256_ENV = "R8_LIQUID_U3_SOLVER_CPU_SETTLE_COLD_A_V6_EXPORT_SHA256"
ADMISSION_TOKEN = (
...
OUTPUT_MAGIC = b"R8SOLVEROUTV6\0\0\0"
OUTPUT_VERSION = 6
    ...
    *,
    ...
    for source, name in sources:
        if not isinstance(source, str) or not isinstance(name, str):
            raise SupervisorError(f"v6 {context} source reference type differs")
        if any(marker in source for marker in CONSUMED_ATTEMPT_SOURCE_MARKERS) or any(
            ...
            raise SupervisorError(
                f"consumed v4/v5 identity or transient snapshot appeared in v6 {context}"
            )
...
os.write(1,(json.dumps({{"status":"ROOT_SOLVER_CPU_SETTLE_COLD_A_V6_SNAPSHOT_CREATED_NOT_EXECUTED","files":manifest}},sort_keys=True,separators=(",",":"))+"\\n").encode("ascii"))
'''
...
g={{"__name__":"__main__"}}
exec(compile(p,"<r8-liquid-solver-cpu-settle-cold-a-v6-root-snapshot-bootstrap>","exec",dont_inherit=True,optimize=2),g,g)
'''
...
def verify_policy_static() -> dict[str, Any]:
    ...
    gate = load_module(GATE_PATH, "_r8_c1m_solver_cpu_settle_cold_a_v6_static_gate")
    helper_module = load_module(
        HELPER_PATH, "_r8_c1m_solver_cpu_settle_cold_a_v6_static_helper"
    )
    if (
        ...
        raise SupervisorError("v6 campaign identity differs across runtime artifacts")
...
def parse_success_frame(raw: bytes) -> dict[str, Any]:
    ...
    if (
        metadata["document_type"] != "SMPCC_R8_LIQUID_U3_C1M_CPU_SOLVER_SETTLE_COLD_A_GUEST_FRAME_V6"
        or metadata["status"] != "GUEST_SOLVER_V6_COLD_A_SETTLE_COMPLETE_PENDING_HOST_REAUDIT_AND_O_EXCL_EXPORT"
    ...
    gate = load_module(GATE_PATH, "_r8_c1m_solver_cpu_settle_cold_a_v6_frame_gate")
    ...
    expected_inputs = {}
    helper = load_module(HELPER_PATH, "_r8_c1m_solver_cpu_settle_cold_a_v6_frame_helper")
...
def load_lifecycle_harness() -> Any:
    ...
    module = load_module(HARNESS_PATH, "_r8_c1m_solver_cpu_settle_cold_a_v6_lifecycle_harness")
...
def run_once(*, policy_sha256: str, admission_token: str) -> dict[str, Any]:
    ...
    gate = load_module(GATE_PATH, "_r8_c1m_solver_cpu_settle_cold_a_v6_runtime_gate")
    with harness.TerminationGuard() as termination_guard:
        ...
        start_document = {
            "document_type": "SMPCC_R8_LIQUID_U3_C1M_CPU_SOLVER_SETTLE_COLD_A_V6_START_RECEIPT",
            "case_id": CASE_ID,
        ...
        execution_status = "V6_COLD_A_ATTEMPT_ABORTED_CLEANUP_PENDING"
        try:
            ...
            execution_status = "PASS_U3_C1M_CPU_SOLVER_SETTLE_COLD_A_V6_FRAME_VALID_CLOSED_ZERO_UNEXPECTED_LOGGED_DENIALS_TWO_KNOWN_QUIET_DENY_RULES_CLEANUP_PENDING"
        ...
        if parsed is not None and primary_error is None and not cleanup_errors:
            try:
                ...
                if message.get("status") != "PASS_UID1000_SOLVER_SETTLE_COLD_A_TREE_O_EXCL_EXPORT_V6" or not isinstance(message.get("export"), dict):
                ...
                execution_status = "PASS_U3_C1M_CPU_SOLVER_SETTLE_COLD_A_V6_EXPORTED_CLOSED_ZERO_UNEXPECTED_LOGGED_DENIALS_TWO_KNOWN_QUIET_DENY_RULES_CLEANUP_PENDING"
            except BaseException as exc:
                primary_error = f"{type(exc).__name__}: {exc}"[:4096]
        ...
        execution_document = {
            "document_type": "SMPCC_R8_LIQUID_U3_C1M_CPU_SOLVER_SETTLE_COLD_A_V6_EXECUTION_RECEIPT",
            "case_id": CASE_ID,
            "campaign_id": CAMPAIGN_ID,
            "campaign_role": "cold_a",
            "status": execution_status if primary_error is None and not cleanup_errors else "V6_COLD_A_ATTEMPT_ABORTED_NO_RETRY_SAME_IDENTITY",
            "policy_sha256": policy_sha256,
        ...
        success = execution_document["status"] == "PASS_U3_C1M_CPU_SOLVER_SETTLE_COLD_A_V6_EXPORTED_CLOSED_ZERO_UNEXPECTED_LOGGED_DENIALS_TWO_KNOWN_QUIET_DENY_RULES_CLEANUP_PENDING" and parsed is not None and export_verification is not None
        if success:
            lifecycle_document = {
                "document_type": "SMPCC_R8_LIQUID_U3_C1M_CPU_SOLVER_SETTLE_COLD_A_V6_LIFECYCLE_RECEIPT",
                "case_id": CASE_ID,
                "campaign_id": CAMPAIGN_ID,
                "campaign_role": "cold_a",
                "status": "PASS_U3_C1M_CPU_SOLVER_SETTLE_COLD_A_V6_LIFECYCLE_CLEANUP_EXPORT_CLOSED_ZERO_UNEXPECTED_LOGGED_DENIALS_TWO_KNOWN_QUIET_DENY_RULES",
        ...
        failure_document = {
            "document_type": "SMPCC_R8_LIQUID_U3_C1M_CPU_SOLVER_SETTLE_COLD_A_V6_LIFECYCLE_INCOMPLETE_RECEIPT",
            "case_id": CASE_ID,
            "campaign_id": CAMPAIGN_ID,
            "campaign_role": "cold_a",
        ...
        raise SupervisorError(f"solver cold A v6 failed closed; preserved {failure_record['path']}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if arguments.command == "self-check":
            ...
            print(json.dumps({
                "status": "PASS_SOLVER_CPU_SETTLE_COLD_A_V6_SUPERVISOR_STATIC_EXECUTION_NOT_PERFORMED",
                "review": review,
                "snapshot_bootstrap": {
        ...
        if arguments.command == "export-frame":
            ...
            print(json.dumps({"status": "PASS_UID1000_SOLVER_SETTLE_COLD_A_TREE_O_EXCL_EXPORT_V6", "export": result}, ensure_ascii=False, sort_keys=True))
            return 0
        result = run_once(policy_sha256=arguments.policy_sha256, admission_token=arguments.admission_token)
        print(json.dumps({"status": "SOLVER_CPU_SETTLE_COLD_A_V6_ONE_SHOT_LIFECYCLE_RECORDED", "result": result}, ensure_ascii=False, sort_keys=True))
        return 0
        ...
        print(json.dumps({"status": "SOLVER_CPU_SETTLE_COLD_A_V6_NO_GO", "error": str(exc)}, ensure_ascii=False, sort_keys=True), file=target)
        return 2
