#!/usr/bin/env python3
"""Root lifecycle supervisor for the one-shot harmless v10 successor probe.
...
WORKSPACE_SUPERVISOR_PATH = WORKSPACE_PACKAGE_ROOT / "scripts/r8_liquid_target_u3_stdio_apparmor_transport_probe_supervisor_v10.py"
PROBE_ID = "u3_stdio_apparmor_transport_probe_v10_20260808T045130Z"
SNAPSHOT_ROOT = Path(f"/run/r8-liquid-{PROBE_ID}.snapshot")
SNAPSHOT_ENV = "R8_LIQUID_U3_STDIO_PROBE_V10_SNAPSHOT"
ADMISSION_ENV = "R8_LIQUID_U3_STDIO_PROBE_V10_ADMISSION"
ADMISSION_FD_ENV = "R8_LIQUID_U3_STDIO_PROBE_V10_ADMISSION_FD"
ADMISSION_SHA256_ENV = "R8_LIQUID_U3_STDIO_PROBE_V10_ADMISSION_SHA256"
...
POLICY_NAME = "liquid_zrj_msi_u2404_u3_stdio_apparmor_transport_probe_policy_v10.json"
SCHEMA_NAME = "target_host_u3_stdio_apparmor_transport_probe_policy_v10.json"
PROFILE_NAME = "r8-liquid-u3-stdio-transport-probe-v10.profile"
GATE_NAME = "r8_liquid_target_u3_stdio_apparmor_transport_probe_gate_v10.py"
SUPERVISOR_NAME = SCRIPT_PATH.name
BOOTSTRAP_PROFILE = "r8-liquid-u3-stdio-transport-bootstrap-v10-20260808t045130z"
RUNTIME_PROFILE = "r8-liquid-u3-stdio-transport-runtime-v10-20260808t045130z"
...
SUCCESS_MAGIC = b"R8LQSTDIOPASSV10\x00"
...
EVIDENCE_CLASSIFICATION_CONTRACT = {
    ...
    "success_pass_requires": "returncode_zero_empty_stderr_byte_exact_success_frame_and_zero_fresh_denials_in_closed_boot_cursor_window",
...
EXPECTED_INPUTS = {
    "probe_alpha.bin": {"size_bytes": 275, "sha256": "c45c746f5ced85584fabb1aaf6664c8b4f2a2be8576079d0fc6ad483ffa959f0"},
    "probe_beta.bin": {"size_bytes": 520, "sha256": "fee4d99d59419d5d1bd2e7b3dc680365fd6f0d533a380688c84553ab827d55f9"},
    "probe_gamma.bin": {"size_bytes": 993, "sha256": "3550f5309f79594137d63a09a2f803f35cddb7a03b6f2eb693d92f2d3fe070f4"},
}
...
ROOT="/run"
NAME="r8-liquid-u3_stdio_apparmor_transport_probe_v10_20260808T045130Z.snapshot"
SOURCES=(
 ("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/scripts/r8_liquid_target_u3_stdio_apparmor_transport_probe_gate_v10.py","r8_liquid_target_u3_stdio_apparmor_transport_probe_gate_v10.py"),
 ("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/scripts/r8_liquid_target_u3_stdio_apparmor_transport_probe_supervisor_v10.py","r8_liquid_target_u3_stdio_apparmor_transport_probe_supervisor_v10.py"),
 ("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/config/apparmor_drafts/r8-liquid-u3-stdio-transport-probe-v10.profile","r8-liquid-u3-stdio-transport-probe-v10.profile"),
 ("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/schema/target_host_u3_stdio_apparmor_transport_probe_policy_v10.json","target_host_u3_stdio_apparmor_transport_probe_policy_v10.json"),
 ("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/config/target_hosts/liquid_zrj_msi_u2404_u3_stdio_apparmor_transport_probe_policy_v10.json","liquid_zrj_msi_u2404_u3_stdio_apparmor_transport_probe_policy_v10.json"),
)
def open_abs(path):
...
os.write(1,(json.dumps({"status":"ROOT_SNAPSHOT_V10_CREATED_NOT_EXECUTED","files":manifest},sort_keys=True,separators=(",",":"))+"\n").encode("ascii"))
'''
...
g={{"__name__":"__main__"}}
exec(compile(p,"<r8-liquid-root-snapshot-bootstrap-v10>","exec",dont_inherit=True,optimize=2),g,g)
'''
...
def repository_static_review() -> dict[str, Any]:
    ...
    if policy.get("status") != "STATIC_READY_V10_PENDING_INDEPENDENT_REVIEW_AND_EXPLICIT_SINGLE_ATTEMPT":
        raise SupervisorError("v10 static status differs")
    ...
    if transport.get("success_magic_hex") != SUCCESS_MAGIC.hex():
        raise SupervisorError("fresh v10 success protocol magic differs")
    expected_frozen = {
        "probe_id": PROBE_ID,
    ...
    if (mount_policy.get("proc_rules") != [PROC_MOUNT_RULE]
            ...
            or mount_policy.get("zero_fresh_denials_required") is not True
            or mount_policy.get("rules_may_be_guessed") is not False
            or mount_policy.get("success_frame_required") is not True):
        raise SupervisorError("v10 evidence-derived proc success admission contract differs")
    if policy.get("evidence_classification") != EVIDENCE_CLASSIFICATION_CONTRACT:
        raise SupervisorError("v10 zero-denial success evidence classification contract differs")
    ...
    return {"policy_sha256": sha256_bytes(policy_raw), "policy_size_bytes": len(policy_raw), "artifacts": observed, "bootstrap_source_sha256": SNAPSHOT_BOOTSTRAP_SHA256, "predecessor_v8": predecessor_v8, "predecessor_v9": predecessor_v9, "execution_performed": False}
...
def verify_snapshot_producer_identity() -> dict[str, Any]:
    ...
    if SCRIPT_PATH != WORKSPACE_SUPERVISOR_PATH:
        raise SupervisorError("snapshot producer must be the exact workspace v10 supervisor")
...
def labeled_processes(proc_root: Path = Path("/proc")) -> list[dict[str, Any]]:
    """Authoritatively enumerate every task/thread carrying a fresh v10 label."""
...
def parse_success_frame(frame: bytes) -> dict[str, Any]:
    ...
    if (value["status"] != "PASS_HARMLESS_STDIO_APPARMOR_TRANSPORT_PROBE_V10"
...
def classify_execution(result: Mapping[str, Any], audit: Mapping[str, Any]) -> tuple[str, dict[str, Any]]:
    ...
    if (result.get("failure") is None and type(returncode) is int and returncode == 0
        ...
        return "PASS_HARMLESS_STDIO_APPARMOR_TRANSPORT_PROBE_V10_CLEANUP_PENDING", evidence
    # Revision v10 promotes only a byte-exact success frame inside a boot/cursor
    # bound window with zero fresh denials for either unique v10 label.
...
def execution_receipt_document(
    *,
    ...
    return {
        "document_type": "SMPCC_R8_LIQUID_U3_STDIO_APPARMOR_PROBE_V10_EXECUTION_RECEIPT",
        "probe_id": PROBE_ID,
        "status": status,
...
def lifecycle_document(
    *,
    ...
    return {
        "document_type": "SMPCC_R8_LIQUID_U3_STDIO_APPARMOR_PROBE_V10_LIFECYCLE_RECEIPT",
        "probe_id": PROBE_ID,
        "status": "PASS_V10_PROFILE_PROCESS_SYSCTL_SUDO_LIFECYCLE_CLEANUP",
...
def run_once(*, policy_sha256: str, admission_token: str) -> dict[str, Any]:
    with TerminationGuard() as termination_guard:
        try:
            ...
            document = {
                "document_type": "SMPCC_R8_LIQUID_U3_STDIO_APPARMOR_PROBE_V10_PREFLIGHT_FAILURE_RECEIPT",
                "probe_id": PROBE_ID,
                "status": (
...
def _run_once_guarded(
    *,
    ...
    start_document = {
        "document_type": "SMPCC_R8_LIQUID_U3_STDIO_APPARMOR_PROBE_V10_START_RECEIPT",
        "probe_id": PROBE_ID,
    ...
    failure = {
        "document_type": "SMPCC_R8_LIQUID_U3_STDIO_APPARMOR_PROBE_V10_LIFECYCLE_INCOMPLETE_RECEIPT",
        "probe_id": PROBE_ID,
    ...
    raise SupervisorError(f"v10 lifecycle incomplete; preserved receipt {failure_record['path']}")
...
def recover_only(*, policy_sha256: str, recovery_token: str) -> dict[str, Any]:
    ...
    with TerminationGuard() as termination_guard:
        ...
        document = {
            "document_type": "SMPCC_R8_LIQUID_U3_STDIO_APPARMOR_PROBE_V10_RECOVERY_RECEIPT",
            "probe_id": PROBE_ID,
            "status": "PASS_V10_CLEANUP_ONLY_NO_PROBE_EXECUTED",
            "policy_sha256": policy_sha256,
            "snapshot": snapshot,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if arguments.command == "recover-only":
            ...
            print(json.dumps({"status": "V10_CLEANUP_ONLY_RECORDED", "result": result}, ensure_ascii=False, sort_keys=True))
            return 0
        result = run_once(policy_sha256=arguments.policy_sha256, admission_token=arguments.admission_token)
        print(json.dumps({"status": "V10_ONE_SHOT_LIFECYCLE_RECORDED", "result": result}, ensure_ascii=False, sort_keys=True))
        return 0 if result["execution_status"] == "PASS_HARMLESS_STDIO_APPARMOR_TRANSPORT_PROBE_V10_CLEANUP_PENDING" else 2
