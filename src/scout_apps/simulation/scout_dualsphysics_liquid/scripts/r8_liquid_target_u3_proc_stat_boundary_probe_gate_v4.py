...
PAYLOAD_MAGIC = b"R8PROCSTATPROBE4\x00"
...
exec(compile(p,"<r8-proc-stat-boundary-helper-v4>","exec",dont_inherit=True,optimize=2),g,g)
'''
...
def bwrap_argv() -> list[str]:
    return [
        ...
        "--hostname", "r8-liquid-proc-stat-v4", "--clearenv",
...
def verify_full_authority_delta(candidate_lines: tuple[str, ...]) -> dict[str, Any]:
    ...
    if normalized_digest != EXPECTED_NORMALIZED_PROFILE_LINES_SHA256:
        raise GateError("normalized v4 full-profile digest differs")
...
def verify_supervisor_repository_path_contract(
    ...
    if (
        ...
        raise GateError("supervisor name must be a fresh v4 literal")
...
def verify_static() -> dict[str, Any]:
    ...
    if policy.get("provenance") != expected_provenance:
        raise GateError("frozen v6 and proc-stat v1-v3 NO-GO provenance policy differs")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if arguments.command == "self-check":
            ...
            print(json.dumps({"status": f"PASS_U3_PROC_STAT_APPARMOR_BOUNDARY_PROBE_V4_{state}_EXECUTION_NOT_PERFORMED", "review": review}, ensure_ascii=False, sort_keys=True))
            return 0
