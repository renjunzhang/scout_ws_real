...
SNAPSHOT_ENV = "R8_LIQUID_U3_PROC_STAT_BOUNDARY_PROBE_V4_SNAPSHOT"
ADMISSION_ENV = "R8_LIQUID_U3_PROC_STAT_BOUNDARY_PROBE_V4_ADMISSION"
ADMISSION_FD_ENV = "R8_LIQUID_U3_PROC_STAT_BOUNDARY_PROBE_V4_ADMISSION_FD"
ADMISSION_SHA256_ENV = "R8_LIQUID_U3_PROC_STAT_BOUNDARY_PROBE_V4_ADMISSION_SHA256"
...
POLICY_NAME = "liquid_zrj_msi_u2404_u3_proc_stat_boundary_probe_policy_v4.json"
SCHEMA_NAME = "target_host_u3_proc_stat_boundary_probe_policy_v4.json"
PROFILE_NAME = "r8-liquid-u3-proc-stat-boundary-probe-v4.profile"
GATE_NAME = "r8_liquid_target_u3_proc_stat_boundary_probe_gate_v4.py"
SUPERVISOR_NAME = "r8_liquid_target_u3_proc_stat_boundary_probe_supervisor_v4.py"
WORKSPACE_SUPERVISOR_PATH = WORKSPACE_ROOT / "scripts" / SUPERVISOR_NAME
BOOTSTRAP_PROFILE = "r8-liquid-u3-proc-stat-boundary-bootstrap-v4-20260809t031752z"
RUNTIME_PROFILE = "r8-liquid-u3-proc-stat-boundary-runtime-v4-20260809t031752z"
...
SUCCESS_MAGIC = b"R8PROCSTATPASSV4\x00"
FAILURE_MAGIC = b"R8PROCSTATFAILV4\x00"
...
PROBE_ERROR_CODES = (
    ...
    "leader_absent_from_group_scan", "overflow_owner", "status_other_errno",
    "status_unexpectedly_readable", "pr_set_dumpable", "pr_get_dumpable",
    "child_identity_keys", "child_identity_boundary",
    "live_stat_not_non_owner_root_projection", "child_waitid_wnowait",
...
AUDIT_LOSS_PATTERNS = (
    ("KAUDITD_CALLBACKS_SUPPRESSED", re.compile(r"\bkauditd_printk_skb:\s+[1-9][0-9]*\s+callbacks suppressed\b")),
    ("AUDIT_BACKLOG_LIMIT_EXCEEDED", re.compile(r"\baudit(?:\[[^]]+\])?:.*\bbacklog limit exceeded\b", re.IGNORECASE)),
    ("AUDIT_RECORDS_LOST", re.compile(
        r"\baudit(?:\[[^]]+\])?:.*\b(?:audit_)?lost=[1-9][0-9]*\b",
        re.IGNORECASE,
    )),
)
...
ROOT="/run"
NAME="r8-liquid-u3_proc_stat_apparmor_boundary_probe_v4_20260809T031752Z.snapshot"
SOURCES=(
 ("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/scripts/r8_liquid_target_u3_proc_stat_boundary_probe_gate_v4.py","r8_liquid_target_u3_proc_stat_boundary_probe_gate_v4.py"),
 ("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/scripts/r8_liquid_target_u3_proc_stat_boundary_probe_supervisor_v4.py","r8_liquid_target_u3_proc_stat_boundary_probe_supervisor_v4.py"),
 ("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/config/apparmor_drafts/r8-liquid-u3-proc-stat-boundary-probe-v4.profile","r8-liquid-u3-proc-stat-boundary-probe-v4.profile"),
 ("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/schema/target_host_u3_proc_stat_boundary_probe_policy_v4.json","target_host_u3_proc_stat_boundary_probe_policy_v4.json"),
 ("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/config/target_hosts/liquid_zrj_msi_u2404_u3_proc_stat_boundary_probe_policy_v4.json","liquid_zrj_msi_u2404_u3_proc_stat_boundary_probe_policy_v4.json"),
)
def open_abs(path):
...
os.write(1,(json.dumps({"status":"ROOT_PROC_STAT_PROBE_V4_SNAPSHOT_CREATED_NOT_EXECUTED","files":manifest},sort_keys=True,separators=(",",":"))+"\n").encode("ascii"))
'''
...
g={{"__name__":"__main__"}}
exec(compile(p,"<r8-liquid-root-proc-stat-probe-bootstrap-v4>","exec",dont_inherit=True,optimize=2),g,g)
'''
...
def verify_v2_prestart_no_go_provenance() -> dict[str, Any]:
    ...
    if (
        ...
        raise SupervisorError("frozen v2 pre-start NO-GO snapshot root metadata differs")
    expected_names = {record["name"] for record in snapshot["files"].values()}
    if set(os.listdir(root)) != expected_names:
        raise SupervisorError("frozen v2 pre-start NO-GO snapshot file set differs")
    observed_files: dict[str, dict[str, Any]] = {}
    for name, record in snapshot["files"].items():
        path = root / record["name"]
        file_metadata = os.lstat(path)
        ...
        if (
            ...
            raise SupervisorError(f"frozen v2 pre-start NO-GO snapshot artifact differs: {name}")
        observed_files[name] = {
            "path": str(path),
            "sha256": record["sha256"],
            "size_bytes": record["size_bytes"],
    ...
    try:
        ...
        raise SupervisorError("frozen v2 pre-start NO-GO snapshot policy JSON differs") from exc
    if (
        ...
        raise SupervisorError("frozen v2 pre-start NO-GO snapshot policy identity differs")
    ...
    try:
        ...
        raise SupervisorError("frozen v2 preflight receipt is absent") from exc
    ...
    if (
        ...
        raise SupervisorError("frozen v2 preflight receipt bytes or inode differ")
    try:
        receipt = json.loads(
        ...
        raise SupervisorError("frozen v2 preflight receipt JSON differs") from exc
    receipt_fields = (
    ...
    if (
        ...
        raise SupervisorError("frozen v2 preflight receipt semantics differ")
    for path in frozen["other_receipt_paths"].values():
        try:
            os.lstat(path)
        except FileNotFoundError:
            continue
        raise SupervisorError(f"frozen v2 non-preflight receipt must remain absent: {path}")
    return {
        "decision": frozen["decision"],
        "snapshot": {
            "root": str(root),
...
def verify_v3_runtime_no_go_provenance() -> dict[str, Any]:
    ...
    if (
        ...
        raise SupervisorError("frozen v3 runtime NO-GO snapshot root metadata differs")
    expected_names = {record["name"] for record in snapshot["files"].values()}
    if set(os.listdir(root)) != expected_names:
        raise SupervisorError("frozen v3 runtime NO-GO snapshot file set differs")
    observed_files: dict[str, dict[str, Any]] = {}
    ...
    for name, record in snapshot["files"].items():
        ...
        if (
            ...
            raise SupervisorError(
                f"frozen v3 runtime NO-GO snapshot artifact differs: {name}"
            )
        observed_files[name] = {
    ...
    try:
        v3_policy = json.loads(
            ...
            object_pairs_hook=lambda pairs: _strict_pairs(
                pairs, "frozen v3 runtime NO-GO policy"
            ),
        )
    except (json.JSONDecodeError, UnicodeError) as exc:
        raise SupervisorError("frozen v3 runtime NO-GO snapshot policy JSON differs") from exc
    ...
    if (
        ...
        raise SupervisorError("frozen v3 runtime NO-GO snapshot policy identity differs")
    ...
    for name, record in frozen["receipts"].items():
        ...
        try:
            ...
            raise SupervisorError(
                f"frozen v3 runtime NO-GO receipt is absent: {name}"
            ) from exc
        raw = read_regular_bytes(path)
        if (
            not stat.S_ISREG(receipt_metadata.st_mode)
            ...
            raise SupervisorError(f"frozen v3 runtime NO-GO receipt differs: {name}")
        try:
            value = json.loads(
                raw.decode("utf-8"),
            ...
            raise SupervisorError(
                f"frozen v3 runtime NO-GO receipt JSON differs: {name}"
            ) from exc
        if (
            ...
            raise SupervisorError(
                f"frozen v3 runtime NO-GO receipt semantics differ: {name}"
            )
        receipts[name] = value
    ...
    for path in frozen["other_receipt_paths"].values():
        try:
            ...
            continue
        raise SupervisorError(
            f"frozen v3 non-authoritative receipt must remain absent: {path}"
        )
    ...
    if (
        ...
        raise SupervisorError("frozen v3 runtime NO-GO receipt chain or evidence differs")
    return {
        "decision": frozen["decision"],
        "snapshot": {
...
def verify_full_authority_delta(candidate_lines: tuple[str, ...]) -> dict[str, Any]:
    ...
    if normalized_digest != EXPECTED_NORMALIZED_PROFILE_LINES_SHA256:
        raise SupervisorError("normalized v4 full-profile digest differs")
...
def _top_level_assignment(tree: ast.Module, name: str) -> ast.expr:
    ...
    if len(matches) != 1:
        raise SupervisorError(f"path-contract assignment count differs: {name}")
    return matches[0]
...
def verify_repository_path_contract(source_path: Path | None = None) -> dict[str, Any]:
    """Review both source AST and evaluated paths without trusting import location."""

    path = SCRIPT_PATH if source_path is None else source_path
    raw = read_regular_bytes(path)
    try:
        tree = ast.parse(raw.decode("utf-8"), filename=str(path))
    except (SyntaxError, UnicodeError) as exc:
        raise SupervisorError("supervisor source is not valid UTF-8 Python") from exc
    ...
    if not _ast_expression_matches(
        ...
        raise SupervisorError("canonical WORKSPACE_ROOT AST differs")
    ...
    if (
        ...
        raise SupervisorError("supervisor name must be a fresh v4 literal")
    ...
    if not _ast_expression_matches(
        ...
        raise SupervisorError("canonical supervisor entrypoint AST differs")

    repository = _top_level_assignment(tree, "REPOSITORY_PATHS")
    if not isinstance(repository, ast.Dict):
        raise SupervisorError("REPOSITORY_PATHS must be an exact dict")
    ...
    for key, value in zip(repository.keys, repository.values):
        if not isinstance(key, ast.Constant) or not isinstance(key.value, str):
            raise SupervisorError("REPOSITORY_PATHS key AST differs")
        if key.value in observed:
            raise SupervisorError("REPOSITORY_PATHS has a duplicate key")
        observed[key.value] = value
    if set(observed) != set(expected_expressions) or any(
        not _ast_expression_matches(observed[name], expression)
        for name, expression in expected_expressions.items()
    ):
        raise SupervisorError("canonical REPOSITORY_PATHS AST differs")
    ...
    if not _ast_expression_matches(
        ...
        raise SupervisorError("fresh SNAPSHOT_PATHS AST differs")
    ...
    expected_snapshot = {
        name: SNAPSHOT_ROOT / repository_path.name
        for name, repository_path in expected_repository.items()
    }
    if REPOSITORY_PATHS != expected_repository:
        raise SupervisorError("evaluated canonical REPOSITORY_PATHS differ")
    if SNAPSHOT_PATHS != expected_snapshot:
        raise SupervisorError("evaluated fresh SNAPSHOT_PATHS differ")
    return {
        "source_path": str(path),
        "source_sha256": sha256_bytes(raw),
        "workspace_root": str(WORKSPACE_ROOT),
        "repository_paths": {
            name: str(repository_path)
            for name, repository_path in expected_repository.items()
        },
        "snapshot_paths": {
            name: str(snapshot_path)
            for name, snapshot_path in expected_snapshot.items()
        },
...
def repository_static_review() -> dict[str, Any]:
    path_contract = verify_repository_path_contract()
    ...
    identities = {
        "schema_version": "smpcc-r8-liquid-target-u3-proc-stat-apparmor-boundary-probe-policy-v4",
        "policy_id": "LIQUID_ZRJ_MSI_U2404_U3_PROC_STAT_APPARMOR_BOUNDARY_PROBE_V4",
    ...
    draft_status = "STATIC_DRAFT_V4_PENDING_REVIEWED_BYTE_HASH_FREEZE_AND_INDEPENDENT_REVIEW"
    ready_status = "STATIC_READY_V4_PENDING_INDEPENDENT_REVIEW_AND_EXPLICIT_SINGLE_ATTEMPT"
    ...
    if policy.get("provenance") != expected_provenance:
        raise SupervisorError(
            "frozen v6 and proc-stat v1-v3 NO-GO provenance policy differs"
        )
    ...
    if policy.get("failure_diagnostics") != failure_diagnostics_contract():
        raise SupervisorError("failure diagnostics contract differs")
    ...
    if (
            payload.get("frame_magic_hex") != "523850524f435354415450524f42453400"
        or payload.get("success_magic_hex") != SUCCESS_MAGIC.hex()
        or payload.get("success_status") != "PASS_U3_PROC_STAT_APPARMOR_BOUNDARY_PROBE_V4"
    ...
    expected_commands = {
        ...
        "snapshot_producer_argv": ["/usr/bin/python3.12", "-I", "-B", str(WORKSPACE_SUPERVISOR_PATH), "emit-bootstrap"],
    ...
    if not frozen_ready and reviewed != {
        "all_frozen": False,
        "helper": {"size_bytes": 0, "sha256": "0" * 64},
        "loader": {"size_bytes": 0, "sha256": "0" * 64},
        "payload_frame": {
            "size_bytes": 0,
            "sha256": "0" * 64,
            "maximum_size_bytes": 65_536,
        },
    }:
        raise SupervisorError("draft reviewed payload-byte placeholders differ")
    ...
    if not frozen_ready and (
        ...
        or bootstrap.get("source_sha256") != "0" * 64
        or bootstrap.get("loader_size_bytes") != 0
        or bootstrap.get("loader_sha256") != "0" * 64
    ):
        raise SupervisorError("draft snapshot bootstrap placeholders differ")
    ...
    for name in ("gate", "supervisor", "profile", "schema"):
        ...
        if not path_matches:
            raise SupervisorError(f"trusted artifact canonical path differs: {name}")
        ...
        if not frozen_ready and (
            ...
            raise SupervisorError(f"draft trusted artifact placeholder differs: {name}")
        observed[name] = {
    ...
    return {
        ...
        "repository_path_contract": path_contract,
...
def verify_snapshot_producer_identity() -> dict[str, Any]:
    ...
    if SCRIPT_PATH != WORKSPACE_SUPERVISOR_PATH:
        raise SupervisorError("snapshot producer must be the exact workspace proc-stat v4 supervisor")
...
def verify_snapshot(policy_sha256: str) -> tuple[dict[str, Any], dict[str, dict[str, Any]]]:
    ...
    if (
        policy.get("status") != "STATIC_READY_V4_PENDING_INDEPENDENT_REVIEW_AND_EXPLICIT_SINGLE_ATTEMPT"
    ...
    if policy.get("provenance") != {
        ...
        raise SupervisorError("snapshot v6 and proc-stat v1-v3 NO-GO provenance differs")
    verify_v6_failure_provenance()
    verify_v1_no_go_provenance()
    verify_v2_prestart_no_go_provenance()
    verify_v3_runtime_no_go_provenance()

    if policy.get("failure_diagnostics") != failure_diagnostics_contract():
        raise SupervisorError("snapshot failure diagnostics contract differs")
    if policy.get("journal_contract") != JOURNAL_CONTRACT:
        raise SupervisorError("snapshot journal boundary contract differs")
...
def labeled_processes(proc_root: Path = Path("/proc")) -> list[dict[str, Any]]:
    """Authoritatively enumerate every task/thread carrying a fresh v4 label."""
...
def parse_success_frame(frame: bytes) -> dict[str, Any]:
    ...
    if value["status"] != "PASS_U3_PROC_STAT_APPARMOR_BOUNDARY_PROBE_V4":
...
def parse_failure_frame(frame: bytes) -> dict[str, str]:
    ...
    if len(frame) != EXACT_FAILURE_FRAME_BYTES or not frame.startswith(FAILURE_MAGIC):
        raise SupervisorError("failure frame size or magic differs")
    size = struct.unpack(">H", frame[prefix_size:payload_offset])[0]
    if not 2 <= size <= FAILURE_PAYLOAD_BYTES:
        raise SupervisorError("failure frame declared length differs")
    payload = frame[payload_offset:payload_offset + size]
    padding = frame[payload_offset + size:payload_offset + FAILURE_PAYLOAD_BYTES]
    if padding != b"\x00" * len(padding):
        raise SupervisorError("failure frame padding differs")
    if hashlib.sha256(payload).digest() != frame[-32:]:
        raise SupervisorError("failure frame digest differs")
    try:
        value = json.loads(payload.decode("ascii"))
    except (json.JSONDecodeError, UnicodeError) as exc:
        raise SupervisorError("failure frame JSON differs") from exc
    canonical = json.dumps(value, sort_keys=True, separators=(",", ":")).encode("ascii")
    if canonical != payload or not isinstance(value, dict):
        raise SupervisorError("failure frame is not canonical")
    if set(value) != {"boundary_status", "failure_code"}:
        raise SupervisorError("failure frame key set differs")
    if value["failure_code"] not in HELPER_FAILURE_CODES:
        raise SupervisorError("failure frame code is not enumerated")
    if value["boundary_status"] not in FAILURE_BOUNDARY_STATUSES:
        raise SupervisorError("failure frame boundary status is not enumerated")
    if not failure_frame_semantics_are_compatible(
        value["failure_code"], value["boundary_status"]
    ):
        raise SupervisorError("failure frame code and boundary status are incompatible")
    return value
...
def capture_apparmor_denials(
    ...
    audit_loss_markers: list[dict[str, Any]] = []
    audit_loss_total = 0
    matching_total = expected_total = stat_denial_total = unexpected_total = 0
    for line in lines[:-1]:
        raw_line = line.encode("utf-8")
        marker_codes = [code for code, pattern in AUDIT_LOSS_PATTERNS if pattern.search(line)]
        if marker_codes:
            audit_loss_total += 1
            if len(audit_loss_markers) < AUDIT_STORAGE_CEILING:
                audit_loss_markers.append({
    ...
    if audit_loss_total:
        errors.append("AUDIT_SUPPRESSION_OR_LOSS_MARKER")
    observed.update({
        ...
        "audit_loss_marker_status": (
            "OBSERVED_FAIL_CLOSED" if audit_loss_total else "NOT_OBSERVED"
        ),
        "audit_loss_marker_matching_total": audit_loss_total,
        "audit_loss_marker_storage_overflow": audit_loss_total > len(audit_loss_markers),
        "audit_loss_markers": audit_loss_markers,
...
def _audit_is_closed_success_window(
    ...
    count_keys = (
        ...
        "raw_stdout_size_bytes", "audit_loss_marker_matching_total",
    )
    ...
    if (audit.get("capture_valid") is not True or audit.get("capture_errors") != []
            ...
            or audit.get("stat_denial_total") != 0
            or audit.get("unexpected_total") != 0
            or audit.get("audit_loss_marker_status") != "NOT_OBSERVED"
            or audit.get("audit_loss_marker_matching_total") != 0
    ...
    if (
        audit.get("pre_run_journal_sync") != expected_sync
        or audit.get("journal_sync") != expected_sync
        or type(audit.get("admission_journal_anchor_sha256")) is not str
        or re.fullmatch(r"[0-9a-f]{64}", audit["admission_journal_anchor_sha256"]) is None
    ):
        return False
...
def classify_execution(result: Mapping[str, Any], audit: Mapping[str, Any]) -> tuple[str, dict[str, Any]]:
    ...
    evidence: dict[str, Any] = {
        "capture_status": (
            "BOUNDED_CAPTURE_COMPLETE" if result.get("failure") is None
            else "BOUNDED_CAPTURE_FAILED"
        ),
        "returncode": result.get("returncode") if type(result.get("returncode")) is int else None,
        "stdout_size_bytes": len(stdout) if type(stdout) is bytes else 0,
        "stdout_sha256": sha256_bytes(stdout) if type(stdout) is bytes else sha256_bytes(b""),
        "stderr_size_bytes": len(stderr) if type(stderr) is bytes else 0,
        "stderr_sha256": sha256_bytes(stderr) if type(stderr) is bytes else sha256_bytes(b""),
        "diagnostic_disclosure": "ENUM_ONLY_NO_PID_PATH_ERRNO_TEXT_ARGV_ENV_OR_NONCE",
    ...
    failure_frame: dict[str, str] | None = None
    if type(stderr) is bytes:
        try:
            failure_frame = parse_failure_frame(stderr)
        except SupervisorError:
            pass
    evidence["failure_frame"] = failure_frame
    ...
    if (result.get("failure") is None and type(returncode) is int and returncode == 0
        ...
        return "PASS_U3_PROC_STAT_APPARMOR_BOUNDARY_PROBE_V4_CLEANUP_PENDING", evidence
    if (result.get("failure") is None and returncode == 2 and stdout == b""
            and failure_frame is not None):
        return "ENUMERATED_HELPER_FAILURE_CLEANUP_PENDING", evidence
...
def execution_receipt_document(
    *,
    ...
    return {
        "document_type": "SMPCC_R8_LIQUID_U3_PROC_STAT_APPARMOR_BOUNDARY_PROBE_V4_EXECUTION_RECEIPT",
        "probe_id": PROBE_ID,
        "status": status,
...
def lifecycle_document(
    *,
    ...
    return {
        "document_type": "SMPCC_R8_LIQUID_U3_PROC_STAT_APPARMOR_BOUNDARY_PROBE_V4_LIFECYCLE_RECEIPT",
        "probe_id": PROBE_ID,
...
def run_once(*, policy_sha256: str, admission_token: str) -> dict[str, Any]:
    with TerminationGuard() as termination_guard:
        try:
            ...
            document = {
                "document_type": "SMPCC_R8_LIQUID_U3_PROC_STAT_APPARMOR_BOUNDARY_PROBE_V4_PREFLIGHT_FAILURE_RECEIPT",
                "probe_id": PROBE_ID,
                "status": (
...
def _run_once_guarded(
    *,
    ...
    start_document = {
        "document_type": "SMPCC_R8_LIQUID_U3_PROC_STAT_APPARMOR_BOUNDARY_PROBE_V4_START_RECEIPT",
        "probe_id": PROBE_ID,
    ...
    try:
        ...
        audit["admission_journal_anchor_sha256"] = sha256_bytes(
            json.dumps(audit_anchor, sort_keys=True, separators=(",", ":")).encode("ascii")
        )
        audit["pre_run_journal_sync"] = _command_evidence(pre_run_journal_sync_result)
    ...
    failure = {
        "document_type": "SMPCC_R8_LIQUID_U3_PROC_STAT_APPARMOR_BOUNDARY_PROBE_V4_LIFECYCLE_INCOMPLETE_RECEIPT",
        "probe_id": PROBE_ID,
...
def recover_only(*, policy_sha256: str, recovery_token: str) -> dict[str, Any]:
    ...
    with TerminationGuard() as termination_guard:
        ...
        document = {
            "document_type": "SMPCC_R8_LIQUID_U3_PROC_STAT_APPARMOR_BOUNDARY_PROBE_V4_RECOVERY_RECEIPT",
            "probe_id": PROBE_ID,
            "status": "PASS_U3_PROC_STAT_APPARMOR_BOUNDARY_PROBE_V4_CLEANUP_ONLY_NO_PROBE_EXECUTED",
            "policy_sha256": policy_sha256,
            "snapshot": snapshot,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if arguments.command == "recover-only":
            ...
            print(json.dumps({"status": "PROC_STAT_BOUNDARY_PROBE_V4_CLEANUP_ONLY_RECORDED", "result": result}, ensure_ascii=False, sort_keys=True))
            return 0
        result = run_once(policy_sha256=arguments.policy_sha256, admission_token=arguments.admission_token)
        print(json.dumps({"status": "PROC_STAT_BOUNDARY_PROBE_V4_ONE_SHOT_LIFECYCLE_RECORDED", "result": result}, ensure_ascii=False, sort_keys=True))
        return 0 if result["execution_status"] == "PASS_U3_PROC_STAT_APPARMOR_BOUNDARY_PROBE_V4_CLEANUP_PENDING" else 2
