#!/usr/bin/env python3
"""Guest-only helper for one frozen U3 C1 CPU-solver smoke attempt.
...
FRAME_MAGIC = b"R8SOLVEROUTV2\0\0\0"
FRAME_VERSION = 2
...
def _audit_run_csv(raw: bytes) -> dict[str, Any]:
    ...
    for field in ("Np", "PhysicalTime", "PartFiles", "PartsOut"):
        if field not in row:
