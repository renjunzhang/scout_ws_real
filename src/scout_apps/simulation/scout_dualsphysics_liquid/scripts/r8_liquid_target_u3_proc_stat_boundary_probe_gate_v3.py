...
PAYLOAD_MAGIC = b"R8PROCSTATPROBE3\x00"
...
def main():
    ...
    emit_success({{
        "status": "PASS_U3_PROC_STAT_APPARMOR_BOUNDARY_PROBE_V3",
...
try:
    ...
    try:
        os.write(2, b"R8_PROC_STAT_PROBE_V3_NO_GO\n")
    except OSError:
        pass
    raise SystemExit(2)
'''
...
exec(compile(p,"<r8-proc-stat-boundary-helper-v3>","exec",dont_inherit=True,optimize=2),g,g)
'''
...
def bwrap_argv() -> list[str]:
    return [
        ...
        "--hostname", "r8-liquid-proc-stat-v3", "--clearenv",
...
def verify_full_authority_delta(candidate_lines: tuple[str, ...]) -> dict[str, Any]:
    ...
    if normalized_digest != EXPECTED_NORMALIZED_PROFILE_LINES_SHA256:
        raise GateError("normalized v3 full-profile digest differs")
...
def verify_supervisor_repository_path_contract(
    ...
    if (
        ...
        raise GateError("supervisor name must be a fresh v3 literal")
...
def verify_static() -> dict[str, Any]:
    ...
    if policy.get("provenance") != expected_provenance:
        raise GateError("frozen v6, v1, and v2 NO-GO provenance policy differs")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if arguments.command == "self-check":
            ...
            print(json.dumps({"status": f"PASS_U3_PROC_STAT_APPARMOR_BOUNDARY_PROBE_V3_{state}_EXECUTION_NOT_PERFORMED", "review": review}, ensure_ascii=False, sort_keys=True))
            return 0
