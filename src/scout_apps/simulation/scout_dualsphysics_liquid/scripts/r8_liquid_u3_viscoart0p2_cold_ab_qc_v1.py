#!/usr/bin/env python3
"""Read-only QC for fresh viscosity-0.2 GPU cold-A/B runs.
...
SCHEMA_PATH = PACKAGE_ROOT / "schema/target_host_u3_viscoart0p2_cold_ab_qc_v1.json"
TEST_PATH = PACKAGE_ROOT / "tests/test_u3_viscoart0p2_cold_ab_qc_v1.py"
...
def _trajectory(parts: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if any(not math.isfinite(value) for _, _, value in series):
        raise ColdAbQcError("speed trajectory is non-finite")
...
def adjudicate(metrics: dict[str, float], structural_pass: bool, initial_state_valid: bool) -> dict[str, Any]:
    ...
    return {
        ...
        "selected": selected,
        "status": (
            "PASS_U3_VISCOART0P2_COLD_SETTLING_CANDIDATE"
            if selected else "FAIL_U3_VISCOART0P2_COLD_NOT_SETTLED"
        ),
    }
...
def _parse_solver_proof(run_out_source: bi4.SecureFile) -> dict[str, Any]:
    ...
    if shifting != ["None"] or viscosity != ["Artificial"] or visco != ["0.2"]:
...
def build_report(policy_path: Path) -> dict[str, Any]:
    ...
    report = {
        "schema_version": "r8-liquid-u3-viscoart0p2-cold-ab-qc-v1",
        "document_type": "SMPCC_R8_LIQUID_U3_VISCOART0P2_COLD_AB_QC_V1",
        ...
        "verdict": {
            ...
            "exact_blocker": "NONE" if result["selected"] else f"VISCOART0P2_COLD_{role.upper()}_FAILS_{len(result['failed_absolute_metrics'])}_OF_17_ABSOLUTE_SETTLING_LIMITS",
            "next": (
...
def validate_report(report: dict[str, Any]) -> None:
    ...
    expected_status = "PASS_U3_VISCOART0P2_COLD_SETTLING_CANDIDATE" if selected else "FAIL_U3_VISCOART0P2_COLD_NOT_SETTLED"
...
def static_self_check() -> dict[str, Any]:
    ...
    return {
        "status": "PASS_U3_VISCOART0P2_COLD_AB_QC_V1_STATIC_SELF_CHECK",
