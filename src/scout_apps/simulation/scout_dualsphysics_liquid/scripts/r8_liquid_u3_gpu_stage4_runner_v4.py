...
SCHEMA_PATH = PACKAGE_ROOT / "schema/target_host_u3_gpu_stage4_run_policy_v4.json"
LAB_ROOT = Path("/home/zrj/scout_liquid_lab")
RUNS_PARENT = LAB_ROOT / "gpu_stage4_runs"
RUNS_PARENT_UID = 1000
RUNS_PARENT_GID = 1000
RUNS_PARENT_MODE = 0o700
...
def semantic_validate(policy: dict[str, Any], policy_path: Path) -> None:
    ...
    if delta != expected_delta:
        raise Stage4RunError("CFL sensitivity delta differs")
    if [item for item in argv if item.startswith("-shifting:")] != ["-shifting:none"]:
        raise Stage4RunError("CFL sensitivity Shifting=None invariant differs")
    if [item for item in argv if item.startswith("-cfl:")] != ["-cfl:0.1"]:
        raise Stage4RunError("CFL sensitivity exact override differs")
...
def runs_parent_identity(*, allow_absent: bool) -> dict[str, Any]:
    """Inspect the exact run parent without following a final-component link."""
    try:
        metadata = os.lstat(RUNS_PARENT)
    except FileNotFoundError:
        if allow_absent:
            return {"path": str(RUNS_PARENT), "exists": False, "created_by_this_call": False}
        raise Stage4RunError(f"dedicated run parent is absent: {RUNS_PARENT}")
    if not stat.S_ISDIR(metadata.st_mode):
        raise Stage4RunError(f"dedicated run parent is not a real directory: {RUNS_PARENT}")
    if RUNS_PARENT.resolve(strict=True) != RUNS_PARENT:
        raise Stage4RunError(f"dedicated run parent resolves through a symlink: {RUNS_PARENT}")
    observed_mode = stat.S_IMODE(metadata.st_mode)
    if metadata.st_uid != RUNS_PARENT_UID or metadata.st_gid != RUNS_PARENT_GID:
        raise Stage4RunError(
            f"dedicated run parent owner differs: uid={metadata.st_uid} gid={metadata.st_gid}"
        )
    if observed_mode != RUNS_PARENT_MODE:
        raise Stage4RunError(f"dedicated run parent mode differs: {observed_mode:04o}")
    return {
        "path": str(RUNS_PARENT),
        "exists": True,
...
def ensure_runs_parent() -> dict[str, Any]:
    """Create the one exact lab child if absent, then verify it through dirfds."""
    lab_metadata = os.lstat(LAB_ROOT)
    if not stat.S_ISDIR(lab_metadata.st_mode) or LAB_ROOT.resolve(strict=True) != LAB_ROOT:
        raise Stage4RunError(f"lab root is not a real directory: {LAB_ROOT}")
    lab_fd = os.open(LAB_ROOT, os.O_RDONLY | os.O_DIRECTORY | os.O_NOFOLLOW | os.O_CLOEXEC)
    created = False
    try:
        try:
            os.mkdir(RUNS_PARENT.name, RUNS_PARENT_MODE, dir_fd=lab_fd)
            created = True
        ...
        try:
            if created:
                os.fchown(child_fd, RUNS_PARENT_UID, RUNS_PARENT_GID)
                os.fchmod(child_fd, RUNS_PARENT_MODE)
                os.fsync(child_fd)
            metadata = os.fstat(child_fd)
            if not stat.S_ISDIR(metadata.st_mode):
                raise Stage4RunError("dedicated run parent fd is not a directory")
            if metadata.st_uid != RUNS_PARENT_UID or metadata.st_gid != RUNS_PARENT_GID:
                raise Stage4RunError(
                    f"dedicated run parent fd owner differs: uid={metadata.st_uid} gid={metadata.st_gid}"
                )
            if stat.S_IMODE(metadata.st_mode) != RUNS_PARENT_MODE:
                raise Stage4RunError(
                    f"dedicated run parent fd mode differs: {stat.S_IMODE(metadata.st_mode):04o}"
                )
        finally:
            os.close(child_fd)
    finally:
        os.close(lab_fd)
    observed = runs_parent_identity(allow_absent=False)
    observed["created_by_this_call"] = created
    return observed
...
def preflight(policy_path: Path, *, require_fresh: bool) -> dict[str, Any]:
    ...
    return {
        ...
        "runs_parent": runs_parent_identity(allow_absent=True),
        "fresh": require_fresh,
    }
...
def execute(policy_path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        runs_parent = ensure_runs_parent()
        start = {
            "schema_version": "r8-liquid-u3-gpu-stage4-run-receipt-v4",
            "document_type": "SMPCC_R8_LIQUID_U3_GPU_STAGE4_RUN_START_V4",
        ...
        final = {
            "schema_version": "r8-liquid-u3-gpu-stage4-run-receipt-v4",
            "document_type": "SMPCC_R8_LIQUID_U3_GPU_STAGE4_RUN_FINAL_V4",
        ...
        failure = {
            "schema_version": "r8-liquid-u3-gpu-stage4-run-receipt-v4",
            "document_type": "SMPCC_R8_LIQUID_U3_GPU_STAGE4_RUN_FAILURE_V4",
...
def self_check(policy_path: Path) -> dict[str, Any]:
    ...
    argv = bwrap_argv(policy)
    return {
        "status": "PASS_U3_GPU_STAGE4_RUNNER_V4_SELF_CHECK",
        "policy": identity(policy_path, maximum=MAX_POLICY_BYTES),
        "verified": verified,
        "sandbox_argv": argv,
        "network_used": False,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "self-check":
            ...
            result = {"status": "PASS_U3_GPU_STAGE4_RUNNER_V4_PREFLIGHT", "evidence": preflight(args.policy, require_fresh=True)}
        else:
            result = execute(args.policy)
    except (Stage4RunError, OSError, ValueError, subprocess.SubprocessError) as exc:
        print(json.dumps({"status": "FAIL_U3_GPU_STAGE4_RUNNER_V4", "error_type": type(exc).__name__, "error": str(exc)}, ensure_ascii=False, sort_keys=True), file=sys.stderr)
        return 1
