#!/usr/bin/env python3
"""Fail-closed v3_v5 identity adapter over the frozen strong semantic gate."""
...
v1, v3, HandoffError = base.v1, base.v3, base.HandoffError
ROOT = MODULE_DIR.parent
POLICY_PATH = ROOT / "config/target_hosts/liquid_zrj_msi_u2404_s5a1_primary_handoff_v3_v5.json"
SCHEMA_PATH = ROOT / "schema/target_host_s5a1_handoff_package_v5.json"
BASE_POLICY_PATH, BASE_POLICY_SHA256 = base.BASE_POLICY_PATH, base.BASE_POLICY_SHA256
BASE_SCHEMA_PATH, BASE_SCHEMA_SHA256 = base.BASE_SCHEMA_PATH, base.BASE_SCHEMA_SHA256
TRANSFER_ID = "SIM-S1_CORE_H1_C1_Bsmooth_b01_r01_r8_liquid_handoff_v3_v5"
...
def expand_policy(raw: bytes | None = None):
    if raw is None:
        ...
        identity = {"path": str(POLICY_PATH), "sha256": hashlib.sha256(raw).hexdigest(),
                    "size_bytes": len(raw)}
    delta = _object(raw, "v3_v5 policy delta")
    expected = {
        "schema_version": "smpcc-r8-liquid-s5a1-policy-delta-v5",
        "document_type": "SMPCC_R8_LIQUID_S5A1_PRIMARY_HANDOFF_POLICY_DELTA_V5",
        "base": {"path": str(BASE_POLICY_PATH), "sha256": BASE_POLICY_SHA256},
        "overrides": {
            "policy_id": "liquid_zrj_msi_u2404_s5a1_primary_handoff_v3_v5",
            "transfer_id": TRANSFER_ID, "planned_partial_root": PARTIAL_ROOT,
            "planned_final_root": FINAL_ROOT,
        },
    }
    if delta != expected:
        raise HandoffError("v3_v5 policy delta differs or is not closed")
...
def expand_schema(raw: bytes | None = None):
    if raw is None:
        ...
        identity = {"path": str(SCHEMA_PATH), "sha256": hashlib.sha256(raw).hexdigest(),
                    "size_bytes": len(raw)}
    delta = _object(raw, "v5 package schema delta")
    expected = {
        "revision_kind": "DETERMINISTIC_CLOSED_SCHEMA_DELTA_V5",
        "base": {"path": str(BASE_SCHEMA_PATH), "sha256": BASE_SCHEMA_SHA256},
        "overrides": {
            "schema_id": "https://scout.local/schema/target_host_s5a1_handoff_package_v5.json",
            "transfer_id": TRANSFER_ID,
        },
    }
    if delta != expected:
        raise HandoffError("v5 package schema delta differs or is not closed")
...
def self_check() -> dict[str, Any]:
    ...
    if policy["transfer_id"] != TRANSFER_ID or schema["properties"]["transfer_id"] != {"const": TRANSFER_ID}:
        raise HandoffError("effective v3_v5 identity differs")
    return {"status": "PASS_S5A1_HANDOFF_GATE_V7_V3_V5_ADAPTER_STATIC_CONTRACT",
