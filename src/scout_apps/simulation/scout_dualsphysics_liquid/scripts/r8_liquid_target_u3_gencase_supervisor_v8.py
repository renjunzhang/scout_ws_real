#!/usr/bin/env python3
"""Root lifecycle supervisor and host frame auditor for U3 C1M GenCase v8.
...
a separate UID-1000/NNP exporter to create the three expected outputs with
...
WORKSPACE_SUPERVISOR_PATH = WORKSPACE_PACKAGE_ROOT / "scripts/r8_liquid_target_u3_gencase_supervisor_v8.py"
...
EXPORT_FD_ENV = "R8_LIQUID_U3_GENCASE_V8_EXPORT_FD"
EXPORT_SHA256_ENV = "R8_LIQUID_U3_GENCASE_V8_EXPORT_SHA256"
ADMISSION_TOKEN = f"{CASE_ID}:single-gencase-attempt"
SNAPSHOT_ROOT = Path(f"/run/r8-liquid-{CASE_ID}.snapshot")

POLICY_NAME = "liquid_zrj_msi_u2404_u3_gencase_execution_policy_v8.json"
SCHEMA_NAME = "target_host_u3_gencase_execution_policy_v8.json"
PROFILE_NAME = "r8-liquid-u3-gencase-v8.profile"
GATE_NAME = "r8_liquid_target_u3_gencase_gate_v8.py"
HELPER_NAME = "r8_liquid_u3_gencase_bootstrap_helper_v8.py"
SUPERVISOR_NAME = "r8_liquid_target_u3_gencase_supervisor_v8.py"
...
OUTPUT_MAGIC = b"R8C1GENCASEV8\0\0\0"
OUTPUT_HEADER = struct.Struct(">16sIIQQQQ")
OUTPUT_VERSION = 1
OUTPUT_METADATA_LIMIT = 65_536
OUTPUT_BI4_LIMIT = 16_777_216
OUTPUT_XML_LIMIT = 524_288
OUTPUT_OUT_LIMIT = 1_048_576
OUTPUT_CONSOLE_LIMIT = 1_048_576
OUTPUT_FRAME_LIMIT = (
    OUTPUT_HEADER.size
    + OUTPUT_METADATA_LIMIT
    + OUTPUT_BI4_LIMIT
    + OUTPUT_XML_LIMIT
    + OUTPUT_OUT_LIMIT
    + OUTPUT_CONSOLE_LIMIT
)
...
SNAPSHOT_SOURCE_PAIRS = (
    ...
    (str(SEED_INPUT_ROOT / "C1M_moving_zero_Def.xml"), "C1M_moving_zero_Def.xml"),
)
...
os.write(1,(json.dumps({{"status":"ROOT_GENCASE_V8_SNAPSHOT_CREATED_NOT_EXECUTED","files":manifest}},sort_keys=True,separators=(",",":"))+"\\n").encode("ascii"))
'''
...
g={{"__name__":"__main__"}}
exec(compile(p,"<r8-liquid-gencase-v8-root-snapshot-bootstrap>","exec",dont_inherit=True,optimize=2),g,g)
'''
...
def verify_policy_static() -> dict[str, Any]:
    ...
    if policy.get("status") != "ADMITTED_SINGLE_C1M_ZERO_MOTION_GENCASE_RUNTIME_V8_AFTER_FROZEN_V7_DUAL_MOTION_AUDIT_FAILURE_AND_V11_ZERO_DENIAL_PROBE":
        raise SupervisorError("v8 one-shot admission status differs")
    if policy.get("allowed_gate_commands") != ["self-check", "internal-run"]:
        raise SupervisorError("v8 gate command surface differs")
    provenance = policy.get("seed_provenance", {})
    if (
        provenance.get("seed_id") != SEED_ID
        or provenance.get("seed_root") != str(SEED_ROOT)
        or provenance.get("input_root") != str(SEED_INPUT_ROOT)
        or provenance.get("receipt") != str(SEED_RECEIPT)
        or provenance.get("receipt_sha256") != SEED_RECEIPT_SHA256
        or provenance.get("receipt_status") != SEED_RECEIPT_STATUS
        or set(provenance.get("files", {})) != set(INPUT_CONTRACT)
    ):
        raise SupervisorError("v8 C1M seed v3 provenance differs")
    ...
    if (probe.get("required_receipt_sha256") != frozen_receipts["execution"][1]
        ...
        raise SupervisorError("v8 policy does not pin the frozen v11 PASS lifecycle")
    predecessor = policy.get("rejected_predecessor", {})
    v5_paths = {
        "start": (
            Path("/home/zrj/scout_liquid_lab/audits/u3_c1_gencase_v5_20260808T070530Z.start.json"),
            "9b26b2334eefc6549d315176be69823fc40e4de87495dc07daeefd38e8d0bac3",
        ),
        "execution": (
            Path("/home/zrj/scout_liquid_lab/audits/u3_c1_gencase_v5_20260808T070530Z.execution.json"),
            "49b23780ff1d6085c7ae1c5073ca1ee71fcf806657da6138d3dbfd39d893ba83",
        ),
        "lifecycle": (
            Path("/home/zrj/scout_liquid_lab/audits/u3_c1_gencase_v5_20260808T070530Z.lifecycle_incomplete.json"),
            "7bf446a6aacf58b6459f40c6d3214f3c9317d0c76a3fbb67af66a66d539bd374",
        ),
    }
    v5_documents: dict[str, Any] = {}
    for name, (path, digest) in v5_paths.items():
        raw = read_regular_bytes(path, limit=256 * 1024)
        if sha256_bytes(raw) != digest:
            raise SupervisorError(f"frozen v5 {name} receipt digest differs")
        v5_documents[name] = json.loads(raw.decode("utf-8"))
    v5_start = v5_documents["start"]
    v5_execution = v5_documents["execution"]
    v5_lifecycle = v5_documents["lifecycle"]
    v5_audit = v5_execution.get("audit", {})
    gate = v5_execution.get("gate", {})
    if (predecessor.get("identity") != "u3_c1_gencase_v5_20260808T070530Z"
            or predecessor.get("start_receipt_sha256") != v5_paths["start"][1]
            or predecessor.get("execution_receipt_sha256") != v5_paths["execution"][1]
            or predecessor.get("lifecycle_receipt_sha256") != v5_paths["lifecycle"][1]
            or v5_start.get("status") != "ONE_SHOT_STARTED_NO_RETRY_SAME_IDENTITY"
            or v5_execution.get("status") != "ATTEMPT_ABORTED_NO_RETRY_SAME_IDENTITY"
            or v5_execution.get("primary_error") != "SupervisorError: UID-1000 gate did not return one clean successful frame"
            or gate.get("returncode") != 2
            or gate.get("stdout_size_bytes") != 0
            or "generated C1 XML enables a forbidden dynamic feature" not in gate.get("stderr_utf8_prefix", "")
            or v5_audit.get("capture_valid") is not True
            or v5_audit.get("capture_errors") != []
            or v5_audit.get("matching_total") != 0
            or v5_audit.get("unexpected_total") != 0
            or v5_audit.get("sanitized_denials") != []
            or v5_lifecycle.get("status") != "LIFECYCLE_OR_EXECUTION_INCOMPLETE_NO_RETRY_SAME_IDENTITY"
            or v5_lifecycle.get("cleanup_errors") != []
            or v5_lifecycle.get("cleanup_observation", {}).get("post_unload_stable_zero_scans") != [[], [], []]
            or any(v5_lifecycle.get("profiles_after_observation", {}).get("kernel_exact_counts", {}).values())
            or v5_lifecycle.get("sysctls_after_observation") != policy.get("profile_lifecycle", {}).get("expected_sysctls")
            or os.path.lexists("/home/zrj/scout_liquid_lab/cases/u3_c1_gencase_v5_20260808T070530Z.partial")
            or os.path.lexists("/home/zrj/scout_liquid_lab/audits/u3_c1_gencase_v5_20260808T070530Z.console.log")):
        raise SupervisorError("frozen v5 fail-closed provenance differs")
    rejected_v7 = policy.get("rejected_v7_attempt", {})
    v7_paths = {
    ...
    v7_documents: dict[str, Any] = {}
    v7_evidence: dict[str, Any] = {}
    for name, (path, digest) in v7_paths.items():
        raw = read_regular_bytes(path, limit=256 * 1024)
        if sha256_bytes(raw) != digest:
            raise SupervisorError(f"frozen v7 {name} receipt digest differs")
        v7_documents[name] = json.loads(raw.decode("utf-8"))
        v7_evidence[name] = {"path": str(path), "sha256": digest, "size_bytes": len(raw)}
    v7_start = v7_documents["start"]
    v7_execution = v7_documents["execution"]
    v7_lifecycle = v7_documents["lifecycle"]
    v7_gate = v7_execution.get("gate", {})
    v7_audit = v7_execution.get("audit", {})
    v7_profiles = v7_lifecycle.get("profiles_after_observation", {})
    v7_console = str(v7_gate.get("stderr_utf8_prefix", ""))
    required_v7_console = (
        'generated C1M XML motion container location or attributes differ',
        'Fixed....: 0',
        'Moving...: 2,669',
        'Floating.: 0',
    ...
    if (
        rejected_v7.get("identity") != V7_CASE_ID
        or rejected_v7.get("permanent_status") != "ATTEMPT_ABORTED_NO_RETRY_SAME_IDENTITY"
        or rejected_v7.get("failure_class")
        != "SOURCE_PROVEN_DUAL_MOTION_COPY_AUDITOR_FALSE_NEGATIVE"
        or rejected_v7.get("start_receipt_sha256") != V7_START_RECEIPT_SHA256
        or rejected_v7.get("execution_receipt_sha256") != V7_EXECUTION_RECEIPT_SHA256
        or rejected_v7.get("lifecycle_receipt_sha256") != V7_LIFECYCLE_RECEIPT_SHA256
        or rejected_v7.get("gencase_completed") is not True
        or rejected_v7.get("particle_summary")
        != {"fixed": 0, "moving": 2669, "floating": 0, "fluid": 6409, "total": 9078}
        or rejected_v7.get("case_exported") is not False
        or rejected_v7.get("cleanup_complete") is not True
        or v7_start.get("case_id") != V7_CASE_ID
        or v7_start.get("status") != "ONE_SHOT_STARTED_NO_RETRY_SAME_IDENTITY"
        or v7_execution.get("case_id") != V7_CASE_ID
        or v7_execution.get("status") != "ATTEMPT_ABORTED_NO_RETRY_SAME_IDENTITY"
        or v7_execution.get("primary_error")
        != "SupervisorError: UID-1000 gate did not return one clean successful frame"
        or v7_execution.get("cleanup_errors") != []
        or v7_gate.get("returncode") != 2
        or v7_gate.get("stdout_size_bytes") != 0
        or v7_gate.get("stdin_fully_written") is not True
        or any(marker not in v7_console for marker in required_v7_console)
        or v7_audit.get("capture_valid") is not True
        or v7_audit.get("capture_errors") != []
        or v7_audit.get("matching_total") != 0
        or v7_audit.get("unexpected_total") != 0
        or v7_audit.get("sanitized_denials") != []
        or v7_execution.get("export") is not None
        or v7_execution.get("export_verification") is not None
        or v7_lifecycle.get("case_id") != V7_CASE_ID
        or v7_lifecycle.get("status") != "LIFECYCLE_OR_EXECUTION_INCOMPLETE_NO_RETRY_SAME_IDENTITY"
        or v7_lifecycle.get("cleanup_errors") != []
        or v7_lifecycle.get("cleanup_observation", {}).get("post_unload_stable_zero_scans")
        != [[], [], []]
        or any(v7_profiles.get("kernel_exact_counts", {}).values())
        or any(v7_profiles.get("aa_status_exact_presence", {}).values())
        or v7_lifecycle.get("sysctls_after_observation")
        != policy.get("profile_lifecycle", {}).get("expected_sysctls")
        or v7_lifecycle.get("sudo_timestamp_observation", {})
        .get("noninteractive_true_must_fail", {}).get("returncode") != 1
        ...
        raise SupervisorError("frozen v7 dual-motion audit failure provenance differs")
    ...
    return {
        ...
        "frozen_v5_failure": {name: {"path": str(path), "sha256": digest} for name, (path, digest) in v5_paths.items()},
        "frozen_v7_failure": v7_evidence,
...
def verify_seed_receipt_and_read_inputs() -> dict[str, bytes]:
    for directory in (SEED_ROOT, SEED_INPUT_ROOT):
        ...
        if (
            ...
            raise SupervisorError(f"C1M seed v3 directory contract differs: {directory}")
    receipt_raw = read_regular_bytes(SEED_RECEIPT, limit=64 * 1024)
    if sha256_bytes(receipt_raw) != SEED_RECEIPT_SHA256:
        raise SupervisorError("C1M seed v3 receipt digest differs")
    try:
        receipt = json.loads(receipt_raw.decode("utf-8"))
    except (json.JSONDecodeError, UnicodeError) as exc:
        raise SupervisorError("C1M seed v3 receipt is invalid") from exc
    if (
        receipt.get("seed_id") != SEED_ID
        or receipt.get("status") != SEED_RECEIPT_STATUS
        or receipt.get("predecessor_seed_used_as_source") is not False
    ):
        raise SupervisorError("C1M seed v3 receipt is not the required non-executing PASS")
    for field in (
        ...
        if receipt.get(field) is not False:
            raise SupervisorError(f"C1M seed v3 receipt violates non-execution: {field}")
    seed_inventory = receipt.get("seed_input")
    if (
        not isinstance(seed_inventory, dict)
        or seed_inventory.get("root") != str(SEED_INPUT_ROOT)
        or seed_inventory.get("files")
        != {
            name: {"mode": "0400", "sha256": digest, "size_bytes": size}
            for name, (digest, size) in sorted(INPUT_CONTRACT.items())
        }
    ):
        raise SupervisorError("C1M seed v3 receipt inventory differs")
    result: dict[str, bytes] = {}
    names = sorted(entry.name for entry in os.scandir(SEED_INPUT_ROOT))
    if names != sorted(INPUT_CONTRACT):
        raise SupervisorError("C1M seed v3 input file set differs")
    for name, (digest, size) in INPUT_CONTRACT.items():
        ...
        if (
            ...
            raise SupervisorError(f"C1M seed v3 file contract differs: {name}")
        raw = read_regular_bytes(path, limit=16 * 1024 * 1024)
        if sha256_bytes(raw) != digest:
            raise SupervisorError(f"C1M seed v3 file digest differs: {name}")
        result[name] = raw
    return result
...
def _validate_generated_xml(raw: bytes) -> None:
    ...
    if (keyed.get("MinFluidStop") != "1" or keyed.get("Shifting") != "1"
            or keyed.get("DtAllParticles") != "1" or "PartsOutMax" in keyed):
        raise SupervisorError("framed XML C1M parameter contract differs")
    particles = root.find("./execution/particles")
    if particles is None or particles.attrib != {
        "np": "9078", "nb": "2669", "nbf": "0", "mkboundfirst": "2", "mkfluidfirst": "1"
    }:
        raise SupervisorError("framed XML C1M particle-header contract differs")
    if particles.findall("./fixed") or len(particles.findall("./moving")) != 1:
        raise SupervisorError("framed XML is not exactly one moving boundary and zero fixed blocks")
    moving = particles.find("./moving")
    fluid = particles.find("./fluid")
    if moving is None or moving.attrib != {
        "mkbound": "0", "mk": "2", "begin": "0", "count": "2669", "refmotion": "0"
    }:
        raise SupervisorError("framed XML C1M moving block contract differs")
    if fluid is None or fluid.attrib != {
        "mkfluid": "0", "mk": "1", "begin": "2669", "count": "6409"
    }:
        raise SupervisorError("framed XML C1M fluid block contract differs")
    definition_motion = root.find("./casedef/motion")
    execution_motion = root.find("./execution/motion")
    all_motion = [element for element in root.iter() if element.tag == "motion"]
    if (
        definition_motion is None
        or execution_motion is None
        or all_motion != [definition_motion, execution_motion]
        or len(root.findall(".//objreal")) != 2
        or len(root.findall(".//begin")) != 2
        or len(root.findall(".//mvnull")) != 2
    ):
        raise SupervisorError("framed XML must contain exactly the casedef and execution motion copies")
    for location, motion in (
        ("casedef", definition_motion),
        ("execution", execution_motion),
    ):
        if (
            motion.attrib
            or (motion.text or "").strip()
            or (motion.tail or "").strip()
            or [child.tag for child in motion] != ["objreal"]
        ):
            raise SupervisorError(f"framed XML C1M {location} motion container differs")
        objreal = motion[0]
        if (objreal.attrib != {"ref": "0"}
                or [child.tag for child in objreal] != ["begin", "mvnull"]
                or (objreal.text or "").strip()
                or (objreal.tail or "").strip()):
            raise SupervisorError(f"framed XML C1M {location} zero-motion object differs")
        begin, mvnull = objreal
        try:
            begin_start = float(begin.get("start", "nan"))
        except ValueError as exc:
            raise SupervisorError(f"framed XML C1M {location} begin start is not numeric") from exc
        if (set(begin.attrib) != {"mov", "start"} or begin.get("mov") != "1"
                or begin_start != 0.0 or len(begin)
                or (begin.text or "").strip() or (begin.tail or "").strip()
                or mvnull.attrib != {"id": "1"} or len(mvnull)
                or (mvnull.text or "").strip() or (mvnull.tail or "").strip()):
            raise SupervisorError(f"framed XML C1M {location} mvnull event differs")
    forbidden = {"floating", "inout", "wavegen", "special", "wavepaddles", "mlayerpistons",
                 "chrono", "accinputs", "relaxationzones", "obj", "wait", "mvrect", "mvrectace",
                 "mvrot", "mvrotace", "mvcir", "mvcirace", "mvrectsinu", "mvrotsinu",
                 "mvcirsinu", "mvpredef", "mvfile", "mvrectfile", "mvrotfile",
                 "mvrotadvfile", "mvpathfile", "file"}
    if forbidden.intersection(element.tag for element in root.iter()):
        raise SupervisorError("framed XML enables a forbidden dynamic or external feature")
...
def _validate_generated_out(raw: bytes) -> None:
    if b"\0" in raw:
        raise SupervisorError("framed OUT contains a NUL byte")
    try:
        text = raw.decode("utf-8", "strict")
    except UnicodeDecodeError as exc:
        raise SupervisorError("framed OUT is not strict UTF-8 text") from exc
    required = (
        "GenCase v5.4.354.01",
    ...
    if any(marker not in text for marker in required):
        raise SupervisorError("framed OUT lacks the frozen GenCase/particle summary")
...
def parse_success_frame(raw: bytes) -> dict[str, Any]:
    ...
    magic, version, metadata_size, bi4_size, xml_size, out_size, console_size = OUTPUT_HEADER.unpack_from(raw)
    ...
    if (not 1 <= bi4_size <= OUTPUT_BI4_LIMIT
            or not 1 <= xml_size <= OUTPUT_XML_LIMIT
            or not 1 <= out_size <= OUTPUT_OUT_LIMIT):
    ...
    expected_total = OUTPUT_HEADER.size + metadata_size + bi4_size + xml_size + out_size + console_size
    ...
    out_start = xml_start + xml_size
    xml = raw[xml_start:out_start]
    console_start = out_start + out_size
    out = raw[out_start:console_start]
    ...
    if metadata["document_type"] != "SMPCC_R8_LIQUID_U3_C1M_GENCASE_GUEST_FRAME_V8" or metadata["status"] != "GUEST_FRAME_COMPLETE_PENDING_HOST_REAUDIT_AND_O_EXCL_EXPORT":
    ...
    payloads = {"C1M_zero.bi4": bi4, "C1M_zero.xml": xml, "C1M_zero.out": out}
    expected_outputs = {
...
def export_frame_o_excl(frame: bytes) -> dict[str, Any]:
    ...
    try:
        ...
        for name in ("C1M_zero.bi4", "C1M_zero.xml", "C1M_zero.out"):
            ...
            try:
                ...
                os.close(descriptor)
        if sorted(os.listdir(output_fd)) != ["C1M_zero.bi4", "C1M_zero.out", "C1M_zero.xml"]:
...
def load_lifecycle_harness() -> Any:
    ...
    spec = importlib.util.spec_from_file_location("_r8_gencase_v8_lifecycle_harness", HARNESS_PATH)
...
def verify_exported_outputs(parsed: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if sorted(os.listdir(OUTPUT_ROOT)) != ["C1M_zero.bi4", "C1M_zero.out", "C1M_zero.xml"]:
    ...
    for name, expected in parsed["payloads"].items():
        ...
        metadata = os.lstat(path)
        limit = (
            OUTPUT_BI4_LIMIT if name.endswith(".bi4")
            else OUTPUT_XML_LIMIT if name.endswith(".xml")
            else OUTPUT_OUT_LIMIT
        )
        raw = read_regular_bytes(path, limit=limit)
...
def run_once(*, policy_sha256: str, admission_token: str) -> dict[str, Any]:
    ...
    with harness.TerminationGuard() as termination_guard:
        ...
        if policy.get("profile_lifecycle", {}).get("expected_sysctls") != sysctls_before:
            raise SupervisorError("current sysctls differ from the frozen v8 baseline")
        ...
        start_document = {
            "document_type": "SMPCC_R8_LIQUID_U3_C1M_GENCASE_V8_START_RECEIPT",
        ...
        try:
            ...
            execution_status = "PASS_U3_C1M_GENCASE_V8_FRAME_VALID_CLEANUP_PENDING"
        ...
        if parsed is not None and primary_error is None and not cleanup_errors:
            try:
                ...
                if (not isinstance(export_message, dict) or export_message.get("status") != "PASS_UID1000_O_EXCL_EXPORT_V8"
                ...
                execution_status = "PASS_U3_C1M_GENCASE_V8_CASE_EXPORTED_CLEANUP_PENDING"
            except BaseException as exc:
                primary_error = f"{type(exc).__name__}: {exc}"[:4096]
        ...
        execution_document = {
            "document_type": "SMPCC_R8_LIQUID_U3_C1M_GENCASE_V8_EXECUTION_RECEIPT",
        ...
        success = (
            execution_document["status"] == "PASS_U3_C1M_GENCASE_V8_CASE_EXPORTED_CLEANUP_PENDING"
            and not cleanup_errors and primary_error is None and parsed is not None
            and export_verification is not None
        )
        if success:
            lifecycle_document = {
                "document_type": "SMPCC_R8_LIQUID_U3_C1M_GENCASE_V8_LIFECYCLE_RECEIPT",
                "case_id": CASE_ID, "status": "PASS_U3_C1M_GENCASE_V8_LIFECYCLE_CLEANUP_AND_CASE_EXPORT",
        ...
        failure_document = {
            "document_type": "SMPCC_R8_LIQUID_U3_C1M_GENCASE_V8_LIFECYCLE_INCOMPLETE_RECEIPT",
        ...
        raise SupervisorError(f"v8 attempt failed closed; preserved {failure_record['path']}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if arguments.command == "self-check":
            ...
            print(json.dumps({
                "status": "PASS_V8_SUPERVISOR_STATIC_ADMITTED_EXECUTION_NOT_PERFORMED",
                "review": review,
                "snapshot_bootstrap": {
        ...
        if arguments.command == "export-frame":
            ...
            print(json.dumps({"status": "PASS_UID1000_O_EXCL_EXPORT_V8", "export": result}, ensure_ascii=False, sort_keys=True))
            return 0
        result = run_once(policy_sha256=arguments.policy_sha256, admission_token=arguments.admission_token)
        print(json.dumps({"status": "V8_ONE_SHOT_LIFECYCLE_RECORDED", "result": result}, ensure_ascii=False, sort_keys=True))
        return 0
