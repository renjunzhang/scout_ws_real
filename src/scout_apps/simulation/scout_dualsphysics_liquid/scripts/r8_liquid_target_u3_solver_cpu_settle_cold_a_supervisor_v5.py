#!/usr/bin/env python3
"""Root lifecycle supervisor for one fresh U3 C1M cold-A settle v5.
...
WORKSPACE_SUPERVISOR_PATH = WORKSPACE_PACKAGE_ROOT / "scripts/r8_liquid_target_u3_solver_cpu_settle_cold_a_supervisor_v5.py"
CASE_ID = "u3_c1m_solver_cpu_settle_cold_a_v5_20260808T203712Z"
CAMPAIGN_ID = "u3_c1m_solver_cpu_settle_ab_campaign_20260808T203712Z"
HOST_UID = 1000
HOST_GID = 1000
BOOTSTRAP_PROFILE = "r8-liquid-u3-c1m-solver-cpu-settle-cold-a-bootstrap-v5-20260808t203712z"
RUNTIME_PROFILE = "r8-liquid-u3-c1m-solver-cpu-settle-cold-a-runtime-v5-20260808t203712z"
LABELS = (BOOTSTRAP_PROFILE, RUNTIME_PROFILE)
SNAPSHOT_ROOT = Path(f"/run/r8-liquid-{CASE_ID}.snapshot")
SNAPSHOT_ENV = "R8_LIQUID_U3_SOLVER_CPU_SETTLE_COLD_A_V5_SNAPSHOT"
ADMISSION_FD_ENV = "R8_LIQUID_U3_SOLVER_CPU_SETTLE_COLD_A_V5_ADMISSION_FD"
ADMISSION_SHA256_ENV = "R8_LIQUID_U3_SOLVER_CPU_SETTLE_COLD_A_V5_ADMISSION_SHA256"
EXPORT_FD_ENV = "R8_LIQUID_U3_SOLVER_CPU_SETTLE_COLD_A_V5_EXPORT_FD"
EXPORT_SHA256_ENV = "R8_LIQUID_U3_SOLVER_CPU_SETTLE_COLD_A_V5_EXPORT_SHA256"
ADMISSION_TOKEN = (
    f"{CAMPAIGN_ID}:{CASE_ID}:single-cpu-solver-settle-cold-a-v5-attempt"
)
POLICY_STATUS = "REVIEWED_FRESH_C1M_CPU_SOLVER_SETTLE_COLD_A_V5_TWO_KNOWN_QUIET_DENY_RULES_PENDING_STATIC_VERIFICATION"
...
POLICY_NAME = "liquid_zrj_msi_u2404_u3_solver_cpu_settle_cold_a_execution_policy_v5.json"
SCHEMA_NAME = "target_host_u3_solver_cpu_settle_cold_a_execution_policy_v5.json"
PROFILE_NAME = "r8-liquid-u3-solver-cpu-settle-cold-a-v5.profile"
GATE_NAME = "r8_liquid_target_u3_solver_cpu_settle_cold_a_gate_v5.py"
HELPER_NAME = "r8_liquid_u3_solver_cpu_settle_cold_a_bootstrap_helper_v5.py"
SUPERVISOR_NAME = "r8_liquid_target_u3_solver_cpu_settle_cold_a_supervisor_v5.py"
...
OUTPUT_MAGIC = b"R8SOLVEROUTV5\0\0\0"
OUTPUT_VERSION = 5
...
os.write(1,(json.dumps({{"status":"ROOT_SOLVER_CPU_SETTLE_COLD_A_V5_SNAPSHOT_CREATED_NOT_EXECUTED","files":manifest}},sort_keys=True,separators=(",",":"))+"\\n").encode("ascii"))
'''
...
g={{"__name__":"__main__"}}
exec(compile(p,"<r8-liquid-solver-cpu-settle-cold-a-v5-root-snapshot-bootstrap>","exec",dont_inherit=True,optimize=2),g,g)
'''
...
def verify_provenance_and_inputs() -> tuple[dict[str, Any], dict[str, bytes]]:
    ...
    for name, source, digest, size in INPUT_SOURCES:
        if V4_CASE_ID in source or "/run/r8-liquid-" in source:
            raise SupervisorError("consumed v4 identity or snapshot appeared in v5 input sources")
...
def verify_policy_static() -> dict[str, Any]:
    ...
    gate = load_module(GATE_PATH, "_r8_c1m_solver_cpu_settle_cold_a_v5_static_gate")
    helper_module = load_module(
        HELPER_PATH, "_r8_c1m_solver_cpu_settle_cold_a_v5_static_helper"
    )
    if (
        ...
        raise SupervisorError("v5 campaign identity differs across runtime artifacts")
    ...
    for source, name in SNAPSHOT_SOURCE_PAIRS:
        if V4_CASE_ID in source or "cold_a_v4" in name:
            raise SupervisorError("consumed v4 identity appeared in v5 snapshot sources")
        raw = read_regular_bytes(Path(source), limit=80 * 1024 * 1024)
...
def parse_success_frame(raw: bytes) -> dict[str, Any]:
    ...
    if (
        metadata["document_type"] != "SMPCC_R8_LIQUID_U3_C1M_CPU_SOLVER_SETTLE_COLD_A_GUEST_FRAME_V5"
        or metadata["status"] != "GUEST_SOLVER_V5_COLD_A_SETTLE_COMPLETE_PENDING_HOST_REAUDIT_AND_O_EXCL_EXPORT"
    ...
    gate = load_module(GATE_PATH, "_r8_c1m_solver_cpu_settle_cold_a_v5_frame_gate")
    ...
    expected_inputs = {}
    helper = load_module(HELPER_PATH, "_r8_c1m_solver_cpu_settle_cold_a_v5_frame_helper")
...
def load_lifecycle_harness() -> Any:
    ...
    module = load_module(HARNESS_PATH, "_r8_c1m_solver_cpu_settle_cold_a_v5_lifecycle_harness")
...
def run_once(*, policy_sha256: str, admission_token: str) -> dict[str, Any]:
    ...
    gate = load_module(GATE_PATH, "_r8_c1m_solver_cpu_settle_cold_a_v5_runtime_gate")
    with harness.TerminationGuard() as termination_guard:
        ...
        start_document = {
            "document_type": "SMPCC_R8_LIQUID_U3_C1M_CPU_SOLVER_SETTLE_COLD_A_V5_START_RECEIPT",
            "case_id": CASE_ID,
        ...
        execution_status = "V5_COLD_A_ATTEMPT_ABORTED_CLEANUP_PENDING"
        try:
            ...
            execution_status = "PASS_U3_C1M_CPU_SOLVER_SETTLE_COLD_A_V5_FRAME_VALID_CLOSED_ZERO_UNEXPECTED_LOGGED_DENIALS_TWO_KNOWN_QUIET_DENY_RULES_CLEANUP_PENDING"
        ...
        if parsed is not None and primary_error is None and not cleanup_errors:
            try:
                ...
                if message.get("status") != "PASS_UID1000_SOLVER_SETTLE_COLD_A_TREE_O_EXCL_EXPORT_V5" or not isinstance(message.get("export"), dict):
                ...
                execution_status = "PASS_U3_C1M_CPU_SOLVER_SETTLE_COLD_A_V5_EXPORTED_CLOSED_ZERO_UNEXPECTED_LOGGED_DENIALS_TWO_KNOWN_QUIET_DENY_RULES_CLEANUP_PENDING"
            except BaseException as exc:
                primary_error = f"{type(exc).__name__}: {exc}"[:4096]
        ...
        execution_document = {
            "document_type": "SMPCC_R8_LIQUID_U3_C1M_CPU_SOLVER_SETTLE_COLD_A_V5_EXECUTION_RECEIPT",
            "case_id": CASE_ID,
            "campaign_id": CAMPAIGN_ID,
            "campaign_role": "cold_a",
            "status": execution_status if primary_error is None and not cleanup_errors else "V5_COLD_A_ATTEMPT_ABORTED_NO_RETRY_SAME_IDENTITY",
            "policy_sha256": policy_sha256,
        ...
        success = execution_document["status"] == "PASS_U3_C1M_CPU_SOLVER_SETTLE_COLD_A_V5_EXPORTED_CLOSED_ZERO_UNEXPECTED_LOGGED_DENIALS_TWO_KNOWN_QUIET_DENY_RULES_CLEANUP_PENDING" and parsed is not None and export_verification is not None
        if success:
            lifecycle_document = {
                "document_type": "SMPCC_R8_LIQUID_U3_C1M_CPU_SOLVER_SETTLE_COLD_A_V5_LIFECYCLE_RECEIPT",
                "case_id": CASE_ID,
                "campaign_id": CAMPAIGN_ID,
                "campaign_role": "cold_a",
                "status": "PASS_U3_C1M_CPU_SOLVER_SETTLE_COLD_A_V5_LIFECYCLE_CLEANUP_EXPORT_CLOSED_ZERO_UNEXPECTED_LOGGED_DENIALS_TWO_KNOWN_QUIET_DENY_RULES",
        ...
        failure_document = {
            "document_type": "SMPCC_R8_LIQUID_U3_C1M_CPU_SOLVER_SETTLE_COLD_A_V5_LIFECYCLE_INCOMPLETE_RECEIPT",
            "case_id": CASE_ID,
            "campaign_id": CAMPAIGN_ID,
            "campaign_role": "cold_a",
        ...
        raise SupervisorError(f"solver cold A v5 failed closed; preserved {failure_record['path']}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if arguments.command == "self-check":
            ...
            print(json.dumps({
                "status": "PASS_SOLVER_CPU_SETTLE_COLD_A_V5_SUPERVISOR_STATIC_EXECUTION_NOT_PERFORMED",
                "review": review,
                "snapshot_bootstrap": {
        ...
        if arguments.command == "export-frame":
            ...
            print(json.dumps({"status": "PASS_UID1000_SOLVER_SETTLE_COLD_A_TREE_O_EXCL_EXPORT_V5", "export": result}, ensure_ascii=False, sort_keys=True))
            return 0
        result = run_once(policy_sha256=arguments.policy_sha256, admission_token=arguments.admission_token)
        print(json.dumps({"status": "SOLVER_CPU_SETTLE_COLD_A_V5_ONE_SHOT_LIFECYCLE_RECORDED", "result": result}, ensure_ascii=False, sort_keys=True))
        return 0
        ...
        print(json.dumps({"status": "SOLVER_CPU_SETTLE_COLD_A_V5_NO_GO", "error": str(exc)}, ensure_ascii=False, sort_keys=True), file=target)
        return 2
