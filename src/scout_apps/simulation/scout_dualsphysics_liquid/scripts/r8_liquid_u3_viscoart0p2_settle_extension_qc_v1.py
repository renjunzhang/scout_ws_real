#!/usr/bin/env python3
"""Read-only QC for the v9 35.05--36.05 s settling extension.

The tool parses only preserved BI4 outputs, proves exact restart-state
identity at Part_0701, verifies the closed 21-frame inventory and the single
end-time delta, evaluates the unchanged 17 settled-state limits over the full
one-second extension, and publishes one O_EXCL receipt.  It never runs the
solver or exposes a GPU.
"""
...
import r8_liquid_u3_gpu_viscoart0p2_settle_extension_runner_v1 as runner
...
SCHEMA_PATH = PACKAGE_ROOT / "schema/target_host_u3_viscoart0p2_settle_extension_qc_v1.json"
TEST_PATH = PACKAGE_ROOT / "tests/test_u3_viscoart0p2_settle_extension_qc_v1.py"
...
PART_INDICES = tuple(range(701, 722))
...
class SettleExtensionQcError(ValueError):
    """Unsafe, drifted, incomplete or inconsistent settling-extension evidence."""
...
def _trajectory(parts: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if not series or any(not math.isfinite(value) or value < 0 for _, _, value in series):
        raise SettleExtensionQcError("speed trajectory is empty, negative or non-finite")
...
def adjudicate(metrics: dict[str, float], structural_pass: bool, restart_matches: bool) -> dict[str, Any]:
    if tuple(sorted(metrics)) != METRIC_NAMES:
        raise SettleExtensionQcError("metric vector differs from the frozen 17 items")
    if any(not isinstance(value, (int, float)) or not math.isfinite(float(value)) or float(value) < 0 for value in metrics.values()):
        raise SettleExtensionQcError("metric vector contains invalid values")
    ...
    return {
        ...
        "selected": selected,
        "status": (
            "PASS_U3_VISCOART0P2_SETTLING_EXTENSION_CANDIDATE"
            if selected else "FAIL_U3_VISCOART0P2_SETTLING_EXTENSION_NOT_SETTLED"
        ),
    }
...
def _parse_solver_proof(run_out_source: bi4.SecureFile) -> dict[str, Any]:
    ...
    if shifting != ["None"] or viscosity != ["Artificial"] or visco != ["0.2"]:
        raise SettleExtensionQcError(f"solver numerical identity differs: {shifting!r}/{viscosity!r}/{visco!r}")
...
def build_report(policy_path: Path) -> dict[str, Any]:
    ...
    if policy_path != runner.POLICY_PATH:
        raise SettleExtensionQcError("policy path differs")
    ...
    required_final = (
        ...
        and final.get("purpose") == "viscoart0p2_settling_extension"
    ...
    if not required_final:
        raise SettleExtensionQcError("final receipt semantics differ")
    ...
    for key in ("file_count", "part_file_count", "part_first", "part_last", "total_bytes", "canonical_sha256"):
        if observed_inventory.get(key) != frozen_inventory.get(key):
            raise SettleExtensionQcError(f"output inventory differs at {key}")
    if any(item["mode"] != "0440" or item["nlink"] != 1 for item in observed_inventory["files"].values()):
        raise SettleExtensionQcError("output mode/link invariant differs")
    ...
    structural = {
        "exact_inventory": observed_inventory["file_count"] == 30,
        ...
        "start_time_matches": abs(times[0] - 35.05) <= metric_contract.CONTRACT.physical_time_tolerance_s,
        "end_time_matches": abs(times[-1] - 36.05) <= metric_contract.CONTRACT.physical_time_tolerance_s,
        ...
        "run_csv_matches": fields["Np"] == 9078 and fields["PartFiles"] == 722 and abs(fields["PhysicalTime"] - times[-1]) <= metric_contract.CONTRACT.physical_time_tolerance_s,
    ...
    if not all(structural.values()):
        raise SettleExtensionQcError(f"structural checks failed: {[name for name, value in structural.items() if not value]}")

    restart_input = _canonical_state(runner.RESTART_PART_PATH)
    restart_output = _canonical_state(root / "data/Part_0701.bi4")
    restart_matches = restart_input["canonical_array_sha256"] == restart_output["canonical_array_sha256"]
    if not restart_matches:
        raise SettleExtensionQcError("restart Part_0701 state arrays differ")
    ...
    if inputs["plan"]["sha256"] != PLAN_SHA256:
        raise SettleExtensionQcError("frozen plan identity differs")
    report = {
        "schema_version": "r8-liquid-u3-viscoart0p2-settle-extension-qc-v1",
        "document_type": "SMPCC_R8_LIQUID_U3_VISCOART0P2_SETTLE_EXTENSION_QC_V1",
        "captured_at_utc": datetime.now(timezone.utc).isoformat(),
        "inputs": inputs,
        "contract": {
            "window_start_s": 35.05,
            "window_end_s": 36.05,
            "tail_window_s": 1.0,
            "part_first": 701,
            "part_last": 721,
        ...
        "run": {
            ...
            "final_checkpoint": _identity(root / "data/Part_0721.bi4"),
        },
        "verdict": {
            ...
            "exact_blocker": "NONE" if result["selected"] else f"VISCOART0P2_SETTLING_EXTENSION_FAILS_{len(result['failed_absolute_metrics'])}_OF_17_ABSOLUTE_SETTLING_LIMITS",
            "next": "FREEZE_FRESH_0_TO_36P05_REPEATABILITY_AND_RESTART_EQUIVALENCE_CONTRACT" if result["selected"] else "STOP_OR_FREEZE_ONE_NEW_NUMERICAL_DELTA",
...
def validate_report(report: dict[str, Any]) -> None:
    ...
    if tuple(sorted(metrics)) != METRIC_NAMES or tuple(sorted(passes)) != METRIC_NAMES:
        raise SettleExtensionQcError("report metric/pass keys differ")
    ...
    if failed != expected_failed or any(passes[name] is not (name not in expected_failed) for name in METRIC_NAMES):
        raise SettleExtensionQcError("metric values, pass vector and failure list disagree")
    ...
    expected_status = "PASS_U3_VISCOART0P2_SETTLING_EXTENSION_CANDIDATE" if selected else "FAIL_U3_VISCOART0P2_SETTLING_EXTENSION_NOT_SETTLED"
    if verdict.get("status") != expected_status or verdict.get("u3_remediation_candidate_pass") is not selected:
        raise SettleExtensionQcError("verdict disagrees with frozen adjudication")
    ...
    if errors:
        first = errors[0]
        raise SettleExtensionQcError(f"QC schema failure at {list(first.absolute_path)}: {first.message}")
...
def write_exclusive(path: Path, report: dict[str, Any]) -> dict[str, Any]:
    ...
    if len(data) > MAX_JSON_BYTES:
        raise SettleExtensionQcError("QC output exceeds bound")
    ...
    try:
        ...
        while view:
            ...
            if written <= 0:
                raise SettleExtensionQcError("short QC write")
            view = view[written:]
        os.fsync(descriptor)
    finally:
        os.close(descriptor)
...
def static_self_check() -> dict[str, Any]:
    ...
    if METRIC_LIMITS != dict(sorted(metric_contract.METRIC_LIMITS.items())):
        raise SettleExtensionQcError("metric contract drifted")
    return {
        "status": "PASS_U3_VISCOART0P2_SETTLE_EXTENSION_QC_V1_STATIC_SELF_CHECK",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.output is None:
            raise SettleExtensionQcError("--output is required")
    ...
    except (SettleExtensionQcError, runner.SettleExtensionRunError, legacy.Stage4RunError, metric_contract.DdtRampQcError, qc_v3.QcError, bi4.Bi4FormatError, OSError, UnicodeError, ValueError, subprocess.SubprocessError) as exc:
