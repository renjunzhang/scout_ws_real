#!/usr/bin/env python3
"""Read-only QC for the fresh viscosity-0.3 GPU cold-B 45.05 s run.

The tool parses only preserved BI4 outputs, verifies the closed 902-frame
...
import r8_liquid_u3_gpu_viscoart0p3_cold_b45_runner_v1 as runner
...
SCHEMA_PATH = PACKAGE_ROOT / "schema/target_host_u3_viscoart0p3_cold_b45_qc_v1.json"
TEST_PATH = PACKAGE_ROOT / "tests/test_u3_viscoart0p3_cold_b45_qc_v1.py"
...
PART_INDICES = tuple(range(0, 902))
...
class ColdAbQcError(ValueError):
    """Unsafe, drifted, incomplete or inconsistent cold-B evidence."""
...
def build_report(policy_path: Path) -> dict[str, Any]:
    ...
    structural = {
        "exact_inventory": observed_inventory["file_count"] == 911,
    ...
    report = {
        "schema_version": "r8-liquid-u3-viscoart0p3-cold-b45-qc-v1",
        "document_type": "SMPCC_R8_LIQUID_U3_VISCOART0P3_COLD_B45_QC_V1",
...
def static_self_check() -> dict[str, Any]:
    ...
    probe = _trajectory([
        ...
        {"cpart": 901, "time_s": 45.05, "speed": {"rms_m_s": 0.0005}},
    ])
    _validate_trajectory_representation(probe)
    return {
        "status": "PASS_U3_VISCOART0P3_COLD_B45_QC_V1_STATIC_SELF_CHECK",
