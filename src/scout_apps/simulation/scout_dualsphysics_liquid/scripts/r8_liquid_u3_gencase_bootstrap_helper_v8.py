#!/usr/bin/env python3
"""Guest-only bootstrap helper for the admitted U3 C1M GenCase v8 runtime.
...
INPUTS: dict[str, tuple[str, int, int]] = {
    ...
    "C1M_moving_zero_Def.xml": (
        "1711b10f366770cb1b6aa2af799c093596c9d606c914c4a68b9bcdb9fa844b52",
        6_070,
        0o400,
    ),
}
EXPECTED_OUTPUTS: dict[str, tuple[int, int]] = {
    "C1M_zero.bi4": (1, 16_777_216),
    "C1M_zero.xml": (1, 524_288),
    "C1M_zero.out": (1, 1_048_576),
}
GENCASE_ARGV = [
    GENCASE,
    f"{RUNTIME_ROOT}/C1M_moving_zero_Def",
    f"{OUTPUT_ROOT}/C1M_zero",
    "-dp:0.002",
    "-threads:1",
    "-save:-all,+bi",
    "-createdirs:1",
...
FRAME_MAGIC = b"R8C1GENCASEV8\0\0\0"
    ...
    for name, (minimum, maximum) in EXPECTED_OUTPUTS.items():
        ...
        if name.endswith(".xml"):
            ...
            if (keyed.get("MinFluidStop") != "1" or keyed.get("Shifting") != "1"
                ...
                raise GuestError("generated C1M XML lost its fixed parameter contract")
            _audit_zero_motion_generated_features(root)
...
def _audit_zero_motion_node(motion: ET.Element, *, location: str) -> None:
    """Require one source-proven zero-motion object in a known container."""

    if motion.attrib or (motion.text or "").strip() or (motion.tail or "").strip():
        raise GuestError(f"generated C1M XML {location} motion container differs")
    if [child.tag for child in motion] != ["objreal"]:
        raise GuestError(f"generated C1M XML {location} motion object set differs")
    objreal = motion[0]
    if (objreal.attrib != {"ref": "0"}
            or [child.tag for child in objreal] != ["begin", "mvnull"]
            or (objreal.text or "").strip()
            or (objreal.tail or "").strip()):
        raise GuestError(f"generated C1M XML {location} zero-motion object structure differs")
    begin, mvnull = objreal
    try:
        begin_start = float(begin.get("start", "nan"))
    except ValueError as exc:
        raise GuestError(f"generated C1M XML {location} begin start is not numeric") from exc
    if (set(begin.attrib) != {"mov", "start"} or begin.get("mov") != "1"
        ...
        raise GuestError(f"generated C1M XML {location} begin event is not the exact zero-time mvnull link")
    if (mvnull.attrib != {"id": "1"} or len(mvnull)
            or (mvnull.text or "").strip() or (mvnull.tail or "").strip()):
        raise GuestError(f"generated C1M XML {location} mvnull definition differs")


def _audit_zero_motion_generated_features(root: ET.Element) -> None:
    """Require one moving block and both source-proven zero-motion copies."""

    particles = root.find("./execution/particles")
    if particles is None or particles.attrib != {
        "np": "9078", "nb": "2669", "nbf": "0", "mkboundfirst": "2", "mkfluidfirst": "1"
    }:
        raise GuestError("generated C1M XML particle-header contract differs")
    if particles.findall("./fixed") or len(particles.findall("./moving")) != 1:
        raise GuestError("generated C1M XML is not exactly one moving boundary and zero fixed blocks")
    ...
    if moving is None or moving.attrib != {
        ...
        raise GuestError("generated C1M XML moving block contract differs")
    if fluid is None or fluid.attrib != {
        "mkfluid": "0", "mk": "1", "begin": "2669", "count": "6409"
    }:
        raise GuestError("generated C1M XML fluid block contract differs")
    ...
    if (
        ...
        raise GuestError("generated C1M XML must contain exactly the casedef and execution motion copies")
    _audit_zero_motion_node(definition_motion, location="casedef")
    _audit_zero_motion_node(execution_motion, location="execution")
    ...
    if forbidden.intersection(element.tag for element in root.iter()):
        raise GuestError("generated C1M XML enables a forbidden dynamic or external feature")
...
def _success_frame(
    *,
    ...
    bi4 = payloads["C1M_zero.bi4"]
    xml = payloads["C1M_zero.xml"]
    out = payloads["C1M_zero.out"]
    return (
