#!/usr/bin/env python3
"""Root lifecycle supervisor for one fresh U3 C1 CPU-solver smoke v2.
...
exact 29-file solver tree with dirfd/O_NOFOLLOW/O_EXCL.
"""
...
WORKSPACE_SUPERVISOR_PATH = WORKSPACE_PACKAGE_ROOT / "scripts/r8_liquid_target_u3_solver_cpu_supervisor_v2.py"
CASE_ID = "u3_c1_solver_cpu_smoke_v2_20260808T134452Z"
HOST_UID = 1000
HOST_GID = 1000
BOOTSTRAP_PROFILE = "r8-liquid-u3-solver-cpu-bootstrap-v2-20260808t134452z"
RUNTIME_PROFILE = "r8-liquid-u3-solver-cpu-runtime-v2-20260808t134452z"
LABELS = (BOOTSTRAP_PROFILE, RUNTIME_PROFILE)
SNAPSHOT_ROOT = Path(f"/run/r8-liquid-{CASE_ID}.snapshot")
SNAPSHOT_ENV = "R8_LIQUID_U3_SOLVER_CPU_V2_SNAPSHOT"
ADMISSION_FD_ENV = "R8_LIQUID_U3_SOLVER_CPU_V2_ADMISSION_FD"
ADMISSION_SHA256_ENV = "R8_LIQUID_U3_SOLVER_CPU_V2_ADMISSION_SHA256"
EXPORT_FD_ENV = "R8_LIQUID_U3_SOLVER_CPU_V2_EXPORT_FD"
EXPORT_SHA256_ENV = "R8_LIQUID_U3_SOLVER_CPU_V2_EXPORT_SHA256"
ADMISSION_TOKEN = f"{CASE_ID}:single-cpu-solver-smoke-v2-attempt"
POLICY_STATUS = "REVIEWED_FRESH_SINGLE_CPU_SOLVER_SMOKE_V2_TWO_KNOWN_QUIET_DENY_RULES_PENDING_STATIC_VERIFICATION"
...
KNOWN_QUIET_DENY_ORIGINS = {
    CACHE_QUIET_DENY_RULE: "new_v2_rule_for_v1_observed_bwrap_dynamic_loader_probe",
    CAPABILITY_QUIET_DENY_RULE: "preexisting_v1_bwrap_capability_rule",
}
DENIAL_VISIBILITY_BOUNDARY = (
    "closed_zero_unexpected_logged_DENIED_records_with_two_explicit_unlogged_deny_rules;"
    "one_new_cache_and_one_preexisting_capability;"
    "does_not_claim_zero_denied_operations"
)

POLICY_NAME = "liquid_zrj_msi_u2404_u3_solver_cpu_execution_policy_v2.json"
SCHEMA_NAME = "target_host_u3_solver_cpu_execution_policy_v2.json"
PROFILE_NAME = "r8-liquid-u3-solver-cpu-v2.profile"
GATE_NAME = "r8_liquid_target_u3_solver_cpu_gate_v2.py"
HELPER_NAME = "r8_liquid_u3_solver_cpu_bootstrap_helper_v2.py"
SUPERVISOR_NAME = "r8_liquid_target_u3_solver_cpu_supervisor_v2.py"
...
OUTPUT_MAGIC = b"R8SOLVEROUTV2\0\0\0"
OUTPUT_VERSION = 2
...
os.write(1,(json.dumps({{"status":"ROOT_SOLVER_CPU_V2_SNAPSHOT_CREATED_NOT_EXECUTED","files":manifest}},sort_keys=True,separators=(",",":"))+"\\n").encode("ascii"))
'''
...
g={{"__name__":"__main__"}}
exec(compile(p,"<r8-liquid-solver-cpu-v2-root-snapshot-bootstrap>","exec",dont_inherit=True,optimize=2),g,g)
'''
...
def verify_policy_static() -> dict[str, Any]:
    ...
    gate = load_module(GATE_PATH, "_r8_solver_cpu_v2_static_gate")
...
def parse_success_frame(raw: bytes) -> dict[str, Any]:
    ...
    if (
        metadata["document_type"] != "SMPCC_R8_LIQUID_U3_C1_CPU_SOLVER_GUEST_FRAME_V2"
        or metadata["status"] != "GUEST_SOLVER_V2_SMOKE_COMPLETE_PENDING_HOST_REAUDIT_AND_O_EXCL_EXPORT"
    ...
    gate = load_module(GATE_PATH, "_r8_solver_cpu_v2_frame_gate")
    ...
    expected_inputs = {}
    helper = load_module(HELPER_PATH, "_r8_solver_cpu_v2_frame_helper")
...
def load_lifecycle_harness() -> Any:
    ...
    module = load_module(HARNESS_PATH, "_r8_solver_cpu_v2_lifecycle_harness")
...
def run_once(*, policy_sha256: str, admission_token: str) -> dict[str, Any]:
    ...
    gate = load_module(GATE_PATH, "_r8_solver_cpu_v2_runtime_gate")
    with harness.TerminationGuard() as termination_guard:
        ...
        start_document = {
            "document_type": "SMPCC_R8_LIQUID_U3_C1_CPU_SOLVER_V2_START_RECEIPT",
            "case_id": CASE_ID,
        ...
        execution_status = "V2_ATTEMPT_ABORTED_CLEANUP_PENDING"
        try:
            ...
            execution_status = "PASS_U3_C1_CPU_SOLVER_V2_FRAME_VALID_CLOSED_ZERO_UNEXPECTED_LOGGED_DENIALS_TWO_KNOWN_QUIET_DENY_RULES_CLEANUP_PENDING"
        ...
        if parsed is not None and primary_error is None and not cleanup_errors:
            try:
                ...
                if message.get("status") != "PASS_UID1000_SOLVER_TREE_O_EXCL_EXPORT_V2" or not isinstance(message.get("export"), dict):
                ...
                execution_status = "PASS_U3_C1_CPU_SOLVER_V2_SMOKE_EXPORTED_CLOSED_ZERO_UNEXPECTED_LOGGED_DENIALS_TWO_KNOWN_QUIET_DENY_RULES_CLEANUP_PENDING"
            except BaseException as exc:
                primary_error = f"{type(exc).__name__}: {exc}"[:4096]
        ...
        execution_document = {
            "document_type": "SMPCC_R8_LIQUID_U3_C1_CPU_SOLVER_V2_EXECUTION_RECEIPT",
            "case_id": CASE_ID,
            "status": execution_status if primary_error is None and not cleanup_errors else "V2_ATTEMPT_ABORTED_NO_RETRY_SAME_IDENTITY",
            "policy_sha256": policy_sha256,
        ...
        success = execution_document["status"] == "PASS_U3_C1_CPU_SOLVER_V2_SMOKE_EXPORTED_CLOSED_ZERO_UNEXPECTED_LOGGED_DENIALS_TWO_KNOWN_QUIET_DENY_RULES_CLEANUP_PENDING" and parsed is not None and export_verification is not None
        if success:
            lifecycle_document = {
                "document_type": "SMPCC_R8_LIQUID_U3_C1_CPU_SOLVER_V2_LIFECYCLE_RECEIPT",
                "case_id": CASE_ID,
        ...
        failure_document = {
            "document_type": "SMPCC_R8_LIQUID_U3_C1_CPU_SOLVER_V2_LIFECYCLE_INCOMPLETE_RECEIPT",
            "case_id": CASE_ID,
            "status": "V2_LIFECYCLE_OR_EXECUTION_INCOMPLETE_NO_RETRY_SAME_IDENTITY",
            "execution_receipt": execution_record,
            "primary_error": primary_error,
            "cleanup_errors": cleanup_errors,
        ...
        raise SupervisorError(f"solver v2 failed closed; preserved {failure_record['path']}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if arguments.command == "self-check":
            ...
            print(json.dumps({
                "status": "PASS_SOLVER_CPU_V2_SUPERVISOR_STATIC_EXECUTION_NOT_PERFORMED",
                "review": review,
                "snapshot_bootstrap": {
        ...
        if arguments.command == "export-frame":
            ...
            print(json.dumps({"status": "PASS_UID1000_SOLVER_TREE_O_EXCL_EXPORT_V2", "export": result}, ensure_ascii=False, sort_keys=True))
            return 0
        result = run_once(policy_sha256=arguments.policy_sha256, admission_token=arguments.admission_token)
        print(json.dumps({"status": "SOLVER_CPU_V2_ONE_SHOT_LIFECYCLE_RECORDED", "result": result}, ensure_ascii=False, sort_keys=True))
        return 0
        ...
        print(json.dumps({"status": "SOLVER_CPU_V2_NO_GO", "error": str(exc)}, ensure_ascii=False, sort_keys=True), file=target)
        return 2
