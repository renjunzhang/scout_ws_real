...
SNAPSHOT_ENV = "R8_LIQUID_U3_PROC_STAT_BOUNDARY_PROBE_V3_SNAPSHOT"
ADMISSION_ENV = "R8_LIQUID_U3_PROC_STAT_BOUNDARY_PROBE_V3_ADMISSION"
ADMISSION_FD_ENV = "R8_LIQUID_U3_PROC_STAT_BOUNDARY_PROBE_V3_ADMISSION_FD"
ADMISSION_SHA256_ENV = "R8_LIQUID_U3_PROC_STAT_BOUNDARY_PROBE_V3_ADMISSION_SHA256"
...
POLICY_NAME = "liquid_zrj_msi_u2404_u3_proc_stat_boundary_probe_policy_v3.json"
SCHEMA_NAME = "target_host_u3_proc_stat_boundary_probe_policy_v3.json"
PROFILE_NAME = "r8-liquid-u3-proc-stat-boundary-probe-v3.profile"
GATE_NAME = "r8_liquid_target_u3_proc_stat_boundary_probe_gate_v3.py"
SUPERVISOR_NAME = "r8_liquid_target_u3_proc_stat_boundary_probe_supervisor_v3.py"
WORKSPACE_SUPERVISOR_PATH = WORKSPACE_ROOT / "scripts" / SUPERVISOR_NAME
BOOTSTRAP_PROFILE = "r8-liquid-u3-proc-stat-boundary-bootstrap-v3-20260809t022402z"
RUNTIME_PROFILE = "r8-liquid-u3-proc-stat-boundary-runtime-v3-20260809t022402z"
...
SUCCESS_MAGIC = b"R8PROCSTATPASSV3\x00"
...
ROOT="/run"
NAME="r8-liquid-u3_proc_stat_apparmor_boundary_probe_v3_20260809T022402Z.snapshot"
SOURCES=(
 ("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/scripts/r8_liquid_target_u3_proc_stat_boundary_probe_gate_v3.py","r8_liquid_target_u3_proc_stat_boundary_probe_gate_v3.py"),
 ("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/scripts/r8_liquid_target_u3_proc_stat_boundary_probe_supervisor_v3.py","r8_liquid_target_u3_proc_stat_boundary_probe_supervisor_v3.py"),
 ("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/config/apparmor_drafts/r8-liquid-u3-proc-stat-boundary-probe-v3.profile","r8-liquid-u3-proc-stat-boundary-probe-v3.profile"),
 ("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/schema/target_host_u3_proc_stat_boundary_probe_policy_v3.json","target_host_u3_proc_stat_boundary_probe_policy_v3.json"),
 ("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/config/target_hosts/liquid_zrj_msi_u2404_u3_proc_stat_boundary_probe_policy_v3.json","liquid_zrj_msi_u2404_u3_proc_stat_boundary_probe_policy_v3.json"),
)
def open_abs(path):
...
os.write(1,(json.dumps({"status":"ROOT_PROC_STAT_PROBE_V3_SNAPSHOT_CREATED_NOT_EXECUTED","files":manifest},sort_keys=True,separators=(",",":"))+"\n").encode("ascii"))
'''
...
g={{"__name__":"__main__"}}
exec(compile(p,"<r8-liquid-root-proc-stat-probe-bootstrap-v3>","exec",dont_inherit=True,optimize=2),g,g)
'''
...
def verify_full_authority_delta(candidate_lines: tuple[str, ...]) -> dict[str, Any]:
    ...
    if normalized_digest != EXPECTED_NORMALIZED_PROFILE_LINES_SHA256:
        raise SupervisorError("normalized v3 full-profile digest differs")
...
def verify_repository_path_contract(source_path: Path | None = None) -> dict[str, Any]:
    ...
    if (
        ...
        raise SupervisorError("supervisor name must be a fresh v3 literal")
...
def repository_static_review() -> dict[str, Any]:
    ...
    identities = {
        "schema_version": "smpcc-r8-liquid-target-u3-proc-stat-apparmor-boundary-probe-policy-v3",
        "policy_id": "LIQUID_ZRJ_MSI_U2404_U3_PROC_STAT_APPARMOR_BOUNDARY_PROBE_V3",
    ...
    draft_status = "STATIC_DRAFT_V3_PENDING_REVIEWED_BYTE_HASH_FREEZE_AND_INDEPENDENT_REVIEW"
    ready_status = "STATIC_READY_V3_PENDING_INDEPENDENT_REVIEW_AND_EXPLICIT_SINGLE_ATTEMPT"
    ...
    if policy.get("provenance") != expected_provenance:
        raise SupervisorError("frozen v6, v1, and v2 NO-GO provenance policy differs")
    ...
    if (
        payload.get("frame_magic_hex") != "523850524f435354415450524f42453300"
        or payload.get("success_magic_hex") != SUCCESS_MAGIC.hex()
        or payload.get("success_status") != "PASS_U3_PROC_STAT_APPARMOR_BOUNDARY_PROBE_V3"
...
def verify_snapshot_producer_identity() -> dict[str, Any]:
    ...
    if SCRIPT_PATH != WORKSPACE_SUPERVISOR_PATH:
        raise SupervisorError("snapshot producer must be the exact workspace proc-stat v3 supervisor")
...
def verify_snapshot(policy_sha256: str) -> tuple[dict[str, Any], dict[str, dict[str, Any]]]:
    ...
    if (
        policy.get("status") != "STATIC_READY_V3_PENDING_INDEPENDENT_REVIEW_AND_EXPLICIT_SINGLE_ATTEMPT"
    ...
    if policy.get("provenance") != {
        ...
        raise SupervisorError("snapshot v6, v1, and v2 NO-GO provenance differs")
...
def labeled_processes(proc_root: Path = Path("/proc")) -> list[dict[str, Any]]:
    """Authoritatively enumerate every task/thread carrying a fresh v3 label."""
...
def parse_success_frame(frame: bytes) -> dict[str, Any]:
    ...
    if value["status"] != "PASS_U3_PROC_STAT_APPARMOR_BOUNDARY_PROBE_V3":
...
def classify_execution(result: Mapping[str, Any], audit: Mapping[str, Any]) -> tuple[str, dict[str, Any]]:
    ...
    if (result.get("failure") is None and type(returncode) is int and returncode == 0
        ...
        return "PASS_U3_PROC_STAT_APPARMOR_BOUNDARY_PROBE_V3_CLEANUP_PENDING", evidence
...
def execution_receipt_document(
    *,
    ...
    return {
        "document_type": "SMPCC_R8_LIQUID_U3_PROC_STAT_APPARMOR_BOUNDARY_PROBE_V3_EXECUTION_RECEIPT",
        "probe_id": PROBE_ID,
        "status": status,
...
def lifecycle_document(
    *,
    ...
    return {
        "document_type": "SMPCC_R8_LIQUID_U3_PROC_STAT_APPARMOR_BOUNDARY_PROBE_V3_LIFECYCLE_RECEIPT",
        "probe_id": PROBE_ID,
...
def run_once(*, policy_sha256: str, admission_token: str) -> dict[str, Any]:
    with TerminationGuard() as termination_guard:
        try:
            ...
            document = {
                "document_type": "SMPCC_R8_LIQUID_U3_PROC_STAT_APPARMOR_BOUNDARY_PROBE_V3_PREFLIGHT_FAILURE_RECEIPT",
                "probe_id": PROBE_ID,
                "status": (
...
def _run_once_guarded(
    *,
    ...
    start_document = {
        "document_type": "SMPCC_R8_LIQUID_U3_PROC_STAT_APPARMOR_BOUNDARY_PROBE_V3_START_RECEIPT",
        "probe_id": PROBE_ID,
    ...
    failure = {
        "document_type": "SMPCC_R8_LIQUID_U3_PROC_STAT_APPARMOR_BOUNDARY_PROBE_V3_LIFECYCLE_INCOMPLETE_RECEIPT",
        "probe_id": PROBE_ID,
...
def recover_only(*, policy_sha256: str, recovery_token: str) -> dict[str, Any]:
    ...
    with TerminationGuard() as termination_guard:
        ...
        document = {
            "document_type": "SMPCC_R8_LIQUID_U3_PROC_STAT_APPARMOR_BOUNDARY_PROBE_V3_RECOVERY_RECEIPT",
            "probe_id": PROBE_ID,
            "status": "PASS_U3_PROC_STAT_APPARMOR_BOUNDARY_PROBE_V3_CLEANUP_ONLY_NO_PROBE_EXECUTED",
            "policy_sha256": policy_sha256,
            "snapshot": snapshot,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if arguments.command == "recover-only":
            ...
            print(json.dumps({"status": "PROC_STAT_BOUNDARY_PROBE_V3_CLEANUP_ONLY_RECORDED", "result": result}, ensure_ascii=False, sort_keys=True))
            return 0
        result = run_once(policy_sha256=arguments.policy_sha256, admission_token=arguments.admission_token)
        print(json.dumps({"status": "PROC_STAT_BOUNDARY_PROBE_V3_ONE_SHOT_LIFECYCLE_RECORDED", "result": result}, ensure_ascii=False, sort_keys=True))
        return 0 if result["execution_status"] == "PASS_U3_PROC_STAT_APPARMOR_BOUNDARY_PROBE_V3_CLEANUP_PENDING" else 2
