#!/usr/bin/env python3
"""Read-only QC for fresh viscosity-0.3 GPU cold-A/B runs.

The tool parses only preserved BI4 outputs, verifies the closed 702-frame
inventory and frozen numerical identity, evaluates the unchanged 17 settled-
state limits over the final one-second tail, and publishes one O_EXCL receipt.
It never runs the solver or uses a GPU.
"""
...
import r8_liquid_u3_gpu_viscoart0p3_cold_ab_runner_v1 as runner
...
SCHEMA_PATH = PACKAGE_ROOT / "schema/target_host_u3_viscoart0p3_cold_ab_qc_v1.json"
TEST_PATH = PACKAGE_ROOT / "tests/test_u3_viscoart0p3_cold_ab_qc_v1.py"
...
PART_INDICES = tuple(range(0, 702))
...
class ColdAbQcError(ValueError):
    """Unsafe, drifted, incomplete or inconsistent cold-A/B evidence."""
...
def _trajectory(parts: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if not series or any(not math.isfinite(value) or value < 0 for _, _, value in series):
        raise ColdAbQcError("speed trajectory is empty, negative or non-finite")
...
def _validate_trajectory_representation(value: dict[str, Any]) -> None:
    ...
    if not isinstance(minimum, (int, float)) or not math.isfinite(float(minimum)) or minimum < 0:
        raise ColdAbQcError("trajectory minimum is invalid")
    is_zero = float(minimum) == 0.0
    if flag is not is_zero:
        raise ColdAbQcError("minimum_is_zero disagrees with the minimum")
    if is_zero and ratio is not None:
        raise ColdAbQcError("zero minimum must have a null ratio")
    if not is_zero and (
        not isinstance(ratio, (int, float))
        or not math.isfinite(float(ratio))
        or ratio < 1
    ):
        raise ColdAbQcError("positive minimum must have a finite ratio >= 1")


def adjudicate(metrics: dict[str, float], structural_pass: bool, initial_state_valid: bool) -> dict[str, Any]:
    if tuple(sorted(metrics)) != METRIC_NAMES:
        raise ColdAbQcError("metric vector differs from the frozen 17 items")
    if any(not isinstance(value, (int, float)) or not math.isfinite(float(value)) or float(value) < 0 for value in metrics.values()):
        raise ColdAbQcError("metric vector contains invalid values")
    ...
    selected = structural_pass and initial_state_valid and not failed
    return {
        "metric_absolute_pass": passes,
        "failed_absolute_metrics": failed,
        "selected": selected,
        "status": (
            "PASS_U3_VISCOART0P3_COLD_SETTLING_CANDIDATE"
            if selected else "FAIL_U3_VISCOART0P3_COLD_NOT_SETTLED"
        ),
    }
...
def _parse_solver_proof(run_out_source: bi4.SecureFile) -> dict[str, Any]:
    ...
    if shifting != ["None"] or viscosity != ["Artificial"] or visco != ["0.3"]:
        raise ColdAbQcError(f"solver numerical identity differs: {shifting!r}/{viscosity!r}/{visco!r}")
...
def build_report(policy_path: Path) -> dict[str, Any]:
    ...
    if len(roles) != 1:
        raise ColdAbQcError("policy path differs")
    role = roles[0]
    ...
    required_final = (
        ...
        and final.get("run_id") == runner.RUN_IDS[role]
        and final.get("purpose") == runner.PURPOSES[role]
    ...
    if not required_final:
        raise ColdAbQcError("final receipt semantics differ")
    ...
    for key in ("file_count", "part_file_count", "part_first", "part_last", "total_bytes", "canonical_sha256"):
        if observed_inventory.get(key) != frozen_inventory.get(key):
            raise ColdAbQcError(f"output inventory differs at {key}")
    if any(item["mode"] != "0440" or item["nlink"] != 1 for item in observed_inventory["files"].values()):
        raise ColdAbQcError("output mode/link invariant differs")
    ...
    structural = {
        "exact_inventory": observed_inventory["file_count"] == 711,
        ...
        "start_time_matches": abs(times[0] - 0.0) <= metric_contract.CONTRACT.physical_time_tolerance_s,
        "end_time_matches": abs(times[-1] - 35.05) <= metric_contract.CONTRACT.physical_time_tolerance_s,
        ...
        "run_csv_matches": fields["Np"] == 9078 and fields["PartFiles"] == 702 and abs(fields["PhysicalTime"] - times[-1]) <= metric_contract.CONTRACT.physical_time_tolerance_s,
    ...
    if not all(structural.values()):
        raise ColdAbQcError(f"structural checks failed: {[name for name, value in structural.items() if not value]}")

    initial_state = _canonical_state(root / "data/Part_0000.bi4")
    initial_state_valid = initial_state["particle_count"] == 9078 and parts[0]["cpart"] == 0
    if not initial_state_valid:
        raise ColdAbQcError("fresh Part_0000 state differs")
    metrics, tail = metric_contract._metric_vector(parts, case)
    result = adjudicate(metrics, all(structural.values()), initial_state_valid)
    ...
    if inputs["plan"]["sha256"] != PLAN_SHA256:
        raise ColdAbQcError("frozen plan identity differs")
    report = {
        "schema_version": "r8-liquid-u3-viscoart0p3-cold-ab-qc-v1",
        "document_type": "SMPCC_R8_LIQUID_U3_VISCOART0P3_COLD_AB_QC_V1",
        ...
        "run": {
            ...
            "final_checkpoint": _identity(root / "data/Part_0701.bi4"),
        },
        "verdict": {
            ...
            "exact_blocker": "NONE" if result["selected"] else f"VISCOART0P3_COLD_{role.upper()}_FAILS_{len(result['failed_absolute_metrics'])}_OF_17_ABSOLUTE_SETTLING_LIMITS",
            "next": (
                "RUN_FRESH_COLD_B" if role == "a" and result["selected"]
                else "COMPARE_COLD_AB_AND_PREPARE_RESTART_EQUIVALENCE" if result["selected"]
                else "STOP_OR_FREEZE_ONE_NEW_NUMERICAL_DELTA"
            ),
...
def validate_report(report: dict[str, Any]) -> None:
    run = report.get("run", {})
    _validate_trajectory_representation(run.get("trajectory", {}))
    ...
    if tuple(sorted(metrics)) != METRIC_NAMES or tuple(sorted(passes)) != METRIC_NAMES:
        raise ColdAbQcError("report metric/pass keys differ")
    ...
    if failed != expected_failed or any(passes[name] is not (name not in expected_failed) for name in METRIC_NAMES):
        raise ColdAbQcError("metric values, pass vector and failure list disagree")
    selected = not expected_failed and all(run.get("structural_checks", {}).values()) and run.get("initial_state_valid") is True
    verdict = report.get("verdict", {})
    expected_status = "PASS_U3_VISCOART0P3_COLD_SETTLING_CANDIDATE" if selected else "FAIL_U3_VISCOART0P3_COLD_NOT_SETTLED"
    if verdict.get("status") != expected_status or verdict.get("u3_remediation_candidate_pass") is not selected:
        raise ColdAbQcError("verdict disagrees with frozen adjudication")
    role = run.get("run_role")
    expected_next = (
        "RUN_FRESH_COLD_B" if selected and role == "a"
        else "COMPARE_COLD_AB_AND_PREPARE_RESTART_EQUIVALENCE" if selected and role == "b"
        else "STOP_OR_FREEZE_ONE_NEW_NUMERICAL_DELTA"
    )
    if role not in ("a", "b") or verdict.get("next") != expected_next:
        raise ColdAbQcError("run role and next gate disagree")
    ...
    if errors:
        first = errors[0]
        raise ColdAbQcError(f"QC schema failure at {list(first.absolute_path)}: {first.message}")
...
def write_exclusive(path: Path, report: dict[str, Any]) -> dict[str, Any]:
    ...
    if len(data) > MAX_JSON_BYTES:
        raise ColdAbQcError("QC output exceeds bound")
    ...
    try:
        ...
        while view:
            ...
            if written <= 0:
                raise ColdAbQcError("short QC write")
            view = view[written:]
        os.fsync(descriptor)
    finally:
        os.close(descriptor)
...
def static_self_check() -> dict[str, Any]:
    ...
    if METRIC_LIMITS != dict(sorted(metric_contract.METRIC_LIMITS.items())):
        raise ColdAbQcError("metric contract drifted")
    if schema["$defs"]["inventory"]["properties"]["total_bytes"]["maximum"] != runner.EXPECTED_OUTPUT["maximum_output_bytes"]:
        raise ColdAbQcError("QC inventory maximum differs from the frozen runner limit")
    probe = _trajectory([
    ...
    return {
        "status": "PASS_U3_VISCOART0P3_COLD_AB_QC_V1_STATIC_SELF_CHECK",
        "zero_initial_speed_representation": "NULL_RATIO_WITH_EXPLICIT_MINIMUM_IS_ZERO",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.output is None:
            raise ColdAbQcError("--output is required")
    ...
    except (ColdAbQcError, runner.ColdAbRunError, legacy.Stage4RunError, metric_contract.DdtRampQcError, qc_v3.QcError, bi4.Bi4FormatError, OSError, UnicodeError, ValueError, subprocess.SubprocessError) as exc:
