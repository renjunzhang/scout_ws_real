#!/usr/bin/env python3
"""Read-only QC v3 for one or two completed U3 C1M CPU output trees.
...
SCHEMA_VERSION = "r8-liquid-u3-solver-output-qc-v3"
...
MOTION_REF_NAME = "PartMotionRef.ibi4"
...
def _require_value_type(item: bi4.JbdItem, name: str, expected_type: int) -> Any:
    if name not in item.values or item.value_types.get(name) != expected_type:
        raise QcError(
            f"JBD value {name!r} is missing or has type "
            f"{item.value_types.get(name)!r}, expected {expected_type}"
        )
...
def parse_case_xml(source: bi4.SecureFile) -> dict[str, Any]:
    ...
    casedef_motion = root.find("./casedef/motion")
    execution_motion = root.find("./execution/motion")
    if motions != [casedef_motion, execution_motion] or any(item is None for item in motions):
        raise QcError("C1M case must contain only casedef/motion and execution/motion")
    for motion in motions:
        assert motion is not None
        if motion.attrib or len(motion) != 1 or (motion.text and motion.text.strip()):
            raise QcError("C1M zero-motion container differs")
        objreal = motion[0]
        if objreal.tag != "objreal" or objreal.attrib != {"ref": "0"} or len(objreal) != 2:
            raise QcError("C1M objreal motion differs")
        begin, mvnull = objreal
        if (
            begin.tag != "begin"
            or begin.attrib != {"mov": "1", "start": "0"}
            or len(begin)
            or mvnull.tag != "mvnull"
            or mvnull.attrib != {"id": "1"}
            or len(mvnull)
        ):
            raise QcError("C1M zero-motion primitive differs")
        for node in (objreal, begin, mvnull):
            if node.text and node.text.strip():
                raise QcError("C1M motion contains unexpected text")
    fixed = particles.find("./fixed")
    moving = particles.find("./moving")
    fluid = particles.find("./fluid")
    if fixed is not None or moving is None or fluid is None:
        raise QcError("C1M case must have moving/fluid and zero fixed blocks")
    parameters = {
    ...
    if parameters.get("Shifting") != "1" or parameters.get("DtAllParticles") != "1":
        raise QcError("C1M Shifting/DtAllParticles identity differs")
    ...
    return {
        ...
        "fixed": 0,
        "moving": _uint_text(moving.attrib.get("count", ""), "moving.count"),
        "moving_begin": _uint_text(moving.attrib.get("begin", ""), "moving.begin"),
        "moving_mkbound": _uint_text(
            moving.attrib.get("mkbound", ""), "moving.mkbound"
        ),
        "moving_refmotion": _uint_text(
            moving.attrib.get("refmotion", ""), "moving.refmotion"
        ),
        "fluid": _uint_text(fluid.attrib.get("count", ""), "fluid.count"),
        "mkboundfirst": _uint_text(
            particles.attrib.get("mkboundfirst", ""), "particles.mkboundfirst"
        ),
        ...
        "motion_zero_copy_count": 2,
        "motion_primitive": "objreal_ref0_begin_mov1_start0_mvnull_id1",
        "shifting": 1,
        "dt_all_particles": 1,
    }


def load_moving_case(
    ...
    *,
    ...
    try:
        ...
        raise QcError(f"invalid C1M case BI4: {exc}") from exc
    ...
    if (
        xml["fixed"] != counts["fixed_boundary"]
        or xml["moving"] != counts["moving_boundary"]
        or xml["fluid"] != counts["fluid"]
    ):
        errors.append("XML/BI4 particle class mismatch")
    if counts["fixed_boundary"] or not counts["moving_boundary"] or counts["floating"]:
        errors.append("C1M case must have zero fixed/floating and nonzero moving particles")
    ...
    if any(not math.isfinite(v) for vec in particles["positions_m"] for v in vec):
        raise QcError("C1M case contains non-finite positions")
    if any(not math.isfinite(v) for vec in particles["velocities_m_s"] for v in vec):
        raise QcError("C1M case contains non-finite velocities")
    return {
        "bi4": {
...
def _root_case_invariants(
    ...
    """Compare the moving-boundary case invariants carried into a PART root."""

    report: dict[str, dict[str, Any]] = {}
    count_fields = (
        ("CaseNp", case["particle_count"]),
        ("CaseNfixed", case["counts"]["fixed_boundary"]),
        ("CaseNmoving", case["counts"]["moving_boundary"]),
        ("CaseNfloat", case["counts"]["floating"]),
...
def parse_part_file(
    ...
    fixed_count = case["counts"]["fixed_boundary"]
    moving_begin = fixed_count
    moving_end = moving_begin + case["counts"]["moving_boundary"]
    fluid_begin = moving_end + case["counts"]["floating"]
    fixed_ids = [value for value in ids if 0 <= value < fixed_count]
    moving_ids = [value for value in ids if moving_begin <= value < moving_end]
    fluid_ids = [value for value in ids if fluid_begin <= value < expected_total]
    ...
    moving_positions_exact = id_unique and all(
        position_by_id.get(value) == case["positions_by_id"][value]
        for value in range(moving_begin, moving_end)
    )
    moving_velocities_zero = id_unique and all(
    ...
    fluid_indices = [
        ...
        if fluid_begin <= value < expected_total
    ]
    ...
    return {
        ...
        "counts": {
            "fixed_boundary": len(fixed_ids),
            "moving_boundary": len(moving_ids),
            "fluid": fluid_count,
        },
        ...
        "moving_positions_exact": moving_positions_exact,
        "moving_velocities_zero": moving_velocities_zero,
        "matches_moving_case_initial_arrays": initial_arrays_exact,
        ...
        "_positions_by_id": position_by_id,
    }
...
def _parse_jbd_list(source: bi4.SecureFile, expected_code: str) -> list[bi4.JbdItem]:
    """Decode one bounded JBD list stream (root item plus appended items)."""

    data = source.data
    if len(data) < bi4.JBD_HEADER_SIZE + 4:
        raise QcError(f"JBD list file is too small: {source.path.name}")
    header = data[: bi4.JBD_HEADER_SIZE]
    try:
        title = header[:58].decode("ascii").rstrip(" ")
    except UnicodeDecodeError as exc:
        raise QcError(f"JBD list header is not ASCII: {source.path.name}") from exc
    if title != f"#FileJBD {expected_code}" or header[58:60] != b"\n\0":
        raise QcError(f"invalid {expected_code} header: {source.path.name}")
    byte_order, si64, reserved2, reserved3 = header[60:64]
    if byte_order not in (0, 10) or si64 not in (0, 1):
        raise QcError(f"unsupported {expected_code} byte order/size mode")
    if bool(byte_order == 10) != bool(si64) or reserved2 or reserved3:
        raise QcError(f"inconsistent {expected_code} header flags")
    reader = bi4._Reader(data[bi4.JBD_HEADER_SIZE :])  # frozen v1 JBD codec
    items: list[bi4.JbdItem] = []
    try:
        while reader.remaining:
            items.append(bi4._read_item(reader, si64=bool(si64), depth=0))
    except bi4.Bi4FormatError as exc:
        raise QcError(f"invalid {expected_code} item stream: {exc}") from exc
    if not items or items[0].name != expected_code:
        raise QcError(f"{expected_code} stream lacks its root item")
    return items


def parse_motion_ref(
    source: bi4.SecureFile,
    parts: list[dict[str, Any]],
    case: dict[str, Any],
    thresholds: Thresholds,
) -> dict[str, Any]:
    """Strictly validate the C1M zero-motion reference stream."""

    items = _parse_jbd_list(source, "JPartMotRefBi4")
    root, appended = items[0], items[1:]
    if root.hidden or root.values_hidden or root.items:
        raise QcError("PartMotionRef root unexpectedly embeds child items")
    ...
    if set(root.values) != set(expected_root_value_types):
        raise QcError("PartMotionRef root value set differs")
    for name, expected_type in expected_root_value_types.items():
        _require_value_type(root, name, expected_type)
    if not isinstance(root.values["AppName"], str) or not root.values["AppName"]:
        raise QcError("PartMotionRef AppName is empty or invalid")
    expected_values = {
        "FormatVer": 230729,
        "MainFile": True,
        "MkMovingCount": 1,
    ...
    for name, expected in expected_values.items():
        if root.values.get(name) != expected:
            raise QcError(
                f"PartMotionRef {name} differs: {root.values.get(name)!r} != {expected!r}"
            )
    timeout = _finite_float(root.values["TimeOut"], "PartMotionRef TimeOut")
    if not _same_float(timeout, case["xml"]["time_out_xml_s"]):
        raise QcError("PartMotionRef TimeOut differs from the case XML")
    if root.values["MkBoundFirst"] != case["xml"]["mkboundfirst"]:
        raise QcError("PartMotionRef MkBoundFirst differs from case XML")
    ...
    if set(root.arrays) != set(expected_root_arrays):
        raise QcError("PartMotionRef root array set differs")
    for name, (type_code, count) in expected_root_arrays.items():
        array = root.arrays[name]
        if array.type_code != type_code or array.count != count or array.hidden:
            raise QcError(f"PartMotionRef root array {name} type/count differs")
    try:
        ...
        raise QcError(f"invalid PartMotionRef root arrays: {exc}") from exc
    if mkbounds != [case["xml"]["moving_mkbound"]] or nids != [3]:
        raise QcError("PartMotionRef moving-block identity differs")
    if case["xml"]["moving_begin"] != 0 or case["xml"]["moving_refmotion"] != 0:
        raise QcError("C1M moving block begin/refmotion differs")
    ...
    if len(set(reference_ids)) != 3 or not all(0 <= value < moving_end for value in reference_ids):
        raise QcError("PartMotionRef reference IDs are not three distinct moving IDs")
    if not all(math.isfinite(value) for vector in root_positions for value in vector):
        raise QcError("PartMotionRef root Ps contains a non-finite value")
    if not all(math.isfinite(value) and value >= 0 for value in distances):
        raise QcError("PartMotionRef root Dis contains an invalid value")
    expected_root_positions = [case["positions_by_id"][value] for value in reference_ids]
    if root_positions != expected_root_positions:
        raise QcError("PartMotionRef root Ps differs from C1M case reference particles")
    expected_distances = [
        1.1
        * math.sqrt(
            sum(
                (position[axis] - root_positions[0][axis]) ** 2
                for axis in range(3)
            )
        )
    ...
    if not all(
        _same_float(observed, expected)
        for observed, expected in zip(distances, expected_distances)
    ):
        raise QcError("PartMotionRef root Dis differs from source-defined geometry")

    if len(appended) != len(parts):
        raise QcError("PartMotionRef PART count differs from particle PART count")
    first_positions: list[Any] | None = None
    part_reports = []
    for index, (item, particle_part) in enumerate(zip(appended, parts)):
        if item.name != f"PART_{index:04d}":
            raise QcError(f"unexpected PartMotionRef item {item.name!r}")
        if not item.hidden or item.values_hidden or item.items or set(item.arrays) != {"PosRef"}:
            raise QcError(f"PartMotionRef PART_{index:04d} structure differs")
        if set(item.values) != {"Cpart", "TimeStep", "Step"}:
            raise QcError(f"PartMotionRef PART_{index:04d} value set differs")
        _require_value_type(item, "Cpart", 8)
        _require_value_type(item, "TimeStep", 12)
        _require_value_type(item, "Step", 8)
        cpart = _required_uint(item.values, "Cpart")
        step = _required_uint(item.values, "Step")
        time_s = _finite_float(item.values["TimeStep"], "PartMotionRef TimeStep")
        positions_array = item.arrays["PosRef"]
        if (
            positions_array.type_code != 23
            or positions_array.count != 3
            or positions_array.hidden
        ):
            raise QcError(f"PartMotionRef PART_{index:04d} PosRef type/count differs")
        try:
            positions = positions_array.records()
        except bi4.Bi4FormatError as exc:
            raise QcError(f"invalid PartMotionRef PART_{index:04d} PosRef: {exc}") from exc
        if not all(math.isfinite(value) for vector in positions for value in vector):
            raise QcError(f"PartMotionRef PART_{index:04d} contains non-finite PosRef")
        if cpart != particle_part["cpart"] or step != particle_part["step"]:
            raise QcError(f"PartMotionRef PART_{index:04d} index/step is misaligned")
        if abs(time_s - particle_part["time_s"]) > thresholds.physical_time_tolerance_s:
            raise QcError(f"PartMotionRef PART_{index:04d} time is misaligned")
        expected_part_positions = [
            particle_part["_positions_by_id"].get(value) for value in reference_ids
        ]
        if positions != expected_part_positions:
            raise QcError(
                f"PartMotionRef PART_{index:04d} PosRef differs from particle PART"
            )
        if first_positions is None:
            first_positions = positions
        elif positions != first_positions:
            raise QcError("PartMotionRef PosRef changes during a zero-motion run")
        part_reports.append(
            {"name": item.name, "cpart": cpart, "time_s": time_s, "step": step}
        )
    return {
        "name": source.path.name,
        "sha256": source.sha256,
...
def analyze_run(run_dir: str | os.PathLike[str], case: dict[str, Any], thresholds: Thresholds) -> dict[str, Any]:
    ...
    motion_ref = parse_motion_ref(
        _read(data_dir / MOTION_REF_NAME, maximum=MAX_PART_BYTES),
        parts,
        case,
        thresholds,
    )
    ...
    first_matches_case = (
        parts[0]["matches_moving_case_initial_arrays"]
    ...
    structural_checks = {
        ...
        "first_snapshot_matches_moving_case": first_matches_case,
        ...
        "class_counts_constant": all(
            ...
            and part["counts"]["moving_boundary"] == case["counts"]["moving_boundary"]
            and part["counts"]["fluid"] == case["counts"]["fluid"]
            for part in parts
        ),
        "all_part_root_invariants_match_moving_case": all(
            part["root_case_invariants_match"] for part in parts
        ),
        "moving_boundary_unchanged": all(
            part["moving_positions_exact"] and part["moving_velocities_zero"]
            for part in parts
        ),
        "motion_reference_zero_and_aligned": (
            motion_ref["all_posref_finite"]
            and motion_ref["all_posref_unchanged"]
            and motion_ref["aligned_with_particle_parts"]
        ),
    ...
    return {
        ...
        "inventory": {
            ...
            "motion_reference_sha256": motion_ref["sha256"],
        },
        "run_csv": run_csv,
        "run_out": run_out,
        "parts": parts,
...
def compare_runs(first: dict[str, Any], second: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "motion_reference_sha256_exact": (
            first["inventory"]["motion_reference_sha256"]
            == second["inventory"]["motion_reference_sha256"]
        ),
...
def build_report(
    ...
    *,
    ...
    if any_hard_failure:
        ...
        status = (
            "C1M_ZERO_MOTION_TWO_RUN_SMOKE_PASS"
            if comparison_pass
            else "FAIL_U3_C1M_SMOKE_REPRODUCIBILITY_QC"
        )
    ...
    return {
        ...
        "document_type": "SMPCC_R8_LIQUID_U3_SOLVER_OUTPUT_QC_V3",
