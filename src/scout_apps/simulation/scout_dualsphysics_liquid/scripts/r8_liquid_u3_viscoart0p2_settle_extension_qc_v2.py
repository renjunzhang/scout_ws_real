#!/usr/bin/env python3
"""Read-only QC for the v10 36.05--40.05 s settling extension.

The tool parses only preserved BI4 outputs, proves exact restart-state
identity at Part_0721, verifies the closed 81-frame inventory and the single
end-time delta, evaluates the unchanged 17 settled-state limits over the full
final one-second tail, and publishes one O_EXCL receipt.  It never runs the
...
SCHEMA_PATH = PACKAGE_ROOT / "schema/target_host_u3_viscoart0p2_settle_extension_qc_v2.json"
TEST_PATH = PACKAGE_ROOT / "tests/test_u3_viscoart0p2_settle_extension_qc_v2.py"
...
PART_INDICES = tuple(range(721, 802))
...
def build_report(policy_path: Path) -> dict[str, Any]:
    ...
    structural = {
        "exact_inventory": observed_inventory["file_count"] == 90,
        ...
        "start_time_matches": abs(times[0] - 36.05) <= metric_contract.CONTRACT.physical_time_tolerance_s,
        "end_time_matches": abs(times[-1] - 40.05) <= metric_contract.CONTRACT.physical_time_tolerance_s,
        ...
        "run_csv_matches": fields["Np"] == 9078 and fields["PartFiles"] == 802 and abs(fields["PhysicalTime"] - times[-1]) <= metric_contract.CONTRACT.physical_time_tolerance_s,
    ...
    restart_output = _canonical_state(root / "data/Part_0721.bi4")
    restart_matches = restart_input["canonical_array_sha256"] == restart_output["canonical_array_sha256"]
    if not restart_matches:
        raise SettleExtensionQcError("restart Part_0721 state arrays differ")
    ...
    report = {
        "schema_version": "r8-liquid-u3-viscoart0p2-settle-extension-qc-v2",
        "document_type": "SMPCC_R8_LIQUID_U3_VISCOART0P2_SETTLE_EXTENSION_QC_V2",
        "captured_at_utc": datetime.now(timezone.utc).isoformat(),
        "inputs": inputs,
        "contract": {
            "window_start_s": 36.05,
            "window_end_s": 40.05,
            "tail_window_s": 1.0,
            "part_first": 721,
            "part_last": 801,
        ...
        "run": {
            ...
            "final_checkpoint": _identity(root / "data/Part_0801.bi4"),
        },
        "verdict": {
...
def static_self_check() -> dict[str, Any]:
    ...
    return {
        "status": "PASS_U3_VISCOART0P2_SETTLE_EXTENSION_QC_V2_STATIC_SELF_CHECK",
