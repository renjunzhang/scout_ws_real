#!/usr/bin/env python3
"""Static gate and UID-1000 conduit for one U3 C1M cold-A settle v5.
...
HELPER_MAGIC = b"R8SOLVERHELPV5\0\0"
INPUT_MAGIC = b"R8SOLVERINPUT5\0\0"
...
def guest_loader(*, helper_size: int, helper_sha256: str) -> str:
    return (
        ...
        "g={'__name__':'__main__','__file__':'<r8-liquid-u3-c1m-solver-cpu-settle-cold-a-helper-v5>'}\n"
...
def verify_review_artifacts(*, verify_tools: bool = True) -> dict[str, Any]:
    ...
    if (
        ...
        raise GateError("cold-A v5 campaign or one-shot identity differs")
    ...
    if (
        input_frame.get("helper_magic") != "R8SOLVERHELPV5\\0\\0"
        or input_frame.get("input_magic") != "R8SOLVERINPUT5\\0\\0"
        or output_frame.get("magic") != "R8SOLVEROUTV5\\0\\0\\0"
        or output_frame.get("version") != 5
        ...
        or transport.get("abi_baseline") != "cold_a_v4_normalized"
        or transport.get("abi_delta_from_v4") != "version_discriminator_only"
    ):
        raise GateError("cold-A v5 immutable transport contract differs")
...
def main() -> int:
    ...
    try:
        if arguments.command == "self-check":
            ...
            print(json.dumps({"status": "PASS_SOLVER_CPU_SETTLE_COLD_A_V5_STATIC_EXECUTION_NOT_PERFORMED", "review": review}, ensure_ascii=False, sort_keys=True))
            return 0
        ...
        print(json.dumps({"status": "SOLVER_CPU_SETTLE_COLD_A_V5_NO_GO", "error": str(exc)}, ensure_ascii=False, sort_keys=True), file=stream)
        return 2
