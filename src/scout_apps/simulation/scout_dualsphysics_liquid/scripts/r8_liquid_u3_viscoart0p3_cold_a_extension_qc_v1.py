#!/usr/bin/env python3
"""Read-only QC for the v13 35.05--45.05 s cold-A settling extension.

The tool parses only preserved BI4 outputs, proves exact restart-state
identity at Part_0701, verifies the closed 201-frame inventory and the single
...
import r8_liquid_u3_gpu_viscoart0p3_cold_a_extension_runner_v1 as runner
...
SCHEMA_PATH = PACKAGE_ROOT / "schema/target_host_u3_viscoart0p3_cold_a_extension_qc_v1.json"
TEST_PATH = PACKAGE_ROOT / "tests/test_u3_viscoart0p3_cold_a_extension_qc_v1.py"
...
PART_INDICES = tuple(range(701, 902))
...
def adjudicate(metrics: dict[str, float], structural_pass: bool, restart_matches: bool) -> dict[str, Any]:
    ...
    return {
        ...
        "selected": selected,
        "status": (
            "PASS_U3_VISCOART0P3_COLD_A_SETTLING_EXTENSION_CANDIDATE"
            if selected else "FAIL_U3_VISCOART0P3_COLD_A_SETTLING_EXTENSION_NOT_SETTLED"
        ),
    }
...
def build_report(policy_path: Path) -> dict[str, Any]:
    ...
    required_final = (
        ...
        and final.get("purpose") == "viscoart0p3_cold_a_settling_extension"
    ...
    structural = {
        "exact_inventory": observed_inventory["file_count"] == 210,
    ...
    report = {
        "schema_version": "r8-liquid-u3-viscoart0p3-cold-a-extension-qc-v1",
        "document_type": "SMPCC_R8_LIQUID_U3_VISCOART0P3_COLD_A_EXTENSION_QC_V1",
        ...
        "verdict": {
            ...
            "exact_blocker": "NONE" if result["selected"] else f"VISCOART0P3_COLD_A_SETTLING_EXTENSION_FAILS_{len(result['failed_absolute_metrics'])}_OF_17_ABSOLUTE_SETTLING_LIMITS",
            "next": "FREEZE_FRESH_0_TO_45P05_COLD_B_REPEATABILITY_AND_RESTART_EQUIVALENCE_CONTRACT" if result["selected"] else "STOP_VISCOART0P3_PARAMETER_BRANCH",
...
def validate_report(report: dict[str, Any]) -> None:
    ...
    expected_status = "PASS_U3_VISCOART0P3_COLD_A_SETTLING_EXTENSION_CANDIDATE" if selected else "FAIL_U3_VISCOART0P3_COLD_A_SETTLING_EXTENSION_NOT_SETTLED"
...
def static_self_check() -> dict[str, Any]:
    ...
    return {
        "status": "PASS_U3_VISCOART0P3_COLD_A_EXTENSION_QC_V1_STATIC_SELF_CHECK",
