#!/usr/bin/env python3
"""Read-only QC for the v7 30.05--35.05 s viscosity-0.2 probe.

The tool parses only preserved BI4 outputs.  It proves exact restart-state
equivalence at Part_0601, verifies the closed 101-frame inventory and exact
...
import r8_liquid_u3_gpu_viscoart0p2_probe_runner_v1 as runner
...
SCHEMA_PATH = PACKAGE_ROOT / "schema/target_host_u3_viscoart0p2_probe_qc_v1.json"
TEST_PATH = PACKAGE_ROOT / "tests/test_u3_viscoart0p2_probe_qc_v1.py"
...
PART_INDICES = tuple(range(601, 702))
...
def adjudicate(metrics: dict[str, float], structural_pass: bool, restart_matches: bool) -> dict[str, Any]:
    ...
    return {
        ...
        "selected": selected,
        "status": (
            "PASS_U3_VISCOART0P2_SENSITIVITY_CANDIDATE"
            if selected else "FAIL_U3_VISCOART0P2_SENSITIVITY_NOT_SETTLED"
        ),
    }
...
def build_report(policy_path: Path) -> dict[str, Any]:
    ...
    required_final = (
        ...
        and final.get("purpose") == "viscoart0p2_sensitivity_probe"
    ...
    structural = {
        ...
        "start_time_matches": abs(times[0] - 30.05) <= metric_contract.CONTRACT.physical_time_tolerance_s,
    ...
    restart_output = _canonical_state(root / "data/Part_0601.bi4")
    restart_matches = restart_input["canonical_array_sha256"] == restart_output["canonical_array_sha256"]
    if not restart_matches:
        raise ViscoartProbeQcError("restart Part_0601 state arrays differ")
    ...
    report = {
        "schema_version": "r8-liquid-u3-viscoart0p2-probe-qc-v1",
        "document_type": "SMPCC_R8_LIQUID_U3_VISCOART0P2_PROBE_QC_V1",
        "captured_at_utc": datetime.now(timezone.utc).isoformat(),
        "inputs": inputs,
        "contract": {
            "window_start_s": 30.05,
            "window_end_s": 35.05,
            "tail_window_s": 1.0,
            "part_first": 601,
            "part_last": 701,
        ...
        "verdict": {
            ...
            "exact_blocker": "NONE" if result["selected"] else f"VISCOART0P2_PROBE_FAILS_{len(result['failed_absolute_metrics'])}_OF_17_ABSOLUTE_SETTLING_LIMITS",
...
def validate_report(report: dict[str, Any]) -> None:
    ...
    expected_status = "PASS_U3_VISCOART0P2_SENSITIVITY_CANDIDATE" if selected else "FAIL_U3_VISCOART0P2_SENSITIVITY_NOT_SETTLED"
...
def static_self_check() -> dict[str, Any]:
    ...
    return {
        "status": "PASS_U3_VISCOART0P2_PROBE_QC_V1_STATIC_SELF_CHECK",
