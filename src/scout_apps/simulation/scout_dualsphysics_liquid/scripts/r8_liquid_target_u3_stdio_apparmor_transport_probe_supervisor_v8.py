#!/usr/bin/env python3
"""Root lifecycle supervisor for the one-shot harmless v8 successor probe.
...
WORKSPACE_SUPERVISOR_PATH = Path("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/scripts/r8_liquid_target_u3_stdio_apparmor_transport_probe_supervisor_v8.py")
PROBE_ID = "u3_stdio_apparmor_transport_probe_v8_20260807T183304Z"
SNAPSHOT_ROOT = Path(f"/run/r8-liquid-{PROBE_ID}.snapshot")
SNAPSHOT_ENV = "R8_LIQUID_U3_STDIO_PROBE_V8_SNAPSHOT"
ADMISSION_ENV = "R8_LIQUID_U3_STDIO_PROBE_V8_ADMISSION"
ADMISSION_FD_ENV = "R8_LIQUID_U3_STDIO_PROBE_V8_ADMISSION_FD"
ADMISSION_SHA256_ENV = "R8_LIQUID_U3_STDIO_PROBE_V8_ADMISSION_SHA256"
...
POLICY_NAME = "liquid_zrj_msi_u2404_u3_stdio_apparmor_transport_probe_policy_v8.json"
SCHEMA_NAME = "target_host_u3_stdio_apparmor_transport_probe_policy_v8.json"
PROFILE_NAME = "r8-liquid-u3-stdio-transport-probe-v8.profile"
GATE_NAME = "r8_liquid_target_u3_stdio_apparmor_transport_probe_gate_v8.py"
SUPERVISOR_NAME = SCRIPT_PATH.name
BOOTSTRAP_PROFILE = "r8-liquid-u3-stdio-transport-bootstrap-v8-20260807t183304z"
RUNTIME_PROFILE = "r8-liquid-u3-stdio-transport-runtime-v8-20260807t183304z"
...
SUCCESS_MAGIC = b"R8LQSTDIOPASSV8\x00"
MAX_SUCCESS_FRAME_BYTES = 16_384
TMPFS_BYTES = 67_108_864
EXPECTED_INPUTS = {
    "probe_alpha.bin": {"size_bytes": 264, "sha256": "7446f9a12f6457a6331f446c62eaff46233375e58b89cad376d301d7db9a8805"},
    "probe_beta.bin": {"size_bytes": 519, "sha256": "0a623aeccfeab44a99e9e8405d620deb83a7df37e8d70488efe2d5a6ea2b3602"},
    "probe_gamma.bin": {"size_bytes": 993, "sha256": "41dbd0f0b5d24cf8a21cbf5806975b9ded7ab4baf23c1097ede297a171ec1118"},
}
...
ROOT="/run"
NAME="r8-liquid-u3_stdio_apparmor_transport_probe_v8_20260807T183304Z.snapshot"
SOURCES=(
 ("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/scripts/r8_liquid_target_u3_stdio_apparmor_transport_probe_gate_v8.py","r8_liquid_target_u3_stdio_apparmor_transport_probe_gate_v8.py"),
 ("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/scripts/r8_liquid_target_u3_stdio_apparmor_transport_probe_supervisor_v8.py","r8_liquid_target_u3_stdio_apparmor_transport_probe_supervisor_v8.py"),
 ("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/config/apparmor_drafts/r8-liquid-u3-stdio-transport-probe-v8.profile","r8-liquid-u3-stdio-transport-probe-v8.profile"),
 ("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/schema/target_host_u3_stdio_apparmor_transport_probe_policy_v8.json","target_host_u3_stdio_apparmor_transport_probe_policy_v8.json"),
 ("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/config/target_hosts/liquid_zrj_msi_u2404_u3_stdio_apparmor_transport_probe_policy_v8.json","liquid_zrj_msi_u2404_u3_stdio_apparmor_transport_probe_policy_v8.json"),
)
def open_abs(path):
...
os.write(1,(json.dumps({"status":"ROOT_SNAPSHOT_V8_CREATED_NOT_EXECUTED","files":manifest},sort_keys=True,separators=(",",":"))+"\n").encode("ascii"))
'''
...
g={{"__name__":"__main__"}}
exec(compile(p,"<r8-liquid-root-snapshot-bootstrap-v8>","exec",dont_inherit=True,optimize=2),g,g)
'''
...
def repository_static_review() -> dict[str, Any]:
    ...
    if policy.get("status") != "STATIC_READY_V8_PENDING_INDEPENDENT_REVIEW_AND_EXPLICIT_SINGLE_ATTEMPT":
        raise SupervisorError("v8 static status differs")
    ...
    if transport.get("success_magic_hex") != SUCCESS_MAGIC.hex():
        raise SupervisorError("fresh v8 success protocol magic differs")
    expected_frozen = {
        "probe_id": PROBE_ID,
...
def verify_snapshot_producer_identity() -> dict[str, Any]:
    ...
    if SCRIPT_PATH != WORKSPACE_SUPERVISOR_PATH:
        raise SupervisorError("snapshot producer must be the exact workspace v8 supervisor")
...
def labeled_processes(proc_root: Path = Path("/proc")) -> list[dict[str, Any]]:
    """Authoritatively enumerate every task/thread carrying a fresh v8 label."""
...
def parse_success_frame(frame: bytes) -> dict[str, Any]:
    ...
    if (value["status"] != "PASS_HARMLESS_STDIO_APPARMOR_TRANSPORT_PROBE_V8"
...
def classify_execution(result: Mapping[str, Any], audit: Mapping[str, Any]) -> tuple[str, dict[str, Any]]:
    ...
    gate_no_go_envelope = (
        ...
        or b"R8_STDIO_PROBE_V8_NO_GO" in stderr
    )
...
def execution_receipt_document(
    *,
    ...
    return {
        "document_type": "SMPCC_R8_LIQUID_U3_STDIO_APPARMOR_PROBE_V8_EXECUTION_RECEIPT",
        "probe_id": PROBE_ID,
        "status": status,
...
def lifecycle_document(
    *,
    ...
    return {
        "document_type": "SMPCC_R8_LIQUID_U3_STDIO_APPARMOR_PROBE_V8_LIFECYCLE_RECEIPT",
        "probe_id": PROBE_ID,
        "status": "PASS_V8_PROFILE_PROCESS_SYSCTL_SUDO_LIFECYCLE_CLEANUP",
...
def run_once(*, policy_sha256: str, admission_token: str) -> dict[str, Any]:
    with TerminationGuard() as termination_guard:
        try:
            ...
            document = {
                "document_type": "SMPCC_R8_LIQUID_U3_STDIO_APPARMOR_PROBE_V8_PREFLIGHT_FAILURE_RECEIPT",
                "probe_id": PROBE_ID,
                "status": (
...
def _run_once_guarded(
    *,
    ...
    start_document = {
        "document_type": "SMPCC_R8_LIQUID_U3_STDIO_APPARMOR_PROBE_V8_START_RECEIPT",
        "probe_id": PROBE_ID,
    ...
    failure = {
        "document_type": "SMPCC_R8_LIQUID_U3_STDIO_APPARMOR_PROBE_V8_LIFECYCLE_INCOMPLETE_RECEIPT",
        "probe_id": PROBE_ID,
    ...
    raise SupervisorError(f"v8 lifecycle incomplete; preserved receipt {failure_record['path']}")
...
def recover_only(*, policy_sha256: str, recovery_token: str) -> dict[str, Any]:
    ...
    with TerminationGuard() as termination_guard:
        ...
        document = {
            "document_type": "SMPCC_R8_LIQUID_U3_STDIO_APPARMOR_PROBE_V8_RECOVERY_RECEIPT",
            "probe_id": PROBE_ID,
            "status": "PASS_V8_CLEANUP_ONLY_NO_PROBE_EXECUTED",
            "policy_sha256": policy_sha256,
            "snapshot": snapshot,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if arguments.command == "recover-only":
            ...
            print(json.dumps({"status": "V8_CLEANUP_ONLY_RECORDED", "result": result}, ensure_ascii=False, sort_keys=True))
            return 0
