#!/usr/bin/env python3
"""Static gate and unprivileged runner for the fresh harmless v5 successor.
...
SUPERVISOR_NAME = "r8_liquid_target_u3_stdio_apparmor_transport_probe_supervisor_v5.py"
...
STDIN_MAGIC = b"R8LQSTDIOPROBE5\x00"
...
INPUT_PAYLOADS: dict[str, bytes] = {
    "probe_alpha.bin": b"R8_STDIO_PROBE_ALPHA_V5\n" * 11,
    "probe_beta.bin": bytes(range(256)) * 2 + b"BETA_V5",
    "probe_gamma.bin": hashlib.sha256(b"r8-stdio-probe-v5-gamma-seed").digest() * 31 + b"G",
}
...
try:
    ...
    try:
        os.write(2, b"R8_STDIO_PROBE_V5_NO_GO\n")
    except OSError:
        pass
    raise SystemExit(2)
'''
...
exec(compile(p,"<r8-stdio-probe-helper-v5>","exec",dont_inherit=True,optimize=2),g,g)
'''
...
def bwrap_argv() -> list[str]:
    return [
        ...
        "--hostname", "r8-liquid-stdio-probe-v5", "--clearenv",
...
def verify_static() -> dict[str, Any]:
    ...
    expected = {
        "schema_version": "smpcc-r8-liquid-target-u3-stdio-apparmor-transport-probe-policy-v5",
        "policy_id": "LIQUID_ZRJ_MSI_U2404_U3_STDIO_APPARMOR_TRANSPORT_PROBE_V5",
        "host_id": "LIQUID_ZRJ_MSI_U2404",
        "status": "STATIC_READY_V5_PENDING_INDEPENDENT_REVIEW_AND_EXPLICIT_SINGLE_ATTEMPT",
    ...
    if policy.get("provenance", {}).get("predecessor_no_go") != PREDECESSOR_NO_GO:
        raise GateError("predecessor command-mode NO_GO record differs")
    ...
    if (transport.get("stdin_magic_hex") != STDIN_MAGIC.hex()
        ...
        raise GateError("fresh v5 stdio protocol magic differs")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if arguments.command == "self-check":
            review = verify_static()
            print(json.dumps({"status": "PASS_V5_STATIC_DRAFT_EXECUTION_NOT_PERFORMED", "review": review}, ensure_ascii=False, sort_keys=True))
            return 0
