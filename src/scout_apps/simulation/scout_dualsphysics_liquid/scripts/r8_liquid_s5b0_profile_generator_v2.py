#!/usr/bin/env python3
"""Deterministic, non-loading AppArmor profile renderer for S5B0 v2."""
...
TEMPLATE_PATH = ROOT / "config/apparmor_drafts/r8-liquid-s5b0-replay-v2.profile.template"
TOKENS = (
...
def _safe_path(value: str, label: str) -> str:
    ...
    if not value.startswith("/") or str(path) != value or any(char in value for char in "*?[]{}\n\r"):
        raise ProfileV2Error(f"unsafe {label} path")
    return value


def render_profile(template: str, replacements: Mapping[str, str]) -> str:
    if set(replacements) != set(TOKENS):
        raise ProfileV2Error("replacement token set differs")
    if not re.fullmatch(r"r8-liquid-s5b0-[a-z0-9-]{8,80}", replacements["PROFILE_NAME"]):
        raise ProfileV2Error("profile name is not exact-safe")
    for name in TOKENS[1:]:
        _safe_path(replacements[name], name)
    for name, expected in EXACT_DEVICES.items():
        if replacements[name] != expected:
            raise ProfileV2Error(f"device differs: {name}")
    if any("nvidia-uvm-tools" in replacements[name] for name in replacements):
        raise ProfileV2Error("uvm-tools cannot be pre-admitted")
    rendered = template
    for name in TOKENS:
        marker = f"@@{name}@@"
        if rendered.count(marker) != TOKEN_COUNTS[name]:
            raise ProfileV2Error(f"placeholder cardinality differs: {name}")
        rendered = rendered.replace(marker, replacements[name])
    if "@@" in rendered or "NOT LOADABLE AS-IS" not in rendered:
        raise ProfileV2Error("template marker/placeholder contract differs")
    ...
    if host_writable != [f"{output}/ rw,", f"{output}/** rwk,"]:
        raise ProfileV2Error("OUTPUT_ROOT is not the unique host writable tree")
    return rendered


def fixture_replacements() -> dict[str, str]:
    return {
        "PROFILE_NAME": "r8-liquid-s5b0-fixture-replay-v2",
        ...
        "NVIDIA0": "/dev/nvidia0", "NVIDIACTL": "/dev/nvidiactl",
        "NVIDIAUVM": "/dev/nvidia-uvm", "OUTPUT_ROOT": "/output",
    }
...
def self_check() -> dict[str, object]:
    ...
    return {
        "status": "PASS_S5B0_PROFILE_V2_STATIC_NON_LOADING_QUERY",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        result = self_check()
    except (OSError, UnicodeError, ProfileV2Error) as exc:
        print(json.dumps({"status": "FAIL_S5B0_PROFILE_V2", "error": str(exc)}, sort_keys=True), file=sys.stderr)
        return 2
