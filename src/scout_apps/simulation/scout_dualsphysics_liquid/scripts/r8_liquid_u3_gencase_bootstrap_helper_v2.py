#!/usr/bin/env python3
"""Guest-only bootstrap helper for the U3 C1 GenCase v2 design.
...
requires EOF, closes stdin, verifies all three final guest copies with the
pinned sha256sum binary, and executes exactly one fixed GenCase argv.
"""
...
FRAME_MAGIC = b"R8C1GENCASEV2\0\0\0"
...
def _run_candidate() -> tuple[int, bytes, bool, bool]:
    ...
    return returncode, bytes(console), overflow, timed_out
...
def _success_frame(
    *,
    ...
    return (
        FRAME_HEADER.pack(FRAME_MAGIC, FRAME_VERSION, len(encoded), len(bi4), len(xml), len(console))
        + encoded
        + bi4
        + xml
        + console
...
def main() -> int:
    ...
    try:
        ...
        returncode, console, overflow, timed_out = _run_candidate()
        if overflow:
