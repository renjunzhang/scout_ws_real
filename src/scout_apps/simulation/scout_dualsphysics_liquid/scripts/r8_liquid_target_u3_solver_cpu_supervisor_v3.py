#!/usr/bin/env python3
"""Root lifecycle supervisor for one fresh U3 C1M CPU-solver smoke v3.
...
exact 30-file solver tree with dirfd/O_NOFOLLOW/O_EXCL.
"""
...
WORKSPACE_SUPERVISOR_PATH = WORKSPACE_PACKAGE_ROOT / "scripts/r8_liquid_target_u3_solver_cpu_supervisor_v3.py"
...
EXPORT_FD_ENV = "R8_LIQUID_U3_SOLVER_CPU_V3_EXPORT_FD"
EXPORT_SHA256_ENV = "R8_LIQUID_U3_SOLVER_CPU_V3_EXPORT_SHA256"
ADMISSION_TOKEN = f"{CASE_ID}:single-cpu-solver-smoke-v3-attempt"
...
OUTPUT_MAGIC = b"R8SOLVEROUTV3\0\0\0"
OUTPUT_VERSION = 3
...
os.write(1,(json.dumps({{"status":"ROOT_SOLVER_CPU_V3_SNAPSHOT_CREATED_NOT_EXECUTED","files":manifest}},sort_keys=True,separators=(",",":"))+"\\n").encode("ascii"))
'''
...
g={{"__name__":"__main__"}}
exec(compile(p,"<r8-liquid-solver-cpu-v3-root-snapshot-bootstrap>","exec",dont_inherit=True,optimize=2),g,g)
'''
...
def verify_policy_static() -> dict[str, Any]:
    ...
    gate = load_module(GATE_PATH, "_r8_c1m_solver_cpu_v3_static_gate")
...
def parse_success_frame(raw: bytes) -> dict[str, Any]:
    ...
    if not 1 <= metadata_size <= OUTPUT_METADATA_LIMIT or file_count != 30:
    ...
    if (
        metadata["document_type"] != "SMPCC_R8_LIQUID_U3_C1M_CPU_SOLVER_GUEST_FRAME_V3"
        or metadata["status"] != "GUEST_SOLVER_V3_SMOKE_COMPLETE_PENDING_HOST_REAUDIT_AND_O_EXCL_EXPORT"
    ...
    gate = load_module(GATE_PATH, "_r8_c1m_solver_cpu_v3_frame_gate")
    ...
    expected_inputs = {}
    helper = load_module(HELPER_PATH, "_r8_c1m_solver_cpu_v3_frame_helper")
    ...
    if len(paths) != 30 or tuple(paths) != EXPECTED_PATHS:
    ...
    if output_audit.get("file_count") != 30 or output_audit.get("total_bytes") != len(payload_raw):
...
def load_lifecycle_harness() -> Any:
    ...
    module = load_module(HARNESS_PATH, "_r8_c1m_solver_cpu_v3_lifecycle_harness")
...
def run_once(*, policy_sha256: str, admission_token: str) -> dict[str, Any]:
    ...
    gate = load_module(GATE_PATH, "_r8_c1m_solver_cpu_v3_runtime_gate")
    with harness.TerminationGuard() as termination_guard:
        ...
        start_document = {
            "document_type": "SMPCC_R8_LIQUID_U3_C1M_CPU_SOLVER_V3_START_RECEIPT",
            "case_id": CASE_ID,
        ...
        execution_status = "V3_ATTEMPT_ABORTED_CLEANUP_PENDING"
        try:
            ...
            execution_status = "PASS_U3_C1M_CPU_SOLVER_V3_FRAME_VALID_CLOSED_ZERO_UNEXPECTED_LOGGED_DENIALS_TWO_KNOWN_QUIET_DENY_RULES_CLEANUP_PENDING"
        ...
        if parsed is not None and primary_error is None and not cleanup_errors:
            try:
                ...
                if message.get("status") != "PASS_UID1000_SOLVER_TREE_O_EXCL_EXPORT_V3" or not isinstance(message.get("export"), dict):
                ...
                execution_status = "PASS_U3_C1M_CPU_SOLVER_V3_SMOKE_EXPORTED_CLOSED_ZERO_UNEXPECTED_LOGGED_DENIALS_TWO_KNOWN_QUIET_DENY_RULES_CLEANUP_PENDING"
            except BaseException as exc:
                primary_error = f"{type(exc).__name__}: {exc}"[:4096]
        ...
        execution_document = {
            "document_type": "SMPCC_R8_LIQUID_U3_C1M_CPU_SOLVER_V3_EXECUTION_RECEIPT",
            "case_id": CASE_ID,
            "status": execution_status if primary_error is None and not cleanup_errors else "V3_ATTEMPT_ABORTED_NO_RETRY_SAME_IDENTITY",
            "policy_sha256": policy_sha256,
        ...
        success = execution_document["status"] == "PASS_U3_C1M_CPU_SOLVER_V3_SMOKE_EXPORTED_CLOSED_ZERO_UNEXPECTED_LOGGED_DENIALS_TWO_KNOWN_QUIET_DENY_RULES_CLEANUP_PENDING" and parsed is not None and export_verification is not None
        if success:
            lifecycle_document = {
                "document_type": "SMPCC_R8_LIQUID_U3_C1M_CPU_SOLVER_V3_LIFECYCLE_RECEIPT",
                "case_id": CASE_ID,
        ...
        failure_document = {
            "document_type": "SMPCC_R8_LIQUID_U3_C1M_CPU_SOLVER_V3_LIFECYCLE_INCOMPLETE_RECEIPT",
            "case_id": CASE_ID,
            "status": "V3_LIFECYCLE_OR_EXECUTION_INCOMPLETE_NO_RETRY_SAME_IDENTITY",
            "execution_receipt": execution_record,
            "primary_error": primary_error,
            "cleanup_errors": cleanup_errors,
        ...
        raise SupervisorError(f"solver v3 failed closed; preserved {failure_record['path']}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if arguments.command == "self-check":
            ...
            print(json.dumps({
                "status": "PASS_SOLVER_CPU_V3_SUPERVISOR_STATIC_EXECUTION_NOT_PERFORMED",
                "review": review,
                "snapshot_bootstrap": {
        ...
        if arguments.command == "export-frame":
            ...
            print(json.dumps({"status": "PASS_UID1000_SOLVER_TREE_O_EXCL_EXPORT_V3", "export": result}, ensure_ascii=False, sort_keys=True))
            return 0
        result = run_once(policy_sha256=arguments.policy_sha256, admission_token=arguments.admission_token)
        print(json.dumps({"status": "SOLVER_CPU_V3_ONE_SHOT_LIFECYCLE_RECORDED", "result": result}, ensure_ascii=False, sort_keys=True))
        return 0
        ...
        print(json.dumps({"status": "SOLVER_CPU_V3_NO_GO", "error": str(exc)}, ensure_ascii=False, sort_keys=True), file=target)
        return 2
