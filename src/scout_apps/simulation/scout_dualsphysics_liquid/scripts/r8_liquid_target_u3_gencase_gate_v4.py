#!/usr/bin/env python3
"""Static and UID-1000 child gate for one admitted U3 C1 GenCase v4 run.
...
POLICY_REPOSITORY_PATH = PACKAGE_DIR / "config/target_hosts/liquid_zrj_msi_u2404_u3_gencase_execution_policy_v4.json"
SCHEMA_REPOSITORY_PATH = PACKAGE_DIR / "schema/target_host_u3_gencase_execution_policy_v4.json"
PROFILE_REPOSITORY_PATH = PACKAGE_DIR / "config/apparmor_drafts/r8-liquid-u3-gencase-v4.profile"
HELPER_REPOSITORY_PATH = SCRIPT_PATH.with_name("r8_liquid_u3_gencase_bootstrap_helper_v4.py")
SUPERVISOR_REPOSITORY_PATH = SCRIPT_PATH.with_name("r8_liquid_target_u3_gencase_supervisor_v4.py")
...
def guest_loader(*, helper_size: int, helper_sha256: str) -> str:
    return (
        ...
        "g={'__name__':'__main__','__file__':'<r8-liquid-u3-gencase-helper-v4>'}\n"
...
def verify_profile(*, run_parser: bool) -> dict[str, Any]:
    ...
    if digest != expected_hash:
        raise GateError("v4 AppArmor profile digest differs")
    ...
    if effective.count(bootstrap_header) != 1 or effective.count(runtime_header) != 1:
        raise GateError("v4 AppArmor label identity differs")
...
def verify_review_artifacts(*, run_parser: bool = True) -> dict[str, Any]:
    ...
    if schema.get("additionalProperties") is not False or set(policy) != set(schema.get("required", [])):
        raise GateError("v4 policy schema is not a closed exact top-level contract")
    expected_top = {
        "schema_version": "smpcc-r8-liquid-target-u3-gencase-execution-policy-v4",
        "document_type": "SMPCC_R8_LIQUID_TARGET_U3_GENCASE_EXECUTION_POLICY",
        "policy_id": "LIQUID_ZRJ_MSI_U2404_U3_GENCASE_EXECUTION_V4",
        "host_id": "LIQUID_ZRJ_MSI_U2404",
        "development_only": True,
        "formal": False,
        "physical_primary_eligible": False,
        "execution_scope": "one_exact_c1_gencase_v4_attempt_after_frozen_v11_probe",
        "allowed_gate_commands": ["self-check", "internal-run"],
        "status": "ADMITTED_SINGLE_GENCASE_RUNTIME_V4_AFTER_FROZEN_V11_ZERO_DENIAL_PROBE",
        "next_allowed_stage": "EXECUTE_ONCE_THEN_VALIDATE_CASE_AND_VISUALIZE_OR_FREEZE_FAILURE",
    }
    for key, expected in expected_top.items():
        if policy.get(key) != expected:
            raise GateError(f"v4 policy field differs: {key}")
    if policy.get("trusted_tools") != trusted_tool_policy():
        raise GateError("v4 trusted-tool policy differs")
    ...
    if not isinstance(artifacts, dict) or set(artifacts) != set(_artifact_paths()):
        raise GateError("v4 trusted-artifact set differs")
    ...
    for name, path in _artifact_paths().items():
        ...
        if expected.get("path") != policy_artifact_relative_path(name):
            raise GateError(f"v4 artifact path differs: {name}")
        digest = sha256_regular_file(path, limit=2 * 1024 * 1024)
        if digest != expected.get("sha256"):
            raise GateError(f"v4 artifact digest differs: {name}")
    ...
    if frozen != expected_frozen:
        raise GateError("v4 frozen attempt identity differs")
    ...
    if policy["isolation"].get("guest_output_tmpfs_byte_ceiling") != TMPFS_OUTPUT_BYTES or policy["isolation"].get("guest_output_tmpfs_inode_ceiling") != "UNCLAIMED_BWRAP_0_9_HAS_NO_NR_INODES_OPTION":
        raise GateError("v4 tmpfs byte/inode claim differs")
    ...
    if (command_policy.get("bootstrap_helper_argv_canonical_sha256") != canonical_hash(expected_guest)
        ...
        raise GateError("v4 fixed guest command differs")
...
def policy_artifact_relative_path(name: str) -> str:
    return {
        "gate": "scripts/r8_liquid_target_u3_gencase_gate_v4.py",
        "helper": "scripts/r8_liquid_u3_gencase_bootstrap_helper_v4.py",
        "supervisor": "scripts/r8_liquid_target_u3_gencase_supervisor_v4.py",
        "profile": "config/apparmor_drafts/r8-liquid-u3-gencase-v4.profile",
        "schema": "schema/target_host_u3_gencase_execution_policy_v4.json",
    }[name]
...
def verify_snapshot_runtime() -> dict[str, Any]:
    ...
    if (not stat.S_ISDIR(root.st_mode) or root.st_uid != 0 or root.st_gid != 0
        ...
        raise GateError("v4 snapshot directory metadata differs")
    for path in (SCRIPT_PATH, POLICY_PATH, SCHEMA_PATH, PROFILE_PATH, HELPER_PATH, SUPERVISOR_PATH):
        ...
        if (not stat.S_ISREG(metadata.st_mode) or metadata.st_uid != 0 or metadata.st_gid != 0
            ...
            raise GateError(f"v4 snapshot artifact metadata differs: {path.name}")
...
def main() -> int:
    ...
    try:
        if arguments.command == "self-check":
            ...
            print(json.dumps({"status": "PASS_V4_STATIC_ADMITTED_EXECUTION_NOT_PERFORMED", "review": review}, ensure_ascii=False, sort_keys=True))
            return 0
