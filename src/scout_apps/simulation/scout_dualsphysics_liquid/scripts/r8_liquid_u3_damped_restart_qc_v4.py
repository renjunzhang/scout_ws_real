#!/usr/bin/env python3
"""QC wrapper for the upstream-default artificial-viscosity remediation.

The inherited QC still computes the original 17 metrics on the undamped tail.
This wrapper additionally proves that the only numerical delta from v3 is one
``-viscoart:0.01`` override in each phase, verifies the sealed upstream v5.4
template evidence, and publishes a create-new, closed-schema adjudication.
...
SCHEMA_PATH = PACKAGE_ROOT / "schema/target_host_u3_damped_restart_viscoart_qc_v1.json"
TEST_PATH = PACKAGE_ROOT / "tests/test_u3_damped_restart_qc_v4.py"
PRIOR_POLICY_PATH = PACKAGE_ROOT / "config/target_hosts/liquid_zrj_msi_u2404_u3_stage4_damped_restart_20260811T040906Z_v3.json"
PRIOR_POLICY_SHA256 = "35c5bf821b2482bf4aec13d89b68e4d753988b376b02a21579cf22333d0b2686"
PRIOR_QC_PATH = Path("/home/zrj/scout_liquid_lab/audits/u3_c1m_gpu_damped_init_undamped_tail_20260811T040906Z_v3.qc_v1.json")
...
def _upstream_template_proof() -> dict[str, Any]:
    base_argv = ["/usr/bin/git", f"--git-dir={UPSTREAM_GIT}"]
    environment = {"PATH": "/usr/bin", "LC_ALL": "C", "LANG": "C"}
    revision = f"{runner.UPSTREAM_COMMIT}:{UPSTREAM_TEMPLATE}"
    blob = subprocess.run(
    ...
    if blob.returncode != 0 or content.returncode != 0:
        raise ViscoartQcError("sealed upstream template cannot be read")
    ...
    if blob_text != runner.UPSTREAM_PARAMETER_TEMPLATE_BLOB or digest != runner.UPSTREAM_PARAMETER_TEMPLATE_SHA256:
        raise ViscoartQcError("sealed upstream template identity drifted")
    if 'key="ViscoTreatment" value="1"' not in text or 'key="Visco" value="0.01"' not in text:
        raise ViscoartQcError("upstream artificial-viscosity defaults are absent")
    return {
...
def policy_delta_proof(policy_path: Path) -> dict[str, Any]:
    ...
    for phase_name in base.PHASE_KEYS:
        ...
        normalized = [item for item in current_phase["solver_argv"] if item != runner.VISCO_ARG]
        phase_checks[f"{phase_name}_argv_only_inserts_viscoart"] = (
        ...
        argv_hashes[phase_name] = {
            ...
            "current_without_viscoart_canonical_sha256": canonical_sha256(normalized),
        }
    invariant_checks = {
    ...
    if not all(checks.values()):
        raise ViscoartQcError(f"v4 policy delta is not isolated: {[key for key, value in checks.items() if not value]}")
    return {
        "parameter": "VISCOSITY_FORMULATION",
        "baseline": "LAMINAR_1E-6",
        "candidate": "UPSTREAM_DEFAULT_ARTIFICIAL_0P01",
        "inserted_argument": runner.VISCO_ARG,
...
def _static_self_check(policy_path: Path) -> dict[str, Any]:
    ...
    upstream = _upstream_template_proof()
    schema = base.read_json(SCHEMA_PATH)
    Draft202012Validator.check_schema(schema)
    return {
        "status": "PASS_U3_DAMPED_RESTART_VISCOART_QC_V4_STATIC_SELF_CHECK",
        "policy_delta": delta,
...
def build_report(policy_path: Path) -> dict[str, Any]:
    ...
    prior_tail = prior_qc.get("undamped_tail", {})
    if (
        prior_verdict.get("status") != "FAIL_U3_DAMPED_INITIALIZATION_UNDAMPED_TAIL_NOT_SETTLED"
        or prior_verdict.get("u3_remediation_candidate_pass") is not False
        or len(prior_tail.get("failed_absolute_metrics", [])) != 10
    ):
        raise ViscoartQcError("prior failed damped-tail adjudication semantics differ")
    prior_viscosity = {
        phase_name: _run_viscosity(
            Path(prior_policy["phases"][phase_name]["run"]["output_root"]) / "Run.out", "Laminar", 0.000001
        )
        for phase_name in base.PHASE_KEYS
    }
    current_viscosity = {
        phase_name: _run_viscosity(
            Path(policy["phases"][phase_name]["run"]["output_root"]) / "Run.out", "Artificial", 0.01
        )
        for phase_name in base.PHASE_KEYS
    }
    ...
    return {
        ...
        "upstream_template_proof": upstream,
        ...
        "verdict": {
            "status": (
                "PASS_U3_VISCOART_DAMPED_INITIALIZATION_UNDAMPED_TAIL_SETTLED_CANDIDATE"
                if selected else "FAIL_U3_VISCOART_DAMPED_INITIALIZATION_UNDAMPED_TAIL_NOT_SETTLED"
            ),
...
def validate_report(report: dict[str, Any]) -> None:
    ...
    for group, expected_name, expected_value in (
        ("prior", "Laminar", 0.000001),
        ("candidate", "Artificial", 0.01),
    ):
