#!/usr/bin/env python3
"""Root lifecycle supervisor for the one-shot harmless v9 successor probe.
...
WORKSPACE_SUPERVISOR_PATH = WORKSPACE_PACKAGE_ROOT / "scripts/r8_liquid_target_u3_stdio_apparmor_transport_probe_supervisor_v9.py"
PROBE_ID = "u3_stdio_apparmor_transport_probe_v9_20260808T034143Z"
SNAPSHOT_ROOT = Path(f"/run/r8-liquid-{PROBE_ID}.snapshot")
SNAPSHOT_ENV = "R8_LIQUID_U3_STDIO_PROBE_V9_SNAPSHOT"
ADMISSION_ENV = "R8_LIQUID_U3_STDIO_PROBE_V9_ADMISSION"
ADMISSION_FD_ENV = "R8_LIQUID_U3_STDIO_PROBE_V9_ADMISSION_FD"
ADMISSION_SHA256_ENV = "R8_LIQUID_U3_STDIO_PROBE_V9_ADMISSION_SHA256"
...
POLICY_NAME = "liquid_zrj_msi_u2404_u3_stdio_apparmor_transport_probe_policy_v9.json"
SCHEMA_NAME = "target_host_u3_stdio_apparmor_transport_probe_policy_v9.json"
PROFILE_NAME = "r8-liquid-u3-stdio-transport-probe-v9.profile"
GATE_NAME = "r8_liquid_target_u3_stdio_apparmor_transport_probe_gate_v9.py"
SUPERVISOR_NAME = SCRIPT_PATH.name
BOOTSTRAP_PROFILE = "r8-liquid-u3-stdio-transport-bootstrap-v9-20260808t034143z"
RUNTIME_PROFILE = "r8-liquid-u3-stdio-transport-runtime-v9-20260808t034143z"
...
SUCCESS_MAGIC = b"R8LQSTDIOPASSV9\x00"
...
EXPECTED_INPUTS = {
    "probe_alpha.bin": {"size_bytes": 264, "sha256": "2b9ad1bd5d0b10f5c9081c18c542235ff84b018d16a63dcedefbaf50393b5c77"},
    "probe_beta.bin": {"size_bytes": 519, "sha256": "db45dc90e06c23abd44325077021e5ef0f0c58ad1548008181efa8e1ff4f46c1"},
    "probe_gamma.bin": {"size_bytes": 993, "sha256": "80cd1b4107a69f1080ce5be4ef5162bc931207b668bf5efbebd440afb3179ec3"},
}
...
ROOT="/run"
NAME="r8-liquid-u3_stdio_apparmor_transport_probe_v9_20260808T034143Z.snapshot"
SOURCES=(
 ("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/scripts/r8_liquid_target_u3_stdio_apparmor_transport_probe_gate_v9.py","r8_liquid_target_u3_stdio_apparmor_transport_probe_gate_v9.py"),
 ("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/scripts/r8_liquid_target_u3_stdio_apparmor_transport_probe_supervisor_v9.py","r8_liquid_target_u3_stdio_apparmor_transport_probe_supervisor_v9.py"),
 ("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/config/apparmor_drafts/r8-liquid-u3-stdio-transport-probe-v9.profile","r8-liquid-u3-stdio-transport-probe-v9.profile"),
 ("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/schema/target_host_u3_stdio_apparmor_transport_probe_policy_v9.json","target_host_u3_stdio_apparmor_transport_probe_policy_v9.json"),
 ("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/config/target_hosts/liquid_zrj_msi_u2404_u3_stdio_apparmor_transport_probe_policy_v9.json","liquid_zrj_msi_u2404_u3_stdio_apparmor_transport_probe_policy_v9.json"),
)
def open_abs(path):
...
os.write(1,(json.dumps({"status":"ROOT_SNAPSHOT_V9_CREATED_NOT_EXECUTED","files":manifest},sort_keys=True,separators=(",",":"))+"\n").encode("ascii"))
'''
...
g={{"__name__":"__main__"}}
exec(compile(p,"<r8-liquid-root-snapshot-bootstrap-v9>","exec",dont_inherit=True,optimize=2),g,g)
'''
...
def repository_static_review() -> dict[str, Any]:
    ...
    if policy.get("status") != "STATIC_READY_V9_PENDING_INDEPENDENT_REVIEW_AND_EXPLICIT_SINGLE_ATTEMPT":
        raise SupervisorError("v9 static status differs")
    ...
    if transport.get("success_magic_hex") != SUCCESS_MAGIC.hex():
        raise SupervisorError("fresh v9 success protocol magic differs")
    expected_frozen = {
        "probe_id": PROBE_ID,
    ...
    return {"policy_sha256": sha256_bytes(policy_raw), "policy_size_bytes": len(policy_raw), "artifacts": observed, "bootstrap_source_sha256": SNAPSHOT_BOOTSTRAP_SHA256, "predecessor_v8": predecessor_v8, "execution_performed": False}
...
def verify_snapshot_producer_identity() -> dict[str, Any]:
    ...
    if SCRIPT_PATH != WORKSPACE_SUPERVISOR_PATH:
        raise SupervisorError("snapshot producer must be the exact workspace v9 supervisor")
...
def labeled_processes(proc_root: Path = Path("/proc")) -> list[dict[str, Any]]:
    """Authoritatively enumerate every task/thread carrying a fresh v9 label."""
...
def parse_success_frame(frame: bytes) -> dict[str, Any]:
    ...
    if (value["status"] != "PASS_HARMLESS_STDIO_APPARMOR_TRANSPORT_PROBE_V9"
...
def capture_apparmor_denials(anchor: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if (JOURNAL_CURSOR_RE.fullmatch(end_cursor) is None or end_cursor == cursor
...
def _audit_is_closed_mount_discovery(audit: Mapping[str, Any]) -> bool:
    count_keys = (
    ...
    if (audit.get("capture_valid") is not True or audit.get("capture_errors") != []
            ...
            or audit.get("expected_mount_total") != 1
    ...
    if (type(sanitized) is not list or type(expected) is not list or type(unexpected) is not list
            or len(sanitized) != 1 or len(expected) != 1 or unexpected != []
            or sanitized[0] != expected[0] or not _is_exact_mount_discovery(expected[0])):
        return False
    return True
...
def classify_execution(result: Mapping[str, Any], audit: Mapping[str, Any]) -> tuple[str, dict[str, Any]]:
    ...
    stderr_envelope = _denial_stderr_envelope(stderr)
    evidence["mount_denial_stderr"] = stderr_envelope
    returncode = result.get("returncode")
    exact_discovery = _audit_is_closed_mount_discovery(audit)
    if (result.get("failure") is None and type(returncode) is int and returncode == 1
            and stdout == b"" and stderr_envelope["valid"] is True and exact_discovery):
        return "PASS_FAIL_CLOSED_APPARMOR_MOUNT_DENIAL_DISCOVERY_CLEANUP_PENDING", evidence
    # Revision v9 authorizes only the exact mount-denial discovery.  A zero
    # exit means an unreviewed proc mount unexpectedly succeeded and is never
    # promoted, even when a syntactically valid success frame is present.
...
def execution_receipt_document(
    *,
    ...
    return {
        "document_type": "SMPCC_R8_LIQUID_U3_STDIO_APPARMOR_PROBE_V9_EXECUTION_RECEIPT",
        "probe_id": PROBE_ID,
        "status": status,
...
def lifecycle_document(
    *,
    ...
    return {
        "document_type": "SMPCC_R8_LIQUID_U3_STDIO_APPARMOR_PROBE_V9_LIFECYCLE_RECEIPT",
        "probe_id": PROBE_ID,
        "status": "PASS_V9_PROFILE_PROCESS_SYSCTL_SUDO_LIFECYCLE_CLEANUP",
...
def run_once(*, policy_sha256: str, admission_token: str) -> dict[str, Any]:
    with TerminationGuard() as termination_guard:
        try:
            ...
            document = {
                "document_type": "SMPCC_R8_LIQUID_U3_STDIO_APPARMOR_PROBE_V9_PREFLIGHT_FAILURE_RECEIPT",
                "probe_id": PROBE_ID,
                "status": (
...
def _run_once_guarded(
    *,
    ...
    start_document = {
        "document_type": "SMPCC_R8_LIQUID_U3_STDIO_APPARMOR_PROBE_V9_START_RECEIPT",
        "probe_id": PROBE_ID,
    ...
    failure = {
        "document_type": "SMPCC_R8_LIQUID_U3_STDIO_APPARMOR_PROBE_V9_LIFECYCLE_INCOMPLETE_RECEIPT",
        "probe_id": PROBE_ID,
    ...
    raise SupervisorError(f"v9 lifecycle incomplete; preserved receipt {failure_record['path']}")
...
def recover_only(*, policy_sha256: str, recovery_token: str) -> dict[str, Any]:
    ...
    with TerminationGuard() as termination_guard:
        ...
        document = {
            "document_type": "SMPCC_R8_LIQUID_U3_STDIO_APPARMOR_PROBE_V9_RECOVERY_RECEIPT",
            "probe_id": PROBE_ID,
            "status": "PASS_V9_CLEANUP_ONLY_NO_PROBE_EXECUTED",
            "policy_sha256": policy_sha256,
            "snapshot": snapshot,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if arguments.command == "recover-only":
            ...
            print(json.dumps({"status": "V9_CLEANUP_ONLY_RECORDED", "result": result}, ensure_ascii=False, sort_keys=True))
            return 0
        result = run_once(policy_sha256=arguments.policy_sha256, admission_token=arguments.admission_token)
        print(json.dumps({"status": "V9_ONE_SHOT_LIFECYCLE_RECORDED", "result": result}, ensure_ascii=False, sort_keys=True))
        return 0 if result["execution_status"] == "PASS_FAIL_CLOSED_APPARMOR_MOUNT_DENIAL_DISCOVERY_CLEANUP_PENDING" else 2
