...
SCHEMA_PATH = PACKAGE_ROOT / "schema/target_host_u3_gpu_stage4_run_policy_v2.json"
...
def execute(policy_path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        start = {
            "schema_version": "r8-liquid-u3-gpu-stage4-run-receipt-v2",
            "document_type": "SMPCC_R8_LIQUID_U3_GPU_STAGE4_RUN_START_V2",
        ...
        final = {
            "schema_version": "r8-liquid-u3-gpu-stage4-run-receipt-v2",
            "document_type": "SMPCC_R8_LIQUID_U3_GPU_STAGE4_RUN_FINAL_V2",
        ...
        failure = {
            "schema_version": "r8-liquid-u3-gpu-stage4-run-receipt-v2",
            "document_type": "SMPCC_R8_LIQUID_U3_GPU_STAGE4_RUN_FAILURE_V2",
...
def self_check(policy_path: Path) -> dict[str, Any]:
    ...
    argv = bwrap_argv(policy)
    return {
        "status": "PASS_U3_GPU_STAGE4_RUNNER_V2_SELF_CHECK",
        "policy": identity(policy_path, maximum=MAX_POLICY_BYTES),
        "verified": verified,
        "sandbox_argv": argv,
        "network_used": False,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "self-check":
            ...
            result = {"status": "PASS_U3_GPU_STAGE4_RUNNER_V2_PREFLIGHT", "evidence": preflight(args.policy, require_fresh=True)}
        else:
            result = execute(args.policy)
    except (Stage4RunError, OSError, ValueError, subprocess.SubprocessError) as exc:
        print(json.dumps({"status": "FAIL_U3_GPU_STAGE4_RUNNER_V2", "error_type": type(exc).__name__, "error": str(exc)}, ensure_ascii=False, sort_keys=True), file=sys.stderr)
        return 1
