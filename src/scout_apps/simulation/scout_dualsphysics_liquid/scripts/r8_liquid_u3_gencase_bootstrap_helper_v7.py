#!/usr/bin/env python3
"""Guest-only bootstrap helper for the admitted U3 C1M GenCase v7 runtime.
...
FRAME_MAGIC = b"R8C1GENCASEV7\0\0\0"
...
def _audit_zero_motion_generated_features(root: ET.Element) -> None:
    """Require one moving block driven only by the zero-motion ``mvnull``."""
    ...
    if motion is None or all_motion != [motion] or motion.attrib or (motion.text or "").strip():
        raise GuestError("generated C1M XML motion container location or attributes differ")
    if [child.tag for child in motion] != ["objreal"]:
        raise GuestError("generated C1M XML must contain exactly one real motion object")
    objreal = motion[0]
    if objreal.attrib != {"ref": "0"} or [child.tag for child in objreal] != ["begin", "mvnull"]:
        raise GuestError("generated C1M XML zero-motion object structure differs")
    begin, mvnull = objreal
    try:
        begin_start = float(begin.get("start", "nan"))
    except ValueError as exc:
        raise GuestError("generated C1M XML begin start is not numeric") from exc
    if (set(begin.attrib) != {"mov", "start"} or begin.get("mov") != "1"
            or begin_start != 0.0 or len(begin)
            or (begin.text or "").strip()):
        raise GuestError("generated C1M XML begin event is not the exact zero-time mvnull link")
    if mvnull.attrib != {"id": "1"} or len(mvnull) or (mvnull.text or "").strip():
        raise GuestError("generated C1M XML mvnull definition differs")
...
def _success_frame(
    *,
    ...
    metadata = {
        "document_type": "SMPCC_R8_LIQUID_U3_C1M_GENCASE_GUEST_FRAME_V7",
