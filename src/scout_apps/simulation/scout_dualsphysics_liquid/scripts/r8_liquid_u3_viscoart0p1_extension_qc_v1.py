#!/usr/bin/env python3
"""Read-only QC for the v6 20.05--30.05 s undamped extension.

The tool parses only preserved BI4 outputs.  It proves exact restart-state
equivalence at Part_0401, verifies the closed 201-frame inventory and solver
configuration, evaluates the existing 17 settled-state limits over the final
one-second tail, and publishes one O_EXCL receipt.  It never runs the solver or
uses a GPU.
"""
...
SCHEMA_PATH = PACKAGE_ROOT / "schema/target_host_u3_viscoart0p1_extension_qc_v1.json"
TEST_PATH = PACKAGE_ROOT / "tests/test_u3_viscoart0p1_extension_qc_v1.py"
...
PART_INDICES = tuple(range(401, 602))
...
class ExtensionQcError(ValueError):
    """Unsafe, drifted, incomplete or inconsistent extension evidence."""
...
def _trajectory(parts: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if any(not math.isfinite(value) for _, _, value in series):
        raise ExtensionQcError("speed trajectory is non-finite")
...
def adjudicate(metrics: dict[str, float], structural_pass: bool, restart_matches: bool) -> dict[str, Any]:
    if tuple(sorted(metrics)) != METRIC_NAMES:
        raise ExtensionQcError("metric vector differs from the frozen 17 items")
    if any(not isinstance(value, (int, float)) or not math.isfinite(float(value)) or float(value) < 0 for value in metrics.values()):
        raise ExtensionQcError("metric vector contains invalid values")
    ...
    return {
        ...
        "selected": selected,
        "status": (
            "PASS_U3_VISCOART0P1_EXTENDED_UNDAMPED_SETTLING_CANDIDATE"
            if selected else "FAIL_U3_VISCOART0P1_EXTENDED_UNDAMPED_NOT_SETTLED"
        ),
    }
...
def _parse_solver_proof(run_out_source: bi4.SecureFile) -> dict[str, Any]:
    ...
    if shifting != ["None"] or viscosity != ["Artificial"] or visco != ["0.1"]:
        raise ExtensionQcError(f"solver numerical identity differs: {shifting!r}/{viscosity!r}/{visco!r}")
...
def build_report(policy_path: Path) -> dict[str, Any]:
    ...
    if policy_path != runner.POLICY_PATH:
        raise ExtensionQcError("policy path differs")
    ...
    required_final = (
        ...
        and final.get("purpose") == "extended_undamped_settling"
    ...
    if not required_final:
        raise ExtensionQcError("final receipt semantics differ")
    ...
    for key in ("file_count", "part_file_count", "part_first", "part_last", "total_bytes", "canonical_sha256"):
        if observed_inventory.get(key) != frozen_inventory.get(key):
            raise ExtensionQcError(f"output inventory differs at {key}")
    if any(item["mode"] != "0440" or item["nlink"] != 1 for item in observed_inventory["files"].values()):
        raise ExtensionQcError("output mode/link invariant differs")
    ...
    structural = {
        ...
        "start_time_matches": abs(times[0] - 20.05) <= metric_contract.CONTRACT.physical_time_tolerance_s,
        "end_time_matches": abs(times[-1] - 30.05) <= metric_contract.CONTRACT.physical_time_tolerance_s,
        ...
        "run_csv_matches": fields["Np"] == 9078 and fields["PartFiles"] == 602 and abs(fields["PhysicalTime"] - times[-1]) <= metric_contract.CONTRACT.physical_time_tolerance_s,
    ...
    if not all(structural.values()):
        raise ExtensionQcError(f"structural checks failed: {[name for name, value in structural.items() if not value]}")

    restart_input = _canonical_state(runner.RESTART_PART_PATH)
    restart_output = _canonical_state(root / "data/Part_0401.bi4")
    restart_matches = restart_input["canonical_array_sha256"] == restart_output["canonical_array_sha256"]
    if not restart_matches:
        raise ExtensionQcError("restart Part_0401 state arrays differ")
    ...
    if inputs["plan"]["sha256"] != PLAN_SHA256:
        raise ExtensionQcError("frozen plan identity differs")
    report = {
        "schema_version": "r8-liquid-u3-viscoart0p1-extension-qc-v1",
        "document_type": "SMPCC_R8_LIQUID_U3_VISCOART0P1_EXTENSION_QC_V1",
        "captured_at_utc": datetime.now(timezone.utc).isoformat(),
        "inputs": inputs,
        "contract": {
            "window_start_s": 20.05,
            "window_end_s": 30.05,
            "tail_window_s": 1.0,
            "part_first": 401,
            "part_last": 601,
        ...
        "run": {
            ...
            "final_checkpoint": _identity(root / "data/Part_0601.bi4"),
        },
        "verdict": {
            ...
            "exact_blocker": "NONE" if result["selected"] else f"EXTENDED_TAIL_FAILS_{len(result['failed_absolute_metrics'])}_OF_17_ABSOLUTE_SETTLING_LIMITS",
            "next": "FRESH_COLD_B_REPEATABILITY_RESTART_EQUIVALENCE" if result["selected"] else "STOP_OR_FREEZE_ONE_NEW_NUMERICAL_DELTA",
...
def validate_report(report: dict[str, Any]) -> None:
    ...
    if tuple(sorted(metrics)) != METRIC_NAMES or tuple(sorted(passes)) != METRIC_NAMES:
        raise ExtensionQcError("report metric/pass keys differ")
    ...
    if failed != expected_failed or any(passes[name] is not (name not in expected_failed) for name in METRIC_NAMES):
        raise ExtensionQcError("metric values, pass vector and failure list disagree")
    ...
    expected_status = "PASS_U3_VISCOART0P1_EXTENDED_UNDAMPED_SETTLING_CANDIDATE" if selected else "FAIL_U3_VISCOART0P1_EXTENDED_UNDAMPED_NOT_SETTLED"
    if verdict.get("status") != expected_status or verdict.get("u3_remediation_candidate_pass") is not selected:
        raise ExtensionQcError("verdict disagrees with frozen adjudication")
    ...
    if errors:
        first = errors[0]
        raise ExtensionQcError(f"QC schema failure at {list(first.absolute_path)}: {first.message}")
...
def write_exclusive(path: Path, report: dict[str, Any]) -> dict[str, Any]:
    ...
    if len(data) > MAX_JSON_BYTES:
        raise ExtensionQcError("QC output exceeds bound")
    ...
    try:
        ...
        while view:
            ...
            if written <= 0:
                raise ExtensionQcError("short QC write")
            view = view[written:]
        os.fsync(descriptor)
    finally:
        os.close(descriptor)
...
def static_self_check() -> dict[str, Any]:
    ...
    if METRIC_LIMITS != dict(sorted(metric_contract.METRIC_LIMITS.items())):
        raise ExtensionQcError("metric contract drifted")
    return {
        "status": "PASS_U3_VISCOART0P1_EXTENSION_QC_V1_STATIC_SELF_CHECK",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.output is None:
            raise ExtensionQcError("--output is required")
    ...
    except (ExtensionQcError, runner.ExtensionRunError, legacy.Stage4RunError, metric_contract.DdtRampQcError, qc_v3.QcError, bi4.Bi4FormatError, OSError, UnicodeError, ValueError, subprocess.SubprocessError) as exc:
