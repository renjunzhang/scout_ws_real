#!/usr/bin/env python3
"""Fresh GPU artificial-viscosity 0.3 cold-B 45.05 s runner.

The independent policy start from the exact sealed C1M initial state and
run the accepted development candidate through 45.05 s with identical
...
SCHEMA_PATH = PACKAGE_ROOT / "schema/target_host_u3_viscoart0p3_cold_b45_policy_v1.json"
TEST_PATH = PACKAGE_ROOT / "tests/test_u3_gpu_viscoart0p3_cold_b45_runner_v1.py"
QC_PATH = SCRIPT_PATH.parent / "r8_liquid_u3_viscoart0p3_cold_b45_qc_v1.py"
QC_SCHEMA_PATH = PACKAGE_ROOT / "schema/target_host_u3_viscoart0p3_cold_b45_qc_v1.json"
QC_TEST_PATH = PACKAGE_ROOT / "tests/test_u3_viscoart0p3_cold_b45_qc_v1.py"
POLICY_PATHS = {
    "b": PACKAGE_ROOT / "config/target_hosts/liquid_zrj_msi_u2404_u3_stage4_viscoart0p3_cold_b_20260811T113902Z_v14.json",
}
RUN_IDS = {
    "b": "u3_c1m_gpu_viscoart0p3_cold_b_20260811T113902Z_v14",
}
PURPOSES = {"b": "viscoart0p3_fresh_continuous_cold_b_45s"}
PRIOR_QC_PATH = Path("/home/zrj/scout_liquid_lab/audits/u3_c1m_gpu_viscoart0p3_cold_a_extend_from_0701_20260811T110912Z_v13.qc_v1.json")
PRIOR_QC_SHA256 = "982d1a5a5bcebc4d85bfdb09f9d9e9fe03d84729a0c84f84f0f9cbd66990dd25"
PRIOR_FINAL_PATH = Path("/home/zrj/scout_liquid_lab/audits/u3_c1m_gpu_viscoart0p3_cold_a_extend_from_0701_20260811T110912Z_v13.final.json")
PRIOR_FINAL_SHA256 = "6126ff3dd61d031a36db5d4d87188fde46daad86be2abeeef1f7f078dabf9099"
...
EXPECTED_DELTA = {
    "parameter": "CONTINUATION_MODE",
    "baseline": "V13_CHECKPOINT_RESTART_COMPOSITE",
    "candidate": "FRESH_CONTINUOUS_CASE",
...
def semantic_validate(policy: dict[str, Any], policy_path: Path) -> None:
    ...
    if sandbox["apparmor_profile_used"]:
        raise ColdAbRunError("cold-B must not use an AppArmor profile")
    ...
    if policy["parents"]["prior_qc"]["sha256"] != PRIOR_QC_SHA256:
        raise ColdAbRunError("v13 QC identity differs")
    if policy["parents"]["prior_tail_final"]["sha256"] != PRIOR_FINAL_SHA256:
        raise ColdAbRunError("v13 final receipt identity differs")
...
def self_check(policy_path: Path) -> dict[str, Any]:
    ...
    return {
        "status": "PASS_U3_VISCOART0P3_COLD_B45_RUNNER_V1_SELF_CHECK",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "self-check":
            ...
            result = {"status": "PASS_U3_VISCOART0P3_COLD_B45_PREFLIGHT", "evidence": base.preflight(args.policy, require_fresh=True)}
        else:
        ...
        print(json.dumps({"status": "FAIL_U3_VISCOART0P3_COLD_B45_RUNNER", "error_type": type(exc).__name__, "error": str(exc)}, ensure_ascii=False, sort_keys=True), file=sys.stderr)
        return 1
