#!/usr/bin/env python3
"""Read-only QC for one or two completed U3 DualSPHysics CPU output trees.
...
SCHEMA_VERSION = "r8-liquid-u3-solver-output-qc-v1"
...
def load_fixed_case(
    ...
    *,
    ...
    return {
        ...
        "initial_fluid_mass_kg": counts["fluid"] * massfluid,
...
def parse_part_file(
    ...
    root_identity_exact = (
        _required_uint(root.values, "CaseNp") == case["particle_count"]
        and _required_uint(root.values, "CaseNfixed") == case["counts"]["fixed_boundary"]
        and _required_uint(root.values, "CaseNmoving") == 0
        and _required_uint(root.values, "CaseNfloat") == 0
        and _required_uint(root.values, "CaseNfluid") == case["counts"]["fluid"]
        and _same_float(_finite_float(root.values.get("Dp"), "PART root Dp"), case["dp_m"])
        and _same_float(_finite_float(root.values.get("MassFluid"), "PART root MassFluid"), case["massfluid_kg"])
    )
    ...
    fluid_mass = fluid_count * case["massfluid_kg"]
    initial_mass = case["initial_fluid_mass_kg"]
    return {
        ...
        "mass_relative_error": (fluid_mass - initial_mass) / initial_mass,
        ...
        "root_case_identity_exact": root_identity_exact,
...
def analyze_run(run_dir: str | os.PathLike[str], case: dict[str, Any], thresholds: Thresholds) -> dict[str, Any]:
    ...
    first_matches_case = (
        ...
        and parts[0]["root_case_identity_exact"]
    ...
    structural_checks = {
        ...
        "all_part_roots_match_fixed_case": all(part["root_case_identity_exact"] for part in parts),
...
def build_report(
    ...
    *,
    ...
    return {
        ...
        "document_type": "SMPCC_R8_LIQUID_U3_SOLVER_OUTPUT_QC",
        ...
        "fixed_case": {
            ...
            "counts": case["counts"],
            "dp_m": case["dp_m"],
            "massfluid_kg": case["massfluid_kg"],
            "initial_fluid_mass_kg": case["initial_fluid_mass_kg"],
        },
        "run": first,
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--expected-case-bi4-sha256")
    parser.add_argument("--expected-case-xml-sha256")
    return parser
