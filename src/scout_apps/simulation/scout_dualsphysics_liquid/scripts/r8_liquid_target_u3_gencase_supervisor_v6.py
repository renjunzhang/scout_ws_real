#!/usr/bin/env python3
"""Root lifecycle supervisor and host frame auditor for U3 C1 GenCase v6.
...
WORKSPACE_SUPERVISOR_PATH = WORKSPACE_PACKAGE_ROOT / "scripts/r8_liquid_target_u3_gencase_supervisor_v6.py"
CASE_ID = "u3_c1_gencase_v6_20260808T072315Z"
HOST_UID = 1000
HOST_GID = 1000
BOOTSTRAP_PROFILE = "r8-liquid-u3-gencase-bootstrap-v6-20260808t072315z"
RUNTIME_PROFILE = "r8-liquid-u3-gencase-runtime-v6-20260808t072315z"
LABELS = (BOOTSTRAP_PROFILE, RUNTIME_PROFILE)
SNAPSHOT_ENV = "R8_LIQUID_U3_GENCASE_V6_SNAPSHOT"
ADMISSION_FD_ENV = "R8_LIQUID_U3_GENCASE_V6_ADMISSION_FD"
ADMISSION_SHA256_ENV = "R8_LIQUID_U3_GENCASE_V6_ADMISSION_SHA256"
EXPORT_FD_ENV = "R8_LIQUID_U3_GENCASE_V6_EXPORT_FD"
EXPORT_SHA256_ENV = "R8_LIQUID_U3_GENCASE_V6_EXPORT_SHA256"
ADMISSION_TOKEN = f"{CASE_ID}:single-gencase-attempt"
SNAPSHOT_ROOT = Path(f"/run/r8-liquid-{CASE_ID}.snapshot")

POLICY_NAME = "liquid_zrj_msi_u2404_u3_gencase_execution_policy_v6.json"
SCHEMA_NAME = "target_host_u3_gencase_execution_policy_v6.json"
PROFILE_NAME = "r8-liquid-u3-gencase-v6.profile"
GATE_NAME = "r8_liquid_target_u3_gencase_gate_v6.py"
HELPER_NAME = "r8_liquid_u3_gencase_bootstrap_helper_v6.py"
SUPERVISOR_NAME = "r8_liquid_target_u3_gencase_supervisor_v6.py"
...
HELPER_MAGIC = b"R8C1HELPERV6\0\0\0\0\0"
INPUT_MAGIC = b"R8C1INPUTV6\0\0\0\0\0"
OUTPUT_MAGIC = b"R8C1GENCASEV6\0\0\0"
...
os.write(1,(json.dumps({{"status":"ROOT_GENCASE_V6_SNAPSHOT_CREATED_NOT_EXECUTED","files":manifest}},sort_keys=True,separators=(",",":"))+"\\n").encode("ascii"))
'''
...
g={{"__name__":"__main__"}}
exec(compile(p,"<r8-liquid-gencase-v6-root-snapshot-bootstrap>","exec",dont_inherit=True,optimize=2),g,g)
'''
...
def verify_policy_static() -> dict[str, Any]:
    ...
    if policy.get("status") != "ADMITTED_SINGLE_GENCASE_RUNTIME_V6_AFTER_FROZEN_V11_ZERO_DENIAL_PROBE":
        raise SupervisorError("v6 one-shot admission status differs")
    if policy.get("allowed_gate_commands") != ["self-check", "internal-run"]:
        raise SupervisorError("v6 gate command surface differs")
    ...
    if (probe.get("required_receipt_sha256") != frozen_receipts["execution"][1]
        ...
        raise SupervisorError("v6 policy does not pin the frozen v11 PASS lifecycle")
...
def _validate_generated_xml(raw: bytes) -> None:
    ...
    if (motion is None or all_motion != [motion] or motion.attrib or len(motion)
            or (motion.text or "").strip()):
        raise SupervisorError("framed XML motion container is not exactly one empty execution node")
    if {"floating", "inout", "wavegen"}.intersection(element.tag for element in root.iter()):
...
def parse_success_frame(raw: bytes) -> dict[str, Any]:
    ...
    if metadata["document_type"] != "SMPCC_R8_LIQUID_U3_C1_GENCASE_GUEST_FRAME_V6" or metadata["status"] != "GUEST_FRAME_COMPLETE_PENDING_HOST_REAUDIT_AND_O_EXCL_EXPORT":
    ...
    payloads = {"C1_static.bi4": bi4, "C1_static.xml": xml, "C1_static.out": out}
    expected_outputs = {
...
def export_frame_o_excl(frame: bytes) -> dict[str, Any]:
    ...
    try:
        ...
        for name in ("C1_static.bi4", "C1_static.xml", "C1_static.out"):
            ...
            try:
                ...
                os.close(descriptor)
        if sorted(os.listdir(output_fd)) != ["C1_static.bi4", "C1_static.out", "C1_static.xml"]:
...
def load_lifecycle_harness() -> Any:
    ...
    spec = importlib.util.spec_from_file_location("_r8_gencase_v6_lifecycle_harness", HARNESS_PATH)
...
def verify_exported_outputs(parsed: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if sorted(os.listdir(OUTPUT_ROOT)) != ["C1_static.bi4", "C1_static.out", "C1_static.xml"]:
...
def run_once(*, policy_sha256: str, admission_token: str) -> dict[str, Any]:
    ...
    with harness.TerminationGuard() as termination_guard:
        ...
        if policy.get("profile_lifecycle", {}).get("expected_sysctls") != sysctls_before:
            raise SupervisorError("current sysctls differ from the frozen v6 baseline")
        ...
        start_document = {
            "document_type": "SMPCC_R8_LIQUID_U3_C1_GENCASE_V6_START_RECEIPT",
        ...
        try:
            ...
            execution_status = "PASS_U3_C1_GENCASE_V6_FRAME_VALID_CLEANUP_PENDING"
        ...
        if parsed is not None and primary_error is None and not cleanup_errors:
            try:
                ...
                if (not isinstance(export_message, dict) or export_message.get("status") != "PASS_UID1000_O_EXCL_EXPORT_V6"
                ...
                execution_status = "PASS_U3_C1_GENCASE_V6_CASE_EXPORTED_CLEANUP_PENDING"
            except BaseException as exc:
                primary_error = f"{type(exc).__name__}: {exc}"[:4096]
        ...
        execution_document = {
            "document_type": "SMPCC_R8_LIQUID_U3_C1_GENCASE_V6_EXECUTION_RECEIPT",
        ...
        success = (
            execution_document["status"] == "PASS_U3_C1_GENCASE_V6_CASE_EXPORTED_CLEANUP_PENDING"
            and not cleanup_errors and primary_error is None and parsed is not None
            and export_verification is not None
        )
        if success:
            lifecycle_document = {
                "document_type": "SMPCC_R8_LIQUID_U3_C1_GENCASE_V6_LIFECYCLE_RECEIPT",
                "case_id": CASE_ID, "status": "PASS_U3_C1_GENCASE_V6_LIFECYCLE_CLEANUP_AND_CASE_EXPORT",
        ...
        failure_document = {
            "document_type": "SMPCC_R8_LIQUID_U3_C1_GENCASE_V6_LIFECYCLE_INCOMPLETE_RECEIPT",
        ...
        raise SupervisorError(f"v6 attempt failed closed; preserved {failure_record['path']}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if arguments.command == "self-check":
            ...
            print(json.dumps({
                "status": "PASS_V6_SUPERVISOR_STATIC_ADMITTED_EXECUTION_NOT_PERFORMED",
                "review": review,
                "snapshot_bootstrap": {
        ...
        if arguments.command == "export-frame":
            ...
            print(json.dumps({"status": "PASS_UID1000_O_EXCL_EXPORT_V6", "export": result}, ensure_ascii=False, sort_keys=True))
            return 0
