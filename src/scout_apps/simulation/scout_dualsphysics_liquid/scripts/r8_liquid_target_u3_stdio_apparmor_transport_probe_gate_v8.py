#!/usr/bin/env python3
"""Static gate and unprivileged runner for the fresh harmless v8 successor.
...
SUPERVISOR_NAME = "r8_liquid_target_u3_stdio_apparmor_transport_probe_supervisor_v8.py"
...
STDIN_MAGIC = b"R8LQSTDIOPROBE8\x00"
...
INPUT_PAYLOADS: dict[str, bytes] = {
    "probe_alpha.bin": b"R8_STDIO_PROBE_ALPHA_V8\n" * 11,
    "probe_beta.bin": bytes(range(256)) * 2 + b"BETA_V8",
    "probe_gamma.bin": hashlib.sha256(b"r8-stdio-probe-v8-gamma-seed").digest() * 31 + b"G",
}
...
def main():
    ...
    emit_success({{"status": "PASS_HARMLESS_STDIO_APPARMOR_TRANSPORT_PROBE_V8", "bootstrap_profile": BOOTSTRAP_PROFILE, "bootstrap_label": bootstrap_label, "bootstrap_identity": bootstrap_identity, "work_tmpfs": work_tmpfs, "inputs": observed, "host_stdin_consumed_and_fd0_replaced_with_eof_pipe": True, "fds_after_stdin": fds_after_stdin, "runtime": runtime, "fds_before_success": fds_before_success, "host_writable_mounts": []}})
    return 0
...
try:
    ...
    try:
        os.write(2, b"R8_STDIO_PROBE_V8_NO_GO\n")
    except OSError:
        pass
    raise SystemExit(2)
'''
...
exec(compile(p,"<r8-stdio-probe-helper-v8>","exec",dont_inherit=True,optimize=2),g,g)
'''
...
def bwrap_argv() -> list[str]:
    return [
        ...
        "--hostname", "r8-liquid-stdio-probe-v8", "--clearenv",
...
def verify_static() -> dict[str, Any]:
    ...
    expected = {
        "schema_version": "smpcc-r8-liquid-target-u3-stdio-apparmor-transport-probe-policy-v8",
        "policy_id": "LIQUID_ZRJ_MSI_U2404_U3_STDIO_APPARMOR_TRANSPORT_PROBE_V8",
        "host_id": "LIQUID_ZRJ_MSI_U2404",
        "status": "STATIC_READY_V8_PENDING_INDEPENDENT_REVIEW_AND_EXPLICIT_SINGLE_ATTEMPT",
    ...
    if (transport.get("stdin_magic_hex") != STDIN_MAGIC.hex()
        ...
        raise GateError("fresh v8 stdio protocol magic differs")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if arguments.command == "self-check":
            review = verify_static()
            print(json.dumps({"status": "PASS_V8_STATIC_DRAFT_EXECUTION_NOT_PERFORMED", "review": review}, ensure_ascii=False, sort_keys=True))
            return 0
