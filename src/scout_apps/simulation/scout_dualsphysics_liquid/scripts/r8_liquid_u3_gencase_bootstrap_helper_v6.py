#!/usr/bin/env python3
"""Guest-only bootstrap helper for the admitted U3 C1 GenCase v6 runtime.
...
FRAME_MAGIC = b"R8C1GENCASEV6\0\0\0"
...
def _audit_transient_outputs() -> tuple[dict[str, dict[str, Any]], dict[str, bytes]]:
    ...
    for name, (minimum, maximum) in EXPECTED_OUTPUTS.items():
        ...
        if name.endswith(".xml"):
            ...
            _audit_inactive_generated_features(root)
...
def _audit_inactive_generated_features(root: ET.Element) -> None:
    """Accept only GenCase's source-proven empty execution/motion container."""
    ...
    if (motion is None or all_motion != [motion] or motion.attrib or len(motion)
        ...
        raise GuestError("generated C1 XML motion container is not exactly one empty execution node")
    forbidden = {"floating", "inout", "wavegen"}
