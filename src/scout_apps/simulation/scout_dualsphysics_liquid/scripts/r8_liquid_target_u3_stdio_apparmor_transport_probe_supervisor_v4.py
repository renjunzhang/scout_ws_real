#!/usr/bin/env python3
"""Root lifecycle supervisor for the one-shot harmless v4 successor probe.
...
WORKSPACE_SUPERVISOR_PATH = Path("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/scripts/r8_liquid_target_u3_stdio_apparmor_transport_probe_supervisor_v4.py")
PROBE_ID = "u3_stdio_apparmor_transport_probe_v4_20260807T163041Z"
SNAPSHOT_ROOT = Path(f"/run/r8-liquid-{PROBE_ID}.snapshot")
SNAPSHOT_ENV = "R8_LIQUID_U3_STDIO_PROBE_V4_SNAPSHOT"
ADMISSION_ENV = "R8_LIQUID_U3_STDIO_PROBE_V4_ADMISSION"
ADMISSION_FD_ENV = "R8_LIQUID_U3_STDIO_PROBE_V4_ADMISSION_FD"
ADMISSION_SHA256_ENV = "R8_LIQUID_U3_STDIO_PROBE_V4_ADMISSION_SHA256"
...
PREDECESSOR_NO_GO = {
    ...
    "snapshot_root": "/run/r8-liquid-u3_stdio_apparmor_transport_probe_v3_20260807T144659Z.snapshot",
    "status": "SNAPSHOT_CREATED_NOT_EXECUTED_COMMAND_MODE_NO_GO",
...
POLICY_NAME = "liquid_zrj_msi_u2404_u3_stdio_apparmor_transport_probe_policy_v4.json"
SCHEMA_NAME = "target_host_u3_stdio_apparmor_transport_probe_policy_v4.json"
PROFILE_NAME = "r8-liquid-u3-stdio-transport-probe-v4.profile"
GATE_NAME = "r8_liquid_target_u3_stdio_apparmor_transport_probe_gate_v4.py"
SUPERVISOR_NAME = SCRIPT_PATH.name
BOOTSTRAP_PROFILE = "r8-liquid-u3-stdio-transport-bootstrap-v4-20260807t163041z"
RUNTIME_PROFILE = "r8-liquid-u3-stdio-transport-runtime-v4-20260807t163041z"
LABELS = (BOOTSTRAP_PROFILE, RUNTIME_PROFILE)
HOST_UID = 1000
HOST_GID = 1000
SUCCESS_MAGIC = b"R8LQSTDIOPASSV4\x00"
MAX_SUCCESS_FRAME_BYTES = 16_384
TMPFS_BYTES = 67_108_864
EXPECTED_INPUTS = {
    "probe_alpha.bin": {"size_bytes": 264, "sha256": "8295a02eeabe3d024a5a8cee2c410389ecc8dbfe13d346f577efbc135c18b1f8"},
    "probe_beta.bin": {"size_bytes": 519, "sha256": "35ce09bb6862ed0f83558c0f98357be6f8027649059764a0659d0b73bb581542"},
    "probe_gamma.bin": {"size_bytes": 993, "sha256": "4dbbb53252d08ada678ac81f259c67ffacb77d4986b8d2fe47a4622a8b73f6b5"},
}
...
ROOT="/run"
NAME="r8-liquid-u3_stdio_apparmor_transport_probe_v4_20260807T163041Z.snapshot"
SOURCES=(
 ("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/scripts/r8_liquid_target_u3_stdio_apparmor_transport_probe_gate_v4.py","r8_liquid_target_u3_stdio_apparmor_transport_probe_gate_v4.py"),
 ("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/scripts/r8_liquid_target_u3_stdio_apparmor_transport_probe_supervisor_v4.py","r8_liquid_target_u3_stdio_apparmor_transport_probe_supervisor_v4.py"),
 ("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/config/apparmor_drafts/r8-liquid-u3-stdio-transport-probe-v4.profile","r8-liquid-u3-stdio-transport-probe-v4.profile"),
 ("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/schema/target_host_u3_stdio_apparmor_transport_probe_policy_v4.json","target_host_u3_stdio_apparmor_transport_probe_policy_v4.json"),
 ("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/config/target_hosts/liquid_zrj_msi_u2404_u3_stdio_apparmor_transport_probe_policy_v4.json","liquid_zrj_msi_u2404_u3_stdio_apparmor_transport_probe_policy_v4.json"),
)
def open_abs(path):
...
os.write(1,(json.dumps({"status":"ROOT_SNAPSHOT_V4_CREATED_NOT_EXECUTED","files":manifest},sort_keys=True,separators=(",",":"))+"\n").encode("ascii"))
'''
...
g={{"__name__":"__main__"}}
exec(compile(p,"<r8-liquid-root-snapshot-bootstrap-v4>","exec",dont_inherit=True,optimize=2),g,g)
'''
...
def repository_static_review() -> dict[str, Any]:
    ...
    if policy.get("status") != "STATIC_READY_V4_PENDING_INDEPENDENT_REVIEW_AND_EXPLICIT_SINGLE_ATTEMPT":
        raise SupervisorError("v4 static status differs")
...
def verify_snapshot_producer_identity() -> dict[str, Any]:
    ...
    if SCRIPT_PATH != WORKSPACE_SUPERVISOR_PATH:
        raise SupervisorError("snapshot producer must be the exact workspace v4 supervisor")
...
def _json_contains_exact(value: Any, needle: str) -> bool:
    ...
    if isinstance(value, dict):
        return needle in value or any(_json_contains_exact(item, needle) for item in value.values())
    return False
...
def profile_state() -> dict[str, Any]:
    ...
    result = run_bounded_command(["/usr/sbin/aa-status", "--json", "--show", "profiles"], stdout_limit=2 * 1024 * 1024, stderr_limit=16_384, timeout_seconds=15)
    ...
    presence = {label: _json_contains_exact(parsed, label) for label in LABELS}
    for label in LABELS:
...
def _assert_one_shot_paths_absent() -> None:
    for path in (START_RECEIPT, EXECUTION_RECEIPT, LIFECYCLE_RECEIPT, LIFECYCLE_FAILURE_RECEIPT, RECOVERY_RECEIPT):
        try:
            os.lstat(path)
        except FileNotFoundError:
            continue
...
def parse_success_frame(frame: bytes) -> dict[str, Any]:
    ...
    if (value["status"] != "PASS_HARMLESS_STDIO_APPARMOR_TRANSPORT_PROBE_V4"
...
def classify_execution(result: Mapping[str, Any], audit: Mapping[str, Any]) -> tuple[str, dict[str, Any]]:
    ...
    gate_no_go_envelope = (
        ...
        or b"R8_STDIO_PROBE_V4_NO_GO" in stderr
    )
...
def execution_receipt_document(
    *,
    ...
    return {
        "document_type": "SMPCC_R8_LIQUID_U3_STDIO_APPARMOR_PROBE_V4_EXECUTION_RECEIPT",
        "probe_id": PROBE_ID,
        "status": status,
...
def lifecycle_document(
    *,
    ...
    return {
        "document_type": "SMPCC_R8_LIQUID_U3_STDIO_APPARMOR_PROBE_V4_LIFECYCLE_RECEIPT",
        "probe_id": PROBE_ID,
        "status": "PASS_V4_PROFILE_PROCESS_SYSCTL_SUDO_LIFECYCLE_CLEANUP",
...
def _run_once_guarded(
    *,
    ...
    start_document = {
        "document_type": "SMPCC_R8_LIQUID_U3_STDIO_APPARMOR_PROBE_V4_START_RECEIPT",
        "probe_id": PROBE_ID,
    ...
    profiles_after: dict[str, Any] = {"kernel_exact_counts": {label: -1 for label in LABELS}, "aa_status_exact_presence": {label: True for label in LABELS}}
    ...
    failure = {
        "document_type": "SMPCC_R8_LIQUID_U3_STDIO_APPARMOR_PROBE_V4_LIFECYCLE_INCOMPLETE_RECEIPT",
        "probe_id": PROBE_ID,
    ...
    raise SupervisorError(f"v4 lifecycle incomplete; preserved receipt {failure_record['path']}")
...
def recover_only(*, policy_sha256: str, recovery_token: str) -> dict[str, Any]:
    ...
    with TerminationGuard() as termination_guard:
        ...
        document = {
            "document_type": "SMPCC_R8_LIQUID_U3_STDIO_APPARMOR_PROBE_V4_RECOVERY_RECEIPT",
            "probe_id": PROBE_ID,
            "status": "PASS_V4_CLEANUP_ONLY_NO_PROBE_EXECUTED",
            "policy_sha256": policy_sha256,
            "snapshot": snapshot,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if arguments.command == "recover-only":
            ...
            print(json.dumps({"status": "V4_CLEANUP_ONLY_RECORDED", "result": result}, ensure_ascii=False, sort_keys=True))
            return 0
