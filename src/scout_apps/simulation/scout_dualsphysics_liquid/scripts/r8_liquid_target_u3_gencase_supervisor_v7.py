#!/usr/bin/env python3
"""Root lifecycle supervisor and host frame auditor for U3 C1M GenCase v7.
...
WORKSPACE_SUPERVISOR_PATH = WORKSPACE_PACKAGE_ROOT / "scripts/r8_liquid_target_u3_gencase_supervisor_v7.py"
CASE_ID = "u3_c1m_gencase_v7_20260808T151500Z"
HOST_UID = 1000
HOST_GID = 1000
BOOTSTRAP_PROFILE = "r8-liquid-u3-c1m-gencase-bootstrap-v7-20260808t151500z"
RUNTIME_PROFILE = "r8-liquid-u3-c1m-gencase-runtime-v7-20260808t151500z"
LABELS = (BOOTSTRAP_PROFILE, RUNTIME_PROFILE)
SNAPSHOT_ENV = "R8_LIQUID_U3_GENCASE_V7_SNAPSHOT"
ADMISSION_FD_ENV = "R8_LIQUID_U3_GENCASE_V7_ADMISSION_FD"
ADMISSION_SHA256_ENV = "R8_LIQUID_U3_GENCASE_V7_ADMISSION_SHA256"
EXPORT_FD_ENV = "R8_LIQUID_U3_GENCASE_V7_EXPORT_FD"
EXPORT_SHA256_ENV = "R8_LIQUID_U3_GENCASE_V7_EXPORT_SHA256"
ADMISSION_TOKEN = f"{CASE_ID}:single-gencase-attempt"
SNAPSHOT_ROOT = Path(f"/run/r8-liquid-{CASE_ID}.snapshot")

POLICY_NAME = "liquid_zrj_msi_u2404_u3_gencase_execution_policy_v7.json"
SCHEMA_NAME = "target_host_u3_gencase_execution_policy_v7.json"
PROFILE_NAME = "r8-liquid-u3-gencase-v7.profile"
GATE_NAME = "r8_liquid_target_u3_gencase_gate_v7.py"
HELPER_NAME = "r8_liquid_u3_gencase_bootstrap_helper_v7.py"
SUPERVISOR_NAME = "r8_liquid_target_u3_gencase_supervisor_v7.py"
...
HELPER_MAGIC = b"R8C1HELPERV7\0\0\0\0\0"
INPUT_MAGIC = b"R8C1INPUTV7\0\0\0\0\0"
OUTPUT_MAGIC = b"R8C1GENCASEV7\0\0\0"
...
os.write(1,(json.dumps({{"status":"ROOT_GENCASE_V7_SNAPSHOT_CREATED_NOT_EXECUTED","files":manifest}},sort_keys=True,separators=(",",":"))+"\\n").encode("ascii"))
'''
...
g={{"__name__":"__main__"}}
exec(compile(p,"<r8-liquid-gencase-v7-root-snapshot-bootstrap>","exec",dont_inherit=True,optimize=2),g,g)
'''
...
def verify_policy_static() -> dict[str, Any]:
    ...
    if policy.get("status") != "ADMITTED_SINGLE_C1M_ZERO_MOTION_GENCASE_RUNTIME_V7_AFTER_FROZEN_V11_ZERO_DENIAL_PROBE":
        raise SupervisorError("v7 one-shot admission status differs")
    if policy.get("allowed_gate_commands") != ["self-check", "internal-run"]:
        raise SupervisorError("v7 gate command surface differs")
    ...
    if (
        ...
        raise SupervisorError("v7 C1M seed v3 provenance differs")
    ...
    if (probe.get("required_receipt_sha256") != frozen_receipts["execution"][1]
        ...
        raise SupervisorError("v7 policy does not pin the frozen v11 PASS lifecycle")
...
def _validate_generated_xml(raw: bytes) -> None:
    ...
    if (motion is None or all_motion != [motion] or motion.attrib or (motion.text or "").strip()
            or [child.tag for child in motion] != ["objreal"]):
        raise SupervisorError("framed XML C1M motion container differs")
    objreal = motion[0]
    if objreal.attrib != {"ref": "0"} or [child.tag for child in objreal] != ["begin", "mvnull"]:
        raise SupervisorError("framed XML C1M zero-motion object differs")
    begin, mvnull = objreal
    try:
        begin_start = float(begin.get("start", "nan"))
    except ValueError as exc:
        raise SupervisorError("framed XML C1M begin start is not numeric") from exc
    if (set(begin.attrib) != {"mov", "start"} or begin.get("mov") != "1"
            or begin_start != 0.0 or len(begin)
            or (begin.text or "").strip() or mvnull.attrib != {"id": "1"}
            or len(mvnull) or (mvnull.text or "").strip()):
        raise SupervisorError("framed XML C1M mvnull event differs")
    forbidden = {"floating", "inout", "wavegen", "obj", "wait", "mvrect", "mvrectace",
...
def parse_success_frame(raw: bytes) -> dict[str, Any]:
    ...
    if metadata["document_type"] != "SMPCC_R8_LIQUID_U3_C1M_GENCASE_GUEST_FRAME_V7" or metadata["status"] != "GUEST_FRAME_COMPLETE_PENDING_HOST_REAUDIT_AND_O_EXCL_EXPORT":
...
def load_lifecycle_harness() -> Any:
    ...
    spec = importlib.util.spec_from_file_location("_r8_gencase_v7_lifecycle_harness", HARNESS_PATH)
...
def run_once(*, policy_sha256: str, admission_token: str) -> dict[str, Any]:
    ...
    with harness.TerminationGuard() as termination_guard:
        ...
        if policy.get("profile_lifecycle", {}).get("expected_sysctls") != sysctls_before:
            raise SupervisorError("current sysctls differ from the frozen v7 baseline")
        ...
        start_document = {
            "document_type": "SMPCC_R8_LIQUID_U3_C1M_GENCASE_V7_START_RECEIPT",
        ...
        try:
            ...
            execution_status = "PASS_U3_C1M_GENCASE_V7_FRAME_VALID_CLEANUP_PENDING"
        ...
        if parsed is not None and primary_error is None and not cleanup_errors:
            try:
                ...
                if (not isinstance(export_message, dict) or export_message.get("status") != "PASS_UID1000_O_EXCL_EXPORT_V7"
                ...
                execution_status = "PASS_U3_C1M_GENCASE_V7_CASE_EXPORTED_CLEANUP_PENDING"
            except BaseException as exc:
                primary_error = f"{type(exc).__name__}: {exc}"[:4096]
        ...
        execution_document = {
            "document_type": "SMPCC_R8_LIQUID_U3_C1M_GENCASE_V7_EXECUTION_RECEIPT",
        ...
        success = (
            execution_document["status"] == "PASS_U3_C1M_GENCASE_V7_CASE_EXPORTED_CLEANUP_PENDING"
            and not cleanup_errors and primary_error is None and parsed is not None
            and export_verification is not None
        )
        if success:
            lifecycle_document = {
                "document_type": "SMPCC_R8_LIQUID_U3_C1M_GENCASE_V7_LIFECYCLE_RECEIPT",
                "case_id": CASE_ID, "status": "PASS_U3_C1M_GENCASE_V7_LIFECYCLE_CLEANUP_AND_CASE_EXPORT",
        ...
        failure_document = {
            "document_type": "SMPCC_R8_LIQUID_U3_C1M_GENCASE_V7_LIFECYCLE_INCOMPLETE_RECEIPT",
        ...
        raise SupervisorError(f"v7 attempt failed closed; preserved {failure_record['path']}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if arguments.command == "self-check":
            ...
            print(json.dumps({
                "status": "PASS_V7_SUPERVISOR_STATIC_ADMITTED_EXECUTION_NOT_PERFORMED",
                "review": review,
                "snapshot_bootstrap": {
        ...
        if arguments.command == "export-frame":
            ...
            print(json.dumps({"status": "PASS_UID1000_O_EXCL_EXPORT_V7", "export": result}, ensure_ascii=False, sort_keys=True))
            return 0
        result = run_once(policy_sha256=arguments.policy_sha256, admission_token=arguments.admission_token)
        print(json.dumps({"status": "V7_ONE_SHOT_LIFECYCLE_RECORDED", "result": result}, ensure_ascii=False, sort_keys=True))
        return 0
