...
WORKSPACE_SUPERVISOR_PATH = WORKSPACE_PACKAGE_ROOT / "scripts/r8_liquid_target_u3_proc_stat_boundary_probe_supervisor_v1.py"
...
GATE_NAME = "r8_liquid_target_u3_proc_stat_boundary_probe_gate_v1.py"
...
ROOT="/run"
NAME="r8-liquid-u3_proc_stat_apparmor_boundary_probe_v1_20260809T002729Z.snapshot"
SOURCES=(
 ("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/scripts/r8_liquid_target_u3_proc_stat_boundary_probe_gate_v1.py","r8_liquid_target_u3_proc_stat_boundary_probe_gate_v1.py"),
 ("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/scripts/r8_liquid_target_u3_proc_stat_boundary_probe_supervisor_v1.py","r8_liquid_target_u3_proc_stat_boundary_probe_supervisor_v1.py"),
 ("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/config/apparmor_drafts/r8-liquid-u3-proc-stat-boundary-probe-v1.profile","r8-liquid-u3-proc-stat-boundary-probe-v1.profile"),
 ("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/schema/target_host_u3_proc_stat_boundary_probe_policy_v1.json","target_host_u3_proc_stat_boundary_probe_policy_v1.json"),
 ("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/config/target_hosts/liquid_zrj_msi_u2404_u3_proc_stat_boundary_probe_policy_v1.json","liquid_zrj_msi_u2404_u3_proc_stat_boundary_probe_policy_v1.json"),
)
def open_abs(path):
...
os.write(1,(json.dumps({"status":"ROOT_PROC_STAT_PROBE_V1_SNAPSHOT_CREATED_NOT_EXECUTED","files":manifest},sort_keys=True,separators=(",",":"))+"\n").encode("ascii"))
'''
...
g={{"__name__":"__main__"}}
exec(compile(p,"<r8-liquid-root-proc-stat-probe-bootstrap-v1>","exec",dont_inherit=True,optimize=2),g,g)
'''
...
def repository_static_review() -> dict[str, Any]:
    ...
    if policy.get("provenance") != {"cold_a_v6_failure": FROZEN_V6_FAILURE}:
        raise SupervisorError("frozen v6 failure provenance policy differs")
    ...
    if (
        ...
        or semantics.get("explicit_silent_denials") != ["deny capability dac_override,"]
    ):
    ...
    if (
        ...
        or candidate.get("candidate_delta_from_v6") != "one_non_owner_numeric_pid_stat_read_rule_only"
    ...
    if (
        payload.get("frame_magic_hex") != "523850524f435354415450524f42453100"
...
def verify_snapshot_producer_identity() -> dict[str, Any]:
    ...
    if SCRIPT_PATH != WORKSPACE_SUPERVISOR_PATH:
        raise SupervisorError("snapshot producer must be the exact workspace proc-stat v1 supervisor")
...
def verify_snapshot(policy_sha256: str) -> tuple[dict[str, Any], dict[str, dict[str, Any]]]:
    ...
    if (
        policy.get("status") != "STATIC_READY_V1_PENDING_INDEPENDENT_REVIEW_AND_EXPLICIT_SINGLE_ATTEMPT"
    ...
    if policy.get("provenance") != {"cold_a_v6_failure": FROZEN_V6_FAILURE}:
        raise SupervisorError("snapshot v6 failure provenance differs")
...
def labeled_processes(proc_root: Path = Path("/proc")) -> list[dict[str, Any]]:
    """Authoritatively enumerate every task/thread carrying a fresh v1 label."""
...
def classify_execution(result: Mapping[str, Any], audit: Mapping[str, Any]) -> tuple[str, dict[str, Any]]:
    ...
    if (result.get("failure") is None and type(returncode) is int and returncode == 0
        ...
        return "PASS_U3_PROC_STAT_APPARMOR_BOUNDARY_PROBE_V1_CLEANUP_PENDING", evidence
...
def execution_receipt_document(
    *,
    ...
    return {
        "document_type": "SMPCC_R8_LIQUID_U3_PROC_STAT_APPARMOR_BOUNDARY_PROBE_V1_EXECUTION_RECEIPT",
        "probe_id": PROBE_ID,
        "status": status,
...
def lifecycle_document(
    *,
    ...
    return {
        "document_type": "SMPCC_R8_LIQUID_U3_PROC_STAT_APPARMOR_BOUNDARY_PROBE_V1_LIFECYCLE_RECEIPT",
        "probe_id": PROBE_ID,
        "status": "PASS_U3_PROC_STAT_APPARMOR_BOUNDARY_PROBE_V1_LIFECYCLE_CLEANUP",
...
def run_once(*, policy_sha256: str, admission_token: str) -> dict[str, Any]:
    with TerminationGuard() as termination_guard:
        try:
            ...
            document = {
                "document_type": "SMPCC_R8_LIQUID_U3_PROC_STAT_APPARMOR_BOUNDARY_PROBE_V1_PREFLIGHT_FAILURE_RECEIPT",
                "probe_id": PROBE_ID,
                "status": (
...
def _run_once_guarded(
    *,
    ...
    start_document = {
        "document_type": "SMPCC_R8_LIQUID_U3_PROC_STAT_APPARMOR_BOUNDARY_PROBE_V1_START_RECEIPT",
        "probe_id": PROBE_ID,
    ...
    failure = {
        "document_type": "SMPCC_R8_LIQUID_U3_PROC_STAT_APPARMOR_BOUNDARY_PROBE_V1_LIFECYCLE_INCOMPLETE_RECEIPT",
        "probe_id": PROBE_ID,
...
def recover_only(*, policy_sha256: str, recovery_token: str) -> dict[str, Any]:
    ...
    with TerminationGuard() as termination_guard:
        ...
        document = {
            "document_type": "SMPCC_R8_LIQUID_U3_PROC_STAT_APPARMOR_BOUNDARY_PROBE_V1_RECOVERY_RECEIPT",
            "probe_id": PROBE_ID,
            "status": "PASS_U3_PROC_STAT_APPARMOR_BOUNDARY_PROBE_V1_CLEANUP_ONLY_NO_PROBE_EXECUTED",
            "policy_sha256": policy_sha256,
            "snapshot": snapshot,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if arguments.command == "recover-only":
            ...
            print(json.dumps({"status": "PROC_STAT_BOUNDARY_PROBE_V1_CLEANUP_ONLY_RECORDED", "result": result}, ensure_ascii=False, sort_keys=True))
            return 0
        result = run_once(policy_sha256=arguments.policy_sha256, admission_token=arguments.admission_token)
        print(json.dumps({"status": "PROC_STAT_BOUNDARY_PROBE_V1_ONE_SHOT_LIFECYCLE_RECORDED", "result": result}, ensure_ascii=False, sort_keys=True))
        return 0 if result["execution_status"] == "PASS_U3_PROC_STAT_APPARMOR_BOUNDARY_PROBE_V1_CLEANUP_PENDING" else 2
