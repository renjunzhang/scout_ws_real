#!/usr/bin/env python3
"""Root lifecycle supervisor for the one-shot harmless v3 denial probe.
...
GATE_NAME = "r8_liquid_target_u3_stdio_apparmor_transport_probe_gate_v3.py"
...
EXPECTED_INPUTS = {
    "probe_alpha.bin": {"size_bytes": 264, "sha256": "4cd51fb6ff9e87408a487cb726471e8e38e196d1b4646604ab1a23001bb7c9b2"},
    "probe_beta.bin": {"size_bytes": 519, "sha256": "e359c8230d068e60614da79938b10fdc9a8827f241313baa79dff0aabe8b09fa"},
    "probe_gamma.bin": {"size_bytes": 993, "sha256": "ba44cbf4bdc09698034f47fc8809892a5e64bd876b0e34e35dbdc1379e8bcc76"},
}
...
ROOT="/run"
NAME="r8-liquid-u3_stdio_apparmor_transport_probe_v3_20260807T144659Z.snapshot"
SOURCES=(
 ("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/scripts/r8_liquid_target_u3_stdio_apparmor_transport_probe_gate_v3.py","r8_liquid_target_u3_stdio_apparmor_transport_probe_gate_v3.py"),
 ("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/scripts/r8_liquid_target_u3_stdio_apparmor_transport_probe_supervisor_v3.py","r8_liquid_target_u3_stdio_apparmor_transport_probe_supervisor_v3.py"),
 ("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/config/apparmor_drafts/r8-liquid-u3-stdio-transport-probe-v3.profile","r8-liquid-u3-stdio-transport-probe-v3.profile"),
 ("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/schema/target_host_u3_stdio_apparmor_transport_probe_policy_v3.json","target_host_u3_stdio_apparmor_transport_probe_policy_v3.json"),
 ("/home/zrj/scout_ws/src/scout_apps/simulation/scout_dualsphysics_liquid/config/target_hosts/liquid_zrj_msi_u2404_u3_stdio_apparmor_transport_probe_policy_v3.json","liquid_zrj_msi_u2404_u3_stdio_apparmor_transport_probe_policy_v3.json"),
)
def open_abs(path):
...
os.write(1,(json.dumps({"status":"ROOT_SNAPSHOT_CREATED_NOT_EXECUTED","files":manifest},sort_keys=True,separators=(",",":"))+"\n").encode("ascii"))
'''
...
g={{"__name__":"__main__"}}
exec(compile(p,"<r8-liquid-root-snapshot-bootstrap-v3>","exec",dont_inherit=True,optimize=2),g,g)
'''
...
def repository_static_review() -> dict[str, Any]:
    ...
    if policy.get("status") != "STATIC_READY_V3_PENDING_INDEPENDENT_REVIEW_AND_EXPLICIT_SINGLE_ATTEMPT":
        raise SupervisorError("v3 static status differs")
    ...
    expected_commands = {
        ...
        "run_argv_template": [
            "/usr/bin/sudo", str(SNAPSHOT_PATHS["supervisor"]), "run", "--policy-sha256",
            "<EXTERNALLY_FROZEN_POLICY_SHA256>", "--admission-token", ADMISSION_TOKEN,
        ],
        "recover_argv_template": [
            "/usr/bin/sudo", str(SNAPSHOT_PATHS["supervisor"]), "recover-only", "--policy-sha256",
    ...
    if commands.get("operator_transport") != "snapshot producer stdout is root loader stdin; sudo password is read only from the inherited /dev/tty":
...
def classify_execution(result: Mapping[str, Any], audit: Mapping[str, Any]) -> tuple[str, dict[str, Any]]:
    ...
    gate_no_go_envelope = (
        ...
        or b"R8_STDIO_PROBE_V3_NO_GO" in stderr
    )
...
def execution_receipt_document(
    *,
    ...
    return {
        "document_type": "SMPCC_R8_LIQUID_U3_STDIO_APPARMOR_PROBE_V3_EXECUTION_RECEIPT",
        "probe_id": PROBE_ID,
        "status": status,
...
def lifecycle_document(
    *,
    ...
    return {
        "document_type": "SMPCC_R8_LIQUID_U3_STDIO_APPARMOR_PROBE_V3_LIFECYCLE_RECEIPT",
        "probe_id": PROBE_ID,
        "status": "PASS_V3_PROFILE_PROCESS_SYSCTL_SUDO_LIFECYCLE_CLEANUP",
...
def _run_once_guarded(
    *,
    ...
    start_document = {
        "document_type": "SMPCC_R8_LIQUID_U3_STDIO_APPARMOR_PROBE_V3_START_RECEIPT",
        "probe_id": PROBE_ID,
    ...
    failure = {
        "document_type": "SMPCC_R8_LIQUID_U3_STDIO_APPARMOR_PROBE_V3_LIFECYCLE_INCOMPLETE_RECEIPT",
        "probe_id": PROBE_ID,
    ...
    raise SupervisorError(f"v3 lifecycle incomplete; preserved receipt {failure_record['path']}")
...
def recover_only(*, policy_sha256: str, recovery_token: str) -> dict[str, Any]:
    ...
    with TerminationGuard() as termination_guard:
        ...
        document = {
            "document_type": "SMPCC_R8_LIQUID_U3_STDIO_APPARMOR_PROBE_V3_RECOVERY_RECEIPT",
            "probe_id": PROBE_ID,
            "status": "PASS_V3_CLEANUP_ONLY_NO_PROBE_EXECUTED",
            "policy_sha256": policy_sha256,
            "snapshot": snapshot,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if arguments.command == "recover-only":
            ...
            print(json.dumps({"status": "V3_CLEANUP_ONLY_RECORDED", "result": result}, ensure_ascii=False, sort_keys=True))
            return 0
        result = run_once(policy_sha256=arguments.policy_sha256, admission_token=arguments.admission_token)
        print(json.dumps({"status": "V3_ONE_SHOT_LIFECYCLE_RECORDED", "result": result}, ensure_ascii=False, sort_keys=True))
