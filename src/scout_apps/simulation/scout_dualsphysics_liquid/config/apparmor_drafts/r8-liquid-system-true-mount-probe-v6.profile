# R8 liquid target transient AppArmor mount-confinement probe v6.
...
# self-bind of newroot, and the exact pivot_root from /tmp/ with /tmp/oldroot/.
# Bubblewrap 0.9.0 documents that it always creates a new mount namespace, and
# both paths exist under the just-mounted tmpfs.  This profile never permits a
# host bind, remount, umount, procfs, devtmpfs, output bind, source bind, or
...
profile r8-liquid-system-true-mount-probe-v6 flags=(attach_disconnected,mediate_deleted) {
  ...
  # pivot_root moves only that temporary root and its temporary oldroot.
  ...
  ptrace (read, readby) peer=r8-liquid-system-true-mount-probe-v6,
  network unix dgram,
  network inet dgram,
  network inet6 dgram,
  network netlink raw,
