# R8 liquid target transient AppArmor mount-confinement probe v3.
...
#
# Scope: allow only the two observed bwrap setup mounts and the creation of its
# ephemeral /tmp/newroot/ tree.  Bubblewrap 0.9.0 documents that it always
# creates a new mount namespace.  The immediately preceding exact tmpfs mount
# hides the host /tmp before this tree is created.  This profile never permits
# a bind, remount, umount, procfs, devtmpfs, output bind, source bind, or
# host-writable mount.  It must be removed immediately after the bounded probe
# and must never be copied to /etc/apparmor.d or enabled for boot-time loading.
abi <abi/4.0>,

include <tunables/global>

profile r8-liquid-system-true-mount-probe-v3 flags=(attach_disconnected,mediate_deleted) {
  ...
  # These two rules exactly match the observed bwrap setup mounts.  The
  # first makes root propagation private to bwrap's mount namespace; the
  # second creates only an ephemeral, nodev/nosuid tmpfs at /tmp/ there.
  mount options=(rw, silent, rslave) -> /,
  mount fstype=tmpfs options=(rw, nosuid, nodev) -> /tmp/,

  # bwrap's next documented setup action creates only this transient newroot
  # tree under that just-mounted tmpfs.  It does not grant /tmp/**, /home,
  # source, output, or any other host path.
  /tmp/newroot/ rw,
  /tmp/newroot/** rw,
  ...
  ptrace (read, readby) peer=r8-liquid-system-true-mount-probe-v3,
  network unix dgram,
  network inet dgram,
  network inet6 dgram,
  network netlink raw,
