# R8 liquid U3 CPU-build profile v3.
# REVIEWED_SINGLE_ATTEMPT_ONLY: companion profile to source-copy v3.  Its fixed
# Make argv supplies SHELL=/usr/bin/dash, so no synthetic /bin link is needed;
# /lib64 -> usr/lib64 is the sole synthetic link.  The outer sudo -> setpriv
# handoff drops to zrj:zrj with zero supplementary groups before aa-exec.
# Never install this named profile under /etc/apparmor.d.
abi <abi/4.0>,

include <tunables/global>

profile r8-liquid-u3-cpu-build-v3 flags=(attach_disconnected,mediate_deleted) {
  userns create,
  ...
  ptrace (read, readby) peer=r8-liquid-u3-cpu-build-v3,
  signal (send, receive) peer=r8-liquid-u3-cpu-build-v3,
  network unix dgram,
  network inet dgram,
  network inet6 dgram,
  network netlink raw,

  # Intentionally absent: /bin and /lib links, compiled-artifact execution,
  # all upstream binaries, source/repository/workspace binds, GPU/DRM devices,
  # broad host paths, generic mount/remount/umount, stream network, dbus,
