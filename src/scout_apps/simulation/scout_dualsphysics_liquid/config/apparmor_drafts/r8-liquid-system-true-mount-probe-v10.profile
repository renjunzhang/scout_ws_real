# R8 liquid target transient AppArmor mount-confinement probe v10.
...
# rprivate propagation setting on temporary /oldroot/, its exact unmount, the
# read-only open of the synthetic root, and the audited second pivot_root whose
# new-root and old-root names are both /newroot/. Bubblewrap 0.9.0 documents
...
profile r8-liquid-system-true-mount-probe-v10 flags=(attach_disconnected,mediate_deleted) {
  ...
  # both pivot roots, rprivate, and the oldroot unmount only affect bwrap's
  # new mount namespace after its root was made rslave.
  ...
  ptrace (read, readby) peer=r8-liquid-system-true-mount-probe-v10,
  network unix dgram,
  network inet dgram,
  network inet6 dgram,
  network netlink raw,
