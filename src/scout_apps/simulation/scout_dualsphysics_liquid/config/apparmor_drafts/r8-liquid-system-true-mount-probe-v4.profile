# R8 liquid target transient AppArmor mount-confinement probe v4.
...
# propagation, nodev/nosuid /tmp tmpfs, /tmp/newroot/ creation, and a recursive
# bind of that same newroot path onto itself.  Bubblewrap 0.9.0 documents that
# it always creates a new mount namespace, and the newroot is under the just-
# mounted tmpfs.  This profile never permits a host bind, remount, umount,
# procfs, devtmpfs, output bind, source bind, or host-writable mount.  It must
# be removed immediately after the bounded probe and must never be copied to
# /etc/apparmor.d or enabled for boot-time loading.
abi <abi/4.0>,

include <tunables/global>

profile r8-liquid-system-true-mount-probe-v4 flags=(attach_disconnected,mediate_deleted) {
  ...
  # These three rules exactly match the observed setup sequence.  The rbind
  # source and target are the same just-created directory within the tmpfs.
  mount options=(rw, silent, rslave) -> /,
  mount fstype=tmpfs options=(rw, nosuid, nodev) -> /tmp/,
  mount options=(rw, rbind) /tmp/newroot/ -> /tmp/newroot/,

  # This is the only writable subtree and it exists only beneath bwrap's
  # already-mounted /tmp tmpfs.  It does not grant /tmp/**, /home, source,
  ...
  ptrace (read, readby) peer=r8-liquid-system-true-mount-probe-v4,
  network unix dgram,
  network inet dgram,
  network inet6 dgram,
  network netlink raw,
