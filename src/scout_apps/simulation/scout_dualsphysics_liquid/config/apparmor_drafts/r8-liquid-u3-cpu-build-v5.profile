# R8 liquid U3 CPU-build profile v5.
# REVIEWED_SINGLE_ATTEMPT_ONLY: v5 uses only the successful /usr + /lib64 +
# tmpfs /work guest layout, fixed SHELL=/usr/bin/dash, and the pre-aa-exec
# zero-group setpriv handoff. Never install this named profile persistently.
abi <abi/4.0>,

include <tunables/global>

profile r8-liquid-u3-cpu-build-v5 flags=(attach_disconnected,mediate_deleted) {
  userns create,
  ...
  ptrace (read, readby) peer=r8-liquid-u3-cpu-build-v5,
  signal (send, receive) peer=r8-liquid-u3-cpu-build-v5,
  network unix dgram,
  network inet dgram,
  network inet6 dgram,
  network netlink raw,

  # Intentionally absent: guest tmpfs /tmp, proc/dev guest mount, artifact
  # execution, upstream binaries, source/repository/workspace binds, GPU,
