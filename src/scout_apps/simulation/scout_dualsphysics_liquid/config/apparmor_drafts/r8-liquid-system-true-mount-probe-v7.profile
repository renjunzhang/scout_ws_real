# R8 liquid target transient AppArmor mount-confinement probe v7.
...
# self-bind of newroot, the exact pivot_root from /tmp/ with /tmp/oldroot/, and
# the following rprivate propagation setting on the resulting temporary
# /oldroot/. Bubblewrap 0.9.0 documents that it always creates a new mount
# namespace. This profile never permits a host bind, remount, umount, procfs,
...
profile r8-liquid-system-true-mount-probe-v7 flags=(attach_disconnected,mediate_deleted) {
  ...
  # pivot_root and rprivate only affect the synthetic root and oldroot inside
  # bwrap's new mount namespace.
  ...
  ptrace (read, readby) peer=r8-liquid-system-true-mount-probe-v7,
  network unix dgram,
  network inet dgram,
  network inet6 dgram,
  network netlink raw,
