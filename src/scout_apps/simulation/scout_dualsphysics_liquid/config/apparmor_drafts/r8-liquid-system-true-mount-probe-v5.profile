# R8 liquid target transient AppArmor mount-confinement probe v5.
...
# propagation, nodev/nosuid /tmp tmpfs, newroot/oldroot creation, and a
# recursive self-bind of newroot.  Bubblewrap 0.9.0 documents that it always
# creates a new mount namespace, and both trees exist under the just-mounted
# tmpfs.  This profile never permits a host bind, remount, umount, procfs,
...
profile r8-liquid-system-true-mount-probe-v5 flags=(attach_disconnected,mediate_deleted) {
  ...
  # These three rules exactly match the observed setup mounts.  The rbind
  ...
  ptrace (read, readby) peer=r8-liquid-system-true-mount-probe-v5,
  network unix dgram,
  network inet dgram,
  network inet6 dgram,
  network netlink raw,
