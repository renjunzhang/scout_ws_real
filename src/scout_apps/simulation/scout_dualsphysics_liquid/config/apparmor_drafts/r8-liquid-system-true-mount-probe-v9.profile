# R8 liquid target transient AppArmor mount-confinement probe v9.
...
# self-bind of newroot, the exact pivot_root from /tmp/ with /tmp/oldroot/, the
# rprivate propagation setting on temporary /oldroot/, its exact unmount, and
# the read-only open of the resulting synthetic root directory. Bubblewrap 0.9.0
# documents that it always creates a new mount namespace. This profile never
# permits a host bind, generic umount, remount, read of a host file, procfs,
...
profile r8-liquid-system-true-mount-probe-v9 flags=(attach_disconnected,mediate_deleted) {
  ...
  # only the current synthetic root directory; no concrete host file path is
  ...
  # pivot_root, rprivate, and the oldroot unmount only affect bwrap's new
  ...
  ptrace (read, readby) peer=r8-liquid-system-true-mount-probe-v9,
  network unix dgram,
  network inet dgram,
  network inet6 dgram,
  network netlink raw,
