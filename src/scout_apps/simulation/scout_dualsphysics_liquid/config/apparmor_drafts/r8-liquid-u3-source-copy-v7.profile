# R8 liquid U3 source-copy profile v7.
# REVIEWED_SINGLE_ATTEMPT_ONLY: v6 allowed a read of the exact bwrap
# --disable-userns sysctl, but bubblewrap 0.9.0 opens that node O_WRONLY and
# writes the literal value 1 only after clone(CLONE_NEWUSER) has entered its
# first user namespace.  It then creates a second user namespace, so that
# namespace-local limit prevents further nested user namespaces.  The exact
# write below is therefore required for the frozen bwrap feature, is not a
# host-initial-namespace sysctl permission, and is paired with a before/after
# host-value invariant in the gate.  No other /proc/sys write is admitted.
abi <abi/4.0>,

include <tunables/global>

profile r8-liquid-u3-source-copy-v7 flags=(attach_disconnected,mediate_deleted) {
  userns create,
  ...
  ptrace (read, readby) peer=r8-liquid-u3-source-copy-v7,
  signal (send, receive) peer=r8-liquid-u3-source-copy-v7,
  network unix dgram,
  network inet dgram,
  network inet6 dgram,
  network netlink raw,
