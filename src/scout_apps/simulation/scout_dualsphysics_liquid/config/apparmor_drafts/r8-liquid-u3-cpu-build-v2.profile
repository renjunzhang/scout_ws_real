# R8 liquid U3 CPU-build profile v2.
# REVIEWED_SINGLE_ATTEMPT_ONLY: companion revision for the post-v1 attempt.
# sudo may only invoke setpriv to drop to zrj:zrj with zero supplementary
# groups before aa-exec.  This named profile is transient and must never be
# installed under /etc/apparmor.d.
abi <abi/4.0>,

include <tunables/global>

profile r8-liquid-u3-cpu-build-v2 flags=(attach_disconnected,mediate_deleted) {
  userns create,
  ...
  ptrace (read, readby) peer=r8-liquid-u3-cpu-build-v2,
  signal (send, receive) peer=r8-liquid-u3-cpu-build-v2,
  network unix dgram,
  network inet dgram,
  network inet6 dgram,
  network netlink raw,
