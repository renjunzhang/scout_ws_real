"""Static/mock-only tests for the NO-GO U3 C1 GenCase runtime v2 draft."""
...
GATE_PATH = PACKAGE_DIR / "scripts/r8_liquid_target_u3_gencase_gate_v2.py"
HELPER_PATH = PACKAGE_DIR / "scripts/r8_liquid_u3_gencase_bootstrap_helper_v2.py"
SUPERVISOR_PATH = PACKAGE_DIR / "scripts/r8_liquid_target_u3_gencase_supervisor_v2.py"
...
class TargetU3GenCaseV2Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.gate = load_module("u3_gencase_gate_v2_test", GATE_PATH)
        cls.helper = load_module("u3_gencase_helper_v2_test", HELPER_PATH)
        cls.supervisor = load_module("u3_gencase_supervisor_v2_test", SUPERVISOR_PATH)
    ...
    def success_frame(self, *, bi4: bytes | None = None, xml: bytes | None = None, console: bytes = b"console") -> bytes:
        ...
        metadata = {
            "document_type": "SMPCC_R8_LIQUID_U3_C1_GENCASE_GUEST_FRAME_V1",
    ...
    def test_policy_schema_artifact_hashes_and_no_go_status(self):
        ...
        self.assertTrue(self.policy["status"].startswith("NO_GO_V2_"))
        self.assertEqual(self.policy["allowed_gate_commands"], ["self-check"])
        self.assertEqual(self.policy["required_harmless_probes"]["admission"], "REQUIRED_NOT_YET_EXECUTED")
        self.gate.verify_review_artifacts(run_parser=False)
        self.supervisor.verify_policy_static()

    def test_profile_is_lowercase_fail_closed_runtime_and_unloadable_draft(self):
        text = PROFILE_PATH.read_text(encoding="utf-8")
        effective = self.gate._effective_profile(text)
        self.assertIn("/work/runtime/GenCase_linux64 rpx -> r8-liquid-u3-gencase-runtime-v2,", effective)
        self.assertNotIn(" rPx ->", effective)
        runtime = effective.split("profile r8-liquid-u3-gencase-runtime-v2", 1)[1]
        for token in ("/work/export", "/work/transport", "userns ", "mount ", "capability ", "network ", "/proc", "/dev", "/home/", "/usr/bin/", " rpx"):
            self.assertNotIn(token, runtime)
        self.assertIn("signal (send,receive) set=(term,kill,exists) peer=r8-liquid-u3-gencase-bootstrap-v2,", effective)
        self.assertFalse(self.gate.verify_profile(run_parser=False)["production_load_allowed"])
    ...
    def test_uid1000_o_excl_export_preserves_partial_and_rejects_reuse(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                ...
                mock.patch.object(self.supervisor, "require_stable_zero_labels", return_value=[[], [], []]),
                mock.patch.object(self.supervisor, "loaded_profile_counts", return_value={name: 0 for name in self.supervisor.LABELS}),
            ):
    ...
    def test_run_refuses_before_any_one_shot_path_mutation(self):
        before = {path: path.exists() for path in (self.supervisor.SNAPSHOT_ROOT, self.supervisor.ATTEMPT_ROOT, self.supervisor.EXECUTION_RECEIPT, self.supervisor.LIFECYCLE_RECEIPT)}
        with self.assertRaises(self.supervisor.SupervisorError):
            self.supervisor.refuse_current_v2_run_before_mutation()
    ...
    def test_v1_is_append_only_rejected_and_seed_v2_receipt_is_pinned(self):
        rejected = self.policy["rejected_predecessor"]
        self.assertEqual(rejected["permanent_status"], "NO_GO_NEVER_EXECUTE_OR_REUSE")
