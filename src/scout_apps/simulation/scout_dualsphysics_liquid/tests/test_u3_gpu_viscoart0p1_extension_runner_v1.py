#!/usr/bin/env python3
"""Static and negative tests for the v6 undamped extension runner."""
...
import r8_liquid_u3_gpu_viscoart0p1_extension_runner_v1 as runner  # noqa: E402
...
class ExtensionRunnerTests(unittest.TestCase):
    ...
    def test_exact_argv_has_one_official_viscosity_and_restart(self) -> None:
        ...
        self.assertEqual([item for item in argv if item.startswith("-visco")], ["-viscoart:0.1"])
        self.assertEqual(argv.count("-partbegin:401:401"), 1)
    ...
    def _reject(self, mutate) -> None:
        ...
        mutate(candidate)
        with self.assertRaises(runner.ExtensionRunError):
            runner.semantic_validate(candidate, runner.POLICY_PATH)

    def test_rejects_viscosity_drift(self) -> None:
        self._reject(lambda value: value["solver"]["argv"].__setitem__(12, "-viscoart:0.2"))

    def test_rejects_restart_drift(self) -> None:
        self._reject(lambda value: value["restart"].__setitem__("part_index", 400))

    def test_rejects_numerical_delta_drift(self) -> None:
        self._reject(lambda value: value["single_delta"].__setitem__("parameter", "VISCOSITY"))
