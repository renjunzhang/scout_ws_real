"""Static/mock-only tests for the admitted one-shot U3 C1 GenCase v6 runtime."""
...
GATE_PATH = PACKAGE_DIR / "scripts/r8_liquid_target_u3_gencase_gate_v6.py"
HELPER_PATH = PACKAGE_DIR / "scripts/r8_liquid_u3_gencase_bootstrap_helper_v6.py"
SUPERVISOR_PATH = PACKAGE_DIR / "scripts/r8_liquid_target_u3_gencase_supervisor_v6.py"
POLICY_PATH = PACKAGE_DIR / "config/target_hosts/liquid_zrj_msi_u2404_u3_gencase_execution_policy_v6.json"
SCHEMA_PATH = PACKAGE_DIR / "schema/target_host_u3_gencase_execution_policy_v6.json"
PROFILE_PATH = PACKAGE_DIR / "config/apparmor_drafts/r8-liquid-u3-gencase-v6.profile"
...
def make_xml() -> bytes:
    ...
    <parameter key="MinFluidStop" value="1"/></parameters><motion/></execution></case>"""
...
class TargetU3GenCaseV6Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.gate = load_module("u3_gencase_gate_v6_test", GATE_PATH)
        cls.helper = load_module("u3_gencase_helper_v6_test", HELPER_PATH)
        cls.supervisor = load_module("u3_gencase_supervisor_v6_test", SUPERVISOR_PATH)
    ...
    def success_frame(
        ...
        *,
        ...
        metadata = {
            "document_type": "SMPCC_R8_LIQUID_U3_C1_GENCASE_GUEST_FRAME_V6",
    ...
    def test_policy_schema_artifact_hashes_and_admitted_status(self):
        ...
        self.assertEqual(self.policy["status"], "ADMITTED_SINGLE_GENCASE_RUNTIME_V6_AFTER_FROZEN_V11_ZERO_DENIAL_PROBE")
    ...
    def test_frame_rejects_trailing_overdeclared_bad_bi4_xml_and_out(self):
        ...
        for active_xml in (
            ...
            make_xml().replace(b"<motion/>", b"<motion><objreal/></motion>"),
            make_xml().replace(b"<motion/>", b"<motion/><floating/>"),
        ):
            with self.assertRaises(self.supervisor.SupervisorError):
                self.supervisor.parse_success_frame(self.success_frame(xml=active_xml))
        with self.assertRaises(self.supervisor.SupervisorError):
    ...
    def test_v5_failure_is_frozen_and_seed_v2_receipt_is_pinned(self):
