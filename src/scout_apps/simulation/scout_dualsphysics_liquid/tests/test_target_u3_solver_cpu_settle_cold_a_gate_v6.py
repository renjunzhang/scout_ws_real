"""Static/mock-only tests for the fresh U3 C1M cold-A settle v6."""
...
GATE_PATH = PACKAGE_DIR / "scripts/r8_liquid_target_u3_solver_cpu_settle_cold_a_gate_v6.py"
HELPER_PATH = PACKAGE_DIR / "scripts/r8_liquid_u3_solver_cpu_settle_cold_a_bootstrap_helper_v6.py"
SUPERVISOR_PATH = PACKAGE_DIR / "scripts/r8_liquid_target_u3_solver_cpu_settle_cold_a_supervisor_v6.py"
POLICY_PATH = PACKAGE_DIR / "config/target_hosts/liquid_zrj_msi_u2404_u3_solver_cpu_settle_cold_a_execution_policy_v6.json"
V4_POLICY_PATH = PACKAGE_DIR / "config/target_hosts/liquid_zrj_msi_u2404_u3_solver_cpu_settle_cold_a_execution_policy_v4.json"
V5_POLICY_PATH = PACKAGE_DIR / "config/target_hosts/liquid_zrj_msi_u2404_u3_solver_cpu_settle_cold_a_execution_policy_v5.json"
V5_PROFILE_PATH = PACKAGE_DIR / "config/apparmor_drafts/r8-liquid-u3-solver-cpu-settle-cold-a-v5.profile"
SCHEMA_PATH = PACKAGE_DIR / "schema/target_host_u3_solver_cpu_settle_cold_a_execution_policy_v6.json"
PROFILE_PATH = PACKAGE_DIR / "config/apparmor_drafts/r8-liquid-u3-solver-cpu-settle-cold-a-v6.profile"
...
class TargetU3SolverCpuSettleColdAV6Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.gate = load_module("u3_solver_cpu_settle_cold_a_gate_v6_test", GATE_PATH)
        cls.helper = load_module("u3_solver_cpu_settle_cold_a_helper_v6_test", HELPER_PATH)
        cls.supervisor = load_module("u3_solver_cpu_settle_cold_a_supervisor_v6_test", SUPERVISOR_PATH)
    ...
    def success_frame(self, *, console: bytes = b"") -> bytes:
        ...
        metadata = {
            "document_type": "SMPCC_R8_LIQUID_U3_C1M_CPU_SOLVER_SETTLE_COLD_A_GUEST_FRAME_V6",
            "status": "GUEST_SOLVER_V6_COLD_A_SETTLE_COMPLETE_PENDING_HOST_REAUDIT_AND_O_EXCL_EXPORT",
    ...
    def test_snapshot_bootstrap_and_loader_sources_compile(self):
        compile(
            ...
            "<solver-cpu-settle-cold-a-v6-snapshot-bootstrap-test>",
            "exec",
        )
        compile(
            self.supervisor.SNAPSHOT_BOOTSTRAP_LOADER_BYTES,
            "<solver-cpu-settle-cold-a-v6-snapshot-loader-test>",
            "exec",
        )
    ...
    def test_proc_directory_rule_is_the_only_v5_profile_delta_and_exact(self):
        v5_policy = json.loads(V5_POLICY_PATH.read_text(encoding="utf-8"))
        v5_text = V5_PROFILE_PATH.read_text(encoding="utf-8")
        v6_text = PROFILE_PATH.read_text(encoding="utf-8")

        def normalized_lines(text: str, bootstrap: str, runtime: str) -> list[str]:
            effective = "\n".join(
            ...
            canonical = canonical.replace(bootstrap, "<BOOTSTRAP_PROFILE>")
            canonical = canonical.replace(runtime, "<RUNTIME_PROFILE>")
            return canonical.splitlines()

        v5_lines = normalized_lines(
            v5_text,
            v5_policy["frozen_attempt"]["bootstrap_profile"],
            v5_policy["frozen_attempt"]["runtime_profile"],
        )
        v6_lines = normalized_lines(
            v6_text, self.gate.BOOTSTRAP_PROFILE, self.gate.RUNTIME_PROFILE
        )
        rule = self.gate.PROC_DIRECTORY_READ_RULE
        self.assertEqual(v6_lines.count(rule), 1)
        self.assertEqual([line for line in v6_lines if line != rule], v5_lines)
        self.assertEqual(self.policy["profile_lifecycle"]["proc_directory_read_rule"], rule)
        self.assertEqual(
            self.policy["profile_lifecycle"]["proc_authority_delta_from_v5"],
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            assert_rejected(v6_text.replace(f"  {rule}\n", "", 1))
            assert_rejected(v6_text.replace(f"  {rule}\n", f"  {rule}\n  {rule}\n", 1))
            for variant in (
                "owner /proc/ r,",
                "/proc/* r,",
                ...
                assert_rejected(v6_text.replace(f"  {rule}\n", f"  {variant}\n", 1))
            assert_rejected(
                v6_text.replace("  owner /proc/** r,\n", "  /proc/** r,\n", 1)
            )
            assert_rejected(v6_text.rsplit("}", 1)[0] + f"  {rule}\n}}\n")
            assert_rejected(v6_text.replace(f"  {rule}\n", f"  {rule}\n  /proc/version r,\n", 1))
            for directive in (
                ...
                assert_rejected(directive + "\n" + v6_text)

    def test_fresh_cold_a_v6_protocol_identity_and_visibility_boundary(self):
        self.assertEqual(self.gate.CASE_ID, "u3_c1m_solver_cpu_settle_cold_a_v6_20260808T212044Z")
        self.assertEqual(
            self.gate.CAMPAIGN_ID,
            "u3_c1m_solver_cpu_settle_ab_campaign_20260808T212044Z",
        )
        ...
        self.assertEqual(self.gate.HELPER_MAGIC, b"R8SOLVERHELPV6\0\0")
        self.assertEqual(self.gate.INPUT_MAGIC, b"R8SOLVERINPUT6\0\0")
        self.assertEqual(self.helper.INPUT_MAGIC, self.gate.INPUT_MAGIC)
        self.assertEqual(self.helper.FRAME_MAGIC, b"R8SOLVEROUTV6\0\0\0")
        self.assertEqual(self.helper.FRAME_VERSION, 6)
        ...
        self.assertEqual(
            ...
            "cold_a_v5_normalized",
        )
        self.assertEqual(
            self.policy["immutable_transport"]["abi_delta_from_v5"],
            "version_discriminator_only",
        )
        self.assertIn("_COLD_A_V6_", self.gate.SNAPSHOT_ENV)
        self.assertIn("_COLD_A_V6_", self.supervisor.EXPORT_FD_ENV)
        self.assertEqual(
    ...
    def test_v6_normalized_abi_and_scientific_contract_equal_v5(self):
        v5 = json.loads(V5_POLICY_PATH.read_text(encoding="utf-8"))
        self.assertEqual(self.policy["input_contract"], v5["input_contract"])
        self.assertEqual(self.policy["resources"], v5["resources"])
        self.assertEqual(self.policy["output_contract"], v5["output_contract"])
        self.assertEqual(
            self.policy["fixed_guest_command"]["solver_argv"],
            v5["fixed_guest_command"]["solver_argv"],
        )
        self.assertEqual(
            self.policy["fixed_guest_command"]["environment"],
            v5["fixed_guest_command"]["environment"],
        )
        self.assertEqual(
            self.policy["fixed_guest_command"]["solver_process_group"],
            v5["fixed_guest_command"]["solver_process_group"],
        )
        ...
        def normalized_transport(policy):
            ...
            transport.pop("abi_delta_from_v5", None)
        ...
        self.assertEqual(normalized_transport(self.policy), normalized_transport(v5))
        self.assertEqual(
        ...
        expected_policy_delta = {
            ...
            "provenance.consumed_cold_a_v5_attempt",
            "invariants.v5_identity_consumed_and_never_retried",
            "invariants.v5_snapshot_and_runtime_outputs_are_provenance_only",
            "invariants.v6_profile_delta_is_exact_proc_directory_read_only",
            "invariants.no_broad_non_owner_proc_glob_or_write_authority",
            "immutable_transport.abi_baseline",
            "immutable_transport.abi_delta_from_v4",
            "immutable_transport.abi_delta_from_v5",
            ...
            "profile_lifecycle.proc_directory_read_rule",
            "profile_lifecycle.proc_authority_baseline",
            "profile_lifecycle.proc_authority_delta_from_v5",
        }
    ...
    def test_all_v6_receipts_freeze_campaign_and_cold_a_role(self):
        tree = ast.parse(SUPERVISOR_PATH.read_text(encoding="utf-8"))
        run_once = next(
            node
    ...
    def test_consumed_v5_denial_failure_is_exact_and_never_reused(self):
        predecessor = self.policy["provenance"]["consumed_cold_a_v5_attempt"]
        self.assertEqual(predecessor, self.gate.V5_FAILURE_PROVENANCE)
        self.assertEqual(predecessor["case_id"], self.supervisor.V5_CASE_ID)
        self.assertEqual(predecessor["campaign_id"], self.supervisor.V5_CAMPAIGN_ID)
        self.assertTrue(predecessor["identity_consumed"])
        self.assertTrue(predecessor["retry_forbidden"])
        self.assertFalse(predecessor["output_exported"])
        failure = predecessor["observed_failure"]
        self.assertEqual(failure["gate_returncode"], 2)
        self.assertEqual(
            failure["stderr_sha256"],
            "aee2529d45339ce39f1ac66d9517021dd73f32c93882edf72edbca2f755d264d",
        )
        self.assertEqual(failure["required_profile_delta"], "/proc/ r,")
        denials = failure["unexpected_logged_denials"]
        self.assertEqual(denials["matching_total"], 2)
        self.assertEqual(denials["unexpected_total"], 2)
        self.assertFalse(denials["storage_overflow"])
        self.assertEqual(
            denials["common_fields"],
            {
                "apparmor": "DENIED",
                "operation": "open",
                "class": "file",
        ...
        self.assertEqual(cleanup["prequery_stable_zero_labels"], [[], [], []])
        self.assertEqual(cleanup["postquery_stable_zero_labels"], [[], [], []])
        self.assertEqual(cleanup["cleanup_stable_zero_labels"], [[], [], []])
        self.assertEqual(cleanup["post_unload_stable_zero_labels"], [[], [], []])
        self.assertEqual(set(cleanup["profile_exact_counts"].values()), {0})
        self.assertTrue(cleanup["sysctls_unchanged"])
        self.assertTrue(cleanup["execution_receipt_linked"])
        observed = self.supervisor.verify_v5_failure_provenance()
        self.assertEqual(observed, predecessor)
        input_sources = [
            source for _name, source, _digest, _size in self.supervisor.INPUT_SOURCES
        ]
        snapshot_sources = [
            source for source, _name in self.supervisor.SNAPSHOT_SOURCE_PAIRS
        ]
        for source in input_sources + snapshot_sources:
            self.assertNotIn(self.supervisor.V5_CASE_ID, source)
            self.assertNotIn("cold_a_v5", source)
            self.assertNotIn("/run/r8-liquid-", source)
        invariants = self.policy["invariants"]
        self.assertTrue(invariants["v5_identity_consumed_and_never_retried"])
        self.assertTrue(
            invariants["v5_snapshot_and_runtime_outputs_are_provenance_only"]
        )

    def test_input_and_snapshot_sources_reject_consumed_attempt_reuse_before_io(self):
        source_markers = (
            self.supervisor.V4_CASE_ID,
            self.supervisor.V5_CASE_ID,
            "/run/r8-liquid-foreign-attempt.snapshot",
        )
        error = "consumed v4/v5 identity or transient snapshot appeared"

        for marker in source_markers:
            injected = list(self.supervisor.INPUT_SOURCES)
            name, _source, digest, size = injected[0]
            injected[0] = (name, f"/nonexistent/{marker}/input", digest, size)
            with self.subTest(collection="input", marker=marker), mock.patch.object(
                self.supervisor, "INPUT_SOURCES", tuple(injected)
            ), mock.patch.object(
                self.supervisor,
                "read_json_object",
                side_effect=AssertionError("input reuse guard performed I/O"),
            ):
                with self.assertRaisesRegex(self.supervisor.SupervisorError, error):
                    self.supervisor.verify_provenance_and_inputs()

        for marker in source_markers:
            injected = list(self.supervisor.SNAPSHOT_SOURCE_PAIRS)
            _source, name = injected[0]
            injected[0] = (f"/nonexistent/{marker}/snapshot", name)
            with self.subTest(collection="snapshot-source", marker=marker), mock.patch.object(
                self.supervisor, "SNAPSHOT_SOURCE_PAIRS", tuple(injected)
            ), mock.patch.object(
                self.supervisor,
                "read_json_object",
                side_effect=AssertionError("snapshot reuse guard performed I/O"),
            ):
                with self.assertRaisesRegex(self.supervisor.SupervisorError, error):
                    self.supervisor.verify_policy_static()

        for marker in ("cold_a_v4", "cold_a_v5"):
            injected = list(self.supervisor.SNAPSHOT_SOURCE_PAIRS)
            source, _name = injected[0]
            injected[0] = (source, f"{marker}_snapshot")
            with self.subTest(collection="snapshot-name", marker=marker), mock.patch.object(
                self.supervisor, "SNAPSHOT_SOURCE_PAIRS", tuple(injected)
            ), mock.patch.object(
                self.supervisor,
                "read_json_object",
                side_effect=AssertionError("snapshot name guard performed I/O"),
            ):
                with self.assertRaisesRegex(self.supervisor.SupervisorError, error):
                    self.supervisor.verify_policy_static()

    def test_v5_denial_semantic_tamper_fails_closed(self):
        documents = {
            self.supervisor.V5_START_RECEIPT: json.loads(
                self.supervisor.V5_START_RECEIPT.read_text(encoding="utf-8")
            ),
            self.supervisor.V5_EXECUTION_RECEIPT: json.loads(
                self.supervisor.V5_EXECUTION_RECEIPT.read_text(encoding="utf-8")
            ),
            self.supervisor.V5_INCOMPLETE_RECEIPT: json.loads(
                self.supervisor.V5_INCOMPLETE_RECEIPT.read_text(encoding="utf-8")
            ),
        }
        execution = json.loads(
            json.dumps(documents[self.supervisor.V5_EXECUTION_RECEIPT])
        )
        for collection in ("sanitized_denials", "unexpected_denials"):
            execution["audit"][collection][0]["fields"][4]["value"] = "/proc/sys/"
        documents[self.supervisor.V5_EXECUTION_RECEIPT] = execution
        with mock.patch.object(
            self.supervisor,
            "read_json_object",
            side_effect=lambda path: documents[Path(path)],
        ):
            with self.assertRaises(self.supervisor.SupervisorError):
                self.supervisor.verify_v5_failure_provenance()
    ...
    def test_real_guest_pipe_eof_grace_preserves_output_without_signal(self):
        ...
        self.assertEqual(stdout, b"kept:frame-v6")
    ...
    def test_helper_has_fd_process_group_and_output_tree_barriers(self):
        ...
        self.assertIn('"status": "GUEST_SOLVER_V6_COLD_A_NO_GO"', source)
