#!/usr/bin/env python3
"""Closed-schema and negative tests for the v5 viscosity-strength remediation QC."""
...
import r8_liquid_u3_damped_restart_qc_v5 as qc
...
def delta_checks() -> dict[str, bool]:
    ...
    for phase_name in qc.base.PHASE_KEYS:
        result[f"{phase_name}_argv_only_changes_viscoart_strength"] = True
...
def valid_report(*, selected: bool = True) -> dict[str, object]:
    ...
    verdict_status = "PASS_U3_VISCOART0P1_DAMPED_INITIALIZATION_UNDAMPED_TAIL_SETTLED_CANDIDATE"
    blocker = "NONE"
    ...
    if not selected:
        ...
        verdict_status = "FAIL_U3_VISCOART0P1_DAMPED_INITIALIZATION_UNDAMPED_TAIL_NOT_SETTLED"
    ...
    argv_hashes = {
        name: {
            ...
            "current_normalized_to_prior_canonical_sha256": "1" * 64,
        }
    ...
    return {
        ...
        "remediation_delta": {
            ...
            "baseline": 0.01,
            "candidate": 0.1,
            "baseline_argument": "-viscoart:0.01",
            "candidate_argument": "-viscoart:0.1",
            "applied_to_phases": ["damped_init", "undamped_tail"],
            "other_numerical_parameters_changed": False,
            "checks": delta_checks(),
            "argv_canonical_sha256": argv_hashes,
        },
        "upstream_example_proof": {
            "commit": qc.runner.UPSTREAM_COMMIT,
            "path": qc.UPSTREAM_EXAMPLE,
            "blob": qc.runner.UPSTREAM_EXAMPLE_BLOB,
            "content_sha256": qc.runner.UPSTREAM_EXAMPLE_SHA256,
        ...
        "runtime_viscosity_proof": {
            "prior": {
                phase: viscosity("Artificial", 0.01, f"prior-{phase}")
                for phase in qc.base.PHASE_KEYS
            },
            "candidate": {
                phase: viscosity("Artificial", 0.1, f"candidate-{phase}")
                for phase in qc.base.PHASE_KEYS
            },
...
class DampedRestartViscoartQcV5Tests(unittest.TestCase):
