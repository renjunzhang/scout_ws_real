"""Static/mock-only tests for the fresh U3 C1 CPU-solver smoke v2."""
...
GATE_PATH = PACKAGE_DIR / "scripts/r8_liquid_target_u3_solver_cpu_gate_v2.py"
HELPER_PATH = PACKAGE_DIR / "scripts/r8_liquid_u3_solver_cpu_bootstrap_helper_v2.py"
SUPERVISOR_PATH = PACKAGE_DIR / "scripts/r8_liquid_target_u3_solver_cpu_supervisor_v2.py"
POLICY_PATH = PACKAGE_DIR / "config/target_hosts/liquid_zrj_msi_u2404_u3_solver_cpu_execution_policy_v2.json"
SCHEMA_PATH = PACKAGE_DIR / "schema/target_host_u3_solver_cpu_execution_policy_v2.json"
PROFILE_PATH = PACKAGE_DIR / "config/apparmor_drafts/r8-liquid-u3-solver-cpu-v2.profile"
...
class TargetU3SolverCpuV2Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.gate = load_module("u3_solver_cpu_gate_v2_test", GATE_PATH)
        cls.helper = load_module("u3_solver_cpu_helper_v2_test", HELPER_PATH)
        cls.supervisor = load_module("u3_solver_cpu_supervisor_v2_test", SUPERVISOR_PATH)
    ...
    def success_frame(self, *, console: bytes = b"") -> bytes:
        ...
        metadata = {
            "document_type": "SMPCC_R8_LIQUID_U3_C1_CPU_SOLVER_GUEST_FRAME_V2",
            "status": "GUEST_SOLVER_V2_SMOKE_COMPLETE_PENDING_HOST_REAUDIT_AND_O_EXCL_EXPORT",
    ...
    def test_snapshot_bootstrap_and_loader_sources_compile(self):
        compile(
            ...
            "<solver-cpu-v2-snapshot-bootstrap-test>",
            "exec",
        )
        compile(
            self.supervisor.SNAPSHOT_BOOTSTRAP_LOADER_BYTES,
            "<solver-cpu-v2-snapshot-loader-test>",
            "exec",
        )
    ...
    def test_fresh_v2_protocol_identity_and_visibility_boundary(self):
        self.assertEqual(self.gate.CASE_ID, "u3_c1_solver_cpu_smoke_v2_20260808T134452Z")
        self.assertEqual(self.gate.HELPER_MAGIC, b"R8SOLVERHELPV2\0\0")
        self.assertEqual(self.gate.INPUT_MAGIC, b"R8SOLVERINPUT2\0\0")
        self.assertEqual(self.helper.INPUT_MAGIC, self.gate.INPUT_MAGIC)
        self.assertEqual(self.helper.FRAME_MAGIC, b"R8SOLVEROUTV2\0\0\0")
        self.assertEqual(self.helper.FRAME_VERSION, 2)
        ...
        self.assertIn("_V2_", self.gate.SNAPSHOT_ENV)
        self.assertIn("_V2_", self.supervisor.EXPORT_FD_ENV)
        self.assertEqual(
