"""Static/mock-only tests for the fresh U3 C1M CPU-solver smoke v3."""
...
GATE_PATH = PACKAGE_DIR / "scripts/r8_liquid_target_u3_solver_cpu_gate_v3.py"
HELPER_PATH = PACKAGE_DIR / "scripts/r8_liquid_u3_solver_cpu_bootstrap_helper_v3.py"
SUPERVISOR_PATH = PACKAGE_DIR / "scripts/r8_liquid_target_u3_solver_cpu_supervisor_v3.py"
POLICY_PATH = PACKAGE_DIR / "config/target_hosts/liquid_zrj_msi_u2404_u3_solver_cpu_execution_policy_v3.json"
SCHEMA_PATH = PACKAGE_DIR / "schema/target_host_u3_solver_cpu_execution_policy_v3.json"
PROFILE_PATH = PACKAGE_DIR / "config/apparmor_drafts/r8-liquid-u3-solver-cpu-v3.profile"
...
class TargetU3SolverCpuV3Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.gate = load_module("u3_solver_cpu_gate_v3_test", GATE_PATH)
        cls.helper = load_module("u3_solver_cpu_helper_v3_test", HELPER_PATH)
        cls.supervisor = load_module("u3_solver_cpu_supervisor_v3_test", SUPERVISOR_PATH)
    ...
    def success_frame(self, *, console: bytes = b"") -> bytes:
        ...
        metadata = {
            ...
            "output_audit": {"file_count": 30, "total_bytes": len(payload)},
    ...
    def test_snapshot_bootstrap_and_loader_sources_compile(self):
        compile(
            ...
            "<solver-cpu-v3-snapshot-bootstrap-test>",
            "exec",
        )
        compile(
            self.supervisor.SNAPSHOT_BOOTSTRAP_LOADER_BYTES,
            "<solver-cpu-v3-snapshot-loader-test>",
            "exec",
        )
    ...
    def test_fresh_v3_protocol_identity_and_visibility_boundary(self):
        self.assertEqual(self.gate.CASE_ID, "u3_c1m_solver_cpu_smoke_v3_20260808T160108Z")
        self.assertEqual(self.gate.HELPER_MAGIC, b"R8SOLVERHELPV3\0\0")
        self.assertEqual(self.gate.INPUT_MAGIC, b"R8SOLVERINPUT3\0\0")
        self.assertEqual(self.helper.INPUT_MAGIC, self.gate.INPUT_MAGIC)
        self.assertEqual(self.helper.FRAME_MAGIC, b"R8SOLVEROUTV3\0\0\0")
        self.assertEqual(self.helper.FRAME_VERSION, 3)
        ...
        self.assertIn("_V3_", self.gate.SNAPSHOT_ENV)
        self.assertIn("_V3_", self.supervisor.EXPORT_FD_ENV)
        self.assertEqual(
    ...
    def test_c1m_case_has_only_zero_motion_moving_boundary_semantics(self):
        ...
        self.assertEqual(len(self.supervisor.EXPECTED_PATHS), 30)
    ...
    def test_success_frame_exact_30_files_and_tamper_rejection(self):
    ...
    def test_uid1000_o_excl_export_preserves_zero_byte_console_and_reaudits_it(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                ...
                self.assertEqual(verified["file_count"], 30)
