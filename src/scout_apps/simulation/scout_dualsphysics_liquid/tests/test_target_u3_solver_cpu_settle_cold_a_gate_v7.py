"""Static/mock-only tests for the fresh U3 C1M cold-A settle v7."""
...
GATE_PATH = PACKAGE_DIR / "scripts/r8_liquid_target_u3_solver_cpu_settle_cold_a_gate_v7.py"
HELPER_PATH = PACKAGE_DIR / "scripts/r8_liquid_u3_solver_cpu_settle_cold_a_bootstrap_helper_v7.py"
SUPERVISOR_PATH = PACKAGE_DIR / "scripts/r8_liquid_target_u3_solver_cpu_settle_cold_a_supervisor_v7.py"
POLICY_PATH = PACKAGE_DIR / "config/target_hosts/liquid_zrj_msi_u2404_u3_solver_cpu_settle_cold_a_execution_policy_v7.json"
...
SCHEMA_PATH = PACKAGE_DIR / "schema/target_host_u3_solver_cpu_settle_cold_a_execution_policy_v7.json"
PROFILE_PATH = PACKAGE_DIR / "config/apparmor_drafts/r8-liquid-u3-solver-cpu-settle-cold-a-v7.profile"
...
class TargetU3SolverCpuSettleColdAV7Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.gate = load_module("u3_solver_cpu_settle_cold_a_gate_v7_test", GATE_PATH)
        cls.helper = load_module("u3_solver_cpu_settle_cold_a_helper_v7_test", HELPER_PATH)
        cls.supervisor = load_module("u3_solver_cpu_settle_cold_a_supervisor_v7_test", SUPERVISOR_PATH)
    ...
    def test_snapshot_bootstrap_and_loader_sources_compile(self):
        compile(
            ...
            "<solver-cpu-settle-cold-a-v7-snapshot-bootstrap-test>",
            "exec",
        )
        compile(
            self.supervisor.SNAPSHOT_BOOTSTRAP_LOADER_BYTES,
            "<solver-cpu-settle-cold-a-v7-snapshot-loader-test>",
            "exec",
        )
    ...
    def test_proc_directory_rule_is_the_only_v5_profile_delta_and_exact(self):
        ...
        v7_text = PROFILE_PATH.read_text(encoding="utf-8")
        ...
        v7_lines = normalized_lines(
            v7_text, self.gate.BOOTSTRAP_PROFILE, self.gate.RUNTIME_PROFILE
        )
        rule = self.gate.PROC_DIRECTORY_READ_RULE
        self.assertEqual(v7_lines.count(rule), 1)
        self.assertEqual([line for line in v7_lines if line != rule], v5_lines)
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            assert_rejected(v7_text.replace(f"  {rule}\n", "", 1))
            assert_rejected(v7_text.replace(f"  {rule}\n", f"  {rule}\n  {rule}\n", 1))
            for variant in (
                "owner /proc/ r,",
                "/proc/* r,",
                ...
                assert_rejected(v7_text.replace(f"  {rule}\n", f"  {variant}\n", 1))
            assert_rejected(
                v7_text.replace("  owner /proc/** r,\n", "  /proc/** r,\n", 1)
            )
            assert_rejected(v7_text.rsplit("}", 1)[0] + f"  {rule}\n}}\n")
            assert_rejected(v7_text.replace(f"  {rule}\n", f"  {rule}\n  /proc/version r,\n", 1))
            for directive in (
    ...
    def test_fresh_cold_a_v7_protocol_identity_and_visibility_boundary(self):
        self.assertEqual(self.gate.CASE_ID, "u3_c1m_solver_cpu_settle_cold_a_v7_20260809T051659Z")
        self.assertEqual(
            self.gate.CAMPAIGN_ID,
            "u3_c1m_solver_cpu_settle_ab_campaign_20260809T051659Z",
        )
        ...
        self.assertEqual(self.gate.HELPER_MAGIC, b"R8SOLVERHELPV7\0\0")
        self.assertEqual(self.gate.INPUT_MAGIC, b"R8SOLVERINPUT7\0\0")
        self.assertEqual(self.helper.INPUT_MAGIC, self.gate.INPUT_MAGIC)
        self.assertEqual(self.helper.FRAME_MAGIC, b"R8SOLVEROUTV7\0\0\0")
        self.assertEqual(self.helper.FRAME_VERSION, 7)
        ...
        self.assertIn("_COLD_A_V7_", self.gate.SNAPSHOT_ENV)
        self.assertIn("_COLD_A_V7_", self.supervisor.EXPORT_FD_ENV)
        self.assertEqual(
    ...
    def test_v7_normalized_abi_and_scientific_contract_equal_v5(self):
        ...
        expected_policy_delta = {
            ...
            "invariants.v7_profile_delta_is_exact_proc_directory_read_only",
    ...
    def test_all_v7_receipts_freeze_campaign_and_cold_a_role(self):
        tree = ast.parse(SUPERVISOR_PATH.read_text(encoding="utf-8"))
        run_once = next(
            node
    ...
    def test_real_guest_pipe_eof_grace_preserves_output_without_signal(self):
        ...
        self.assertEqual(stdout, b"kept:frame-v7")
    ...
    def test_helper_has_fd_process_group_and_output_tree_barriers(self):
        ...
        self.assertIn('"status": "GUEST_SOLVER_V7_COLD_A_NO_GO"', source)
