#!/usr/bin/env python3
"""Static and negative tests for the 36.05--40.05 s extension QC."""
...
import r8_liquid_u3_viscoart0p2_settle_extension_qc_v2 as qc  # noqa: E402
...
class SettleExtensionQcTests(unittest.TestCase):
    def test_schema_is_closed_and_valid(self) -> None:
        ...
        self.assertEqual(schema["$defs"]["run"]["properties"]["frame_count"]["const"], 81)
    ...
    def test_zero_minimum_has_explicit_null_ratio(self) -> None:
        parts = [
            {"cpart": 721, "time_s": 36.05, "speed": {"rms_m_s": 0.0}},
            {"cpart": 801, "time_s": 40.05, "speed": {"rms_m_s": 0.0001}},
        ]
    ...
    def test_contract_is_only_end_time_delta(self) -> None:
        ...
        self.assertEqual(qc.PART_INDICES, tuple(range(721, 802)))
