#!/usr/bin/env python3
"""Offline-only tests for U3 solver output QC v3."""
...
import r8_liquid_u3_solver_output_qc_v3 as qc  # noqa: E402
...
def _array(
    name: str, type_code: int, count: int, payload: bytes, *, hidden: bool = False
) -> bytes:
    definition = (
        _string("\nARRAY")
        + _string(name)
        + struct.pack("<iiII", int(hidden), type_code, count, len(payload))
    )
...
def _item(
    ...
    definition = (
        _string("\nITEM\n")
        + _string(name)
        + struct.pack("<ii", int(hidden), int(values_hidden))
        + _string("%.7E")
        + _string("%.15E")
...
def particle_fixture() -> tuple[list[int], list[tuple[float, float, float]], list[tuple[float, float, float]], list[float]]:
    ...
    densities = [1000.0, 1000.0, 1000.0] + [1001.0] * (len(ids) - 3)
...
def make_partout(*, nout: int = 0) -> bytes:
    root = _item(
        "JPartOutBi4",
        [
            _value("CaseNp", 10, struct.pack("<Q", 131)),
            _value("CaseNfixed", 10, struct.pack("<Q", 0)),
            _value("CaseNmoving", 10, struct.pack("<Q", 3)),
    ...
    if nout:
        appended = _item(
            ...
            arrays=[
                _array("Idp", 8, nout, b"".join(struct.pack("<I", 130) for _ in range(nout))),
...
def make_motion_ref(
    times: list[float],
    *,
    mutate_part: int | None = None,
    reference_ids: tuple[int, int, int] = (0, 1, 2),
    root_positions: list[tuple[float, float, float]] | None = None,
    root_distances: tuple[float, float, float] | None = None,
    root_arrays_hidden: bool = False,
    appended_hidden: bool = True,
) -> bytes:
    case_positions = [
        (-0.018, 0.0, 0.0),
        (0.018, 0.0, 0.0),
    ...
    root_positions = list(case_positions if root_positions is None else root_positions)
    if root_distances is None:
        root_distances = tuple(
            1.1
            * math.sqrt(
                sum((position[axis] - root_positions[0][axis]) ** 2 for axis in range(3))
            )
    ...
    root = _item(
        "JPartMotRefBi4",
        [
            _value("AppName", 1, _string("DualSPHysics test")),
            _value("FormatVer", 8, struct.pack("<I", 230729)),
            _value("MainFile", 2, struct.pack("<i", 1)),
            _value("TimeOut", 12, struct.pack("<d", 0.05)),
            _value("MkBoundFirst", 6, struct.pack("<H", 2)),
            _value("MkMovingCount", 8, struct.pack("<I", 1)),
            _value("MkFloatCount", 8, struct.pack("<I", 0)),
            _value("MkCount", 8, struct.pack("<I", 1)),
        ],
        arrays=[
            _array(
                "MkBound", 6, 1, struct.pack("<H", 0), hidden=root_arrays_hidden
            ),
            _array("Nid", 8, 1, struct.pack("<I", 3), hidden=root_arrays_hidden),
            _array(
                "Id",
                8,
                3,
                struct.pack("<III", *reference_ids),
                hidden=root_arrays_hidden,
            ),
            _array(
                "Ps",
                23,
                3,
                b"".join(struct.pack("<ddd", *value) for value in root_positions),
                hidden=root_arrays_hidden,
            ),
            _array(
                "Dis",
                12,
                3,
                struct.pack("<ddd", *root_distances),
                hidden=root_arrays_hidden,
            ),
        ],
    )
    ...
    for index, time_s in enumerate(times):
        current = [case_positions[value] for value in reference_ids]
        if mutate_part == index:
            x, y, z = current[0]
            current[0] = (x + 1e-4, y, z)
        appended.append(
            _item(
                f"PART_{index:04d}",
                [
                    _value("Cpart", 8, struct.pack("<I", index)),
                    _value("TimeStep", 12, struct.pack("<d", time_s)),
                    _value("Step", 8, struct.pack("<I", index * 100)),
                ],
                arrays=[
                    _array(
                        "PosRef",
                        23,
                        3,
                        b"".join(struct.pack("<ddd", *value) for value in current),
                    )
                ],
                hidden=appended_hidden,
            )
        )
    return _header("JPartMotRefBi4") + root + b"".join(appended)
...
<case>
  ...
  <execution>
    <parameters>
      ...
      <parameter key="Shifting" value="1"/><parameter key="DtAllParticles" value="1"/>
      <simulationdomain><posmin x="-0.021" y="-0.021" z="-0.002"/><posmax x="0.021" y="0.021" z="0.070"/></simulationdomain>
    </parameters>
    <particles np="131" mkboundfirst="2"><moving mkbound="0" refmotion="0" begin="0" count="3"/><fluid begin="3" count="128"/></particles>
...
def make_run_csv(final_time: float, part_count: int, parts_out: int = 0) -> bytes:
    values = [
        "run", "code", "date", "131", "1", "1", "1", "1", "0", "0", "300", "", f"{final_time:g}",
        str(part_count), str(parts_out), "131", "20", "CPU", "CellsFull", "cfg", "3", "0",
...
def write_run(
    ...
    *,
    ...
    mutate_motion_part: int | None = None,
    motion_kwargs: dict[str, object] | None = None,
) -> None:
    root.mkdir()
    data = root / "data"
    data.mkdir()
    ...
    for index, time_s in enumerate(times):
        ...
        if index == 0 and mutate_initial_position is not None:
            ...
            current_positions[3] = (x + mutate_initial_position, y, z)
        if index == len(times) - 1 and mutate_final_velocity is not None:
            current_velocities[3] = (mutate_final_velocity, 0.0, 0.0)
    ...
    (data / "PartMotionRef.ibi4").write_bytes(
        make_motion_ref(
            times,
            mutate_part=mutate_motion_part,
            **(motion_kwargs or {}),
        )
    )
...
class U3SolverOutputQcV3Tests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_real_float32_quantized_mass_passes_smoke_and_never_settles(self) -> None:
        ...
        self.assertEqual(report["verdict"]["status"], "C1M_ZERO_MOTION_SMOKE_PASS")
        ...
        self.assertEqual(
            report["moving_case"]["requested_case_massfluid_kg"], 8e-6
        )
        self.assertEqual(
            report["moving_case"]["solver_effective_massfluid_kg"], effective
        )
        self.assertEqual(
    ...
    def test_later_part_root_mutation_fails_all_part_invariant_check(self) -> None:
        ...
        self.assertTrue(checks["first_snapshot_matches_moving_case"])
        self.assertFalse(checks["all_part_root_invariants_match_moving_case"])
    ...
    def test_initial_particle_array_mutation_remains_a_hard_failure(self) -> None:
        ...
        self.assertFalse(part["matches_moving_case_initial_arrays"])
        self.assertFalse(
            report["run"]["checks"]["structural"][
                "first_snapshot_matches_moving_case"
            ]
        )
        self.assertEqual(report["verdict"]["status"], "FAIL_U3_SOLVER_OUTPUT_QC")
    ...
    def test_cli_requires_both_moving_case_hashes(self) -> None:
    ...
    def test_two_long_identical_outputs_pass_exact_determinism(self) -> None:
        ...
        self.assertTrue(report["comparison"]["motion_reference_sha256_exact"])
    ...
    def test_motion_reference_change_is_rejected(self) -> None:
        run = self.base / "run-motion-change"
        write_run(run, [0.0, 1.0], mutate_motion_part=1)
        with self.assertRaisesRegex(qc.QcError, "PosRef differs"):
            self.report(run)

    def test_motion_reference_scalar_types_are_exact(self) -> None:
        for index, (name, expected_type, wrong_type) in enumerate(
            (("FormatVer", 8, 7), ("MainFile", 2, 7), ("Cpart", 8, 7))
        ):
            with self.subTest(name=name):
                run = self.base / f"run-motion-type-{index}"
                write_run(run, [0.0, 1.0])
                path = run / "data" / "PartMotionRef.ibi4"
                raw = path.read_bytes()
                marker = _string(name) + struct.pack("<i", expected_type)
                self.assertIn(marker, raw)
                path.write_bytes(
                    raw.replace(
                        marker,
                        _string(name) + struct.pack("<i", wrong_type),
                        1,
                    )
                )
                with self.assertRaisesRegex(qc.QcError, "expected"):
                    self.report(run)

        run = self.base / "run-motion-bool-encoding"
        write_run(run, [0.0, 1.0])
        path = run / "data" / "PartMotionRef.ibi4"
        raw = path.read_bytes()
        marker = _string("MainFile") + struct.pack("<ii", 2, 1)
        self.assertIn(marker, raw)
        path.write_bytes(
            raw.replace(
                marker,
                _string("MainFile") + struct.pack("<ii", 2, 2),
                1,
            )
        )
        with self.assertRaisesRegex(qc.QcError, "canonically encoded"):
            self.report(run)

    def test_motion_reference_item_hidden_flag_is_exact(self) -> None:
        run = self.base / "run-motion-hidden"
        write_run(
            run,
            [0.0, 1.0],
            motion_kwargs={"appended_hidden": False},
        )
        with self.assertRaisesRegex(qc.QcError, "structure differs"):
            self.report(run)

        noncanonical = self.base / "run-motion-hidden-noncanonical"
        write_run(
            noncanonical,
            [0.0, 1.0],
            motion_kwargs={"appended_hidden": 2},
        )
        with self.assertRaisesRegex(qc.QcError, "canonically encoded"):
            self.report(noncanonical)

        root_hidden = self.base / "run-motion-root-array-hidden"
        write_run(
            root_hidden,
            [0.0, 1.0],
            motion_kwargs={"root_arrays_hidden": True},
        )
        with self.assertRaisesRegex(qc.QcError, "root array .* differs"):
            self.report(root_hidden)

    def test_motion_reference_root_links_are_exact(self) -> None:
        fixtures = (
            (
                {"reference_ids": (0, 1, 0)},
                "not three distinct moving IDs",
            ),
            (
                {
                    "root_positions": [
            ...
            (
                {"root_distances": (0.0, 99.0, 99.0)},
                "root Dis differs",
            ),
        )
        for index, (kwargs, pattern) in enumerate(fixtures):
            with self.subTest(pattern=pattern):
                run = self.base / f"run-motion-link-{index}"
                write_run(run, [0.0, 1.0], motion_kwargs=kwargs)
                with self.assertRaisesRegex(qc.QcError, pattern):
                    self.report(run)
