"""Static/mock-only tests for the admitted one-shot U3 C1 GenCase v5 runtime."""
...
GATE_PATH = PACKAGE_DIR / "scripts/r8_liquid_target_u3_gencase_gate_v5.py"
HELPER_PATH = PACKAGE_DIR / "scripts/r8_liquid_u3_gencase_bootstrap_helper_v5.py"
SUPERVISOR_PATH = PACKAGE_DIR / "scripts/r8_liquid_target_u3_gencase_supervisor_v5.py"
POLICY_PATH = PACKAGE_DIR / "config/target_hosts/liquid_zrj_msi_u2404_u3_gencase_execution_policy_v5.json"
SCHEMA_PATH = PACKAGE_DIR / "schema/target_host_u3_gencase_execution_policy_v5.json"
PROFILE_PATH = PACKAGE_DIR / "config/apparmor_drafts/r8-liquid-u3-gencase-v5.profile"
...
class TargetU3GenCaseV5Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.gate = load_module("u3_gencase_gate_v5_test", GATE_PATH)
        cls.helper = load_module("u3_gencase_helper_v5_test", HELPER_PATH)
        cls.supervisor = load_module("u3_gencase_supervisor_v5_test", SUPERVISOR_PATH)
    ...
    def success_frame(
        ...
        *,
        ...
        outputs = {
            ...
            "C1_static.out": {"sha256": hashlib.sha256(out).hexdigest(), "size_bytes": len(out)},
        }
        metadata = {
    ...
    def test_policy_schema_artifact_hashes_and_admitted_status(self):
        ...
        self.assertEqual(self.policy["status"], "ADMITTED_SINGLE_GENCASE_RUNTIME_V5_AFTER_FROZEN_V11_ZERO_DENIAL_PROBE")
    ...
    def test_success_frame_exact_parse_and_minimum_bi4_xml_out_reaudit(self):
        ...
        self.assertEqual(parsed["payloads"]["C1_static.out"], make_out())
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                ...
                self.assertEqual(
                    ...
                    ["C1_static.bi4", "C1_static.out", "C1_static.xml"],
                )
    ...
    def test_v4_failure_is_frozen_and_seed_v2_receipt_is_pinned(self):
        ...
        self.assertEqual(rejected["identity"], "u3_c1_gencase_v4_20260808T065518Z")
        self.assertEqual(rejected["execution_receipt_sha256"], "356996f3d8c69839be1ba88f818268071f2c6705791b382f550c8261091ba584")
        self.assertEqual(rejected["lifecycle_receipt_sha256"], "5bf3eb525913cf3cfa9fb393e8e0cecc1f99fe60602279f1907320aaa35adb44")
