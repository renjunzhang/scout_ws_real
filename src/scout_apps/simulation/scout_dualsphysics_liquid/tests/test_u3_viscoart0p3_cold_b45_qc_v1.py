#!/usr/bin/env python3
"""Static and negative tests for the fresh viscosity-0.3 45.05 s cold-B QC."""
...
import r8_liquid_u3_viscoart0p3_cold_b45_qc_v1 as qc  # noqa: E402
...
class ColdAbQcV1Tests(unittest.TestCase):
    def test_schema_is_closed_and_valid(self) -> None:
        ...
        self.assertEqual(
            ...
            "r8-liquid-u3-viscoart0p3-cold-b45-qc-v1",
        )
        self.assertEqual(schema["$defs"]["run"]["properties"]["frame_count"]["const"], 902)
        self.assertEqual(schema["$defs"]["contract"]["properties"]["run_role"]["const"], "b")

    def test_tail_coverage_only_allows_saved_timestamp_roundoff(self) -> None:
        schema = json.loads(qc.SCHEMA_PATH.read_text(encoding="utf-8"))
    ...
    def test_continuation_mode_is_only_frozen_comparison_axis(self) -> None:
        self.assertEqual(qc.runner.EXPECTED_DELTA["parameter"], "CONTINUATION_MODE")
        self.assertEqual(qc.runner.EXPECTED_DELTA["candidate"], "FRESH_CONTINUOUS_CASE")
    ...
    def test_real_inventory_is_admitted_with_exact_frozen_ceiling(self) -> None:
        ...
        self.assertEqual(list(validator.iter_errors(363019515)), [])
    ...
    def test_static_self_check_has_no_execution_or_threshold_delta(self) -> None:
        ...
        self.assertEqual(result["inventory_maximum_bytes"], 536870912)
