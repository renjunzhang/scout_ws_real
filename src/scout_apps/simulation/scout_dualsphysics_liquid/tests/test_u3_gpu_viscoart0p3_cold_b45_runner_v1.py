#!/usr/bin/env python3
"""Static and negative tests for the fresh viscosity-0.3 45.05 s cold-B runner."""
...
import r8_liquid_u3_gpu_viscoart0p3_cold_b45_runner_v1 as runner  # noqa: E402
...
class ColdAbRunnerTests(unittest.TestCase):
    ...
    def test_exact_argv_has_one_official_viscosity_and_no_restart(self) -> None:
        ...
        self.assertEqual([item for item in argv if item.startswith("-visco")], ["-viscoart:0.3"])
    ...
    def test_exact_45s_bounds_and_only_continuation_mode_delta(self) -> None:
        self.assertEqual(runner.EXPECTED_OUTPUT["part_first"], 0)
        self.assertEqual(runner.EXPECTED_OUTPUT["part_last"], 901)
        self.assertEqual(runner.EXPECTED_OUTPUT["part_count"], 902)
        self.assertEqual(runner.EXPECTED_LIMITS["wall_timeout_seconds"], 4200)
        self.assertEqual(runner.EXPECTED_DELTA["parameter"], "CONTINUATION_MODE")
    ...
    def _reject(self, mutate) -> None:
        ...
        with self.assertRaises(runner.ColdAbRunError):
            runner.semantic_validate(candidate, runner.POLICY_PATHS["b"])

    def test_rejects_viscosity_drift(self) -> None:
        self._reject(lambda value: value["solver"]["argv"].__setitem__(10, "-viscoart:0.4"))
