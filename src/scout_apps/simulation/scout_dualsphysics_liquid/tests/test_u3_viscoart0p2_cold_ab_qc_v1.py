#!/usr/bin/env python3
"""Static and negative tests for fresh viscosity-0.2 cold-A/B QC."""
...
import r8_liquid_u3_viscoart0p2_cold_ab_qc_v1 as qc  # noqa: E402


class ColdAbQcTests(unittest.TestCase):
    ...
    def test_single_delta_and_threshold_vector_are_exact(self) -> None:
        ...
        self.assertEqual(delta["parameter"]["const"], "RUN_INITIALIZATION")
        self.assertEqual(delta["baseline"]["const"], "V7_RESTARTED_SENSITIVITY_PROBE")
        self.assertEqual(delta["candidate"]["const"], "FRESH_CASE_INITIAL_STATE")
    ...
    def test_missing_or_nonfinite_metric_rejected(self) -> None:
        ...
        with self.assertRaises(qc.ColdAbQcError):
