...
bool SpmpcLocalPlannerROS::initialize(ros::NodeHandle& nh, ros::NodeHandle& pnh) {
    ...
    if (imu_subscriber_queue_size_ < 1 || imu_subscriber_queue_size_ > 1000) {
        ROS_WARN("[spmpc_sim_local_planner] invalid imu_shadow/subscriber_queue_size=%d; using 10",
                 imu_subscriber_queue_size_);
    ...
    if (!std::isfinite(imu_observer_dt_sec_) || imu_observer_dt_sec_ <= 0.0) {
        ROS_WARN("[spmpc_sim_local_planner] invalid imu_shadow/observer_dt_sec=%.6f; using 0.02 s",
    ...
    if (!parseSloshObserverSource(
        ...
        ROS_FATAL("[spmpc_sim_local_planner] invalid slosh_observer/source='%s'; "
    ...
    if (!parseSloshObserverFallbackPolicy(
        ...
        ROS_FATAL("[spmpc_sim_local_planner] invalid slosh_observer/fallback_policy='%s'; "
    ...
    if (imu_is_nominal && !imu_shadow_enable_) {
        ROS_WARN("[spmpc_sim_local_planner] processed_imu is the nominal liquid observer; "
    ...
    if (!isKnownDelayPhaseMode(delay_phase_mode)) {
        ROS_WARN("[spmpc_sim_local_planner] 未知 delay_phase/mode=\"%s\"，已静默退化为 Off。"
    ...
    if (!isKnownSolverBackend(solver_params.solver_backend)) {
        ROS_FATAL("[spmpc_sim_local_planner] unknown solver_backend '%s'. Valid backends: %s, %s, %s",
    ...
    if (variant_name != "B0" && variant_.name == "B0") {
        ROS_WARN("[spmpc_sim_local_planner] unknown planner_variant '%s'; falling back to B0", variant_name.c_str());
    }
    loadVariantOverrides(variant_.name);
    if (!pnh_.hasParam("variants/" + variant_.name + "/w_contour")) {
        ROS_WARN("[spmpc_sim_local_planner] 未找到 variants/%s/* 参数：config/planner/variants.yaml "
    ...
    if (!validateBackendPolicy(solver_params, variant_, policy_error)) {
        ROS_FATAL("[spmpc_sim_local_planner] backend policy rejected backend=%s role=%s variant=%s: %s",
                  solver_params.solver_backend.c_str(),
    ...
    ROS_INFO("[spmpc_sim_local_planner] backend=%s role=%s variant=%s mode=%s features: slosh=%s slosh_constraint=%s obstacle=%s homotopy=%s corridor=%s corridor_hard_bound=%s",
             solver_params.solver_backend.c_str(),
             solverBackendRole(solver_params.solver_backend),
             variant_.name.c_str(),
    ...
    if (!slosh_observers_.configure(solver_params.slosh, imu_observer_dt_sec_)) {
        ROS_WARN("[spmpc_sim_local_planner] slosh observer configure failed; slosh diagnostics stay zero");
    }
    if (!slosh_observer_selector_.configure(slosh_observer_selector_params_)) {
        ROS_FATAL("[spmpc_sim_local_planner] invalid slosh_observer selector parameters");
        return false;
    }
    if (imu_shadow_enable_ && !slosh_observers_.imuConfigured()) {
        if (imu_is_nominal) {
            ROS_FATAL("[spmpc_sim_local_planner] nominal processed-IMU observer configure failed");
            return false;
        }
        ROS_ERROR("[spmpc_sim_local_planner] IMU shadow observer configure failed; disabling shadow");
        imu_shadow_enable_ = false;
    }
    if (imu_shadow_enable_ &&
        !imu_shadow_adapter_.configure(processed_imu_params, imu_expected_frame_)) {
        if (imu_is_nominal) {
            ROS_FATAL("[spmpc_sim_local_planner] nominal processed-IMU pipeline configure failed");
            return false;
        }
        ROS_ERROR("[spmpc_sim_local_planner] processed-IMU pipeline configure failed; disabling shadow");
        imu_shadow_enable_ = false;
    }
    if (!execution_predictor_.configure(solver_params.slosh)) {
        ROS_WARN("[spmpc_sim_local_planner] delay_phase shadow slosh predictor configure failed; shadow slosh stays pass-through");
    }
    if (!slosh_risk_governor_.configure(solver_params.slosh, slosh_risk_governor_params_) &&
        slosh_risk_governor_params_.enable) {
        ROS_WARN("[spmpc_sim_local_planner] slosh risk governor configure failed; governor will pass through v_ref");
    }
    ...
    // Diagnostics deliberately use a simulation-only root.  The physical
    // controller owns /spmpc/*; publishing there would make a mixed launch
    // look valid even though the binaries are distinct.
    ros::NodeHandle sim_spmpc_nh(nh_, "sim_spmpc");
    diagnostics_.initialize(sim_spmpc_nh);
    ...
    ROS_INFO("[spmpc_sim_local_planner] initialized variant=%s mode=%s path_topic=%s costmap_topic=%s cmd_topic=%s imu_pipeline=%s imu_topic=%s imu_queue=%d observer_source=%s observer_fallback=%s latch_fallback=%s",
             variant_.name.c_str(),
             experiment_mode_.c_str(),
             path_topic_.c_str(),
...
bool SpmpcLocalPlannerROS::loadMapVRefProfile(const std::string& path) {
    ...
    if (!in.is_open()) {
        ROS_WARN("[spmpc_sim_local_planner] map_vref profile open failed: %s", path.c_str());
        return false;
    }
    ...
    while (std::getline(in, line)) {
        ...
        if (!header_parsed) {
            ...
            if (cells.size() >= 2 && parseDoubleStrict(cells[0], first) && parseDoubleStrict(cells[1], second)) {
                ...
                if (s_col < 0 || v_col < 0) {
                    ROS_WARN("[spmpc_sim_local_planner] map_vref profile header must include s_m and v_ref_map_mps: %s", path.c_str());
                    return false;
                }
                continue;
            }
        ...
        if (have_header && (static_cast<int>(cells.size()) <= std::max(s_col, v_col))) {
            ROS_WARN("[spmpc_sim_local_planner] map_vref profile skip short row %d in %s", line_no, path.c_str());
            continue;
        }
        ...
        if (static_cast<int>(cells.size()) <= std::max(s_index, v_index) ||
            ...
            ROS_WARN("[spmpc_sim_local_planner] map_vref profile skip invalid row %d in %s", line_no, path.c_str());
            continue;
        }
        if (s_m < 0.0 || v_ref_mps < 0.0) {
            ROS_WARN("[spmpc_sim_local_planner] map_vref profile skip negative row %d in %s", line_no, path.c_str());
            continue;
        }
    ...
    if (samples.empty()) {
        ROS_WARN("[spmpc_sim_local_planner] map_vref profile has no valid samples: %s", path.c_str());
        return false;
    }
    ...
    ROS_INFO("[spmpc_sim_local_planner] loaded map_vref profile %s with %zu samples", path.c_str(), map_vref_profile_.size());
    return true;
}
...
void SpmpcLocalPlannerROS::imuCallback(const sensor_msgs::ImuConstPtr& msg) {
    ...
    if (output.excitation.valid && !observer_step_ok) {
        ROS_WARN_THROTTLE(1.0,
                          "[spmpc_sim_local_planner] processed-IMU valid but observer step failed");
    }

    if (output.status == ImuPipelineStatusCode::FrameMismatch) {
        ROS_WARN_THROTTLE(2.0,
                          "[spmpc_sim_local_planner] IMU frame mismatch: expected '%s', got '%s'",
        ...
        ROS_WARN_THROTTLE(2.0,
                          "[spmpc_sim_local_planner] processed-IMU calibration unavailable: %s",
...
void SpmpcLocalPlannerROS::pathCallback(const nav_msgs::PathConstPtr& msg) {
    if (!reference_target_frame_.empty() && msg->header.frame_id != reference_target_frame_) {
        ...
        try {
            ...
            ROS_WARN_THROTTLE(1.0,
                              "[spmpc_sim_local_planner] transform reference path %s -> %s failed: %s",
                              msg->header.frame_id.c_str(),
...
void SpmpcLocalPlannerROS::controlTimerCallback(const ros::TimerEvent& event) {
    ...
    if (solver_consumes_selected_state && !observer_selection.valid) {
        ...
        ROS_WARN_THROTTLE(
            1.0,
            "[spmpc_sim_local_planner] fail-closed liquid observer: nominal=%s status=%s reason=%s odom_age=%.3f imu_age=%.3f",
    ...
    if (observer_selection.fallback_active) {
        ROS_WARN_THROTTLE(
            1.0,
            "[spmpc_sim_local_planner] liquid observer fallback active: nominal=%s effective=%s reason=%s epoch=%llu",
            sloshObserverSourceName(observer_selection.nominal_source),
    ...
    // 并按 solver v_max 做同口径 clamp。必须在发布前更新，避免 /sim_spmpc/debug/effective_config 滞后一拍。
...
bool SpmpcLocalPlannerROS::robotStateFromLatest(RobotState& state) {
    ...
    try {
        ...
        if (reference_frame != last_odom_.header.frame_id) {
            ROS_WARN_THROTTLE(1.0,
                              "[spmpc_sim_local_planner] TF pose unavailable %s <- %s: %s; odom frame is %s, refuse mixed-frame fallback",
        ...
        ROS_WARN_THROTTLE(1.0,
                          "[spmpc_sim_local_planner] TF pose unavailable %s <- %s: %s; odom frame matches reference, using odom fallback",
                          reference_frame.c_str(),
                          robot_base_frame_.c_str(),
                          ex.what());
        return true;
...
bool SpmpcLocalPlannerROS::processOdomInput(
    ...
    if (receive_stamp.isZero() || odom.header.stamp.isZero() || !finite_control_state) {
        ...
        ROS_WARN_THROTTLE(
            1.0,
            "[spmpc_sim_local_planner] rejecting odom control input with invalid stamp, pose, or twist");
        if (imu_shadow_publish_diagnostics_) {
    ...
    if (!std::isfinite(dt_msg) || dt_msg <= kMinimumOdomObserverDtSec ||
        ...
        ROS_WARN_THROTTLE(
            1.0,
            "[spmpc_sim_local_planner] rejecting odom slosh update: %s (dt=%.6f s)",
            status,
            dt_msg);
        if (imu_shadow_publish_diagnostics_) {
    ...
    if (!std::isfinite(ax) || !std::isfinite(ay) || !std::isfinite(alpha)) {
        ...
        ROS_WARN_THROTTLE(
            1.0,
            "[spmpc_sim_local_planner] rejecting odom control input with non-finite derived excitation");
        if (imu_shadow_publish_diagnostics_) {
    ...
    if (!observer_updated && odom_observer_configured) {
        ROS_WARN_THROTTLE(1.0, "[spmpc_sim_local_planner] odom slosh observer step rejected");
    } else if (!odom_observer_configured) {
        ROS_WARN_THROTTLE(1.0, "[spmpc_sim_local_planner] slosh observer reconfigure failed");
    }
    if (imu_shadow_publish_diagnostics_) {
        publishOdomSloshObserverDebug(
            odom,
...
SloshModelParams SpmpcLocalPlannerROS::loadSloshParams() const {
    ...
    // 若 container/slosh_height_max 存在且合法，用它覆盖并打 WARN 提示迁移。
    {
        ...
        if (std::isfinite(container_height_max) && container_height_max > 0.0) {
            ROS_WARN_ONCE(
                "[spmpc_sim_local_planner] 检测到 container/slosh_height_max=%.4f m，"
    ...
    if (!std::isfinite(params.slosh_height_max) || params.slosh_height_max <= 0.0) {
        ROS_WARN("[spmpc_sim_local_planner] invalid slosh/slosh_height_max=%.6f, fallback to slosh_height_ref=%.6f",
