#!/usr/bin/env python3
"""Frame-by-frame offline debugger for RealSense liquid height vs MPC slosh height."""

import argparse
import bisect
import csv
import json
import math
import sys
from pathlib import Path
from typing import Dict, List, Optional, Tuple

import cv2
import numpy as np
import rosbag
from cv_bridge import CvBridge, CvBridgeError

from extract_liquid_height_from_bag import (
    default_output_dir,
    detect_liquid_profile,
    load_calibration,
    load_processing_config,
    load_yaml,
    make_debug_frame,
    maybe_mm,
    rectify_roi_and_calibration,
    script_default_config,
)


WINDOW_NAME = "Liquid Debug Review"


def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Build a cached frame-by-frame debug session that overlays RealSense liquid "
            "measurements with MPC /slosh/height and lets you inspect frames interactively."
        )
    )
    parser.add_argument("--bag", required=True, help="Path to the rosbag.")
    parser.add_argument("--calibration", required=True, help="Calibration YAML used for extraction.")
    parser.add_argument(
        "--config",
        default="",
        help="Optional processing config YAML. Defaults to the package liquid_measurement.yaml.",
    )
    parser.add_argument(
        "--image-topic",
        default="",
        help="RGB image topic. Defaults to config topic_image or /camera/color/image_raw.",
    )
    parser.add_argument(
        "--out-dir",
        default="",
        help="Cache output directory. Defaults to <bag_stem>_frame_debug next to the bag.",
    )
    parser.add_argument("--encoding", default="bgr8", help="cv_bridge desired encoding.")
    parser.add_argument("--time-offset", type=float, default=0.0, help="Skip frames before bag_start + offset.")
    parser.add_argument("--every", type=int, default=1, help="Process every N-th frame.")
    parser.add_argument("--max-frames", type=int, default=0, help="Maximum number of frames. 0 means all.")
    parser.add_argument(
        "--reuse-cache",
        action="store_true",
        help="Reuse an existing session manifest in out-dir instead of rebuilding it.",
    )
    parser.add_argument(
        "--skip-viewer",
        action="store_true",
        help="Only build cache/CSV and do not open the interactive viewer.",
    )
    parser.add_argument("--start-index", type=int, default=0, help="Viewer start index in cached records.")
    parser.add_argument(
        "--show-frame-index",
        type=int,
        default=-1,
        help="Print one cached frame's metrics to terminal after build/reuse. Negative disables it.",
    )
    parser.add_argument("--photo-width", type=int, default=960, help="Displayed width of the full photo preview.")
    parser.add_argument("--roi-width", type=int, default=520, help="Displayed width of the ROI debug preview.")
    parser.add_argument(
        "--history-window",
        type=int,
        default=180,
        help="Number of frames shown around the current cursor in the mini history plot.",
    )
    parser.add_argument(
        "--jpeg-quality",
        type=int,
        default=90,
        help="JPEG quality used for cached photo/debug frames.",
    )
    return parser.parse_args()


def session_default_dir(bag_path: Path) -> Path:
    return bag_path.parent / f"{bag_path.stem}_frame_debug"


def ensure_dir(path: Path):
    path.mkdir(parents=True, exist_ok=True)


def image_topics_in_bag(bag: rosbag.Bag) -> List[str]:
    topics = []
    for topic, info in bag.get_type_and_topic_info()[1].items():
        if info.msg_type == "sensor_msgs/Image":
            topics.append(topic)
    return sorted(topics)


def resize_to_width(image: np.ndarray, target_width: int) -> np.ndarray:
    target_width = max(8, int(target_width))
    if image.shape[1] == target_width:
        return image
    scale = target_width / float(image.shape[1])
    target_height = max(8, int(round(image.shape[0] * scale)))
    return cv2.resize(image, (target_width, target_height), interpolation=cv2.INTER_AREA)


def interpolate_value(ts: float, series_times: List[float], series_values: List[float]) -> Optional[float]:
    if not series_times:
        return None
    if len(series_times) == 1:
        return float(series_values[0])
    idx = bisect.bisect_left(series_times, ts)
    if idx <= 0:
        return float(series_values[0])
    if idx >= len(series_times):
        return float(series_values[-1])
    t0 = float(series_times[idx - 1])
    t1 = float(series_times[idx])
    v0 = float(series_values[idx - 1])
    v1 = float(series_values[idx])
    if t1 <= t0:
        return v1
    alpha = (ts - t0) / (t1 - t0)
    return float(v0 + alpha * (v1 - v0))


def finite_float(value) -> Optional[float]:
    try:
        value = float(value)
    except (TypeError, ValueError):
        return None
    return value if math.isfinite(value) else None


def describe_record(record: Dict) -> str:
    lines = [
        f"frame_index: {record['frame_index']}",
        f"stamp: {record['stamp']:.6f}",
        f"relative_time_s: {record['relative_time_s']:.3f}",
        f"valid: {record['valid']}",
        f"accept_for_peak_report: {record['accept_for_peak_report']}",
        f"meniscus_confidence: {record['confidence']:.3f}",
        f"peak_source: {record.get('peak_source', 'n/a')}",
        f"height_peak_rel_px: {record.get('peak_rel_px')}",
        f"height_peak_rel_px_signed: {record.get('peak_rel_px_signed')}",
        f"height_peak_rel_mm: {record.get('peak_rel_mm')}",
        f"height_peak_rel_mm_signed: {record.get('peak_rel_mm_signed')}",
        f"/slosh/height [mm]: {record.get('slosh_height_mm')}",
        f"/slosh/height_pred_max [mm]: {record.get('slosh_pred_mm')}",
        f"cmd_vel.linear.x: {record.get('cmd_vel_linear_x')}",
        f"cmd_vel.angular.z: {record.get('cmd_vel_angular_z')}",
        f"imu_ay: {record.get('imu_ay')}",
        f"fit_rms_px: {record.get('fit_rms_px')}",
        f"peak_local_rms_px: {record.get('peak_local_rms_px')}",
        f"left_band_support_ratio: {record.get('left_band_support_ratio')}",
        f"right_band_support_ratio: {record.get('right_band_support_ratio')}",
        f"photo_path: {record['photo_path']}",
        f"roi_debug_path: {record['roi_debug_path']}",
    ]
    return "\n".join(lines)


def print_selected_frame(manifest: Dict, frame_index: int):
    records = manifest.get("records", [])
    if not records:
        raise RuntimeError("debug session has no records")
    idx = None
    for pos, record in enumerate(records):
        if int(record.get("frame_index", -1)) == int(frame_index):
            idx = pos
            break
    if idx is None:
        idx = int(np.clip(frame_index, 0, len(records) - 1))
    print("[INFO] selected cached frame")
    print(describe_record(records[idx]))


def load_auxiliary_series(bag_path: Path) -> Dict[str, List[float]]:
    topics = {
        "/slosh/height": [],
        "/slosh/height_pred_max": [],
        "/cmd_vel_linear_x": [],
        "/cmd_vel_angular_z": [],
        "/imu_ay": [],
    }
    times = {key: [] for key in topics}

    with rosbag.Bag(str(bag_path), "r") as bag:
        for topic, msg, stamp in bag.read_messages(topics=["/slosh/height", "/slosh/height_pred_max", "/cmd_vel", "/imu/data"]):
            ts = stamp.to_sec()
            if topic == "/slosh/height":
                times["/slosh/height"].append(ts)
                topics["/slosh/height"].append(float(msg.data) * 1000.0)
            elif topic == "/slosh/height_pred_max":
                times["/slosh/height_pred_max"].append(ts)
                topics["/slosh/height_pred_max"].append(float(msg.data) * 1000.0)
            elif topic == "/cmd_vel":
                times["/cmd_vel_linear_x"].append(ts)
                topics["/cmd_vel_linear_x"].append(float(msg.linear.x))
                times["/cmd_vel_angular_z"].append(ts)
                topics["/cmd_vel_angular_z"].append(float(msg.angular.z))
            elif topic == "/imu/data":
                times["/imu_ay"].append(ts)
                topics["/imu_ay"].append(float(msg.linear_acceleration.y))

    return {
        "slosh_height_t": times["/slosh/height"],
        "slosh_height_mm": topics["/slosh/height"],
        "slosh_pred_t": times["/slosh/height_pred_max"],
        "slosh_pred_mm": topics["/slosh/height_pred_max"],
        "cmd_vel_t": times["/cmd_vel_linear_x"],
        "cmd_vel_linear_x": topics["/cmd_vel_linear_x"],
        "cmd_vel_angular_z": topics["/cmd_vel_angular_z"],
        "imu_t": times["/imu_ay"],
        "imu_ay": topics["/imu_ay"],
    }


def draw_text_block(canvas: np.ndarray, lines: List[str], origin: Tuple[int, int], color=(245, 245, 245)):
    x0, y0 = origin
    for idx, line in enumerate(lines):
        cv2.putText(
            canvas,
            line,
            (x0, y0 + idx * 22),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.55,
            color,
            1,
            cv2.LINE_AA,
        )


def draw_history_plot(
    canvas: np.ndarray,
    origin: Tuple[int, int],
    size: Tuple[int, int],
    records: List[Dict],
    cursor_idx: int,
):
    plot_x, plot_y = origin
    plot_w, plot_h = size
    plot = canvas[plot_y : plot_y + plot_h, plot_x : plot_x + plot_w]
    plot[:] = (28, 28, 28)
    cv2.rectangle(canvas, (plot_x, plot_y), (plot_x + plot_w - 1, plot_y + plot_h - 1), (90, 90, 90), 1)

    visual_mm = [record.get("peak_rel_mm") for record in records]
    mpc_mm = [record.get("slosh_height_mm") for record in records]
    pred_mm = [record.get("slosh_pred_mm") for record in records]

    finite_values = [
        float(value)
        for series in (visual_mm, mpc_mm, pred_mm)
        for value in series
        if value is not None and math.isfinite(value)
    ]
    if not finite_values:
        return

    y_min = min(0.0, min(finite_values))
    y_max = max(max(finite_values), y_min + 1e-3)
    y_span = max(1e-3, y_max - y_min)
    x_span = max(1, len(records) - 1)

    def to_points(values, color, thick=1, only_reported=False):
        pts = []
        for idx, value in enumerate(values):
            if value is None or not math.isfinite(value):
                continue
            if only_reported and not records[idx]["accept_for_peak_report"]:
                continue
            px = plot_x + int(round((idx / float(x_span)) * (plot_w - 1)))
            py = plot_y + int(round((1.0 - (float(value) - y_min) / y_span) * (plot_h - 1)))
            pts.append((px, py))
        if len(pts) >= 2:
            cv2.polylines(canvas, [np.asarray(pts, dtype=np.int32)], False, color, thick, cv2.LINE_AA)
        elif len(pts) == 1:
            cv2.circle(canvas, pts[0], 2, color, -1)

    to_points(visual_mm, (160, 160, 160), thick=1, only_reported=False)
    to_points(visual_mm, (255, 160, 64), thick=2, only_reported=True)
    to_points(mpc_mm, (64, 180, 255), thick=2)
    to_points(pred_mm, (96, 96, 255), thick=1)

    cursor_x = plot_x + int(round((cursor_idx / float(x_span)) * (plot_w - 1)))
    cv2.line(canvas, (cursor_x, plot_y), (cursor_x, plot_y + plot_h - 1), (0, 255, 255), 1)
    cv2.putText(canvas, "history: valid / reported / slosh / pred", (plot_x + 6, plot_y + 18), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (220, 220, 220), 1, cv2.LINE_AA)


def compose_view(
    photo: np.ndarray,
    roi_debug: np.ndarray,
    record: Dict,
    records: List[Dict],
    cursor_idx: int,
    photo_width: int,
    roi_width: int,
    history_window: int,
) -> np.ndarray:
    del history_window  # reserved for future local-window plotting

    photo_show = resize_to_width(photo, photo_width)
    roi_show = resize_to_width(roi_debug, roi_width)

    panel_w = max(roi_show.shape[1], 560)
    panel_h = max(320, photo_show.shape[0] - roi_show.shape[0])
    canvas_h = max(photo_show.shape[0], roi_show.shape[0] + panel_h)
    canvas_w = photo_show.shape[1] + panel_w
    canvas = np.zeros((canvas_h, canvas_w, 3), dtype=np.uint8)
    canvas[:] = (18, 18, 18)

    canvas[: photo_show.shape[0], : photo_show.shape[1]] = photo_show
    right_x = photo_show.shape[1]
    canvas[: roi_show.shape[0], right_x : right_x + roi_show.shape[1]] = roi_show

    panel_y = roi_show.shape[0]
    cv2.rectangle(canvas, (right_x, panel_y), (canvas_w - 1, canvas_h - 1), (60, 60, 60), 1)

    peak_units = "mm" if record.get("peak_rel_mm") is not None else "px"
    peak_value = record.get("peak_rel_mm") if peak_units == "mm" else record.get("peak_rel_px")
    peak_signed = record.get("peak_rel_mm_signed") if peak_units == "mm" else record.get("peak_rel_px_signed")
    slosh_height = record.get("slosh_height_mm")
    slosh_pred = record.get("slosh_pred_mm")
    err_height = None if peak_value is None or slosh_height is None else peak_value - slosh_height
    err_pred = None if peak_value is None or slosh_pred is None else peak_value - slosh_pred

    def fmt(value, digits=3):
        if value is None or not math.isfinite(float(value)):
            return "n/a"
        return f"{float(value):.{digits}f}"

    lines = [
        f"cache_idx={cursor_idx} frame={record['frame_index']} rel_t={fmt(record['relative_time_s'], 2)}s",
        f"valid={record['valid']} report={record['accept_for_peak_report']} conf={fmt(record['confidence'], 2)}",
        f"peak={fmt(peak_value, 3)} {peak_units} signed={fmt(peak_signed, 3)} {peak_units}",
        f"left={fmt(record.get('left_rel_mm' if peak_units == 'mm' else 'left_rel_px'), 3)}  right={fmt(record.get('right_rel_mm' if peak_units == 'mm' else 'right_rel_px'), 3)}",
        f"peak_source={record.get('peak_source', 'n/a')}",
        f"slosh_height={fmt(slosh_height, 3)} mm",
        f"pred_max={fmt(slosh_pred, 3)} mm",
        f"err_vs_height={fmt(err_height, 3)} {peak_units}",
        f"err_vs_pred={fmt(err_pred, 3)} {peak_units}",
        f"cmd_vx={fmt(record.get('cmd_vel_linear_x'), 3)}  cmd_wz={fmt(record.get('cmd_vel_angular_z'), 3)}",
        f"imu_ay={fmt(record.get('imu_ay'), 3)}",
        f"fit_rms={fmt(record.get('fit_rms_px'), 3)}  peak_local_rms={fmt(record.get('peak_local_rms_px'), 3)}",
        f"Lband={fmt(record.get('left_band_support_ratio'), 2)}  Rband={fmt(record.get('right_band_support_ratio'), 2)}",
        "keys: a/d prev-next, w/s +/-10, e/z +/-50, g jump, h help, q quit",
    ]
    draw_text_block(canvas, lines, (right_x + 12, panel_y + 26))

    history_y = panel_y + 26 + len(lines) * 22 + 8
    history_h = max(120, canvas_h - history_y - 16)
    draw_history_plot(canvas, (right_x + 10, history_y), (panel_w - 20, history_h), records, cursor_idx)
    return canvas


def save_jpeg(path: Path, image: np.ndarray, jpeg_quality: int):
    cv2.imwrite(str(path), image, [int(cv2.IMWRITE_JPEG_QUALITY), int(np.clip(jpeg_quality, 60, 100))])


def write_session_csv(path: Path, records: List[Dict]):
    fieldnames = [
        "frame_index",
        "stamp",
        "relative_time_s",
        "photo_path",
        "roi_debug_path",
        "valid",
        "accept_for_peak_report",
        "confidence",
        "peak_source",
        "left_rel_px",
        "right_rel_px",
        "peak_rel_px",
        "left_rel_px_signed",
        "right_rel_px_signed",
        "peak_rel_px_signed",
        "left_rel_mm",
        "right_rel_mm",
        "peak_rel_mm",
        "left_rel_mm_signed",
        "right_rel_mm_signed",
        "peak_rel_mm_signed",
        "slosh_height_mm",
        "slosh_pred_mm",
        "cmd_vel_linear_x",
        "cmd_vel_angular_z",
        "imu_ay",
        "fit_rms_px",
        "peak_local_rms_px",
        "left_band_support_ratio",
        "right_band_support_ratio",
    ]
    with path.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=fieldnames)
        writer.writeheader()
        for record in records:
            writer.writerow({key: record.get(key, "") for key in fieldnames})


def build_session(args, out_dir: Path) -> Dict:
    bag_path = Path(args.bag).expanduser().resolve()
    calibration_path = Path(args.calibration).expanduser().resolve()
    config_path = Path(args.config).expanduser().resolve() if args.config else script_default_config()

    calibration = load_calibration(calibration_path)
    config_dict = load_yaml(config_path) if config_path.exists() else {}
    _, processing = load_processing_config(config_path if config_path.exists() else None)
    image_topic = args.image_topic or str(config_dict.get("topic_image", "/camera/color/image_raw"))
    bridge = CvBridge()

    photo_dir = out_dir / "photo"
    roi_dir = out_dir / "roi_debug"
    ensure_dir(out_dir)
    ensure_dir(photo_dir)
    ensure_dir(roi_dir)

    auxiliary = load_auxiliary_series(bag_path)
    manifest_records: List[Dict] = []

    with rosbag.Bag(str(bag_path), "r") as bag:
        image_topics = image_topics_in_bag(bag)
        if image_topic not in image_topics:
            raise RuntimeError(f"image topic not found in bag: {image_topic}; available: {image_topics}")
        bag_start = bag.get_start_time()
        min_stamp = bag_start + max(args.time_offset, 0.0)
        previous_state = None
        image_counter = 0
        processed = 0

        for _, msg, stamp in bag.read_messages(topics=[image_topic]):
            stamp_sec = stamp.to_sec()
            if stamp_sec < min_stamp:
                continue
            if image_counter % max(1, args.every) != 0:
                image_counter += 1
                continue
            if args.max_frames > 0 and processed >= args.max_frames:
                break

            try:
                image = bridge.imgmsg_to_cv2(msg, desired_encoding=args.encoding)
            except CvBridgeError as exc:
                print(f"[WARN] skip frame {image_counter}: {exc}", file=sys.stderr)
                image_counter += 1
                continue

            y0 = calibration.roi_y
            y1 = calibration.roi_y + calibration.roi_h
            x0 = calibration.roi_x
            x1 = calibration.roi_x + calibration.roi_w
            if y0 < 0 or x0 < 0 or y1 > image.shape[0] or x1 > image.shape[1]:
                raise RuntimeError(f"calibration ROI exceeds frame bounds at frame {image_counter}")

            roi_bgr = image[y0:y1, x0:x1].copy()
            rectified_roi_bgr, rectified_calibration = rectify_roi_and_calibration(roi_bgr, calibration)
            detection = detect_liquid_profile(
                rectified_roi_bgr,
                rectified_calibration,
                processing,
                previous_state=previous_state,
            )
            if detection["valid"]:
                from extract_liquid_height_from_bag import build_temporal_state  # local import to avoid circular style lint

                new_state = build_temporal_state(detection)
                if new_state is not None:
                    previous_state = new_state

            full_photo = image.copy()
            cv2.rectangle(full_photo, (x0, y0), (x1 - 1, y1 - 1), (0, 255, 255), 2)
            cv2.putText(
                full_photo,
                f"frame={image_counter} t={stamp_sec - bag_start:.2f}s",
                (12, 28),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.75,
                (0, 255, 255),
                2,
                cv2.LINE_AA,
            )
            debug_roi = make_debug_frame(rectified_roi_bgr, rectified_calibration, image_counter, stamp_sec, detection)

            photo_path = photo_dir / f"frame_{image_counter:06d}.jpg"
            roi_path = roi_dir / f"frame_{image_counter:06d}.jpg"
            save_jpeg(photo_path, full_photo, args.jpeg_quality)
            save_jpeg(roi_path, debug_roi, args.jpeg_quality)

            peak_rel_mm = maybe_mm(detection["peak_rel_px"], calibration.mm_per_pixel)
            peak_rel_mm_signed = maybe_mm(detection["peak_rel_px_signed"], calibration.mm_per_pixel)
            left_rel_mm = maybe_mm(detection["left_rel_px"], calibration.mm_per_pixel)
            right_rel_mm = maybe_mm(detection["right_rel_px"], calibration.mm_per_pixel)
            left_rel_mm_signed = maybe_mm(detection["left_rel_px_signed"], calibration.mm_per_pixel)
            right_rel_mm_signed = maybe_mm(detection["right_rel_px_signed"], calibration.mm_per_pixel)

            record = {
                "frame_index": int(image_counter),
                "stamp": float(stamp_sec),
                "relative_time_s": float(stamp_sec - bag_start),
                "photo_path": str(photo_path),
                "roi_debug_path": str(roi_path),
                "valid": int(detection["valid"]),
                "accept_for_peak_report": int(detection["accept_for_peak_report"]),
                "confidence": float(detection["confidence"]),
                "peak_source": detection["peak_source"],
                "left_rel_px": None if detection["left_rel_px"] is None else float(detection["left_rel_px"]),
                "right_rel_px": None if detection["right_rel_px"] is None else float(detection["right_rel_px"]),
                "peak_rel_px": None if detection["peak_rel_px"] is None else float(detection["peak_rel_px"]),
                "left_rel_px_signed": None if detection["left_rel_px_signed"] is None else float(detection["left_rel_px_signed"]),
                "right_rel_px_signed": None if detection["right_rel_px_signed"] is None else float(detection["right_rel_px_signed"]),
                "peak_rel_px_signed": None if detection["peak_rel_px_signed"] is None else float(detection["peak_rel_px_signed"]),
                "left_rel_mm": None if left_rel_mm is None else float(left_rel_mm),
                "right_rel_mm": None if right_rel_mm is None else float(right_rel_mm),
                "peak_rel_mm": None if peak_rel_mm is None else float(peak_rel_mm),
                "left_rel_mm_signed": None if left_rel_mm_signed is None else float(left_rel_mm_signed),
                "right_rel_mm_signed": None if right_rel_mm_signed is None else float(right_rel_mm_signed),
                "peak_rel_mm_signed": None if peak_rel_mm_signed is None else float(peak_rel_mm_signed),
                "fit_rms_px": None if detection["fit_rms_px"] is None else float(detection["fit_rms_px"]),
                "peak_local_rms_px": None if detection["peak_local_rms_px"] is None else float(detection["peak_local_rms_px"]),
                "left_band_support_ratio": float(detection["left_band_support_ratio"]),
                "right_band_support_ratio": float(detection["right_band_support_ratio"]),
                "slosh_height_mm": interpolate_value(stamp_sec, auxiliary["slosh_height_t"], auxiliary["slosh_height_mm"]),
                "slosh_pred_mm": interpolate_value(stamp_sec, auxiliary["slosh_pred_t"], auxiliary["slosh_pred_mm"]),
                "cmd_vel_linear_x": interpolate_value(stamp_sec, auxiliary["cmd_vel_t"], auxiliary["cmd_vel_linear_x"]),
                "cmd_vel_angular_z": interpolate_value(stamp_sec, auxiliary["cmd_vel_t"], auxiliary["cmd_vel_angular_z"]),
                "imu_ay": interpolate_value(stamp_sec, auxiliary["imu_t"], auxiliary["imu_ay"]),
            }
            manifest_records.append(record)
            image_counter += 1
            processed += 1

    manifest = {
        "bag": str(bag_path),
        "calibration": str(calibration_path),
        "config": str(config_path),
        "image_topic": image_topic,
        "records": manifest_records,
    }
    with (out_dir / "debug_session.json").open("w", encoding="utf-8") as handle:
        json.dump(manifest, handle, ensure_ascii=False, indent=2)
    write_session_csv(out_dir / "debug_session.csv", manifest_records)
    return manifest


def load_session(out_dir: Path) -> Dict:
    manifest_path = out_dir / "debug_session.json"
    if not manifest_path.exists():
        raise FileNotFoundError(str(manifest_path))
    with manifest_path.open("r", encoding="utf-8") as handle:
        return json.load(handle)


def open_viewer(manifest: Dict, args):
    records = manifest.get("records", [])
    if not records:
        raise RuntimeError("debug session has no records")

    cv2.namedWindow(WINDOW_NAME, cv2.WINDOW_NORMAL)
    trackbar_name = "cache_idx"
    trackbar_max = max(1, len(records) - 1)
    idx = int(np.clip(args.start_index, 0, len(records) - 1))
    state = {"idx": idx}

    def on_trackbar(pos):
        state["idx"] = int(np.clip(pos, 0, len(records) - 1))

    cv2.createTrackbar(trackbar_name, WINDOW_NAME, idx, trackbar_max, on_trackbar)
    show_help = True
    while True:
        idx = int(np.clip(state["idx"], 0, len(records) - 1))
        record = dict(records[idx])
        photo = cv2.imread(record["photo_path"], cv2.IMREAD_COLOR)
        roi_debug = cv2.imread(record["roi_debug_path"], cv2.IMREAD_COLOR)
        if photo is None or roi_debug is None:
            raise RuntimeError(f"failed to load cached images for frame {record['frame_index']}")

        canvas = compose_view(
            photo,
            roi_debug,
            record,
            records,
            idx,
            args.photo_width,
            args.roi_width,
            args.history_window,
        )
        if show_help:
            help_lines = [
                "drag cache_idx trackbar to choose frame",
                "a/d: prev/next frame",
                "w/s: -10 / +10 frames",
                "z/e: -50 / +50 frames",
                "g: jump to index",
                "h: toggle help",
                "q or Esc: quit",
            ]
            draw_text_block(canvas, help_lines, (20, canvas.shape[0] - 140), color=(180, 255, 180))

        cv2.imshow(WINDOW_NAME, canvas)
        key = cv2.waitKeyEx(0)
        if key in (27, ord("q")):
            break
        if key in (ord("h"), ord("H")):
            show_help = not show_help
            continue
        if key in (ord("d"), ord("D"), ord(" "), 83, 2555904):
            idx = min(len(records) - 1, idx + 1)
            state["idx"] = idx
            cv2.setTrackbarPos(trackbar_name, WINDOW_NAME, idx)
            continue
        if key in (ord("a"), ord("A"), 81, 2424832):
            idx = max(0, idx - 1)
            state["idx"] = idx
            cv2.setTrackbarPos(trackbar_name, WINDOW_NAME, idx)
            continue
        if key in (ord("s"), ord("S"), 84, 2621440):
            idx = min(len(records) - 1, idx + 10)
            state["idx"] = idx
            cv2.setTrackbarPos(trackbar_name, WINDOW_NAME, idx)
            continue
        if key in (ord("w"), ord("W"), 82, 2490368):
            idx = max(0, idx - 10)
            state["idx"] = idx
            cv2.setTrackbarPos(trackbar_name, WINDOW_NAME, idx)
            continue
        if key in (ord("e"), ord("E")):
            idx = min(len(records) - 1, idx + 50)
            state["idx"] = idx
            cv2.setTrackbarPos(trackbar_name, WINDOW_NAME, idx)
            continue
        if key in (ord("z"), ord("Z")):
            idx = max(0, idx - 50)
            state["idx"] = idx
            cv2.setTrackbarPos(trackbar_name, WINDOW_NAME, idx)
            continue
        if key in (ord("g"), ord("G")):
            try:
                raw = input(f"jump to cached index [0, {len(records) - 1}]: ").strip()
                if raw:
                    idx = int(np.clip(int(raw), 0, len(records) - 1))
                    state["idx"] = idx
                    cv2.setTrackbarPos(trackbar_name, WINDOW_NAME, idx)
            except Exception:
                pass
            continue

    cv2.destroyAllWindows()


def main():
    try:
        args = parse_args()
        bag_path = Path(args.bag).expanduser().resolve()
        out_dir = Path(args.out_dir).expanduser().resolve() if args.out_dir else session_default_dir(bag_path)

        if args.reuse_cache:
            manifest = load_session(out_dir)
        else:
            manifest = build_session(args, out_dir)
            print(f"[OK] built debug session: {out_dir}")
            print(f"[OK] manifest: {out_dir / 'debug_session.json'}")
            print(f"[OK] summary csv: {out_dir / 'debug_session.csv'}")

        if args.show_frame_index >= 0:
            print_selected_frame(manifest, args.show_frame_index)

        if args.skip_viewer:
            return 0

        open_viewer(manifest, args)
        return 0
    except RuntimeError as exc:
        print(f"[WARN] {exc}")
        return 1
    except KeyboardInterrupt:
        try:
            cv2.destroyAllWindows()
        except cv2.error:
            pass
        print("\n[INFO] interrupted by Ctrl+C, viewer closed.")
        return 130


if __name__ == "__main__":
    sys.exit(main())
