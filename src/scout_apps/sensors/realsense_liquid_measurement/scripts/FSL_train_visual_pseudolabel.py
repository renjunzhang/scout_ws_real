#!/usr/bin/env python3
"""Train a minimal visual pseudolabel regressor from ROI images to /slosh/height."""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Train a minimal visual pseudolabel regressor from single-frame ROI images to slosh_height_mm, "
    ...
    parser.add_argument(
        "--image-column",
        choices=("roi_debug_path", "photo_path"),
    ...
    parser.add_argument("--patience", type=int, default=8, help="Early-stop patience on val MAE. Default: 8.")
    parser.add_argument("--seed", type=int, default=7, help="Random seed. Default: 7.")
    parser.add_argument("--device", default="auto", help="Torch device: auto/cpu/cuda. Default: auto.")
    parser.add_argument("--show-first", type=int, default=5, help="Print the first N val rows. Default: 5.")
    return parser.parse_args()


class MinimalVisualRegressor(nn.Module):
    def __init__(self, image_height: int, image_width: int, hidden_dim: int):
        super().__init__()
        ...
        reduced_h = 8
        reduced_w = 16
        flattened_dim = 32 * reduced_h * reduced_w
...
def load_single_image(path: Path, image_height: int, image_width: int) -> np.ndarray:
    with Image.open(path) as img:
        gray = img.convert("L")
        gray = gray.resize((int(image_width), int(image_height)), Image.BILINEAR)
...
def build_image_samples(
    ...
    for row in rows:
        target = finite_float(row.get("slosh_height_mm"))
        if target is None:
...
def main() -> int:
    try:
        ...
        train_samples, train_skip_counts = build_image_samples(
            train_rows, image_column=args.image_column, image_height=int(args.image_height), image_width=int(args.image_width)
        )
        val_samples, val_skip_counts = build_image_samples(
            val_rows, image_column=args.image_column, image_height=int(args.image_height), image_width=int(args.image_width)
        )
        test_samples, test_skip_counts = build_image_samples(
            test_rows, image_column=args.image_column, image_height=int(args.image_height), image_width=int(args.image_width)
        )
        ...
        checkpoint_path = out_dir / "FSL_visual_pseudolabel.pt"
        history_csv_path = out_dir / "FSL_visual_pseudolabel_history.csv"
        predictions_csv_path = out_dir / "FSL_visual_pseudolabel_predictions.csv"
        summary_json_path = out_dir / "FSL_visual_pseudolabel_summary.json"
        bagwise_csv_path = out_dir / "FSL_visual_pseudolabel_bagwise.csv"

        torch.save(
            {
                "model_type": "fsl_visual_cnn_regressor_v1",
        ...
        for model_name, pred_key in (
            ...
            for row in group_metrics(
                [
                    {
                        "bag_id": row["bag_id"],
                        "target": row["target_slosh_height_mm"],
        ...
        summary = {
            ...
            "manifest_csv": str(manifest_csv),
            "split_json": str(split_json_path),
            "task": "visual_pseudolabel_regression",
            "target_column": "slosh_height_mm",
            "image_column": str(args.image_column),
            "device": str(device),
            "image_height": int(args.image_height),
        ...
        print(f"[OK] baseline test mean mae: {summary['baseline_metrics']['test_train_mean']['mae']}")
        ...
        for row in prediction_rows[:show_first]:
            print(
                "[OK] row={row_id} split={split} target={target_slosh_height_mm:.6f} "
