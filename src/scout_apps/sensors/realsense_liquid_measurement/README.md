...
- [realsense\_liquid\_measurement](#realsense_liquid_measurement)
  ...
  - [包内文件与目录说明](#包内文件与目录说明)
    - [根目录文件](#根目录文件)
    - [目录一句话说明](#目录一句话说明)
    - [`config/` 目录文件](#config-目录文件)
  - [scripts 逐个文件说明](#scripts-逐个文件说明)
    - [FSL 伪标签预训练链](#fsl-伪标签预训练链)
    - [SL 人工真值监督学习链](#sl-人工真值监督学习链)
    - [v1 标定与提取链](#v1-标定与提取链)
    - [v2 多标尺标定与提取链](#v2-多标尺标定与提取链)
    - [人工标签与误差分析](#人工标签与误差分析)
    - [动力学数据导出与 replay](#动力学数据导出与-replay)
    - [路径可比性与专题分析](#路径可比性与专题分析)
    - [环境与构建](#环境与构建)
  - [脚本用途总览](#脚本用途总览)
    - [1. 环境与 RealSense ROS 构建](#1-环境与-realsense-ros-构建)
    - [2. v1 标定与提取主线](#2-v1-标定与提取主线)
    - [3. v2 多标尺标定与提取主线](#3-v2-多标尺标定与提取主线)
    - [4. 人工标签质量分析](#4-人工标签质量分析)
    - [5. 监督学习数据导出](#5-监督学习数据导出)
    - [6. 监督学习最小闭环](#6-监督学习最小闭环)
    - [7. slosh model 离线重放](#7-slosh-model-离线重放)
    - [8. 路径与激励可比性分析](#8-路径与激励可比性分析)
    - [9. RealSense vs /slosh/height 误差分析](#9-realsense-vs-sloshheight-误差分析)
    - [10. Q5 重点相位差与坏点深挖](#10-q5-重点相位差与坏点深挖)
    - [11. Center 位置可视复核](#11-center-位置可视复核)
    - [12. Q5\_test1 输入与状态分段分析](#12-q5_test1-输入与状态分段分析)
    - [13. 0330 同路径 Q0/Q5 成对比较](#13-0330-同路径-q0q5-成对比较)
    - [14. 0330 test2 分段幅值与 /local\_path 平滑性深挖](#14-0330-test2-分段幅值与-local_path-平滑性深挖)
  - [当前整理步骤](#当前整理步骤)
  - [当前代码流程图](#当前代码流程图)
  - [RealSense ROS 编译脚本](#realsense-ros-编译脚本)
  - [当前推荐用法](#当前推荐用法)
  - [结果分析教程](#结果分析教程)
    - [A. 想看单个 bag 里 `RealSense` 和 `/slosh/height` 是否同趋势](#a-想看单个-bag-里-realsense-和-sloshheight-是否同趋势)
    - [B. 想比较同一路径下 `Q0` 和 `Q5` 谁晃得更大](#b-想比较同一路径下-q0-和-q5-谁晃得更大)
    - [C. 想证明 `Q5` 更小不是因为路径更平滑](#c-想证明-q5-更小不是因为路径更平滑)
    - [D. 想复现 bag 里的 `/slosh/height` 是怎么来的](#d-想复现-bag-里的-sloshheight-是怎么来的)
    - [E. 想逐帧看图，并给明天的严格验证打标签](#e-想逐帧看图并给明天的严格验证打标签)
    - [当前最推荐的明日继续顺序](#当前最推荐的明日继续顺序)
  - [第二步：手动标定 ROI 和参考几何](#第二步手动标定-roi-和参考几何)
    - [模式 A：快速打通单场景](#模式-a快速打通单场景)
    - [模式 B：正式实验场景标定](#模式-b正式实验场景标定)
  - [第三步：批量提取液面像素时序](#第三步批量提取液面像素时序)
  - [从 Auto-Zero 到逐帧调试的完整流程](#从-auto-zero-到逐帧调试的完整流程)
    - [第 0 步：先拿到本场景自己的静止参考帧](#第-0-步先拿到本场景自己的静止参考帧)
    - [第 1 步：生成本场景的 v2 multiscale 标定 YAML](#第-1-步生成本场景的-v2-multiscale-标定-yaml)
    - [第 2 步：先跑静止 bag](#第-2-步先跑静止-bag)
    - [第 3 步：再跑运动 bag](#第-3-步再跑运动-bag)
    - [第 4 步：和 `/slosh/height` 做同时间轴对比](#第-4-步和-sloshheight-做同时间轴对比)
    - [第 5 步：逐帧调试并录入人工标签](#第-5-步逐帧调试并录入人工标签)
    - [第 6 步：需要时再做人工标签分析或训练集导出](#第-6-步需要时再做人工标签分析或训练集导出)
  - [逐帧调试脚本](#逐帧调试脚本)
    - [原理](#原理)
    - [用法](#用法)
  - [曲线图怎么看](#曲线图怎么看)
