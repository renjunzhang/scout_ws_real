...
int runTrial(const Options& options) {
    ...
    for(int k=0;k<=last_k;++k) {
        ...
        if ((!ok || !out.success || out.status.find("SOLVER_FAILURE_STOPPING:")==0) &&
            out.pre_solve_snapshot.valid)
            saveFailureSnapshot(prefix+"_failure_"+std::to_string(k)+".json",t,state,out);
        if(!ok || !out.success) {
